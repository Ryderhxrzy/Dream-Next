...
const defaultSections = [
  {
    ...
    body: "Every successful interior begins with a clear foundation: flooring, wall treatment, base furniture, and the main material direction. These choices create the visual rhythm that the rest of the room follows.",
  },
  {
    title: "The Role Of Texture",
    body: "In a quiet palette, texture becomes the design language. Linen, wood grain, matte ceramics, leather, woven panels, and stone details help the room feel layered without adding visual noise.",
  },
  {
    title: "Lighting As A Design Element",
    body: "A space needs more than one bright ceiling source. Ambient, task, and accent lighting let the room shift from functional daytime use to a softer evening mood.",
  },
  {
    title: "Bringing It All Together",
    body: "The final pass is editing. Remove pieces that do not support the room's purpose, repeat the strongest materials, and let negative space make the key details feel intentional.",
  },
]
            ...
            <div>
              <FadeUp delay={0.1}>
                <div className="flex flex-wrap items-center gap-3">
                  <span className="rounded-full bg-white px-3 py-1 text-xs font-medium tracking-widest text-[var(--accent)] uppercase">
                    {post.category}
                  </span>
              ...
              <FadeUp delay={0.2}>
                <h1 className="mt-6 text-4xl font-medium tracking-tight text-balance text-[var(--foreground)] sm:text-5xl lg:text-6xl">
                  {post.title}
                </h1>
              </FadeUp>
              ...
              <div className="relative aspect-[4/3] overflow-hidden rounded-[2rem] bg-[#e7ded3] shadow-[0_24px_80px_rgba(64,48,36,0.10)]">
                ...
                <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/55 to-transparent p-6">
                  <p className="max-w-md text-sm leading-relaxed text-white/88">
                    {post.designBrief ||
                      "A practical Dreambuild guide for turning design intent into a calmer, more livable home."}
                  </p>
                </div>
              </div>
            </ScaleUp>
            ...
            <FadeUp delay={0.1}>
              <section className="mt-6 grid gap-3 sm:grid-cols-3">
                {takeaways.slice(0, 3).map((item, index) => (
                  <div
                    key={item}
                    className="rounded-[1.25rem] border border-[#e4d8ca] bg-white p-5"
                  >
                    ...
                    {index === 1 && (
                      <div className="mt-8 grid gap-4 sm:grid-cols-2">
                        {galleryImages.slice(0, 2).map((src, imageIndex) => (
                          <div
                            key={src}
                            className="relative aspect-[4/3] overflow-hidden rounded-[1.5rem] bg-[#e7ded3]"
                          >
                            <Image
                              src={src}
            ...
            <FadeUp delay={0.1}>
              <section className="mt-16 rounded-[1.75rem] bg-[#2d2823] p-7 text-white sm:p-9">
                ...
                <h2 className="mt-4 text-2xl font-medium tracking-tight">
                  Turn the article into a room plan.
                </h2>
                <p className="mt-4 max-w-2xl text-sm leading-7 text-white/70">
                  Use this guide as a starting point, then pair it with actual
                  measurements, lighting conditions, and furniture priorities
          ...
          <aside className="lg:pt-1">
            <div className="sticky top-28 rounded-[1.5rem] border border-[#e4d8ca] bg-white p-5">
              ...
              <div className="mt-5 space-y-3">
                <Link
                  href="/projects"
                  className="block rounded-full border border-[#e4d8ca] px-4 py-3 text-sm text-[var(--foreground)] transition-colors hover:bg-[#f8f5f0]"
                >
                  View Projects
                </Link>
                <Link
                ...
                <Link
                  href="/#contact"
                  className="block rounded-full bg-[var(--dark)] px-4 py-3 text-sm font-medium text-white transition-opacity hover:opacity-85"
                >
                  Contact Dreambuild
                </Link>
              </div>
