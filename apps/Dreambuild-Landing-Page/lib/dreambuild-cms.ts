...
const byIndex = <T>(
  items: CmsItem[],
  defaults: readonly T[],
  mapper: (item: CmsItem, fallback: T, index: number) => T
): T[] => {
...
const mapProcessSteps = (items: CmsItem[]): ProcessStepContent[] => {
  ...
  return items
    ...
    .map((item, index) => {
      const fallback =
        processSteps[index] ?? processSteps[processSteps.length - 1]
  ...
  const hero: HeroContent = heroItem
    ? {
        ...
        primaryButtonText: text(
          heroPayload.primary_button_text,
          defaultHero.primaryButtonText
        ),
        primaryButtonUrl: text(
          heroPayload.primary_button_url,
          defaultHero.primaryButtonUrl
        ),
        secondaryButtonText: text(
          heroPayload.secondary_button_text,
          defaultHero.secondaryButtonText
        ),
        secondaryButtonUrl: text(
          heroPayload.secondary_button_url,
          defaultHero.secondaryButtonUrl
        ),
        signatureLabel: text(
          heroPayload.signature_label,
          defaultHero.signatureLabel
        ),
        stats: [
          {
            value: text(heroPayload.stat_1_value, defaultHero.stats[0].value),
            label: text(heroPayload.stat_1_label, defaultHero.stats[0].label),
          },
          {
            value: text(heroPayload.stat_2_value, defaultHero.stats[1].value),
            label: text(heroPayload.stat_2_label, defaultHero.stats[1].label),
          },
          {
            value: text(heroPayload.stat_3_value, defaultHero.stats[2].value),
            label: text(heroPayload.stat_3_label, defaultHero.stats[2].label),
          },
        ],
        carouselSlides: defaultHeroSlides.map((slide, index) => ({
          ...
          label: text(
            (heroPayload as Record<string, unknown>)[
              `slide_${index + 1}_label`
            ],
            slide.label
          ),
        })),
  ...
  return {
    ...
    contact: contactItems[0]
      ? {
          ...
          address: text(
            contactItems[0].payload?.address,
