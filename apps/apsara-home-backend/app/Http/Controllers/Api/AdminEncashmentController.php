...
class AdminEncashmentController extends Controller
{
    ...
    public function destroy(Request $request, int $id)
    {
        ...
        // Only super_admin / accounting can hard-delete encashment requests.
        // If you want a different policy, adjust here.
        $role = $this->roleFromAdmin($admin);
        if (!in_array($role, ['super_admin', 'accounting'], true)) {
            return response()->json(['message' => 'Forbidden: delete access is limited.'], 403);
        }

        $row = EncashmentRequest::query()->where('er_id', $id)->firstOrFail();

        // Allow deleting any request record from tbl_encashment_requests.
        // If you later want to restrict this, re-add the status guard.


        // Delete related uploaded proof file (best-effort) and delete record.
        // NOTE: We are not using Storage here because your project may store proofs via URL.
        // If you use local storage, replace this with Storage::delete().
        DB::transaction(function () use ($row) {
            $row->delete();
        });

        return response()->json(['message' => 'Encashment request deleted.']);
    }
