...
import { CHANNELS, subscriber } from "./redis.js"
...
export async function startServer() {
  ...
  try {
    ...
    console.warn(
      "⚠ Redis unavailable — realtime pub/sub disabled:",
      ...
      try {
        ...
        if (channel === CHANNELS.NEW_COMMENT) {
          ...
          if (
            ...
            payload.postAuthorId !== payload.commentAuthorId
          ) {
            notify
        ...
        if (channel === CHANNELS.NEW_REPLY) {
          ...
          if (
            ...
            payload.parentAuthorId !== payload.replyAuthorId
          ) {
            notify
        ...
        if (channel === CHANNELS.NEW_REACTION) {
          ...
          if (
            payload.postAuthorId &&
            payload.postAuthorId !== payload.reactorId
          ) {
            notify
              .to(`user:${payload.postAuthorId}`)
        ...
        if (channel === CHANNELS.NEW_RSVP) {
          ...
          if (
            payload.postAuthorId &&
            payload.postAuthorId !== payload.rsvpUserId
          ) {
            notify.to(`user:${payload.postAuthorId}`).emit("new_rsvp", payload)
          }
        }
        ...
        if (channel === CHANNELS.NEW_REPOST) {
          ...
          if (
            payload.postAuthorId &&
            payload.postAuthorId !== payload.reposterId
          ) {
            notify
              .to(`user:${payload.postAuthorId}`)
