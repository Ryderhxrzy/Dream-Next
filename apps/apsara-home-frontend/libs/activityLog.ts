...
export async function logActivity(
  ...
  additionalData?: Omit<
    ActivityLogPayload,
    "customer_id" | "activity_type" | "action" | "description"
  >
) {
  try {
...
export async function logLoginActivity(
  customerId: number,
  loginMethod: string = "manual"
) {
  await logActivity(customerId, "login", "create", "Member logged in", {
    details: {
...
export async function logLogoutActivity(customerId: number) {
  await logActivity(customerId, "logout", "create", "Member logged out")
}
