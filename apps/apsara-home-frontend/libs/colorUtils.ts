...
export function searchColorsByName(
  query: string
): { name: string; hex: string }[] {
  const q = query.trim().toLowerCase()
  if (!q) return getAllColors()
  return COLOR_PALETTE.filter((c) => c.name.toLowerCase().includes(q)).map(
    (c) => ({
      name: c.name,
      hex: c.hex,
    })
...
export function colorNameToHex(name: string): string | null {
  ...
  const exact = COLOR_PALETTE.find(
    (color) => normalizeColorName(color.name) === normalized
  )
  if (exact) return exact.hex

  const startsWith = COLOR_PALETTE.find((color) =>
    normalizeColorName(color.name).startsWith(normalized)
  )
  if (startsWith) return startsWith.hex

  const includes = COLOR_PALETTE.find((color) =>
    normalizeColorName(color.name).includes(normalized)
  )
  ...
  if (tokens.length > 1) {
    const matchedColors = tokens
      .map((token) => {
        const tokenExact = COLOR_PALETTE.find(
          (color) => normalizeColorName(color.name) === token
        )
        if (tokenExact) return tokenExact

        const tokenStartsWith = COLOR_PALETTE.find((color) =>
          normalizeColorName(color.name).startsWith(token)
        )
        if (tokenStartsWith) return tokenStartsWith

        return (
          COLOR_PALETTE.find((color) =>
            normalizeColorName(color.name).includes(token)
          ) ?? null
        )
      })
      .filter((color): color is (typeof COLOR_PALETTE)[number] =>
        Boolean(color)
      )
