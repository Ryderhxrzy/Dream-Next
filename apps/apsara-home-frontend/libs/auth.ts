...
export const authOptions: NextAuthOptions = {
  providers: [
    CredentialsProvider({
      ...
      async authorize(credentials, req) {
        ...
        try {
          if (!apiBaseUrl) {
            throw new Error(
              "Authentication service is not configured (LARAVEL_API_URL)."
            )
          }
          ...
          const cfIpCountry = String(
            incomingHeaders["cf-ipcountry"] ?? ""
          ).trim()
          ...
          if (!res.ok) {
            const message =
              data?.message || "QR login failed. Please try again."
    ...
    CredentialsProvider({
      ...
      async authorize(credentials, req) {
        ...
        const hasPasskeyFlow = Boolean(
          credentials?.passkey_challenge_token && credentials?.passkey_assertion
        )
        ...
        if (
          !hasGoogleToken &&
          !hasFacebookToken &&
          (!hasEmail || (!hasPassword && !hasPasskeyFlow))
        ) {
          return null
        }
        ...
        try {
          ...
          const isResendMfaApproval =
            safeCredentials.resend_mfa_approval === "1"
          const isPasskey =
            !isResendOtp && !isResendMfaApproval && hasPasskeyFlow
          const isGoogleOAuth =
            !isResendOtp &&
            !isResendMfaApproval &&
            !hasPasskeyFlow &&
          ...
          const secChUaPlatform = String(
            incomingHeaders["sec-ch-ua-platform"] ?? ""
          )
            .trim()
            .replace(/^"|"$/g, "")
                ...
                : isResendMfaApproval
                  ...
                  : isPasskey
                    ? {
                        ...
                        challenge_token:
                          safeCredentials.passkey_challenge_token,
                        credential: (() => {
                          try {
                            return JSON.parse(
                              String(safeCredentials.passkey_assertion || "{}")
                            )
                          } catch {
                            return null
                          }
                    ...
                    : isGoogleOAuth
                      ...
                      : isFacebookOAuth
                        ...
                        : {
                            ...
                            otp_challenge_token:
                              safeCredentials.otp_challenge_token || undefined,
                            mfa_challenge_token:
                              safeCredentials.mfa_challenge_token || undefined,
                            cf_turnstile_response:
                              safeCredentials.cf_turnstile_response ||
                              undefined,
                          }
            ),
          })
          ...
          throw e instanceof Error
            ? e
            : new Error("Unable to sign in right now.")
        }
      },
    }),
    CredentialsProvider({
  ...
  cookies: {
    ...
    csrfToken: {
      name: isProd
        ? "__Host-member-next-auth.csrf-token"
        : "member-next-auth.csrf-token",
      options: {
        httpOnly: true,
        sameSite: "lax",
        path: "/",
    ...
    callbackUrl: {
      name: isProd
        ? "__Secure-member-next-auth.callback-url"
        : "member-next-auth.callback-url",
      options: {
        sameSite: "lax",
        path: "/",
        secure: isProd,
