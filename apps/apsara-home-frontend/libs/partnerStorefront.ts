...
export const buildPartnerStorefrontPublicUrl = (
  shopUrl: string,
...
export const resolvePartnerStorefrontPublicUrl = (
  config:
    | Pick<PartnerStorefrontConfig, "shopUrl" | "domainLink">
    | null
    | undefined,
  ...
  const resolved = buildPartnerStorefrontPublicUrl(
    config.shopUrl,
    config.domainLink
  )
...
export const getPartnerStorefrontConfig = (
  item: WebPageItem | undefined
): PartnerStorefrontConfig | null => {
  if (!item) return null

  const fields = getPayloadFields(item)
  const slug = String(fields.slug ?? item.key ?? "")
    .trim()
    .toLowerCase()
  if (!slug) return null
  ...
  return {
    slug,
    displayName:
      String(fields.display_name ?? item.title ?? slug).trim() || slug,
    ...
    heroTitle:
      String(fields.hero_title ?? item.subtitle ?? "").trim() || `Shop ${slug}`,
    heroSubtitle: String(fields.hero_subtitle ?? item.body ?? "").trim(),
    themeColor:
      String(fields.theme_color ?? defaultThemeColor).trim() ||
      defaultThemeColor,
    accentColor:
      String(fields.accent_color ?? defaultAccentColor).trim() ||
      defaultAccentColor,
    ...
    enableActivateDiscount: toBoolean(
      fields.activate_discount ?? fields.enable_activate_discount
    ),
...
export const filterPartnerCategories = (
  categories: Category[],
  config: PartnerStorefrontConfig | null
) => {
...
export const filterPartnerProducts = (
  products: Product[],
  config: PartnerStorefrontConfig | null
) => {
...
export const buildPartnerCategoryLink = (
  partnerSlug: string | undefined,
  category: Pick<Category, "url" | "name">
) => {
