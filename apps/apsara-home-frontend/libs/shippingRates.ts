...
export function resolveShippingFee(
  ...
  const normalizedProvince =
    PROVINCE_ALIASES[normalizeKey(province)] ?? normalizeKey(province)
  ...
  const matchedRate = rates.find((rate) => {
    ...
    const rateProvince =
      PROVINCE_ALIASES[normalizeKey(rate.provinceKey || rate.province)] ??
      normalizeKey(rate.provinceKey || rate.province)
    const rateCity =
      CITY_ALIASES[normalizeKey(rate.cityKey || rate.city)] ??
      normalizeKey(rate.cityKey || rate.city)
