...
export function useCart(): CartContextType {
  ...
  useEffect(() => {
    ...
    try {
      ...
      dispatch(
        setCartSelectionAction({ ids: scopedItems.map((item) => item.id) })
      )
    } catch {
  ...
  useEffect(() => {
    if (isLoggedIn && cartData?.cart_items) {
      const backendItems: CartItem[] = cartData.cart_items.map((item) => ({
        ...
        originalPrice: item.product_price_srp
          ? Number(item.product_price_srp)
          : null,
        ...
        selectedSku: null,
        availableStock:
          Number(item.variant_stock ?? item.product_stock ?? 0) || null,
      }))
  ...
  const removeFromCart = (id: string) => {
    ...
    if (isLoggedIn) {
      const cartItemId =
        items.find((item) => item.id === id)?.cartItemId ?? Number(id)
      if (Number.isFinite(cartItemId)) {
        void removeCartItemApi(cartItemId)
          .unwrap()
          .catch(() => {
  ...
  const updateQuantity = (id: string, qty: number) => {
    ...
    if (isLoggedIn) {
      const cartItemId =
        items.find((item) => item.id === id)?.cartItemId ?? Number(id)
      if (Number.isFinite(cartItemId)) {
        void updateCartItemApi({ id: cartItemId, quantity: cappedQuantity })
          .unwrap()
          .catch(() => {
            // RTK Query invalidation will restore the server state if the request fails.
          })
