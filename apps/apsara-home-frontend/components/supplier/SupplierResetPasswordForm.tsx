    ...
    const loadReset = async () => {
      ...
      try {
        const res = await fetch(
          `${apiUrl}/api/supplier/auth/reset-password/${encodeURIComponent(token)}`,
          {
            cache: "no-store",
            headers: {
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(
          firstError || data?.message || "Unable to reset supplier password."
        )
      }
      ...
      setError(
        err instanceof Error
          ? err.message
          : "Unable to reset supplier password."
      )
    } finally {
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="rounded-2xl border border-slate-800 bg-slate-950/40 p-5">
                ...
                <ul className="mt-3 space-y-2 text-sm text-slate-400">
                  <li className={checks.length ? "text-emerald-400" : ""}>
                    At least 8 characters
                  </li>
                  <li className={checks.uppercase ? "text-emerald-400" : ""}>
                    At least one uppercase letter
                  </li>
                  <li className={checks.lowercase ? "text-emerald-400" : ""}>
                    At least one lowercase letter
                  </li>
                  <li className={checks.number ? "text-emerald-400" : ""}>
                    At least one number
                  </li>
                  <li className={checks.special ? "text-emerald-400" : ""}>
