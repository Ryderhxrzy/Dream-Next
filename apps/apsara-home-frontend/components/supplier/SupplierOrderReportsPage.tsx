...
function StatCard({
  ...
  return (
    <div
      className={`relative overflow-hidden rounded-2xl border bg-white p-5 shadow-sm ${accent}`}
    >
...
export default function SupplierOrderReportsPage({
  ...
  const totals = useMemo(
    () => ({
      ...
      avgOrder: filteredRows.length
        ? filteredRows.reduce((s, r) => s + r.amount, 0) / filteredRows.length
        : 0,
      statusBreakdown: STATUS_OPTIONS.slice(1).reduce<Record<string, number>>(
        (acc, s) => {
    ...
    <div className="space-y-5 pb-10">
      ...
      <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
        ...
        <StatCard
          ...
          value={fmtMoney(totals.avgOrder)}
        ...
        <StatCard
          label="Delivered"
          value={String(totals.statusBreakdown["Delivered"] ?? 0)}
              ...
              >
                <span className="flex items-center gap-1.5">
                  {statusFilter !== "All Status" && (
                    <span
                      className={`h-2 w-2 rounded-full ${STATUS_STYLES[statusFilter]?.dot ?? "bg-slate-400"}`}
                    />
                  )}
                  {statusFilter}
                </span>
                ...
                <div className="absolute left-0 top-12 z-50 min-w-full overflow-hidden rounded-xl border border-slate-100 bg-white py-1 shadow-xl dark:border-slate-800 dark:bg-slate-900">
                  {STATUS_OPTIONS.map((opt) => (
                    ...
                    >
                      {opt !== "All Status" && (
                        <span
                          className={`h-2 w-2 shrink-0 rounded-full ${STATUS_STYLES[opt]?.dot ?? "bg-slate-400"}`}
                        />
                      )}
          ...
          {STATUS_OPTIONS.slice(1).map((s) => {
            ...
            return (
              <button
                ...
                type="button"
                onClick={() =>
                  setStatusFilter(statusFilter === s ? "All Status" : s)
                }
              ...
              >
                <span
                  className={`h-1.5 w-1.5 rounded-full ${statusFilter === s ? st.dot : "bg-slate-400"}`}
                />
                {s}
                <span
                  className={`ml-0.5 rounded-full px-1.5 py-px text-[10px] font-bold ${statusFilter === s ? `${st.bg} ${st.text}` : "bg-slate-100 text-slate-500 dark:bg-slate-800 dark:text-slate-400"}`}
                >
                  {count}
                </span>
              </button>
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
              {isLoading ? (
                ...
                filteredRows.map((row, idx) => (
                  <tr
                    key={row.id}
                    className="group transition-colors hover:bg-slate-50/60 dark:hover:bg-slate-800/20"
                  >
                    {/* Customer */}
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-2.5">
                        <div
                          className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-linear-to-br ${AVATAR_COLORS[idx % AVATAR_COLORS.length]} text-[10px] font-bold text-white`}
                        >
