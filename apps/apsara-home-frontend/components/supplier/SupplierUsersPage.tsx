...
import {
  User, AtSign, Mail, Lock, Send, Copy,
  Pencil, Trash2, Users, ShieldCheck, AlertTriangle, X,
} from 'lucide-react'
import {
...
type EditForm = {
  ...
  email: string; password: string; is_main_supplier?: boolean
}
  ...
  return (
    <div className="hidden sm:flex shrink-0 items-center justify-center">
      <div className="relative w-44 h-32">
        {/* Dot grid */}
        <svg className="absolute top-0 right-0 opacity-30" width="60" height="50" viewBox="0 0 60 50">
          {[0,1,2,3].map(r => [0,1,2,3,4].map(c => (
            <circle key={`${r}${c}`} cx={c*14+5} cy={r*12+5} r="2" fill="#64748b"/>
          )))}
        </svg>
        {/* Card */}
...
export default function SupplierUsersPage() {
  ...
  const handleInvite = async (e: React.FormEvent) => {
    e.preventDefault(); setFeedback(null); setSetupUrl(null)
    try {
      const result = await inviteSupplierUser({
        supplier_id: supplierId, fullname: inviteForm.fullname.trim(),
        username: inviteForm.username.trim(), email: inviteForm.email.trim() || undefined,
      }).unwrap()
      setFeedback({ type: 'success', message: result.message })
      setSetupUrl(result.setup_url)
      setInviteForm(defaultInviteForm)
    } catch (err) {
      setFeedback({ type: 'error', message: getErrorMessage(err, 'Unable to create supplier user invite.') })
    }
  }

  const handleUpdate = async (e: React.FormEvent) => {
    e.preventDefault(); if (!editing) return
    setFeedback(null); setSetupUrl(null)
    try {
      const result = await updateSupplierUser({
        id: editing.id, fullname: editing.fullname.trim(), username: editing.username.trim(),
        email: editing.email.trim() || undefined, password: editing.password.trim() || undefined,
      }).unwrap()
      setFeedback({ type: 'success', message: result.message }); setEditing(null)
    } catch (err) {
  ...
  const requestDelete = (user: { id: number; fullname: string; username: string }) => {
    setFeedback(null); setSetupUrl(null)
    setConfirmDelete({ id: user.id, label: user.fullname?.trim() ? user.fullname : `@${user.username}` })
  }

  const confirmDeleteNow = async () => {
    if (!confirmDelete) return; setFeedback(null)
    try {
      const result = await deleteSupplierUser(confirmDelete.id).unwrap()
      setFeedback({ type: 'success', message: result.message }); setConfirmDelete(null)
    } catch (err) {
  ...
  const handleCopy = async () => {
    if (!setupUrl) return; setFeedback(null)
    const fallback = () => {
      const ta = document.createElement('textarea'); ta.value = setupUrl
    ...
    try {
      if (navigator.clipboard?.writeText && window.isSecureContext) {
        ...
        setFeedback({ type: 'success', message: 'Setup link copied to clipboard.' }); return
      }
      const ok = fallback()
      setFeedback({ type: ok ? 'success' : 'error', message: ok ? 'Copied!' : 'Copy failed.' })
    } catch { const ok = fallback(); setFeedback({ type: ok ? 'success' : 'error', message: ok ? 'Copied!' : 'Copy failed.' }) }
  }

  const openEdit = (user: { id: number; fullname: string; username: string; email: string; is_main_supplier?: boolean }) => {
    setFeedback(null); setSetupUrl(null)
    setEditing({ id: user.id, fullname: user.fullname || '', username: user.username || '', email: user.email || '', password: '', is_main_supplier: user.is_main_supplier })
  }

  if (status === 'loading') {
    return <div className="rounded-3xl border border-slate-200 bg-white p-6 text-sm text-slate-500 shadow-sm">Loading session…</div>
  }
  if (supplierId <= 0) {
    return <div className="rounded-3xl border border-amber-200 bg-amber-50 p-6 text-sm text-amber-800">This supplier account is not linked to a supplier company yet.</div>
  }
    ...
    <div className="space-y-5">
      ...
      <section className="relative overflow-hidden rounded-3xl border border-cyan-100 bg-linear-to-br from-cyan-50 via-sky-50/40 to-indigo-50/60 p-6 shadow-sm sm:p-8 dark:border-cyan-900/30 dark:from-cyan-900/20 dark:via-sky-900/10 dark:to-indigo-900/20">
        <div className="flex items-center justify-between gap-6">
          <div className="flex items-center gap-4">
            ...
            <div>
              <p className="text-[11px] font-bold uppercase tracking-[0.22em] text-cyan-700 dark:text-cyan-400">Supplier Users</p>
      ...
      <div className="grid gap-5 xl:grid-cols-[1fr_1.1fr]">
        ...
        <section className="rounded-3xl border border-slate-100 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-950">
          <p className="text-[10px] font-bold uppercase tracking-[0.22em] text-cyan-600 dark:text-cyan-400">Invite</p>
            ...
            <form onSubmit={handleInvite} className="mt-5 space-y-4">
              ...
              {setupUrl && (
                <div className="rounded-2xl border border-cyan-200 bg-cyan-50 p-4 dark:border-cyan-500/20 dark:bg-cyan-500/10">
                  <p className="text-[10px] font-bold uppercase tracking-[0.18em] text-cyan-700 dark:text-cyan-300">Setup Link</p>
        ...
        <section className="rounded-3xl border border-slate-100 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-950">
          <div className="flex items-center justify-between gap-3">
            <div>
              <p className="text-[10px] font-bold uppercase tracking-[0.22em] text-cyan-600 dark:text-cyan-400">Current Team</p>
              <h2 className="mt-1 text-xl font-bold text-slate-900 dark:text-slate-100">Supplier Portal Users</h2>
            </div>
          ...
          <div className="mt-5">
            {isLoading ? (
              <p className="text-sm text-slate-500 dark:text-slate-400">Loading supplier users…</p>
            ) : isError ? (
              ...
              <div className="space-y-3">
                {users.map((user) => {
                  ...
                  return (
                    <div key={user.id} className="flex items-center justify-between gap-3 rounded-2xl border border-slate-100 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900">
                      <div className="flex items-center gap-3 min-w-0">
                        {/* Avatar */}
                        <div className={`flex h-11 w-11 shrink-0 items-center justify-center rounded-full text-sm font-bold ${color.bg} ${color.text}`}>
                          {inits}
                        </div>
                        <div className="min-w-0">
                          <div className="flex flex-wrap items-center gap-2">
                            <p className="text-sm font-bold text-slate-900 dark:text-slate-100">{user.fullname || user.username}</p>
                          ...
                          <p className="text-xs text-slate-400 dark:text-slate-500">{user.email || 'No email provided'}</p>
                        </div>
                      </div>
                      ...
                      <div className="flex shrink-0 items-center gap-2">
                        ...
                        <button
                          type="button" onClick={() => requestDelete(user)}
                          disabled={isDeleting || (!isMainSupplier || Boolean(user.is_main_supplier))}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/40 p-4 backdrop-blur-sm">
          <div className="w-full max-w-lg rounded-3xl border border-slate-200 bg-white p-6 shadow-2xl dark:border-slate-800 dark:bg-slate-950">
            <div className="flex items-start justify-between gap-3">
              <div>
                <p className="text-[10px] font-bold uppercase tracking-[0.22em] text-cyan-600">Edit User</p>
                <h3 className="mt-1 text-lg font-bold text-slate-900 dark:text-slate-100">Update supplier portal user</h3>
              </div>
            ...
            <form onSubmit={handleUpdate} className="mt-5 space-y-4">
              <Field label="Full Name">
                <IconInput icon={<User className="h-4 w-4" />} value={editing.fullname} required
                  onChange={(e) => setEditing(p => p ? { ...p, fullname: e.target.value } : p)} />
              </Field>
              <Field label="Username">
                <IconInput icon={<AtSign className="h-4 w-4" />} value={editing.username} required
                  onChange={(e) => setEditing(p => p ? { ...p, username: e.target.value } : p)} />
              </Field>
              <Field label="Email (Optional)">
                <IconInput icon={<Mail className="h-4 w-4" />} type="email" value={editing.email}
                  onChange={(e) => setEditing(p => p ? { ...p, email: e.target.value } : p)} />
              </Field>
              <Field label="New Password (Optional)">
                <IconInput icon={<Lock className="h-4 w-4" />} type="password" value={editing.password}
      ...
      {confirmDelete && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/40 p-4 backdrop-blur-sm">
          <div className="w-full max-w-md rounded-3xl border border-rose-100 bg-white p-6 shadow-2xl dark:border-rose-900/30 dark:bg-slate-950">
            ...
            <h3 className="mt-3 text-lg font-bold text-slate-900 dark:text-slate-100">Remove supplier user?</h3>
            <p className="mt-1.5 text-sm text-slate-500 dark:text-slate-400">
              This will remove access for{' '}
              <span className="font-semibold text-slate-800 dark:text-slate-200">{confirmDelete.label}</span>.
            </p>
            {feedback && <div className="mt-4"><Feedback type={feedback.type} message={feedback.message} /></div>}
            <div className="mt-5 flex flex-wrap gap-3">
              <button type="button" onClick={() => void confirmDeleteNow()} disabled={isDeleting}
