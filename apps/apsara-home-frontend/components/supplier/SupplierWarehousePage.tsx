...
function NavBtn({
  ...
  const classes = {
    google: 'bg-slate-900 text-white hover:bg-slate-700 shadow-sm dark:bg-white dark:text-slate-900',
    ...
    disabled: 'pointer-events-none border border-slate-200 bg-slate-50 text-slate-400 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-500',
  }
  ...
  return (
    ...
    >
      {icon}
      {label}
      {variant !== 'disabled' ? <ExternalLink className="h-3.5 w-3.5 opacity-60" /> : null}
    </a>
  )
}
...
function WarehouseModal({
  ...
  const [createWarehouse, { isLoading: isCreating }] = useCreateSupplierWarehouseMutation()
  const [updateWarehouse, { isLoading: isUpdating }] = useUpdateSupplierWarehouseMutation()
  ...
  const [imagePreview, setImagePreview] = useState<string>(warehouse?.image_url ?? '')
  ...
  const handleSave = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const fieldError = apiError?.data?.errors ? Object.values(apiError.data.errors)[0]?.[0] : null
      setError(fieldError || apiError?.data?.message || 'Failed to save. Please try again.')
    }
  }
        ...
        <div className="flex items-center justify-between border-b border-slate-100 px-6 py-5 dark:border-slate-800">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[12px] text-slate-400">Fill in the details and upload a photo.</p>
            </div>
          </div>
          <button
            type="button"
                  ...
                  >
                    ...
                    <div className="text-left">
                      <p className="text-sm font-semibold text-slate-700 dark:text-slate-200">
                        Drop an image or <span className="text-indigo-600 dark:text-indigo-400">click to browse</span>
                      </p>
                      <p className="mt-0.5 text-[11px] text-slate-400">JPG, PNG, WEBP · Max 5 MB</p>
                    </div>
                  </label>
                  ...
                  {warehouse?.image_url && !imageFile ? (
                    <div className="flex items-center gap-3 rounded-xl border border-slate-100 bg-slate-50 px-3 py-2.5 dark:border-slate-800 dark:bg-slate-800">
                      <div className="h-11 w-16 shrink-0 overflow-hidden rounded-lg border border-slate-200 dark:border-slate-700">
                        <img src={warehouse.image_url} alt="Current" className="h-full w-full object-cover" />
                      </div>
                      <div className="min-w-0 flex-1">
                        <p className="text-[11px] font-bold text-slate-500 dark:text-slate-400">Current saved photo</p>
                        <p className="text-[11px] text-slate-400">Upload a new image above to replace it.</p>
                      </div>
      ...
      <div className="space-y-6 pb-10">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <h1 className="mt-1 text-[30px] font-black tracking-tight text-slate-900 dark:text-white">Warehouses</h1>
        ...
        {saveSuccess ? (
          <div className="flex items-center gap-3 rounded-2xl border border-emerald-200 bg-emerald-50 px-5 py-3.5 dark:border-emerald-500/20 dark:bg-emerald-500/10">
            ...
            <button type="button" onClick={() => setSaveSuccess(false)} className="ml-auto text-emerald-400 hover:text-emerald-600">
              <X className="h-4 w-4" />
            </button>
          </div>
        ) : null}
          ...
          <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
            {warehouses.length ? (
              ...
              <div className="flex flex-col items-center justify-center gap-5 rounded-2xl border border-dashed border-slate-200 bg-white py-20 text-center shadow-sm dark:border-slate-700 dark:bg-slate-900">
                ...
                <div>
                  <p className="text-lg font-bold text-slate-800 dark:text-slate-100">No warehouses yet</p>
                  <p className="mt-1 text-sm text-slate-400 dark:text-slate-500 max-w-xs mx-auto">
                    Add your first warehouse so your team can keep track of multiple locations.
                  </p>
                </div>
                <button
                  type="button"
          ...
          <div className="sticky top-6 space-y-5">
            <div className="overflow-hidden rounded-[24px] border border-slate-100 bg-white shadow-[0_18px_45px_rgba(15,23,42,0.08)] dark:border-slate-800 dark:bg-slate-900">
              <div className="flex items-center gap-3 border-b border-slate-100 px-5 py-4 dark:border-slate-800">
                ...
                <div>
                  <p className="text-[14px] font-bold text-slate-900 dark:text-slate-100">Map Preview</p>
                  <p className="text-[11px] text-slate-400">Live embedded map from the selected warehouse.</p>
                </div>
              </div>
              <div className="p-4">
                {latestWarehouse?.warehouse_address ? (
                  ...
                  <div className="flex h-56 flex-col items-center justify-center gap-2 rounded-2xl border border-dashed border-slate-200 bg-slate-50 dark:border-slate-700 dark:bg-slate-800">
                    ...
                    <p className="text-xs text-slate-400">Add a warehouse to preview the latest address</p>
                  </div>
                )}
                <div className="mt-3 grid grid-cols-2 gap-2">
                  <NavBtn href={googleMapsUrl} label="Google Maps" icon={<Navigation className="h-4 w-4" />} variant={googleMapsUrl ? 'google' : 'disabled'} />
                  <NavBtn href={wazeUrl} label="Waze" icon={<Navigation className="h-4 w-4" />} variant={wazeUrl ? 'waze' : 'disabled'} />
                </div>
              </div>
            </div>
