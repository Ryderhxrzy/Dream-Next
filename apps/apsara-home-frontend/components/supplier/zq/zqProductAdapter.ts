...
export const adaptZqDetailToDisplayProduct = (
  ...
  const variants = detail.specs.map((spec, index): ZqDisplayVariant => {
    ...
    return {
      ...
      image:
        spec.image ||
        spec.attributes.find((attribute) => attribute.skuImageUrl)
          ?.skuImageUrl ||
        null,
  ...
  const firstPricedVariant =
    variants.find((variant) => typeof variant.price === "number") ?? variants[0]
  const image =
    detail.images[0] ?? variants.find((variant) => variant.image)?.image ?? null

  return {
    id: detail.id,
    name:
      detail.subject || detail.subjectCn || "Untitled Global Supplier product",
