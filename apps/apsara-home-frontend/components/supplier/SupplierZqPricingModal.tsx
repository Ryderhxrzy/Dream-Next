...
function parseDetail(raw: Record<string, unknown>): ZqDetail {
  ...
  return {
    ...
    categoryName:
      ...
      productCategoryFromDescription(str(raw.description)) ||
      null,
...
export default function SupplierZqPricingModal({
  product,
  onClose,
  onSaved,
  showVariantReversedMultiplier = false,
}: Props) {
  ...
  const [variantPricing, setVariantPricing] = useState<
    Record<string, VariantPricingFields>
  >({})
  ...
  const onVariantMouseDown = useCallback((e: React.MouseEvent) => {
    if (
      (e.target as HTMLElement).closest('button,input,a,select,[role="button"]')
    )
      return
  ...
  const onVariantMouseMove = useCallback((e: React.MouseEvent) => {
    ...
    if (!el) return
    el.scrollLeft =
      variantDrag.current.scrollLeft -
      (e.pageX - el.offsetLeft - variantDrag.current.startX) * 1.2
  }, [])
  ...
  const [updatePricing, { isLoading: isSavingProduct }] =
    useUpdateZqProductPricingMutation()
  const [updateVariantPricing, { isLoading: isSavingVariants }] =
    useUpdateZqVariantPricingMutation()
  const [fetchDetail, { isLoading: isFetchingDetail }] =
    useFetchZqImportDetailMutation()
  ...
  useEffect(() => {
    if (product) {
      ...
      setForm({
        dealer_price: centsToPhp(
          product.dealerPrice ?? product.priceMaxCents ?? product.priceMinCents
        ),
        ...
        reversed_pv_multiplier:
          product.reversedPvMultiplier != null
            ? String(product.reversedPvMultiplier)
            : "",
      })
      setDetail(null)
      ...
      fetchDetail(product.externalId)
        ...
        .then((res) => {
          ...
          setDetail(parsed)
          const main =
            parsed.images.find((img) => img.isMain) ?? parsed.images[0]
      ...
      fetchVariantPricing(product.externalId)
        ...
        .then((res) => {
          ...
          for (const row of res.variants) {
            map[row.skuId] = {
              dealer_price:
                row.dealerPrice != null ? String(row.dealerPrice / 100) : "",
              member_price:
                row.memberPrice != null ? String(row.memberPrice / 100) : "",
              pv: row.pv != null ? String(row.pv) : "",
              reversed_pv_multiplier:
                row.reversedPvMultiplier != null
                  ? String(row.reversedPvMultiplier)
                  : "",
            }
          }
  ...
  const setVariantField = (
    skuId: string,
    field: keyof VariantPricingFields,
    value: string
  ) =>
  ...
  const handleSubmit = async (e: { preventDefault(): void }) => {
    ...
    try {
      ...
      const productPv =
        productDealer > 0 && productMult > 0
          ? roundTo(productDealer * productMult, 2)
          : null
      ...
      await updatePricing({
        ...
        reversed_pv_multiplier:
          form.reversed_pv_multiplier !== ""
            ? parseFloat(form.reversed_pv_multiplier)
            : null,
      }).unwrap()
      ...
      const variantRows = Object.entries(variantPricing)
        ...
        .map(([skuId, v]) => {
          ...
          const effDealer = Math.max(
            toSafe(v.dealer_price) || toSafe(form.dealer_price),
            0
          )
          const effMult = showVariantReversedMultiplier
            ? Math.max(
                toSafe(v.reversed_pv_multiplier) ||
                  toSafe(form.reversed_pv_multiplier),
                0
              )
          ...
          const row = {
            ...
            pv:
              effDealer > 0 && effMult > 0
                ? roundTo(effDealer * effMult, 2)
                : null,
          } as {
            skuId: string
          ...
          if (showVariantReversedMultiplier) {
            row.reversed_pv_multiplier =
              v.reversed_pv_multiplier !== ""
                ? parseFloat(v.reversed_pv_multiplier)
                : null
          }
  ...
  const showSummary =
    summary.computedPv > 0 ||
    summary.effectiveMemberPrice > 0 ||
    summary.transferPrice > 0
  const displayImages =
    detail?.images ??
    (product?.primaryImage
      ? [{ image: product.primaryImage, isMain: true }]
      : [])
                    ...
                    <>
                      ...
                      <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
                        ...
                        >
                          <input
                            disabled
                            readOnly
                            value={detail?.subject || product.subject}
                        ...
                        <Field label="External ID">
                          <input
                            ...
                            className={`${inputCls(true)} font-mono text-xs`}
                          />
                        </Field>
                        <Field label="Source">
                          <input
                            ...
                            readOnly
                            value={
                              detail?.sourceType ?? product.sourceType ?? "—"
                            }
                            className={inputCls(true)}
                          />
                        </Field>
                        <Field label="Category">
                          ...
                          <input
                            ...
                            value={
                              ...
                              (product.zqCategoryId
                                ? `ZQ Category ${product.zqCategoryId}`
                                : "—")
                            }
                            className={inputCls(true)}
                          />
                        ...
                        <Field label="Status">
                          <input
                            disabled
                            readOnly
                            value={detail?.status ?? product.status ?? "—"}
                            className={inputCls(true)}
                          />
                        </Field>
                        ...
                        <Field label="Shipping To">
                          <input
                            ...
                            readOnly
                            value={
                              detail?.shippingTo ?? product.shippingTo ?? "—"
                            }
                            className={inputCls(true)}
                          />
                        </Field>
                        ...
                        <Field label="Variants">
                          <input
                            disabled
                            readOnly
                            value={String(product.variantCount ?? 0)}
                            className={inputCls(true)}
                          />
                        </Field>
                        <Field label="Created">
                          <input
                            ...
                            readOnly
                            value={formatDate(
                              detail?.createdAt ?? product.sourceCreatedAt
                            )}
                            className={inputCls(true)}
                          />
                        </Field>
                        <Field label="Last Synced">
                          <input
                            disabled
                            readOnly
                            value={formatDate(product.syncedAt)}
                            className={inputCls(true)}
                          />
                        </Field>
                        {detail?.productUrl ? (
                          ...
                          >
                            <input
                              disabled
                              readOnly
                              value={detail.productUrl}
                              className={`${inputCls(true)} text-xs`}
                            />
                          </Field>
                        ) : null}
                      </div>
                      ...
                      {!hasVariants && (
                        <>
                          ...
                          <p className="text-xs text-slate-500">
                            SRP comes from the AF Home Global source price. Set
                            Member, Dealer, and the Reversed PV Multiplier — PV
                            is auto-computed.
                          </p>
                          <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
                            <Field label="SRP (₱)">
                              <div className="space-y-1">
                                ...
                                <p className="text-[11px] text-slate-500">
                                  From AF Home Global source price.
                                </p>
                              </div>
                            </Field>
                            <Field label="Member Price (₱)">
                              <div className="space-y-1">
                                <input
                                  ...
                                  onChange={(e) =>
                                    set("member_price", e.target.value)
                                  }
                                  placeholder="0.00"
                                  className={inputCls()}
                                />
                                <p className="text-[11px] text-slate-500">
                                  Shown to member accounts. If blank, SRP is
                                  used.
                                </p>
                              </div>
                            </Field>
                            <Field label="Dealer Price (₱)">
                              <div className="space-y-1">
                                <input
                                  ...
                                  onChange={(e) =>
                                    set("dealer_price", e.target.value)
                                  }
                                  placeholder="0.00"
                                  className={inputCls()}
                                />
                                <p className="text-[11px] text-slate-500">
                                  Also used as Transfer Price for PV formula.
                                </p>
                              </div>
                            </Field>
                            <Field label="Reversed PV Multiplier">
                              <div className="space-y-1">
                                <input
                                  ...
                                  min="0"
                                  step="0.0001"
                                  value={form.reversed_pv_multiplier}
                                  onChange={(e) =>
                                    set(
                                      "reversed_pv_multiplier",
                                      e.target.value
                                ...
                                />
                                <p className="text-[11px] text-slate-500">
                                  Formula: PV = Dealer Price × Multiplier.
                                </p>
                              </div>
                            </Field>
                            <Field label="PV Product">
                              <div className="space-y-1">
                                ...
                                />
                                <p className="text-[11px] text-slate-500">
                                  Auto-computed: Dealer × Reversed PV
                                  Multiplier.
                                </p>
                              </div>
                            </Field>
                      ...
                      {detail && detail.specs.length > 0 && (
                        <>
                          ...
                          <p className="text-xs text-slate-500">
                            Set individual pricing per variant. Leave blank to
                            use the default source price. Auto PV = Dealer Price
                            × Multiplier.
                          </p>
                          <div className="overflow-hidden rounded-2xl border border-slate-200 shadow-sm">
                              ...
                              <table className="min-w-full border-collapse text-[13px]">
                                ...
                                <tbody className="divide-y divide-slate-100 bg-white">
                                  {detail.specs.map((spec) => {
                                    ...
                                    const variantDealer = parseFloat(
                                      vp?.dealer_price || ""
                                    )
                                    const effectiveDealer =
                                      Number.isFinite(variantDealer) &&
                                      variantDealer > 0
                                        ? variantDealer
                                    ...
                                    const effectiveMultiplier =
                                      showVariantReversedMultiplier &&
                                      Number.isFinite(variantMultiplier) &&
                                      variantMultiplier > 0
                                        ? variantMultiplier
                                        : mult
                                    const autoPv =
                                    ...
                                    const variantMemberNum = parseFloat(
                                      vp?.member_price || ""
                                    )
                                    const effectiveMember =
                                      Number.isFinite(variantMemberNum) &&
                                      variantMemberNum > 0
                                        ? variantMemberNum
                                    ...
                                    const isExpanded =
                                      expandedSku === spec.skuId
                                    const variantColSpan =
                                      10 +
                                      (showVariantReversedMultiplier ? 1 : 0)

                                    return (
                                      <Fragment
                                        key={spec.skuId || String(spec.id)}
                                      >
                                        <tr
                                          className={`transition-colors ${isExpanded ? "bg-blue-50/40" : hasVariantPricing ? "bg-emerald-50/40" : "hover:bg-slate-50"}`}
                                        >
                                          ...
                                          <td className="px-3 py-3">
                                            {spec.image ? (
                                              ...
                                              <div className="flex h-10 w-10 items-center justify-center rounded-lg border border-slate-200 bg-slate-100 text-[10px] text-slate-400">
                                                —
                                              </div>
                                            )}
                                          </td>
                            ...
                            <div className="flex items-center gap-2 border-t border-slate-200 bg-slate-50 px-4 py-2.5">
                              ...
                              <p className="text-[11px] text-slate-500">
                                ...
                                are editable. Leave blank to use the default
                                source price.
                              </p>
                            </div>
                          </div>
                      ...
                      {showSummary && !hasVariants && (
                        <PricingSummaryPanel summary={summary} />
                      )}
                    </>
                  )}
                </div>
...
function CalcRow({
  ...
  b: string
  result: string
  resultAccent?: "teal" | "emerald" | "rose"
}) {
  const rc =
    resultAccent === "teal"
      ? "text-teal-600"
  ...
  return (
    <div className="flex items-center justify-between gap-2 px-3 py-2.5">
      ...
      <span className={`shrink-0 text-sm font-bold tabular-nums ${rc}`}>
        {result}
      </span>
    </div>
  )
              ...
              <div className="flex items-center gap-2">
                ...
                <div>
                  ...
                  <p className="font-mono text-xs text-blue-200">
                    All rows derived from {pvStr} PV
                  </p>
                </div>
              </div>
