...
const inputCls = (disabled = false) => [
  ...
  disabled
    ? 'border-slate-200 bg-slate-100/70 text-slate-700 font-medium cursor-not-allowed select-none'
    : 'bg-slate-50/85 border-slate-200 text-slate-800 placeholder-slate-400 focus:border-sky-500 focus:bg-white focus:ring-sky-500/20 hover:border-slate-300',
].join(' ')

const variantInputCls = (disabled = false) => [
  'w-full rounded-lg border px-2.5 py-2 text-sm font-semibold tabular-nums shadow-sm transition-all',
  'focus:outline-none focus:ring-2',
  disabled
    ? 'border-slate-100 bg-slate-50 text-slate-400 cursor-not-allowed'
    : 'bg-white border-slate-200 text-slate-900 placeholder-slate-400 placeholder:font-normal focus:border-emerald-500 focus:ring-emerald-500/25 hover:border-emerald-300',
].join(' ')

/* ─── component ───────────────────────────────────────────────────────  */

export default function SupplierZqPricingModal({ product, onClose, onSaved, showVariantReversedMultiplier = false }: Props) {
  ...
  /* per-variant PV summary dropdown — only one open at a time */
  const [expandedSku, setExpandedSku] = useState<string | null>(null)

  /* drag-to-scroll for the variant table (same UX as the Inventory tab) */
  const variantScrollRef = useRef<HTMLDivElement>(null)
  const variantDrag      = useRef({ isDragging: false, startX: 0, scrollLeft: 0 })
  const [isVariantDrag, setIsVariantDrag] = useState(false)

  const onVariantMouseDown = useCallback((e: React.MouseEvent) => {
    if ((e.target as HTMLElement).closest('button,input,a,select,[role="button"]')) return
    const el = variantScrollRef.current
    if (!el) return
    variantDrag.current = { isDragging: true, startX: e.pageX - el.offsetLeft, scrollLeft: el.scrollLeft }
    setIsVariantDrag(true)
  }, [])
  const onVariantMouseMove = useCallback((e: React.MouseEvent) => {
    if (!variantDrag.current.isDragging) return
    e.preventDefault()
    const el = variantScrollRef.current
    if (!el) return
    el.scrollLeft = variantDrag.current.scrollLeft - (e.pageX - el.offsetLeft - variantDrag.current.startX) * 1.2
  }, [])
  const stopVariantDrag = useCallback(() => {
    variantDrag.current.isDragging = false
    setIsVariantDrag(false)
  }, [])
  ...
  useEffect(() => {
    if (product) {
      /* Display saved manual pricing if present; otherwise fall back to the
         AF Home Global source price range so the modal shows the same prices
         visible in the products list instead of blank 0.00 values. */
      setForm({
        dealer_price:           centsToPhp(product.dealerPrice ?? product.priceMaxCents ?? product.priceMinCents),
        member_price:           centsToPhp(product.memberPrice ?? product.priceMinCents),
        pv:                     '', // PV is auto-computed (Dealer × Reversed PV Multiplier)
        pv_tier:                'low_end', // ZQ products have no high-end tier — always low-end
  ...
  const handleSubmit = async (e: { preventDefault(): void }) => {
    ...
    if (!product) return
    try {
      /* PV is always auto-computed: Dealer Price × Reversed PV Multiplier */
      const productDealer = Math.max(toSafe(form.dealer_price), 0)
      const productMult   = Math.max(toSafe(form.reversed_pv_multiplier), 0)
      const productPv     = productDealer > 0 && productMult > 0 ? roundTo(productDealer * productMult, 2) : null
      ...
      const variantRows = Object.entries(variantPricing)
        ...
        .map(([skuId, v]) => {
          /* PV is auto-computed from the effective Dealer × Multiplier (variant override, else product-level) */
          const effDealer = Math.max(toSafe(v.dealer_price) || toSafe(form.dealer_price), 0)
          const effMult   = showVariantReversedMultiplier
            ? Math.max(toSafe(v.reversed_pv_multiplier) || toSafe(form.reversed_pv_multiplier), 0)
            : Math.max(toSafe(form.reversed_pv_multiplier), 0)
          const row = {
            skuId,
            dealer_price: phpToCents(v.dealer_price),
            member_price: phpToCents(v.member_price),
            pv:           effDealer > 0 && effMult > 0 ? roundTo(effDealer * effMult, 2) : null,
          } as {
            skuId: string
  ...
  const hasVariants = variantTotal > 0
  const mult = Math.max(toSafe(form.reversed_pv_multiplier), 0)
                    ...
                    <>
                      ...
                      <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
                        ...
                        <Field label="Category">
                          {/* ZQ category names come back blank — show the mapped AF Home category name instead, falling back to the ZQ id/name. */}
                          <input
                            disabled
                            readOnly
                            value={
                              product.localCategoryName
                              || detail?.categoryName
                              || product.categoryName
                              || (product.zqCategoryId ? `ZQ Category ${product.zqCategoryId}` : '—')
                            }
                      ...
                      {/* ── Product-level Pricing (only when the product has no variants) ── */}
                      {!hasVariants && (
                      <>
                      <SectionLabel>Product Pricing</SectionLabel>
                      <p className="text-xs text-slate-500">SRP comes from the AF Home Global source price. Set Member, Dealer, and the Reversed PV Multiplier — PV is auto-computed.</p>
                      <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
                        <Field label="SRP (₱)">
                          <div className="space-y-1">
                            <input disabled readOnly value={centsToPhp(product.priceMaxCents ?? product.priceMinCents) || '0.00'} className={inputCls(true)} />
                            <p className="text-[11px] text-slate-500">From AF Home Global source price.</p>
                          </div>
                        </Field>
                        <Field label="Member Price (₱)">
                          <div className="space-y-1">
                            ...
                            <p className="text-[11px] text-slate-500">Shown to member accounts. If blank, SRP is used.</p>
                          </div>
                        </Field>
                        ...
                        <Field label="PV Product">
                          <div className="space-y-1">
                            <input disabled readOnly value={summary.formulaPv > 0 ? summary.formulaPv.toFixed(2) : ''} placeholder="0.00" className={inputCls(true)} />
                            <p className="text-[11px] text-slate-500">Auto-computed: Dealer × Reversed PV Multiplier.</p>
                          </div>
                        </Field>
                      </div>
                      </>
                      ...
                      {detail && detail.specs.length > 0 && (
                        <>
                          ...
                          <p className="text-xs text-slate-500">Set individual pricing per variant. Leave blank to use the default source price. Auto PV = Dealer Price × Multiplier.</p>
                          <div className="overflow-hidden rounded-2xl border border-slate-200 shadow-sm">
                            <div
                              ref={variantScrollRef}
                              onMouseDown={onVariantMouseDown}
                              onMouseMove={onVariantMouseMove}
                              onMouseUp={stopVariantDrag}
                              onMouseLeave={stopVariantDrag}
                              className={`overflow-x-auto ${isVariantDrag ? 'cursor-grabbing select-none' : 'cursor-grab'}`}
                            >
                              <table className="min-w-full border-collapse text-[13px]">
                                <thead>
                                  <tr className="border-b border-slate-200 bg-slate-100">
                                    <th className="w-9 px-2 py-3" aria-label="Expand" />
                                    <th className="px-3 py-3 text-left text-[11px] font-bold uppercase tracking-wider text-slate-600">Image</th>
                                    <th className="px-3 py-3 text-left text-[11px] font-bold uppercase tracking-wider text-slate-600">Spec</th>
                                    <th className="px-3 py-3 text-left text-[11px] font-bold uppercase tracking-wider text-slate-600">SKU ID</th>
                                    <th className="whitespace-nowrap px-3 py-3 text-right text-[11px] font-bold uppercase tracking-wider text-slate-600 leading-tight">
                                      <span className="block text-[10px] font-semibold tracking-normal text-slate-500">AF Home Global</span>Cost
                                    </th>
                                    <th className="whitespace-nowrap px-3 py-3 text-right text-[11px] font-bold uppercase tracking-wider text-slate-600 leading-tight">
                                      <span className="block text-[10px] font-semibold tracking-normal text-slate-500">AF Home Global</span>Price
                                    </th>
                                    <th className="px-3 py-3 text-right text-[11px] font-bold uppercase tracking-wider text-slate-600">Stock</th>
                                    <th className="border-l border-emerald-200 bg-emerald-50 px-3 py-3 text-center text-[11px] font-bold uppercase tracking-wider text-emerald-700">Member ₱</th>
                                    <th className="bg-emerald-50 px-3 py-3 text-center text-[11px] font-bold uppercase tracking-wider text-emerald-700">Dealer ₱</th>
                                    {showVariantReversedMultiplier && (
                                      <th className="bg-emerald-50 px-3 py-3 text-center text-[11px] font-bold uppercase tracking-wider text-emerald-700">Multiplier</th>
                                    )}
                                    <th className="border-l border-teal-200 bg-teal-50 px-3 py-3 text-center text-[11px] font-bold uppercase tracking-wider text-teal-700">Auto PV</th>
                                  </tr>
                                </thead>
                                <tbody className="divide-y divide-slate-100 bg-white">
                                  {detail.specs.map((spec) => {
                                    ...
                                    const variantMemberNum = parseFloat(vp?.member_price || '')
                                    const effectiveMember = Number.isFinite(variantMemberNum) && variantMemberNum > 0
                                      ? variantMemberNum
                                      : toSafe(form.member_price)
                                    const variantSummary = buildPricingSummary({
                                      dealer_price:           String(effectiveDealer),
                                      member_price:           String(effectiveMember),
                                      pv:                     '',
                                      pv_tier:                form.pv_tier,
                                      reversed_pv_multiplier: String(effectiveMultiplier),
                                    })
                                    const isExpanded = expandedSku === spec.skuId
                                    const variantColSpan = 10 + (showVariantReversedMultiplier ? 1 : 0)

                                    return (
                                      <Fragment key={spec.skuId || String(spec.id)}>
                                      <tr className={`transition-colors ${isExpanded ? 'bg-blue-50/40' : hasVariantPricing ? 'bg-emerald-50/40' : 'hover:bg-slate-50'}`}>
                                        <td className="px-2 py-3 align-top">
                                          <button
                                            type="button"
                                            onClick={() => setExpandedSku(isExpanded ? null : spec.skuId)}
                                            aria-label={isExpanded ? 'Hide PV summary' : 'Show PV summary'}
                                            className={`flex h-7 w-7 items-center justify-center rounded-lg border transition ${isExpanded ? 'border-blue-300 bg-blue-100 text-blue-600' : 'border-slate-200 bg-white text-slate-400 hover:border-slate-300 hover:text-slate-600'}`}
                                          >
                                        ...
                                        <td className="px-3 py-3">
                                          {spec.image
                                            ? <img src={spec.image} alt={spec.spec} className="h-10 w-10 rounded-lg border border-slate-200 object-cover" />
                                            : <div className="flex h-10 w-10 items-center justify-center rounded-lg border border-slate-200 bg-slate-100 text-[10px] text-slate-400">—</div>}
                                        </td>
                                        ...
                                        <td className="px-3 py-3 font-mono text-[12px] text-slate-600 whitespace-nowrap">{spec.skuId || '—'}</td>
                                        <td className="px-3 py-3 text-right tabular-nums text-slate-600 whitespace-nowrap">{formatCents(spec.cost)}</td>
                                        <td className="px-3 py-3 text-right font-semibold tabular-nums text-slate-800 whitespace-nowrap">{formatCents(spec.salesPrice)}</td>
                                        <td className="px-3 py-3 text-right tabular-nums text-slate-600">{spec.amountOnSale ?? '—'}</td>
                                        {/* editable pricing */}
                                      ...
                                      {isExpanded && (
                                        <tr className="bg-slate-50/80">
                                          <td colSpan={variantColSpan} className="px-3 py-4 sm:px-4">
                                            <p className="mb-2 text-[11px] font-semibold uppercase tracking-wide text-slate-500">
                                              PV breakdown — <span className="text-slate-700">{spec.spec || spec.skuId}</span>
                                            </p>
                                            <PricingSummaryPanel summary={variantSummary} scopeLabel="this variant" />
                                          </td>
                                        </tr>
                                      )}
                                      </Fragment>
                            ...
                            <div className="flex items-center gap-2 border-t border-slate-200 bg-slate-50 px-4 py-2.5">
                              <span className="inline-block h-3 w-3 shrink-0 rounded border border-emerald-200 bg-emerald-50" />
                              <p className="text-[11px] text-slate-500">
                                <span className="font-semibold text-emerald-700">Green columns</span> are editable. Leave blank to use the default source price.
                              </p>
                            </div>
                          </div>
                        </>
                      ...
                      {/* ── PV Summary (product-level only when there are no variants) ── */}
                      {showSummary && !hasVariants && <PricingSummaryPanel summary={summary} />}
                    </>
                  )}
...
function SectionLabel({ children }: { children: React.ReactNode }) {
  return (
    <div className="flex items-center gap-2.5 pt-2">
      ...
      <span className="whitespace-nowrap text-[11px] font-bold uppercase tracking-[0.2em] text-slate-700">{children}</span>
    ...
    <div className="flex items-center justify-between px-3 py-2.5 gap-2">
      <div className="min-w-0">
        ...
        <div className="flex items-center gap-1 mt-0.5 font-mono text-[11px] text-slate-500 flex-wrap">
          <span>{a}</span><span className="text-slate-400">{op}</span>
          <span>{b}</span><span className="text-slate-400">=</span>
...
function PricingSummaryPanel({ summary, scopeLabel = 'product level' }: { summary: PricingSummary; scopeLabel?: string }) {
    ...
    <div className="rounded-2xl border border-blue-100 overflow-hidden shadow-sm">
      <div className="flex items-center justify-between px-4 py-3 bg-gradient-to-r from-blue-600 to-blue-500">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs text-blue-200">— live computation ({scopeLabel})</span>
        </div>
      ...
      <div className="bg-gradient-to-br from-slate-50 to-blue-50/60 divide-y divide-slate-100">
        <div className="px-4 py-3">
          <p className="text-[11px] font-bold uppercase tracking-widest text-slate-500 mb-2">PV Computation</p>
          <div className="rounded-xl bg-white border border-teal-100 px-3 py-3">
            <div className="flex items-start justify-between gap-3">
              <div>
                <p className="text-xs font-semibold text-slate-500 uppercase tracking-wide mb-1">Transfer Price × Reversed PV Multiplier = PV Product</p>
                ...
                <p className="text-xs text-slate-500 mt-2">Encoded PV used in summary: <span className="font-semibold text-slate-700">{pvStr} PV</span></p>
              </div>
        ...
        <div className="px-4 py-3">
          <p className="text-[11px] font-bold uppercase tracking-widest text-slate-500 mb-2">Price Breakdown</p>
        ...
        <div className="px-4 py-3">
          <div className="flex items-center justify-between mb-2">
            <p className="text-[11px] font-bold uppercase tracking-widest text-slate-500">PV Allocation Preview</p>
              ...
              <div key={label} className="flex items-center justify-between px-3 py-2 gap-2">
                <div className="min-w-0">
                  ...
                  <p className="font-mono text-xs text-slate-500 mt-0.5">
                    {pvStr} PV <span className="text-slate-400">×</span> {rate} <span className="text-slate-400">=</span> <span className="font-semibold text-slate-700">{fmtAlloc(value, unit)}</span>
                  </p>
