...
function buildSparkline(
  ...
  const checkpoints = [
    0,
    ...sorted.map((_, i) => getVal(sorted.slice(0, i + 1))),
  ]
...
function AddStockModal({
  ...
  function stepVar(id: number, delta: number) {
    setVarInputs((prev) => ({
      ...prev,
      [id]: String(Math.max(0, (Number(prev[id]) || 0) + delta)),
    }))
  }
      ...
      if (hasVariants) {
        const variants = (product.variants ?? []).map((v) => ({
          ...
          pv_qty:
            v.id != null
              ? mode === "add"
                ? Number(v.qty ?? 0) + (Number(varInputs[v.id] ?? 0) || 0)
                : Number(varInputs[v.id] ?? 0) || 0
              : Number(v.qty ?? 0),
          pv_status: v.status ?? 1,
        }))
        await updateProduct({
          id: product.id,
          data: { pd_variants: variants },
        }).unwrap()
      ...
      <div className="relative w-full max-w-lg overflow-hidden rounded-2xl bg-white shadow-2xl dark:bg-slate-900">
            ...
            <div className="relative h-12 w-12 shrink-0 overflow-hidden rounded-xl border border-slate-200 bg-slate-100 dark:border-slate-700 dark:bg-slate-800">
              {product.image ? (
                ...
                <div className="flex h-full w-full items-center justify-center">
                  <Warehouse className="h-5 w-5 text-slate-300" />
                </div>
              )}
            </div>
            <div>
        ...
        <div className="px-6 py-5">
          ...
          <div className="mb-5 flex overflow-hidden rounded-xl border border-slate-200 bg-slate-50 p-1 dark:border-slate-700 dark:bg-slate-800">
            {(
              [
                { v: "add", l: "+ Add to Stock" },
                { v: "set", l: "= Set Exact Qty" },
              ] as const
            ).map((opt) => (
              <button
                key={opt.v}
            ...
            <div className="max-h-60 space-y-2 overflow-y-auto pr-1">
              ...
              {(product.variants ?? []).map((v) => {
                ...
                return (
                  <div
                    key={v.id}
                    className="flex items-center gap-3 rounded-xl border border-slate-100 bg-slate-50/50 px-4 py-3 dark:border-slate-800"
                  >
                    <div className="min-w-0 flex-1">
                      <p className="line-clamp-1 text-[12px] font-semibold text-slate-800 dark:text-slate-100">
                        {label}
                      </p>
                    ...
                    <div className="flex items-center gap-1">
                      <button
                        type="button"
                        onClick={() => stepVar(v.id!, -1)}
                        className="flex h-7 w-7 items-center justify-center rounded-lg border border-slate-200 bg-white text-slate-500 hover:bg-slate-100 dark:border-slate-700 dark:bg-slate-800"
                      >
    ...
    <tr className="border-b border-slate-100 transition-colors hover:bg-slate-50/60 dark:border-slate-800 dark:hover:bg-slate-800/20">
      ...
      <td className="px-6 py-4">
        <div className="flex items-center gap-3">
          ...
          <div className="min-w-0">
            <p className="line-clamp-1 text-[13px] font-semibold text-slate-900 dark:text-slate-100">
              {product.name}
            </p>
      ...
      <td className="px-6 py-4">
        <p className="text-[13px] font-bold text-slate-900 dark:text-slate-100">
          ₱
          {Number(product.priceSrp ?? 0).toLocaleString("en-PH", {
...
function InventoryRow({
  ...
  const [checkLive, { data, isFetching, isError }] =
    useLazyGetZqInventoryQuery()
  ...
  return (
    <tr
      className={cn(
        "border-b border-slate-100 transition-colors hover:bg-slate-50/60 dark:border-slate-800",
        index % 2 === 1
            ...
            <div className="min-w-27.5 rounded-xl border border-violet-200/60 bg-[linear-gradient(135deg,rgba(139,92,246,0.06),rgba(99,102,241,0.04))] px-3 py-2">
              ...
              {data.variant_count ? (
                <p className="mt-0.5 text-[10px] text-slate-400">
                  {data.variant_count} variants
                </p>
              ) : null}
              {(typeof data.locked === "number" ||
                typeof data.on_transit === "number") && (
                <p className="mt-1 text-[10px] text-slate-400">
                  Locked {(data.locked ?? 0).toLocaleString()} · Transit{" "}
                  {(data.on_transit ?? 0).toLocaleString()}
                </p>
              )}
            </div>
          )}
        ...
        {product.syncedAt ? (
          <div>
            ...
            <p className="mt-0.5 text-[11px] text-slate-400">
              {new Date(product.syncedAt).toLocaleTimeString("en-US", {
                hour: "2-digit",
                minute: "2-digit",
                hour12: true,
              })}
...
export default function SupplierInventoryPage() {
  ...
  const isZqSupplier = useMemo(() => {
    const normName = String(supplierName ?? "")
      .toLowerCase()
      .replace(/[^a-z0-9]/g, "")
    const byName =
      normName.includes("zqsupplier") ||
  ...
  const onMouseMove = useCallback((e: React.MouseEvent) => {
    ...
    el.scrollLeft =
      ...
      (e.pageX - el.offsetLeft - dragState.current.startX) * 1.2
  }, [])
  ...
  const {
    ...
    { supplierId, page, perPage, search: search || undefined },
  ...
  const sparklines = useMemo(
    () => ({
      ...
      inStock: buildSparkline(
        sorted,
        (s) => s.filter((p) => eqty(p) > 5).length
      ),
      lowStock: buildSparkline(
        sorted,
        (s) => s.filter((p) => eqty(p) > 0 && eqty(p) <= 5).length
      ),
      outStock: buildSparkline(
        sorted,
        (s) => s.filter((p) => eqty(p) === 0).length
      ),
      value: buildSparkline(sorted, (s) =>
        s.reduce((sum, p) => sum + eqty(p) * Number(p.priceSrp ?? 0), 0)
      ),
    }),
    [sorted, qtyOverrides]
  ...
  const lowStockCount = spProducts.filter(
    (p) => eqty(p) > 0 && eqty(p) <= 5
  ).length
  const outStockCount = spProducts.filter((p) => eqty(p) === 0).length
  const totalValue = spProducts.reduce(
    (s, p) => s + eqty(p) * Number(p.priceSrp ?? 0),
    0
  )
    ...
    : [
        {
          ...
          displayValue: String(totalProducts),
        ...
        },
        {
          key: "instock",
          label: "In Stock",
          displayValue: String(inStockCount),
        ...
        },
        {
          key: "lowstock",
          label: "Low Stock",
          displayValue: String(lowStockCount),
        ...
        },
        {
          key: "outstock",
          label: "Out of Stock",
          displayValue: String(outStockCount),
        ...
        },
        {
          key: "value",
          label: "Total Value",
          displayValue: fmtVal(totalValue),
        ...
        {isZqSupplier && (
          <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
            {[
              ...
              {
                label: "In Stock",
                value: (zqProducts as ZqCachedProduct[]).filter(
                  (p) => p.totalStock > 5
                ).length,
                icon: <PackageCheck className="h-4 w-4" />,
                iconBg: "bg-emerald-50",
        ...
        {!isZqSupplier && (
          <>
            {lowStockCount > 0 && (
              <button
                type="button"
                className="flex w-full items-center gap-3 rounded-2xl border border-orange-100 bg-orange-50 px-5 py-3.5 text-left transition hover:bg-orange-100 dark:border-orange-500/20 dark:bg-orange-500/8"
              >
            ...
            {outStockCount > 0 && (
              <button
                type="button"
                className="flex w-full items-center gap-3 rounded-2xl border border-red-100 bg-red-50 px-5 py-3.5 text-left transition hover:bg-red-100 dark:border-red-500/20 dark:bg-red-500/8"
              >
                <PackageX className="h-5 w-5 shrink-0 text-red-500" />
                <p className="flex-1 text-sm font-semibold text-red-700 dark:text-red-300">
                  {outStockCount} product{outStockCount !== 1 ? "s are" : " is"}{" "}
                ...
                {isLoading ? (
                    ...
                    <td colSpan={cols.length} className="py-20 text-center">
                      ...
                      <p className="mt-1 text-xs text-slate-400 dark:text-slate-500">
                        {search
                          ...
                          : isZqSupplier
                            ? "Sync products from Global Supplier first."
                            : "Add products to your inventory."}
                      </p>
                    </td>
                  </tr>
                ) : isZqSupplier ? (
                  (products as ZqCachedProduct[]).map((p, i) => (
                  ...
                  (products as Product[]).map((p) => (
                    <ProductInventoryRow
                      ...
                      onAddStock={(prod) =>
                        setModal({ product: prod, mode: "add" })
                      }
                    />
                  ))
                )}
          ...
          <div className="border-t border-slate-100 bg-slate-50/60 px-6 py-3.5 dark:border-slate-800 dark:bg-slate-800/20">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              <p className="text-sm text-slate-500 dark:text-slate-400">
                {meta ? (
                  <>
                    {(meta.from ?? 0).toLocaleString()}–
                    {(meta.to ?? 0).toLocaleString()} of{" "}
              ...
              {meta && meta.last_page > 1 && (
                <div className="flex items-center gap-2">
                  ...
                  <span className="min-w-18 text-center text-sm font-medium text-slate-500">
                    {page} / {meta.last_page}
                  </span>
                  <button
                    type="button"
