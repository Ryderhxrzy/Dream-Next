...
export default function InteriorRequestsPageMain() {
  ...
  const [statusFilter, setStatusFilter] = useState<
    "all" | InteriorRequestStatus
  >("all")
  const [selectedId, setSelectedId] = useState<number>(0)
  const [replyType, setReplyType] =
    useState<InteriorRequestUpdateType>("message")
  ...
  const [updateRequest, { isLoading: isSavingRequest }] =
    useUpdateAdminInteriorRequestMutation()
  const [replyRequest, { isLoading: isSendingReply }] =
    useReplyAdminInteriorRequestMutation()
  ...
  const effectiveReplyStatus: InteriorRequestStatus = selected
    ? replyStatus ||
      (selected.status === "pending" ? "reviewing" : selected.status)
    : "reviewing"
  ...
  const stats = useMemo(
    () => ({
      ...
      estimateReady: requests.filter((r) => r.status === "estimate_ready")
        .length,
  ...
  const handleAssignToMe = async () => {
    ...
    try {
      ...
      setFeedback({
        type: "success",
        text: "Request assigned to your account.",
      })
    } catch {
      setFeedback({
        type: "error",
        text: "Could not assign this request right now.",
      })
    }
  }
  ...
  const handleSendReply = async () => {
    ...
    try {
      ...
      setFeedback({
        type: "error",
        text: "Could not post the update right now.",
      })
    }
  }
    ...
    return (
      <div className="flex items-center gap-3 rounded-2xl border border-red-200 bg-red-50 p-6 dark:border-red-900/40 dark:bg-red-950/30">
        ...
        <p className="text-sm text-red-700">
          Could not load the interior request queue. Please try again.
        </p>
      </div>
    )
  }
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-5 dark:border-slate-800 dark:bg-slate-900 md:p-6">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="mt-1 max-w-xl text-sm text-slate-500">
                Review design requests, assign consultations, and send estimates
                or concept updates directly to customers.
              </p>
            </div>
          </div>
          <div className="rounded-xl border border-cyan-100 bg-cyan-50 px-4 py-3 dark:border-cyan-900/30 dark:bg-cyan-500/10">
            ...
            <p className="mt-1 max-w-55 text-xs text-cyan-800">
              Visible replies trigger inbox notifications and email alerts for
              the customer.
            </p>
          </div>
        </div>
        ...
        <div className="mt-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
          {[
            ...
            <div
              key={stat.label}
              className={`flex items-center gap-3 rounded-xl border px-4 py-3.5 ${stat.color}`}
            >
        ...
        {selected ? (
            ...
            <div className="rounded-2xl border border-slate-200 bg-white p-5 dark:border-slate-800 dark:bg-slate-900 md:p-6">
              <div className="flex flex-wrap items-start justify-between gap-4">
                ...
                <div className="flex flex-wrap items-center gap-2">
                  <span
                    className={`inline-flex items-center gap-1.5 rounded-full border px-3 py-1.5 text-xs font-semibold ${statusMeta[selected.status].pill}`}
                  >
              ...
              <div className="mt-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
                {[
                  {
                    label: "Property Type",
                    value: selected.property_type || "Not provided",
                  },
                  ...
                  {
                    label: "Assigned To",
                    value: selected.assigned_admin?.name || "Unassigned",
                  },
                ].map((item) => (
                  <div
                    key={item.label}
              ...
              <div className="flex flex-wrap items-center justify-between gap-3">
                <div>
                  ...
                  <p className="mt-0.5 text-xs text-slate-500">
                    Visible replies notify the customer via inbox and email.
                  ...
                  <div className="mt-3 flex items-start gap-3 rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 dark:border-slate-800 dark:bg-slate-950">
                    ...
                    <div>
                      <p className="flex items-center gap-1.5 text-sm font-semibold text-slate-800">
                        {replyVisible ? (
                          <>
                            <Eye className="h-3.5 w-3.5 text-sky-600" /> Visible
                            to customer
                          </>
                        ) : (
                          <>
            ...
            <div className="rounded-2xl border border-slate-200 bg-white p-5 dark:border-slate-800 dark:bg-slate-900 md:p-6">
              <div className="flex items-center justify-between gap-3">
                ...
                <span className="rounded-full border border-slate-200 bg-slate-50 px-2.5 py-1 text-[11px] font-medium text-slate-500">
                  ...
                  {selected.updates.length === 1 ? "update" : "updates"}
                </span>
              </div>
                      ...
                      <div key={update.id} className="flex gap-3">
                        ...
                        <div className="flex-1 rounded-xl border border-slate-200 bg-slate-50 px-4 py-3.5 dark:border-slate-800 dark:bg-slate-950">
                          ...
                          <div className="mt-2.5 flex flex-wrap items-center gap-3 text-[11px] text-slate-400">
                            ...
                            <span
                              className={`flex items-center gap-1 font-medium ${update.visible_to_customer ? "text-cyan-600" : "text-slate-400"}`}
                            >
          ...
          <div className="flex flex-col items-center justify-center rounded-2xl border border-dashed border-slate-200 bg-white p-16 dark:border-slate-800 dark:bg-slate-900">
            ...
            <p className="mt-1 text-xs text-slate-400">
              Pick a request from the list to view details.
            </p>
          </div>
        )}
      </div>
