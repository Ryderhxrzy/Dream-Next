...
const openPrintView = (row: AdminEncashmentItem) => {
  ...
  win.document.open()
  win.document
    .write(`<!doctype html><html><head><meta charset="utf-8"/><title>Invoice ${sanitize(row.invoice_no || row.reference_no)}</title>
...
const openBulkPrintView = (rows: AdminEncashmentItem[]) => {
  ...
  const invoices = rows
    .map(
      (row) =>
        `<section class="invoice"><div class="header"><div><div class="title">AF Home Payout Invoice</div><div class="muted">Auto-generated accounting document</div></div><div style="text-align:right"><div class="label">Invoice No</div><div class="value">${sanitize(row.invoice_no || "Pending")}</div></div></div><div class="box"><div class="row"><div><div class="label">Encashment Ref</div><div class="value">${sanitize(row.reference_no)}</div></div><div><div class="label">Released At</div><div class="value">${sanitize(formatDate(row.released_at))}</div></div></div><div class="row"><div><div class="label">Affiliate</div><div class="value">${sanitize(row.affiliate_name || "Affiliate")}</div></div><div><div class="label">Email</div><div class="value">${sanitize(row.affiliate_email || "-")}</div></div></div><div class="row"><div><div class="label">Channel</div><div class="value">${sanitize((row.channel || "").toUpperCase())}</div></div><div><div class="label">Account</div><div class="value">${sanitize(row.account_name || "-")} (${sanitize(row.account_number || "-")})</div></div></div></div><div class="box"><div class="label">Total Released Amount</div><div class="total">${sanitize(formatMoney(row.amount))}</div></div></section>`
    )
    .join('<div class="page-break"></div>')
  win.document.open()
  win.document.write(
    `<!doctype html><html><head><meta charset="utf-8"/><title>Bulk Invoices (${rows.length})</title><style>body{font-family:Arial,sans-serif;margin:24px;color:#0f172a}.invoice{margin-bottom:28px}.header{display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:18px}.title{font-size:22px;font-weight:700}.muted{color:#64748b;font-size:12px}.box{border:1px solid #e2e8f0;border-radius:10px;padding:12px;margin-bottom:12px}.row{display:flex;justify-content:space-between;gap:16px;margin-bottom:8px}.label{color:#475569;font-size:11px;text-transform:uppercase;letter-spacing:.04em}.value{font-size:13px;font-weight:600}.total{font-size:20px;font-weight:700;color:#0f766e}.page-break{page-break-after:always}.summary{margin-bottom:16px;color:#334155;font-size:13px}</style></head><body onload="window.print()"><div class="summary">Generated ${sanitize(formatDate(new Date().toISOString()))} | Selected invoices: ${rows.length}</div>${invoices}</body></html>`
  )
  win.document.close()
}
...
export default function InvoicesPageMain() {
  ...
  const selectedRows = useMemo(
    () => displayRows.filter((r) => selectedIds.includes(r.id)),
      ...
      {isError ? (
        ...
        <div className="animate-pulse space-y-2.5 rounded-2xl border border-slate-200 bg-white p-4 dark:border-slate-700/60 dark:bg-slate-900">
          {Array.from({ length: 6 }).map((_, i) => (
            <div
              key={i}
              className="h-11 rounded-xl bg-slate-100 dark:bg-slate-800"
            ...
            <div className="space-y-4 px-5 py-5">
              <div className="grid grid-cols-2 gap-3">
                {[
                  {
                    label: "Invoice No",
                    value: selectedRow.invoice_no || "Pending",
                  },
              ...
              <div className="grid grid-cols-2 gap-3 text-sm">
                <div className="rounded-xl border border-slate-100 bg-slate-50 px-3 py-2.5 dark:border-slate-800 dark:bg-slate-950">
                  ...
                  <p className="font-semibold text-slate-800 dark:text-slate-200">
                    {selectedRow.affiliate_name || "Affiliate"}
                  </p>
