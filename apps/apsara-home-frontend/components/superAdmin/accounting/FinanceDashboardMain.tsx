...
export default function FinanceDashboardMain() {
  ...
  const byChannel = useMemo(
    () =>
      queue.reduce(
        (acc, r) => {
          acc[r.channel as "bank" | "gcash" | "maya"] =
            (acc[r.channel as "bank" | "gcash" | "maya"] || 0) + r.amount
          return acc
        },
          ...
          <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
            <div>
              ...
              <p className="mt-0.5 text-sm text-sky-300/80">
                Final payout release workspace — approved and ready to disburse
              </p>
            </div>
            <div className="sm:text-right">
              ...
              <p className="mt-0.5 text-xs text-sky-400/50">
                {count} request{count !== 1 ? "s" : ""} pending release
              </p>
            </div>
          </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        {(
          [
            ...
            {
              label: "Bank",
              amount: byChannel.bank,
              from: "from-indigo-500",
              to: "to-indigo-600",
              shadow: "shadow-indigo-500/20",
            },
          ] as const
        ).map(({ label, amount, from, to, shadow }) => (
          <div
            key={label}
            className={`relative overflow-hidden rounded-xl bg-gradient-to-br ${from} ${to} p-4 shadow-md ${shadow}`}
          >
            ...
            <div className="relative">
              ...
              <p className="mt-0.5 text-xs text-white/60">
                {queue.filter((r) => r.channel === label.toLowerCase()).length}{" "}
                items
              </p>
            </div>
          </div>
        ...
        {isError ? (
          ...
          <div className="flex flex-col items-center justify-center gap-3 py-14">
            ...
            <p className="text-xs text-slate-400 dark:text-slate-500">
              No approved requests pending release.
            </p>
          </div>
        )}
      </div>
