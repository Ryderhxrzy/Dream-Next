...
const SCENARIO_COPY: Record<
  Scenario,
  { label: string; helper: string; quantity: number }
> = {
  single: {
    label: "Single Order",
...
const money = (value: number) =>
  `PHP ${numberFormatter.format(Number.isFinite(value) ? value : 0)}`
const pv = (value: number) =>
  `${numberFormatter.format(Number.isFinite(value) ? value : 0)} PV`
...
export default function ProfitSimulationMain() {
  ...
  const computed = useMemo(() => {
    ...
    const estimatedMargin =
      grossSales - totalProductCost - shipping - cashAllocation
    const marginPercent =
      grossSales > 0 ? (estimatedMargin / grossSales) * 100 : 0
    const breakEvenCostPerUnit =
      qty > 0 ? Math.max(0, (grossSales - shipping - cashAllocation) / qty) : 0
    const allocationPercentOfSales =
      grossSales > 0 ? (cashAllocation / grossSales) * 100 : 0
  ...
  const allocationRows = [
    {
      label: "Cashback / e-GC",
      rate: rates.cashback,
      value: computed.cashback,
      color: "bg-emerald-500",
    },
    {
      label: "Unilevel Pool",
      rate: rates.unilevel,
      value: computed.unilevelPool,
      color: "bg-sky-500",
    },
    {
      label: "50K Points Reward",
      rate: rates.directEdge,
      value: computed.directEdge,
      color: "bg-amber-500",
    },
    {
      label: "Global Purchase Bonus",
      rate: rates.global,
      value: computed.globalBonus,
      color: "bg-violet-500",
    },
    {
      label: "Product Purchase Points",
      rate: rates.productPoints,
      value: computed.productPoints,
      color: "bg-slate-700",
    },
  ]
  ...
  const projectionRows = [10, 50, 100].map((orders) => {
    ...
    const cashAllocation =
      totalPv *
      ((rates.cashback + rates.unilevel + rates.directEdge + rates.global) /
        100)
    const margin =
      grossSales - computed.cost * orders - computed.shipping - cashAllocation
  ...
  const decisionStatus = !isProfitable
    ? "loss"
    : meetsTargetMargin
      ? "healthy"
      : "belowTarget"
  ...
  const suggestedMinSellingPrice =
    computed.qty > 0 && targetMarginRate < 100
      ? Math.max(
          0,
          (computed.totalProductCost +
            computed.shipping +
            computed.cashAllocation) /
      ...
      <div className="mx-auto max-w-7xl space-y-6">
            ...
            <div className="relative flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
              <div>
                ...
                <p className="mt-2 max-w-2xl text-sm leading-relaxed text-slate-300">
                  Test product PV, payout allocation, and estimated company
                  margin without changing live orders, wallets, or product
                  records.
                </p>
              </div>
              <button
          ...
          <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900">
            <div className="mb-5">
              ...
              <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">
                Select a product or enter values manually. Product cost is
                editable because true supplier cost may differ.
              </p>
            </div>
                  ...
                  <div className="mt-2 max-h-80 overflow-y-auto rounded-2xl border border-slate-200 dark:border-slate-700">
                    ...
                    {!productsFetching && productOptions.length === 0 && (
                      <div className="bg-slate-50 px-4 py-3 text-xs text-slate-500 dark:bg-slate-950 dark:text-slate-400">
                        No matching products. You can still enter the values
                        manually below.
                      </div>
                    )}
                    {!productsFetching &&
                            ...
                            <span className="flex min-w-0 items-center gap-3">
                              ...
                              <span className="min-w-0">
                                ...
                                <span className="mt-1 block truncate text-xs text-slate-500 dark:text-slate-400">
                                  ...
                                  {money(
                                    product.priceMember || product.priceSrp || 0
                                  )}{" "}
                                  - {pv(product.prodpv || 0)}
                                </span>
                              </span>
          ...
          <div className="space-y-6">
            <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
              {[
                ...
                [
                  "Total Cost",
                  money(computed.totalProductCost),
                  "cost x quantity",
                ],
                [
                  "Cash Allocations",
                  money(computed.cashAllocation),
                  "cashback + pools",
                ],
                [
                  "Company Margin",
                  money(computed.estimatedMargin),
                  `${percent(computed.marginPercent)} margin`,
                ],
            ...
            <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900">
              ...
              <div className="mt-5 grid gap-3 md:grid-cols-2 xl:grid-cols-4">
                <div className="rounded-2xl bg-slate-50 p-4 dark:bg-slate-950">
                  ...
                  <p
                    className={`mt-2 text-xl font-black ${meetsTargetMargin ? "text-emerald-600" : "text-amber-600"}`}
                  >
                ...
                <div className="rounded-2xl bg-slate-50 p-4 dark:bg-slate-950">
                  ...
                  <p className="mt-2 text-xl font-black">
                    {money(suggestedMinSellingPrice)}
                  </p>
                  <p className="mt-1 text-xs text-slate-500 dark:text-slate-400">
                    per unit at current cost/PV
                  </p>
                </div>
                <div className="rounded-2xl bg-slate-50 p-4 dark:bg-slate-950">
                  <p className="text-xs font-bold uppercase tracking-widest text-slate-400">
                  ...
                  <p className="mt-2 text-xl font-black">
                    {money(computed.breakEvenCostPerUnit)}
                  </p>
                </div>
                <div className="rounded-2xl bg-slate-50 p-4 dark:bg-slate-950">
                  <p className="text-xs font-bold uppercase tracking-widest text-slate-400">
                  ...
                  <p className="mt-2 text-xl font-black">
                    {percent(computed.allocationPercentOfSales)}
                  </p>
                </div>
              </div>
              ...
              <div className="mt-3 rounded-2xl bg-slate-50 p-4 dark:bg-slate-950">
                ...
                <p
                  className={`mt-2 text-xl font-black ${Math.abs(rateTotal - 100) < 0.01 ? "text-emerald-600" : "text-amber-600"}`}
                >
                  {percent(rateTotal)}
                </p>
              </div>
                ...
                <div className="mt-4 rounded-2xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-800 dark:border-amber-900 dark:bg-amber-950/30 dark:text-amber-200">
                  {!isProfitable && (
                    <p className="font-bold">
                      Warning: estimated margin is negative for this setup.
                    </p>
                  )}
                  {isProfitable && !meetsTargetMargin && (
                    <p className="font-bold">
                      Margin is profitable but below the target margin.
                    </p>
                  )}
                  {hasHighPayoutRisk && (
                    <p className="mt-1">
                      Cash allocation is above 30% of gross sales. Review
                      pricing/cost assumptions.
                    </p>
                  )}
                  {Math.abs(rateTotal - 100) >= 0.01 && (
                    <p className="mt-1">
                      Allocation rates do not total 100%. Check formula
                      settings.
                    </p>
                  )}
                </div>
        ...
        <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
            <div>
              ...
              <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">
                Product Purchase Points are shown for PV allocation. Cash
                profitability deducts cash/liability pools only.
              </p>
            </div>
            <div className="rounded-2xl border border-slate-200 px-4 py-3 text-sm dark:border-slate-700">
              <span className="font-bold">Total PV:</span>{" "}
              {pv(computed.totalPv)}
            </div>
          </div>
            ...
            {allocationRows.map((row) => {
              const width =
                computed.totalAllocation > 0
                  ? Math.min((row.value / computed.totalAllocation) * 100, 100)
                  : 0
              return (
                <div
                  key={row.label}
                ...
                >
                  <div className="flex items-center justify-between gap-2">
                    ...
                    <span className="text-sm font-black tabular-nums">
                      {row.value === computed.productPoints
                        ? pv(row.value)
                        : money(row.value)}
                    </span>
                  </div>
          ...
          <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900">
            <div className="flex items-center justify-between gap-3">
              ...
              <span className="rounded-full bg-sky-50 px-3 py-1 text-xs font-black text-sky-700 dark:bg-sky-950 dark:text-sky-300">
                {rates.unilevel}% / 10 ={" "}
                {computed.unilevelPerLevelRate.toFixed(2)}%
              </span>
            </div>
              ...
              {projectionRows.map((row) => (
                ...
                >
                  <div className="flex items-center justify-between">
                    ...
                    <p
                      className={`text-sm font-black ${row.margin >= 0 ? "text-emerald-600" : "text-rose-600"}`}
                    >
        ...
        <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900">
          <div className="mb-5">
            ...
            <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">
              These controls affect this simulation only. They do not change
              live system rates.
            </p>
          </div>
