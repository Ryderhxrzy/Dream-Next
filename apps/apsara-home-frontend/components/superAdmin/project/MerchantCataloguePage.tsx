              ...
              <>
                {selected.image ? (
                  <img
                    src={selected.image}
                    alt={selected.name}
                    className="h-7 w-7 rounded-lg object-contain ring-1 ring-slate-200"
                  />
                ) : (
          ...
          >
            <div className="p-2">
              <input
                ...
                className="w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-sm text-slate-800 outline-none focus:border-violet-400 dark:border-slate-700 dark:bg-slate-800 dark:text-white dark:focus:border-violet-400"
              />
            </div>
              ...
              {loading ? (
                ...
                filtered.map((brand) => (
                  <button
                    ...
                    className={`flex w-full items-center gap-3 px-4 py-2.5 text-left text-sm transition hover:bg-slate-50 dark:hover:bg-slate-800 ${selected?.id === brand.id ? "font-semibold text-violet-700 dark:text-violet-400" : "text-slate-700 dark:text-slate-300"}`}
                  >
                    {brand.image ? (
                      <img
                        src={brand.image}
                        alt={brand.name}
                        className="h-7 w-7 rounded-lg object-contain ring-1 ring-slate-200 dark:ring-slate-700"
                      />
                    ) : (
                      <div className="flex h-7 w-7 shrink-0 items-center justify-center rounded-lg bg-slate-100 text-xs font-bold text-slate-500 dark:bg-slate-700 dark:text-slate-400">
                        {brand.name[0]?.toUpperCase()}
                      </div>
                    )}
...
export default function MerchantCataloguePage() {
  ...
  const catalogueItem = selectedBrand
    ? (catalogueData?.items ?? []).find(
        (item) => item.key === brandItemKey(selectedBrand.id)
      )
    : null
    ...
    try {
      ...
      if (catalogueItem) {
        ...
        await createItem({
          type: "merchant-catalogue",
          data: {
            key: brandItemKey(selectedBrand.id),
      ...
      >
        ...
        <div className="relative max-w-[55%]">
          ...
          <p className="mt-1.5 text-sm text-slate-500">
            Select a merchant brand, upload catalogue pages or videos,
            <br />
        ...
        <div className="flex flex-col items-center gap-4 rounded-2xl border border-dashed border-slate-200 py-24">
          ...
          <div className="text-center">
            ...
            <p className="mt-1 text-sm text-slate-400">
              Select a merchant brand above to manage their catalogue
            </p>
          </div>
        </div>
      )}
