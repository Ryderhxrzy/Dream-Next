...
const describeAccess = (
  ...
  if (role === "admin")
    return permissionCount > 0
      ? `${permissionCount} permission areas enabled`
...
export default function AdminProfilePageMain() {
  ...
  const [updateAdminMe, { isLoading: isSavingAvatar }] =
    useUpdateAdminMeMutation()
  ...
  const isRefreshingAdminIdentity =
    Boolean(sessionAccessToken) && !adminMe && (isLoading || isFetching)
  const name =
    String(adminMe?.name ?? fallbackUser?.name ?? "").trim() ||
    (isRefreshingAdminIdentity ? "Refreshing admin profile..." : "Admin User")
  const email =
    String(adminMe?.email ?? fallbackUser?.email ?? "").trim() ||
    (isRefreshingAdminIdentity
      ? "Reloading email and permissions..."
      : "No email on file")
  const role = adminMe?.role ?? fallbackUser?.role ?? ""
  const userLevelId = Number(
    adminMe?.user_level_id ?? fallbackUser?.userLevelId ?? 0
  )
  ...
  const permissions = normalizeAdminPermissions(
    adminMe?.admin_permissions ?? fallbackUser?.adminPermissions ?? []
  )
  ...
  const handleAvatarFileChange = async (
    ...
    if (!file.type.startsWith("image/")) {
      showErrorToast(
        "Please choose a valid image file for the admin profile picture."
      )
      return
    }
    ...
    try {
      ...
      showErrorToast(
        getApiErrorMessage(error, "Failed to update the admin profile picture.")
      )
    } finally {
  ...
  const handleRemoveAvatar = async () => {
    try {
      ...
      showErrorToast(
        getApiErrorMessage(error, "Failed to remove the admin profile picture.")
      )
    }
  }
  ...
  return (
    <section className="space-y-6">
      ...
      >
        <div
          className={`absolute inset-x-0 top-0 h-44 bg-gradient-to-r ${getRoleTone(role)}`}
        />
            ...
            <div className="flex items-start gap-4">
              ...
              <div className="pt-2 text-white">
                ...
                <p className="mt-2 max-w-2xl text-sm text-white/85">
                  Central view of your admin identity, access scope, and
          ...
          <div className="mt-6 grid gap-4 lg:grid-cols-[1.3fr_0.7fr]">
            <div className="rounded-[1.75rem] border border-slate-200 bg-white/96 p-5 shadow-lg shadow-slate-200/50 backdrop-blur">
              <div className="flex items-center justify-between gap-3">
                <div>
                  ...
                  <p className="mt-1 text-sm text-slate-500">
                    Your active admin identity and login-facing profile data.
                  </p>
                </div>
                {isLoading ? (
              ...
              <div className="mt-5 grid gap-4 md:grid-cols-2">
                ...
                <ProfileField
                  label="Admin ID"
                  value={adminMe?.id ? String(adminMe.id) : "Session linked"}
                />
                <ProfileField
                  label="Supplier Link"
                  value={supplierId ? `Supplier #${supplierId}` : "Not linked"}
                />
                <ProfileField
                  ...
                  value={
                    userLevelId === 1 || role === "super_admin"
                      ? "All areas"
                      : String(visiblePermissionCards.length)
                  }
                />
              </div>
            </div>
            ...
            <div className="rounded-[1.75rem] border border-slate-200 bg-[linear-gradient(180deg,#ffffff,rgba(248,250,252,0.94))] p-5 shadow-sm">
              ...
              <p className="mt-2 text-sm text-slate-500">
                Upload a new admin avatar and use it across the admin header,
                ...
                <div className="flex items-center gap-4">
                  <div className="relative h-20 w-20 overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
                    {avatarUrl ? (
                      ...
                      <div
                        className={`flex h-full w-full items-center justify-center bg-gradient-to-br ${getRoleTone(role)} text-xl font-bold text-white`}
                      >
                        {getInitials(name)}
                      </div>
                    )}
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="mt-1 text-xs leading-5 text-slate-500">
                      Recommended: square image, clear face or brand avatar, and
                      under 5MB.
                    </p>
                  </div>
                </div>
              ...
              <div className="mt-4 space-y-3">
                ...
                >
                  <div>
                    ...
                    <p className="mt-1 text-xs text-slate-500">
                      Open your admin-side preferences and platform config.
                    </p>
                  </div>
                  <span className="text-slate-400 transition group-hover:text-slate-700">
                    →
                ...
                >
                  <div>
                    ...
                    <p className="mt-1 text-xs text-slate-500">
                      Manage how operational updates reach your account.
                    </p>
                  </div>
                  <span className="text-slate-400 transition group-hover:text-slate-700">
                    →
      ...
      <div className="grid gap-6 xl:grid-cols-[1.1fr_0.9fr]">
        ...
        >
          <div className="flex items-center justify-between gap-3">
            <div>
              ...
              <p className="mt-1 text-sm text-slate-500">
                The modules and tools currently available to this admin account.
              </p>
            </div>
            <span className="rounded-full bg-slate-100 px-3 py-1 text-xs font-semibold text-slate-600">
              {userLevelId === 1 || role === "super_admin"
                ? "All modules"
                : `${visiblePermissionCards.length} enabled`}
            </span>
          </div>
        ...
        >
          ...
          <p className="mt-1 text-sm text-slate-500">
            A quick readout of how this admin account is scoped inside the
            portal.
          </p>
          ...
          <div className="mt-5 space-y-4">
            ...
            <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
              ...
              <ul className="mt-3 space-y-2 text-sm text-slate-600">
                ...
                <li>
                  Permissions are sourced from the live admin identity endpoint.
                </li>
                <li>
                  {supplierId
                    ? `Linked to supplier ID ${supplierId}.`
                    : "No supplier binding is attached to this profile."}
                </li>
              </ul>
            </div>
            ...
            <div className="rounded-2xl border border-dashed border-slate-300 bg-slate-50/70 p-4">
              ...
              <p className="mt-2 text-sm leading-6 text-slate-600">
                Next we can add editable profile details, password change
                actions, and recent admin activity on the same page.
              </p>
            </div>
          </div>
        </motion.div>
