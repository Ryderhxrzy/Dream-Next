...
export default function WebPageItemsManager({
  type,
  title,
  description,
}: Props) {
    ...
    <div className="space-y-5 dark:bg-slate-950 dark:text-slate-100">
      ...
      <div className="space-y-3 rounded-2xl border border-slate-100 bg-white p-4 shadow-sm">
                  ...
                  rows.map((row) => (
                    ...
                    >
                      ...
                      <td className="px-3 py-2.5">
                        <span
                          className={`inline-flex rounded-full px-2 py-1 text-xs font-semibold ${row.is_active ? "bg-emerald-100 text-emerald-700" : "bg-slate-200 text-slate-700"}`}
                        >
        ...
        <div className="flex items-center justify-between text-xs text-slate-500">
          <p>
            Showing {data?.meta?.from ?? 0} - {data?.meta?.to ?? 0} of{" "}
            {data?.meta?.total ?? 0}
          </p>
          <div className="flex items-center gap-2">
            <button
