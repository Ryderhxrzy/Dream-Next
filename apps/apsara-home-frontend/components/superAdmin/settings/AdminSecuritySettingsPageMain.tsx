...
export default function AdminSecuritySettingsPageMain() {
  ...
  const [forcePasswordChangeEnabled, setForcePasswordChangeEnabled] =
    useState(true)
  ...
  useEffect(() => {
    ...
    setRegistrationOtpEnabled(
      Boolean(settings.registration_otp_enabled ?? true)
    )
    setStrictPasswordPolicy(Boolean(settings.strict_password_policy ?? true))
    setForcePasswordChangeEnabled(
      Boolean(settings.force_password_change_enabled ?? true)
    )
    ...
    <div className="space-y-8">
      <div className="relative overflow-hidden rounded-3xl border border-slate-200 bg-gradient-to-br from-white via-slate-50 to-slate-100 p-6 shadow-sm dark:border-slate-700 dark:from-slate-800 dark:via-slate-800 dark:to-slate-900">
        ...
        <p className="mt-2 max-w-3xl text-sm leading-relaxed text-slate-600 dark:text-slate-300">
          Adjust authentication limits, session safety, and password rules for
          the admin platform.
        </p>
      </div>
          ...
          <div className="flex items-center justify-between rounded-2xl border border-slate-200 bg-gradient-to-br from-slate-50 to-white px-4 py-3 dark:border-slate-700 dark:from-slate-700/50 dark:to-slate-800">
            <div>
              ...
              <p className="text-xs text-slate-500 dark:text-slate-400">
                Optional security layer for admin logins.
              </p>
            </div>
            <button
              type="button"
      ...
      <div className="flex flex-wrap items-center justify-end gap-3">
        <button
          ...
          onClick={async () => {
            ...
            const passwordMinLengthValue = Number.parseInt(
              passwordMinLength,
              10
            )

            if (
              !Number.isFinite(sessionTimeoutValue) ||
              sessionTimeoutValue < 5 ||
              sessionTimeoutValue > 1440
            ) {
              showErrorToast(
            ...
            if (
              !Number.isFinite(maxLoginAttemptsValue) ||
              maxLoginAttemptsValue < 1 ||
              maxLoginAttemptsValue > 20
            ) {
              showErrorToast("Max login attempts must be between 1 and 20.")
              return
            }
            if (
              !Number.isFinite(passwordMinLengthValue) ||
              passwordMinLengthValue < 6 ||
              passwordMinLengthValue > 64
            ) {
              showErrorToast(
            ...
            try {
              ...
              setSessionTimeout(
                String(response.settings.session_timeout_minutes)
              )
              setMaxLoginAttempts(String(response.settings.max_login_attempts))
              setPasswordMinLength(
                String(response.settings.password_min_length)
              )
              setEnable2fa(Boolean(response.settings.enable_2fa))
              setRegistrationOtpEnabled(
                Boolean(response.settings.registration_otp_enabled)
              )
              setStrictPasswordPolicy(
                Boolean(response.settings.strict_password_policy)
              )
              setForcePasswordChangeEnabled(
                Boolean(response.settings.force_password_change_enabled)
              )
