...
const modalDescriptions: Record<
  MembersStatCardKey,
  { intro: string; emptyTitle: string; emptyDescription: string }
> = {
  total_members: {
  ...
  pending: {
    intro:
      "Live pending and KYC-related member records loaded directly from the database.",
    emptyTitle: "No pending or KYC members found.",
    emptyDescription:
      "There are no pending or KYC-review records to show right now.",
  },
  blocked: {
  ...
  new_members: {
    intro:
      "Live recent registrations loaded directly from the database for the selected period.",
    emptyTitle: "No recent members found.",
    emptyDescription:
      "There are no registration records to show for the selected period.",
  },
  total_spent: {
    intro:
      "These are the actual members contributing to Total Spent, ranked from the database.",
    emptyTitle: "No spending records found.",
    emptyDescription:
      "No members with recorded spending were returned from the database.",
  },
  total_earnings: {
    intro:
      "These are the actual members contributing to Total Earnings, ranked from the database.",
    emptyTitle: "No earnings records found.",
    emptyDescription:
      "No members with recorded earnings were returned from the database.",
  },
  total_referrals: {
    intro:
      "These are the actual members contributing to Total Referrals, ranked from the database.",
    emptyTitle: "No referral records found.",
    emptyDescription:
      "No members with referral counts were returned from the database.",
  },
}
...
const MembersPageMain = ({
  ...
  const [expandedReferralMembers, setExpandedReferralMembers] = useState<
    number[]
  >([])
  ...
  const [stableData, setStableData] = useState<MembersResponse | null>(
    initialData
  )
  const [stableStats, setStableStats] = useState<MembersStatsResponse | null>(
    initialStats
  )
  ...
  const shouldSkipInitialMembersRefetch = Boolean(
    initialData && isUsingDefaultView
  )
  const shouldSkipInitialStatsRefetch = Boolean(
    initialStats && statsPeriod === "7d"
  )

  const { data: statsData } = useGetMembersStatsQuery(
    { period: statsPeriod },
    {
  ...
  const effectiveData = shouldSkipInitialMembersRefetch
    ? (initialData ?? null)
    : (data ?? stableData ?? initialData ?? null)
  ...
  const loadStatPage = useCallback(
    async (
      stat: MembersStatCardKey,
      nextPage: number,
      append: boolean,
      searchValue?: string
    ) => {
      ...
      try {
        ...
        setStatModalMembers((prev) =>
          append ? [...prev, ...response.members] : response.members
        )
      } catch (error) {
        if (requestId !== statModalRequestIdRef.current) {
          return
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      const rows = exportRows.map((member) => [
        ...
        member.name,
        member.email,
        member.referredByName ?? member.referredByUsername ?? "",
        member.status,
  ...
  const selectedStatMeta = selectedStatCard
    ? modalDescriptions[selectedStatCard]
    : null
      ...
      >
        <div>
          ...
          <p className="mt-0.5 text-sm text-slate-500 dark:text-slate-300">
            Manage member profiles, tiers and activities
          </p>
        </div>
        <Button
            ...
            <div className="flex items-start justify-between gap-4 border-b border-slate-100 px-5 py-4 dark:border-slate-800">
              <div>
                ...
                <p className="mt-0.5 text-xs text-slate-500">
                  {selectedStatMeta?.intro ??
                    "Live records loaded from the database."}
                </p>
                {statModalMeta && (
              ...
              {isStatModalLoading ? (
                <div className="space-y-3 px-5 py-5">
                  {Array.from({ length: 6 }).map((_, index) => (
                    <div
                      key={index}
                      className="animate-pulse rounded-2xl border border-slate-100 px-4 py-3"
                    >
                ...
                <div className="px-5 py-12 text-center">
                  ...
                  <p className="mt-1 text-xs text-slate-400">
                    Try reopening the modal to fetch the live records again.
                  </p>
                </div>
                ...
                <div className="px-5 py-12 text-center">
                  <p className="text-sm font-medium text-slate-600">
                    {selectedStatMeta?.emptyTitle ?? "No records found."}
                  </p>
                  <p className="mt-1 text-xs text-slate-400">
                    {selectedStatMeta?.emptyDescription ??
                      "No live records were returned from the database."}
                  </p>
                </div>
              )}
            </div>
                ...
                {usePaginationControls && selectedStatCard && statModalMeta ? (
                    ...
                    <Button
                      onPress={() => {
                        const nextPage = Math.max(
                          1,
                          (statModalMeta.current_page ?? statModalPage) - 1
                        )
                      ...
                      isDisabled={
                        isStatModalLoading || statModalMeta.current_page <= 1
                      }
                  ...
                  <Button
                    onPress={() =>
                      loadStatPage(
                        selectedStatCard,
                        (statModalMeta?.current_page ?? 1) + 1,
                        true,
                        debouncedStatModalSearch
                      )
                    }
