...
const STATUS_CONFIG: Record<
  MemberKycStatus,
  { label: string; dot: string; badge: string; tab: string }
> = {
  pending_review: {
...
function DocCard({ label, url }: { label: string; url?: string | null }) {
  ...
  return (
    ...
    >
      ...
      <span
        className={`text-[11px] font-medium ${hasUrl ? "text-teal-600 group-hover:underline" : "text-slate-400"}`}
      >
...
function DetailModal({
  ...
  const isPending =
    item.status === "pending_review" || item.status === "on_hold"
        ...
        <div className="px-6 py-5 max-h-[65vh] overflow-y-auto space-y-5">
          ...
          <div className="grid grid-cols-2 gap-3">
            {[
              ...
              {
                label: "Contact",
                value: item.contact_number || "Not provided",
              },
      ...
      <td className="px-4 py-3.5 text-xs text-slate-500 whitespace-nowrap">
        ...
        {row.reviewed_at && (
          <p className="mt-0.5 text-slate-400">
            Rev: {formatDate(row.reviewed_at)}
          </p>
        )}
      </td>
