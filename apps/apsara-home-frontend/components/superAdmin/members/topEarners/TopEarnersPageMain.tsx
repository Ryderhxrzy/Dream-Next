...
import { php, SortKey, TIERS, TopEarner } from "./types"
...
export default function TopEarnersPageMain() {
  ...
  return (
    <div className="space-y-6">
      ...
      >
        <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-slate-800 via-slate-800 to-slate-900 shadow-xl dark:from-slate-900 dark:to-black">
            ...
            <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
              <div>
                <div className="mb-2 flex flex-wrap items-center gap-2">
                  ...
                  <span className="inline-flex items-center gap-1.5 rounded-full border border-yellow-400/30 bg-yellow-400/15 px-2.5 py-1 text-[10px] font-semibold text-yellow-300">
                ...
                <p className="mt-0.5 text-sm text-slate-400">
                  Members ranked by total commission earnings
                </p>
              </div>
              <div className="flex items-center gap-2 self-start sm:self-auto">
                {summary && (
                  <div className="mr-2 hidden text-right sm:block">
                    <p className="mb-0.5 text-[10px] font-bold tracking-widest text-slate-500 uppercase">
                      Total Pool
                    </p>
                    <p className="text-xl font-black text-white">
                      {php(summary.totalEarnings)}
                    </p>
                  </div>
                )}
                <button
                ...
                {[
                  {
                    ...
                    value: php(summary.totalEarnings),
                    color: "text-yellow-300",
                  },
                  {
                    label: "Active Earners",
                    value: String(summary.activeEarners),
                    color: "text-teal-300",
                  },
                  {
                    label: "Avg / Member",
                    value: php(Math.round(summary.avgEarnings)),
                    color: "text-sky-300",
                  },
                  {
                    label: "Top Earner",
                    value: php(summary.topEarnerAmount),
