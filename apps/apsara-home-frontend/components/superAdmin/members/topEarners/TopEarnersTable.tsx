...
function MemberAvatar({ src, name, bgCls, size }: { src?: string; name: string; bgCls: string; size: string }) {
  ...
  if (src && !err) {
    return <img src={src} alt={name} onError={() => setErr(true)} className={`rounded-full object-cover shrink-0 ${size}`} />
  }
  return (
...
const RANK_STYLE: Record<number, { wrapper: string; text: string }> = {
  1: { wrapper: 'bg-gradient-to-br from-amber-400 to-yellow-500 shadow-amber-200 dark:shadow-amber-900/40 shadow-md',  text: 'text-white' },
  2: { wrapper: 'bg-gradient-to-br from-slate-400 to-slate-500 shadow-slate-200 dark:shadow-slate-900/40 shadow-md',   text: 'text-white' },
  3: { wrapper: 'bg-gradient-to-br from-orange-400 to-amber-500 shadow-orange-200 dark:shadow-orange-900/40 shadow-md', text: 'text-white' },
}
  ...
  if (medal && style) {
    return (
      <div className="flex flex-col items-center gap-0.5">
        <div className={`flex h-9 w-9 items-center justify-center rounded-full text-base ${style.wrapper}`}>
          {medal.crown}
        </div>
        <span className="text-[9px] font-black uppercase tracking-widest text-slate-400 dark:text-slate-500">{medal.label}</span>
      </div>
    )
  }
  return (
    ...
    >
      ...
      <td className="px-4 py-4">
        <div className="flex items-center gap-3">
          <MemberAvatar src={earner.avatar} name={earner.name} bgCls={avatarBg} size="h-10 w-10 text-xs" />
          <div className="min-w-0">
            <p className="text-sm font-bold leading-tight text-slate-800 dark:text-slate-100">{earner.name}</p>
            <p className="mt-0.5 max-w-40 truncate text-[11px] text-slate-400 dark:text-slate-500">{earner.email}</p>
          </div>
        </div>
      </td>
      ...
      <td className="px-4 py-4 min-w-42.5">
        <p className="text-sm font-black tabular-nums text-teal-700 dark:text-teal-300">{php(earner.earnings)}</p>
        ...
        <p className="mt-1 text-[10px] font-medium text-slate-400 dark:text-slate-500">{pct.toFixed(0)}% of top</p>
      </td>
      ...
      <td className="px-4 py-4">
        <div className="inline-flex items-center gap-1.5 rounded-xl bg-indigo-50 px-2.5 py-1.5 dark:bg-indigo-900/20">
          ...
          <span className="text-xs font-bold tabular-nums text-indigo-700 dark:text-indigo-300">{earner.orders}</span>
        </div>
      </td>
      ...
      <td className="px-4 py-4">
        <div className="inline-flex items-center gap-1.5 rounded-xl bg-violet-50 px-2.5 py-1.5 dark:bg-violet-900/20">
          ...
          <span className="text-xs font-bold tabular-nums text-violet-700 dark:text-violet-300">{earner.referrals}</span>
        </div>
      </td>
      ...
      <td className="px-4 py-4">
        <div className="inline-flex items-center gap-1.5 rounded-xl bg-slate-50 px-2.5 py-1.5 dark:bg-slate-800">
          ...
          <span className="text-xs font-bold tabular-nums text-slate-600 dark:text-slate-300">{php(earner.totalSpent)}</span>
        </div>
      </td>

      {/* Status */}
      <td className="px-4 py-4">
        <div className={`inline-flex items-center gap-1.5 rounded-full border px-2.5 py-1 text-[11px] font-semibold bg-white dark:bg-transparent border-slate-200 dark:border-slate-700`}>
    ...
    >
      ...
      <div className="flex items-center justify-between gap-3 border-b border-slate-100 dark:border-slate-700/60 bg-slate-50/80 dark:bg-slate-800/50 px-5 py-4">
        <div className="flex items-center gap-2.5">
          ...
          <h2 className="text-sm font-bold text-slate-700 dark:text-slate-200">Full Rankings</h2>
          ...
          <thead>
            <tr className="border-b border-slate-100 dark:border-slate-700/60 bg-white dark:bg-slate-900">
              {[
                ...
                <th key={label} className={`px-4 py-3 text-[10px] font-black uppercase tracking-widest text-slate-400 dark:text-slate-500 ${cls}`}>
                  {label}
                </th>
              ))}
            </tr>
            ...
            {isLoading ? (
              Array.from({ length: 8 }).map((_, i) => (
                <tr key={i} className="animate-pulse">
                  <td className="px-4 py-4"><div className="h-9 w-9 rounded-full bg-slate-100 dark:bg-slate-800 mx-auto" /></td>
                  <td className="px-4 py-4">
                    <div className="flex items-center gap-3">
              ...
              <tr>
                <td colSpan={9} className="px-5 py-16 text-center">
                  <div className="flex flex-col items-center gap-2">
                    ...
                    <p className="text-sm font-semibold text-rose-500">Unable to load top earners.</p>
                  </div>
                </td>
              </tr>
            ) : earners.length === 0 ? (
      ...
      <div className="flex items-center justify-between border-t border-slate-100 dark:border-slate-700/60 bg-slate-50/60 dark:bg-slate-800/30 px-5 py-3.5">
        <span className="text-xs text-slate-500 dark:text-slate-400">
          Showing <span className="font-bold text-slate-700 dark:text-slate-200">{earners.length}</span> members
        </span>
