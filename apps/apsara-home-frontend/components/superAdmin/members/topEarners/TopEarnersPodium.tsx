...
function MemberAvatar({
  ...
  if (src && !err) {
    return <img src={src} alt={name} onError={() => setErr(true)} className={`rounded-full object-cover ${size} ${ringCls}`} />
  }
  return (
    <div className={`flex items-center justify-center rounded-full font-bold text-white ${size} ${ringCls} ${bgCls}`}>
      {initials}
    </div>
  )
}
...
function AwardLaurel({ color, size = 170 }: { color: string; size?: number }) {
  ...
  const Leaf = ({ deg, i }: { deg: number; i: number }) => {
    ...
    return <ellipse cx={x} cy={y} rx={lx * scale} ry={ly * scale} transform={`rotate(${deg} ${x} ${y})`} fill={color} opacity={0.82} />
  }
  const leftStart  = { x: c + orbitR * Math.cos(toRad(215)), y: c + orbitR * Math.sin(toRad(215)) }
  const leftEnd    = { x: c + orbitR * Math.cos(toRad(95)),  y: c + orbitR * Math.sin(toRad(95))  }
  const rightStart = { x: c + orbitR * Math.cos(toRad(325)), y: c + orbitR * Math.sin(toRad(325)) }
  const rightEnd   = { x: c + orbitR * Math.cos(toRad(85)),  y: c + orbitR * Math.sin(toRad(85))  }
  const leftCtrl   = { x: c + orbitR * Math.cos(toRad(155)), y: c + orbitR * Math.sin(toRad(155)) }
  const rightCtrl  = { x: c + orbitR * Math.cos(toRad(25)),  y: c + orbitR * Math.sin(toRad(25))  }
  return (
    <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`} fill="none">
      <path d={`M ${leftStart.x} ${leftStart.y} Q ${leftCtrl.x} ${leftCtrl.y} ${leftEnd.x} ${leftEnd.y}`} stroke={color} strokeWidth="1.5" strokeOpacity="0.45" fill="none" />
      <path d={`M ${rightStart.x} ${rightStart.y} Q ${rightCtrl.x} ${rightCtrl.y} ${rightEnd.x} ${rightEnd.y}`} stroke={color} strokeWidth="1.5" strokeOpacity="0.45" fill="none" />
      <circle cx={c} cy={c + orbitR + 6} r="3.5" fill={color} opacity="0.55" />
      {leftDeg.map((deg, i)  => <Leaf key={`l${i}`} deg={deg} i={i} />)}
      {rightDeg.map((deg, i) => <Leaf key={`r${i}`} deg={deg} i={i} />)}
    </svg>
  )
}
...
const CONFETTI = [
  ...
  { x: '88%', y: '27%', color: '#60a5fa', w: 6, h: 4, r: 15,  dy: -10, dur: 2.5 },
  { x: '13%', y: '68%', color: '#a78bfa', w: 4, h: 6, r: 73,  dy: -6,  dur: 4.0 },
  { x: '79%', y: '63%', color: '#fb923c', w: 6, h: 4, r: 38,  dy: -14, dur: 3.1 },
  { x: '48%', y: '2%',  color: '#34d399', w: 5, h: 5, r: 20,  dy: -9,  dur: 2.9 },
  { x: '68%', y: '78%', color: '#fbbf24', w: 7, h: 5, r: 52,  dy: -11, dur: 3.5 },
  { x: '24%', y: '14%', color: '#f472b6', w: 4, h: 6, r: 33,  dy: -7,  dur: 3.8 },
  { x: '72%', y: '42%', color: '#60a5fa', w: 4, h: 4, r: 48,  dy: -13, dur: 2.7 },
  { x: '35%', y: '88%', color: '#fb923c', w: 5, h: 4, r: 18,  dy: -5,  dur: 4.2 },
  { x: '55%', y: '75%', color: '#a78bfa', w: 4, h: 6, r: 66,  dy: -10, dur: 3.3 },
]
    ...
    <div className="pointer-events-none absolute inset-0 overflow-hidden rounded-2xl">
      {CONFETTI.map((d, i) => (
        <motion.div
          key={i}
          className="absolute"
          style={{ left: d.x, top: d.y, width: d.w, height: d.h, backgroundColor: d.color, borderRadius: 1, opacity: 0.75 }}
          animate={{
            y: [0, d.dy, 0],
            rotate: [d.r, d.r + 25, d.r - 15, d.r],
            opacity: [0.75, 0.9, 0.6, 0.75],
          }}
          transition={{ duration: d.dur, repeat: Infinity, ease: 'easeInOut', delay: i * 0.18 }}
        />
      ))}
    </div>
  )
...
const RANK_STYLE = {
  1: {
    ...
    badge:       'bg-gradient-to-r from-amber-400 to-yellow-500 text-white shadow-amber-200 dark:shadow-none',
    laurel:      '#f59e0b',
    earningsBox: 'bg-yellow-50 border-yellow-300 dark:bg-yellow-500/10 dark:border-yellow-500/30',
  ...
  3: {
    ...
    laurel:      '#fb923c',
    earningsBox: 'bg-orange-50 border-orange-200 dark:bg-orange-500/10 dark:border-orange-500/20',
...
function PodiumCard({ earner, rank }: PodiumCardProps) {
  ...
  return (
    <motion.div
      ...
      initial="hidden"
      animate="visible"
      transition={{ delay: cardDelay, type: 'spring', stiffness: 160, damping: 18 }}
    ...
    >
      ...
      <div className="relative z-10 flex w-full flex-col items-center">
        ...
        <motion.div
          ...
          transition={{ delay: cardDelay + 0.15, type: 'spring', stiffness: 300, damping: 15 }}
        ...
        <div className="relative mb-3" style={{ width: 170, height: 170 }}>
          ...
          <motion.div
            ...
            transition={{ duration: 5, repeat: Infinity, ease: 'easeInOut', delay: cardDelay }}
          >
          ...
          <div className="absolute inset-0 flex items-center justify-center">
            <div className="relative">
              ...
              <motion.div
                ...
                animate={{ scale: 1, opacity: 1 }}
                transition={{ delay: cardDelay + 0.3, type: 'spring', stiffness: 200, damping: 14 }}
              >
                <MemberAvatar
                  src={earner.avatar}
                  name={earner.name}
        ...
        >
          <p className="text-sm font-black leading-tight text-slate-800 dark:text-white">{earner.name}</p>
          <p className="mt-0.5 max-w-40 truncate text-xs text-slate-400 dark:text-slate-500">{earner.email}</p>
        </motion.div>
        ...
        <motion.div
          ...
          transition={{ delay: cardDelay + 0.5, type: 'spring', stiffness: 250 }}
        >
        ...
        >
          <p className={`mb-0.5 text-[10px] font-semibold uppercase tracking-wide ${style.earningsLbl}`}>
            Total Earnings
          </p>
        ...
        <div className="mt-2.5 w-full grid grid-cols-3 gap-1.5">
          {[
            {
              icon: <ShoppingBag className="mx-auto mb-1 h-3.5 w-3.5 text-slate-400 dark:text-slate-500" />,
              label: 'Orders', value: earner.orders, custom: null,
            },
            {
              icon: <Users className="mx-auto mb-1 h-3.5 w-3.5 text-slate-400 dark:text-slate-500" />,
              label: 'Referrals', value: earner.referrals, custom: null,
            },
            {
              icon: <span className={`mx-auto mb-1 block h-3.5 w-3.5 rounded-full ${status.dot}`} />,
              label: 'Status', value: null,
              custom: <p className={`mt-0.5 text-xs font-bold ${status.text}`}>{status.label}</p>,
            },
          ].map((stat, si) => (
            <motion.div
              key={stat.label}
              ...
              animate={{ opacity: 1, y: 0 }}
              transition={{ delay: cardDelay + 0.62 + si * 0.07, type: 'spring', stiffness: 220 }}
            >
              {stat.icon}
              <p className="text-[10px] leading-none text-slate-400 dark:text-slate-500">{stat.label}</p>
              {stat.custom ?? <p className="mt-0.5 text-xs font-bold text-slate-700 dark:text-slate-200">{stat.value}</p>}
            </motion.div>
          ))}
        </div>
  ...
  if (isLoading) {
    return (
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
        {[2, 1, 3].map(i => (
          <div key={i} className="animate-pulse rounded-2xl border border-slate-200 bg-white p-5 dark:border-slate-700 dark:bg-slate-800">
    ...
    <AnimatePresence>
      <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }}>
        <div className="mb-3 flex items-center gap-2">
          <span className="text-[10px] font-bold uppercase tracking-widest text-slate-400 dark:text-slate-500">Podium</span>
          <span className="h-px flex-1 bg-slate-100 dark:bg-slate-800" />
          <span className="text-xs text-slate-400 dark:text-slate-500">{top3.length} of {top3.length} shown</span>
        </div>
