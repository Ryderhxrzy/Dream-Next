...
function ShippingRateFormFields({
  ...
  const selectedProvince =
    provinceOptions.find(
      (option) => normalizeLocationKey(option.name) === normalizedProvince
    ) ?? null
  const selectedProvinceCode = selectedProvince?.code ?? ""
  const matchedCityCode =
    cityOptions.find(
      (option) => normalizeLocationKey(option.name) === normalizedCity
    )?.code ?? ""
  ...
  const selectedProvinceValue = showCurrentProvinceFallback
    ? CURRENT_PROVINCE_FALLBACK_CODE
    : selectedProvinceCode
  const selectedCityValue = showCurrentCityFallback
    ? CURRENT_CITY_FALLBACK_CODE
    : matchedCityCode
      ...
      <div>
        ...
        <div className="relative">
          ...
          >
            ...
            {showCurrentProvinceFallback ? (
              <option value={CURRENT_PROVINCE_FALLBACK_CODE}>
                {form.province}
              </option>
            ) : null}
      ...
      <div>
        ...
        <div className="relative">
          ...
          >
            ...
            {showCurrentCityFallback ? (
              <option value={CURRENT_CITY_FALLBACK_CODE}>{form.city}</option>
            ) : null}
      ...
      <label className="flex items-center justify-between gap-4 rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 dark:border-slate-700 dark:bg-slate-800/80">
        <div>
          ...
          <p className="text-xs text-slate-500 dark:text-slate-400">
            Only active rates are used for manual checkout.
          </p>
        </div>
        <button
          type="button"
...
function ButtonSpinner({ tone = "light" }: { tone?: "light" | "danger" }) {
  ...
  return (
    <span
      className={`inline-block h-4 w-4 animate-spin rounded-full border-2 ${borderClass}`}
    />
  )
}
...
export default function AdminShippingRatesPageMain() {
  ...
  const [bulkDeleteRates, { isLoading: isBulkDeleting }] =
    useBulkDeleteAdminShippingRatesMutation()
  const [createRate, { isLoading: isCreating }] =
    useCreateAdminShippingRateMutation()
  const [updateRate, { isLoading: isUpdating }] =
    useUpdateAdminShippingRateMutation()
  const [deleteRate, { isLoading: isDeleting }] =
    useDeleteAdminShippingRateMutation()
  ...
  const activeRatesCount = useMemo(
    () => rates.filter((rate) => rate.status).length,
    [rates]
  )
  ...
  const filteredRateIds = useMemo(
    () => filteredRates.map((rate) => rate.id),
    [filteredRates]
  )
  ...
  const allVisibleSelected =
    filteredRateIds.length > 0 &&
    selectedVisibleCount === filteredRateIds.length

  const updateCreateField = (
    key: keyof ShippingRateForm,
  ...
  const handleEditSave = async () => {
    ...
    try {
      ...
      showErrorToast(
        getApiErrorMessage(error, "Failed to update shipping rate.")
      )
    }
  }
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      setSelectedRateIds((current) =>
        current.filter((id) => id !== deletingRate.id)
      )
      ...
      showErrorToast(
        getApiErrorMessage(error, "Failed to delete shipping rate.")
      )
    } finally {
  ...
  const handleBulkDeleteConfirm = async () => {
    ...
    try {
      ...
      showSuccessToast(
        `${selectedRateIds.length} shipping rate${selectedRateIds.length === 1 ? "" : "s"} deleted.`
      )
      clearSelection()
      setBulkDeleteOpen(false)
    } catch (error) {
      showErrorToast(
        getApiErrorMessage(error, "Failed to delete selected shipping rates.")
      )
    }
  }
          ...
          <div className="relative flex flex-col gap-6 xl:flex-row xl:items-end xl:justify-between">
            <div className="max-w-3xl">
              ...
              <p className="mt-3 text-sm leading-6 text-slate-600 dark:text-slate-300">
                Manage the delivery fee matrix for manual checkout. Active
                province and city combinations are matched automatically on the
          ...
          >
            <div className="rounded-[24px] bg-slate-950 px-5 py-5 text-white dark:bg-slate-800">
              ...
              <p className="mt-2 text-sm leading-6 text-slate-300">
                Add a province and city pair with a fee that can immediately
                power manual checkout.
              </p>
            </div>

            <div className="mt-5 space-y-5">
              <ShippingRateFormFields
                form={createForm}
            ...
            <div className="border-b border-slate-100 px-5 py-5 dark:border-slate-800">
              <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                <div>
                  ...
                  <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">
                    Browse, update, or remove delivery locations without leaving
                    the page.
                  </p>
              ...
              {isLoading ? (
                ...
                >
                  ...
                  <h3 className="mt-4 text-lg font-bold text-slate-900 dark:text-white">
                    No matching shipping rates
                  </h3>
                  <p className="mt-2 text-sm text-slate-500 dark:text-slate-400">
                    {searchTerm.trim()
                      ? "Try a different search keyword."
                      : "Create your first location from the panel on the left."}
                  </p>
                </motion.div>
              ) : (
                    ...
                    >
                      <div className="flex flex-col gap-3 border-b border-slate-200 bg-white px-5 py-4 dark:border-slate-700 dark:bg-slate-900 sm:flex-row sm:items-center sm:justify-between">
                        <div className="flex items-center gap-3">
                          <input
                            type="checkbox"
                            checked={provinceRates.every((rate) =>
                              selectedRateIds.includes(rate.id)
                            )}
                            onChange={() => {
                              const provinceIds = provinceRates.map(
                                (rate) => rate.id
                              )
                              const allProvinceSelected = provinceIds.every(
                                (id) => selectedRateIds.includes(id)
                              )
                        ...
                        <div className="rounded-full border border-slate-200 bg-slate-50 px-3 py-1 text-xs font-semibold text-slate-500 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-400">
                          {provinceRates.length} location
                          {provinceRates.length === 1 ? "" : "s"}
                        </div>
                      </div>
                            ...
                            <motion.article
                              ...
                              className={`rounded-[22px] border border-slate-200 bg-white px-4 py-4 transition hover:border-sky-300 hover:shadow-sm dark:border-slate-700 dark:bg-slate-900 ${
                                isDeletingRow
                                  ...
                                  : isSelected
                                    ? "ring-2 ring-sky-200 dark:ring-sky-900/50"
                                    : ""
                              }`}
                            >
                              <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
      ...
      >
        <div className="rounded-[24px] border border-red-100 bg-red-50 p-5 dark:border-red-900/50 dark:bg-red-950/20">
          <p className="text-sm text-slate-700 dark:text-slate-300">
            {deletingRate ? (
              <>
                You are about to delete the rate for{" "}
                <span className="font-bold text-slate-950 dark:text-white">
                  {deletingRate.city}, {deletingRate.province}
                </span>
                . This location will no longer be available for manual checkout
                until you create it again.
              </>
            ) : null}
          </p>
        </div>
        ...
        <div className="space-y-4">
          <div className="rounded-[24px] border border-red-100 bg-red-50 p-5 dark:border-red-900/50 dark:bg-red-950/20">
            <p className="text-sm text-slate-700 dark:text-slate-300">
              ...
              shipping rate{selectedRateIds.length === 1 ? "" : "s"}. This
              action removes them from manual checkout until they are created
              again.
            </p>
          </div>
          <div className="rounded-[24px] border border-slate-200 bg-slate-50 p-4 dark:border-slate-700 dark:bg-slate-800/60">
            ...
            <p className="mt-2 text-sm text-slate-700 dark:text-slate-300">
              {selectedVisibleCount} visible result
              {selectedVisibleCount === 1 ? "" : "s"} selected in the current
              filter, {selectedRateIds.length} total selected overall.
            </p>
          </div>
        </div>
      </ShippingRateModal>
