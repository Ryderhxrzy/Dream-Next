...
const getZqInlineStorageKey = (pathname: string) =>
  `afhome:zq-inline:${pathname}`
...
const buildDuplicateProductDraft = (
  product: Product
): DuplicateAddProductDraft => {
  const variants = (product.variants ?? []).map((variant) => ({
    ...
    pv_colors: variant.color
      ? [
          {
            name: toDraftString(variant.color),
            hex: toDraftString(variant.colorHex) || "#94a3b8",
          },
        ]
      : [],
  ...
  const globalColors = Array.from(
    new Map(
      variants
        .flatMap((variant) => variant.pv_colors)
        .map(
          (color) =>
            [
              `${color.name.toLowerCase()}::${color.hex.toLowerCase()}`,
              color,
            ] as const
        )
    ).values()
  ...
  return {
    ...
    form: {
      ...
      pd_catsubid:
        Number(product.catsubid ?? 0) > 0
          ? toDraftNumberString(product.catsubid)
          : "",
...
const exportZqToCSV = (products: ZqCachedProduct[]) => {
  ...
  ws["!rows"] = [{ hpt: 22 }, ...Array(dataRows.length).fill({ hpt: 18 })]
  ...
  showSuccessToast(
    `Exported ${products.length} AF HOME GLOBAL products to Excel`
  )
}
...
const mapCachedZqProductToLocalRow = (product: ZqCachedProduct): Product => ({
  ...
  description: product.importStatus
    ? `Global Supplier import status: ${product.importStatus}`
    : null,
...
const parseZqPreviewMeta = (payload: Record<string, unknown> | undefined) => {
  ...
  const nextCursor =
    typeof data.nextCursor === "string" || typeof data.nextCursor === "number"
...
function StatCard({
  ...
  return (
    <div className="group relative overflow-hidden rounded-2xl border border-slate-200/60 bg-white p-5 shadow-sm transition-all duration-200 hover:-translate-y-0.5 hover:shadow-md dark:border-slate-700/50 dark:bg-slate-900">
      {/* Icon container */}
      <div
        className={`mb-4 inline-flex h-11 w-11 items-center justify-center rounded-xl ${colorClass} shadow-sm`}
      >
        {icon}
      </div>
      {/* Value */}
...
function ManualCheckoutSelectionModal({
  ...
  const eyebrow = isViewMode
    ? "Manual Checkout Products"
  ...
  const summaryLabel = isViewMode
    ? "manual checkout product"
    : "selected product"
  const primaryLabel = isViewMode
    ? "Already Added to Manual Checkout"
            ...
            >
              ...
              >
                ...
                <p className="mt-1 text-xl font-bold text-slate-900">
                  {progress.batches.toLocaleString()}
                </p>
              </motion.div>
              <motion.div
                variants={{
              ...
              <div className="rounded-2xl border border-emerald-100 bg-emerald-50 px-4 py-3">
                ...
                {hasStarted && progress.skipped > 0 ? (
                  <p className="mt-1 text-sm font-medium text-amber-700">
                    {progress.skipped.toLocaleString()} existing product
...
function ZqCategoryMappingPanel({
  ...
  const mappedCount = categories.filter(
    (category) => category.status === "mapped"
  ).length
      ...
      <div className="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
        <div className="min-w-0">
          ...
          <h3 className="mt-1 text-base font-bold text-slate-900 dark:text-slate-100">
            Map AF HOME GLOBAL categories to AF Home categories
          </h3>
          <p className="mt-1 text-sm text-slate-500 dark:text-slate-300">
            One mapping applies to all synced AF HOME GLOBAL products under that
            category.
          </p>
        </div>
        ...
        <div className="grid w-full gap-3 md:grid-cols-[1.4fr_1.2fr_1fr_auto] xl:max-w-5xl">
          ...
          >
            ...
            {categories.map((category) => {
              ...
              const suffix = category.localCategoryName
              ...
              const label =
                category.zqCategoryName?.trim() ||
                (category.zqCategoryId
                  ? `ZQ Category ${category.zqCategoryId}`
                  : "Uncategorized")
              return (
...
export default function ProductsPageMain({
  ...
  const supplierName = String(
    (
      session?.user as
        | { supplierName?: string | null; name?: string | null }
        | undefined
    )?.supplierName ??
  ...
  const zqInlineStorageKey = useMemo(
    () => getZqInlineStorageKey(pathname || "/products"),
    [pathname]
  )
  ...
  const isSupplierServicesPortal =
    isSupplierPortal &&
    (supplierCategoriesData?.categories ?? []).some(
      (c) => c.name.toLowerCase() === "services"
    )
  const normalizedSupplierName = supplierName
    .toLowerCase()
    .replace(/[^a-z0-9]/g, "")
  const isZqSupplierAccount =
    normalizedSupplierName.includes("zqsupplier") ||
  ...
  const [brandType, setBrandType] = useState<number | undefined>(
    typeof initialBrandType === "number" && initialBrandType > 0
      ? initialBrandType
      : undefined
  )
  const [supplierFilterId, setSupplierFilterId] = useState<number | undefined>(
    undefined
  )
  ...
  const [showManualSelectionModal, setShowManualSelectionModal] =
    useState(false)
  const [manualSelectionProducts, setManualSelectionProducts] = useState<
    Product[]
  >([])
  ...
  const [editZqPricing, setEditZqPricing] = useState<ZqCachedProduct | null>(
    null
  )
    ...
    try {
      return (
        window.localStorage.getItem(
          getZqInlineStorageKey(window.location.pathname || "/products")
        ) === "1"
      )
    } catch {
      return false
  ...
  const [zqImportMode, setZqImportMode] = useState<"resume" | "rescan">(
    "resume"
  )
  ...
  const [zqMappingStatus, setZqMappingStatus] = useState<
    "" | "mapped" | "unmapped" | "missing"
  >("")
  const [selectedZqCategoryKey, setSelectedZqCategoryKey] = useState("")
  const [selectedZqLocalCategoryId, setSelectedZqLocalCategoryId] = useState<
    number | undefined
  >(undefined)
  const [deletingIds, setDeletingIds] = useState<number[]>([])
  const [removingManualCheckoutIds, setRemovingManualCheckoutIds] = useState<
    number[]
  >([])
  const [selectedIds, setSelectedIds] = useState<number[]>([])
  const [applyManualCheckout, { isLoading: isApplyingManualCheckout }] =
    useManualCheckoutApplyMutation()
  const [fetchZqImportPreview] = useFetchZqImportPreviewMutation()
  const [syncZqProducts] = useSyncZqProductsMutation()
  const [saveGeneralSettings, { isLoading: isSavingManualMode }] =
    useUpdateAdminGeneralSettingsMutation()
  ...
  const zqInlineActive =
    canShowZqSupplierSide && (showZqSupplierInline || isSupplierPortal)
  ...
  const isServicesView =
    ...
    (typeof catId === "number" && catId > 0 && catId === servicesCategoryId)
  const { data: adminGeneralSettingsData } = useGetAdminGeneralSettingsQuery(
    undefined,
    { skip: isSupplierPortal }
  )
  const manualHeaderToggle = Boolean(
  ...
  const { data: supplierBrandsData } = useGetPublicProductBrandsQuery(
    undefined,
    { skip: !isSupplierPortal }
  )
  const { data: supplierListData } = useGetSuppliersQuery(undefined, {
    skip: isSupplierPortal,
  })
  ...
  useEffect(() => {
    ...
    const pickBestBrandId = () => {
      const exactMatch = brands.find((brand) => {
        const brandKey = String(brand.name ?? "")
          .toLowerCase()
          .replace(/[^a-z0-9]/g, "")
        return (
          brandKey !== "" &&
          candidates.some((candidate) => candidate === brandKey)
        )
      })
      if (exactMatch?.id) {
  ...
  const { data: adminActiveCountData, refetch: refetchAdminActiveCount } =
    useGetProductsQuery(activeCountArgs, countQueryOpts)
  const { data: adminInactiveCountData, refetch: refetchAdminInactiveCount } =
    useGetProductsQuery(inactiveCountArgs, countQueryOpts)
  const { data: adminPendingCountData, refetch: refetchAdminPendingCount } =
    useGetProductsQuery(pendingCountArgs, countQueryOpts)
  ...
  const { data: zqSummaryData, refetch: refetchZqSummary } =
    useGetZqProductsSummaryQuery(undefined, { skip: !zqInlineActive })
  const { data: zqCategoryMappingsData, refetch: refetchZqCategoryMappings } =
    useGetZqCategoryMappingsQuery(undefined, {
      skip: !zqInlineActive || !isSupplierPortal,
    })
  const [upsertZqCategoryMapping, { isLoading: isSavingZqCategoryMapping }] =
    useUpsertZqCategoryMappingMutation()
  const {
    data: zqCachedData,
  ...
  } = useGetZqCachedProductsQuery(
    {
      ...
      search: debouncedSearch || undefined,
      brandType:
        isSupplierPortal && !isZqSupplierAccount ? brandType : undefined,
  ...
  const normalizeSupplierCreatedProduct = (product: Product): Product => {
    ...
    return {
      ...product,
      supplierId:
        nextSupplierId > 0 ? nextSupplierId : (product.supplierId ?? 0),
  ...
  const handleProductsSaved = (updatedProduct?: Product) => {
    ...
    if (updatedProduct) {
      ...
      setCreatedProducts((prev) => {
        const next = [
          nextProduct,
          ...prev.filter((product) => product.id !== nextProduct.id),
        ]
        return next
      })
    }
  ...
  const products = useMemo(() => {
    const rawProducts = data?.products
      ...
      : useInitialData
        ...
        : []
    const mergedProducts = rawProducts.map(
      (product) => productOverrides[product.id] ?? product
    )
    ...
    return mergedProductList.filter((product) => {
      const matchesSupplier =
        Number(product.supplierId ?? 0) === linkedSupplierId
      const matchesBrand =
        typeof brandType === "number" && brandType > 0
  ...
  const visibleProducts = useMemo(() => {
    ...
    const baseProducts =
      status === "new" ? products.filter(isNewProduct) : products
    const servicesExcluded =
      servicesCategoryId && !isServicesView
        ? baseProducts.filter(
            (p) => Number(p.catid ?? 0) !== servicesCategoryId
          )
        : baseProducts
    const categoryFiltered =
      typeof catId === "number" && catId > 0
        ? servicesExcluded.filter(
            (product) => Number(product.catid ?? 0) === catId
          )
        : servicesExcluded
    const duplicateFiltered = showDuplicateOnly
      ? categoryFiltered.filter((product) =>
          duplicateProductIds.has(product.id)
        )
      : categoryFiltered
  ...
  const selectableProducts = useMemo(() => {
    const selectionSource = selectionData?.products
      ...
      : products
    const mergedSelection = selectionSource.map(
      (product) => productOverrides[product.id] ?? product
    )
    const mergedById = new Map<number, Product>()
    ...
    createdProducts.forEach((product) => {
      ...
      const matchesSupplierFilter =
        !supplierFilterId ||
        supplierFilterId <= 0 ||
        Number(candidate.supplierId ?? 0) === supplierFilterId
      const matchesLinkedSupplier =
        !isSupplierPortal || linkedSupplierId <= 0
          ? true
          : Number(candidate.supplierId ?? 0) === linkedSupplierId ||
            (typeof brandType === "number" &&
              brandType > 0 &&
              Number(candidate.brandType ?? 0) === brandType)
  ...
  const localPendingCount = useMemo(
    () =>
      createdProducts.filter((product) => {
        ...
        const isAlreadyOnServer = serverProducts.some(
  ...
  const handleCatId = (v: number | undefined) => {
    setCatId(v)
    setPage(1)
  }
  const handleBrandType = (v: number | undefined) => {
    setBrandType(v)
    setPage(1)
  }
  const handleSupplierFilterId = (v: number | undefined) => {
  ...
  const handleSelectZqCategory = (value: string) => {
    ...
    const selected = zqCategoryMappings.find(
      (category) =>
        `${category.zqCategoryId ?? ""}::${category.zqCategoryName}` === value
    )
  ...
  const handleSaveZqCategoryMapping = async () => {
    const selected = zqCategoryMappings.find(
      (category) =>
        `${category.zqCategoryId ?? ""}::${category.zqCategoryName}` ===
        selectedZqCategoryKey
    )
    if (!selected) {
    ...
    try {
      ...
      showSuccessToast(
        result.message || "AF HOME GLOBAL category mapping saved."
      )
      ...
      showErrorToast(
        apiError?.data?.message ||
          "Failed to save AF HOME GLOBAL category mapping."
      )
    }
  }
    ...
    try {
      ...
      showSuccessToast(
        nextValue
          ...
          : "Manual checkout mode disabled."
      )
    } catch (error) {
      const apiError = error as { data?: { message?: string } }
      showErrorToast(
        apiError?.data?.message || "Failed to update manual checkout mode."
      )
    }
  }
  ...
  const handleSyncMeilisearch = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${apiBaseUrl}/api/meilisearch/sync-products`,
        {
          method: "POST",
          headers: {
      ...
      setMeilisearchError(null)
      showSuccessToast(
        data?.message || "Products synced to search index successfully!"
      )
    } catch (error) {
      const err = error as { message?: string }
      const errorMessage =
        err?.message || "Failed to sync products to search index"
  ...
  const handleCancelZqImport = () => {
    if (isSyncingAllZq || isDiscoveringZqTotal) {
      ...
      showSuccessToast(
        "Global Supplier import will stop after the current item finishes."
      )
      return
    }
  ...
  const requestSupplierZq = async <T,>(
    path: string,
    ...
    const response = await fetch(
      `${apiBaseUrl}/api/supplier/products/zq/${path}`,
      {
        method: "POST",
        headers: {
          Accept: "application/json",
    ...
    if (!response.ok) {
      if (response.status === 401) {
        void fetch("/api/supplier/auth/signout", {
          method: "POST",
          credentials: "include",
        })
          .catch(() => {})
          .finally(() => {
            window.location.replace("/supplier/login?session=expired")
          })
      }
      throw new Error(
        payload.message ||
          `Global Supplier request failed with status ${response.status}.`
      )
    }
    ...
    try {
      do {
        ...
        const result = isSupplierPortal
          ? await requestSupplierZq<{ zq?: Record<string, unknown> }>(
              "fetch-preview",
              {
                ...previewPayload,
        ...
        const zqPayload =
          result.zq && typeof result.zq === "object" ? result.zq : {}
  ...
  const syncAllZqProducts = async (mode: "resume" | "rescan") => {
    ...
    try {
      ...
      if (!zqImportCancelRef.current) {
        ...
        setShowZqSyncModal(false)
        showSuccessToast(
          "All global supplier products imported to tbl_zqproducts successfully."
        )
      }
    } catch (error) {
      const apiError = error as { data?: { message?: string } }
      showErrorToast(
        apiError?.data?.message ||
          ...
          "Failed to sync all global supplier products."
      )
    } finally {
  ...
  const zqImportedCount = Number(
    zqSummaryData?.total ?? visibleMeta?.total ?? 0
  )
  const zqImportButtonLabel =
    isSyncingAllZq || isDiscoveringZqTotal
      ? "Syncing Global Supplier Products..."
      : isSupplierPortal
        ? zqImportedCount > 0
          ? "Sync Latest Global Supplier Products"
          : "Sync AF HOME GLOBAL Products"
        : zqImportedCount > 0
          ? "Refresh Global Supplier Products"
          : "Start AF HOME GLOBAL SUPPLIER Import"
  const zqImportButtonMobileLabel =
    isSyncingAllZq || isDiscoveringZqTotal
      ? "Syncing..."
      : isSupplierPortal
        ? zqImportedCount > 0
  ...
  const manualCheckoutProducts = useMemo(
    () =>
      selectableProducts.filter((product) =>
        Boolean(product.manualCheckoutEnabled)
      ),
    [selectableProducts]
  )
  ...
  const openManualSelectionModal = (
    ...
    if (products.length === 0) {
      showErrorToast(
        mode === "view"
          ? "No manual checkout products found yet."
  ...
  const downloadExportCSV = async (
    ...
    try {
      ...
      const timestamp = `${now.getFullYear()}-${String(now.getMonth() + 1).padStart(2, "0")}-${String(now.getDate()).padStart(2, "0")}-${String(now.getHours()).padStart(2, "0")}${String(now.getMinutes()).padStart(2, "0")}${String(now.getSeconds()).padStart(2, "0")}`
      a.download =
        match?.[1]?.replace(/['"]/g, "") ?? `products-export-${timestamp}.csv`
  ...
  const handleExportAllCSV = () => {
    downloadExportCSV({
      supplier_id:
        isSupplierPortal && linkedSupplierId > 0 ? linkedSupplierId : undefined,
    })
  }
  ...
  const handleApplyManualCheckout = async () => {
    ...
    try {
      ...
      if (updatedIds.size > 0 && failedCount === 0) {
        showSuccessToast(
          result.message || "Selected products were added to manual checkout."
        )
        return
      }
      ...
      if (updatedIds.size > 0 && failedCount > 0) {
        showSuccessToast(
          `${updatedIds.size} product(s) added to manual checkout.`
        )
      ...
      showErrorToast(
        result.results.find((row) => row.status === "failed")?.message ||
          result.message ||
          "No products were added to manual checkout."
      )
    } catch (error) {
      const apiError = error as { data?: { message?: string } }
      showErrorToast(
        apiError?.data?.message || "Failed to save manual checkout products."
      )
    }
  }

  const handleRemoveManualCheckout = async (product: Product) => {
    setRemovingManualCheckoutIds((current) =>
    ...
    try {
      ...
      const updated = result.results.find(
        (row) => row.product_id === product.id && row.status === "updated"
      )
      if (!updated) {
        showErrorToast(
          result.results.find((row) => row.product_id === product.id)
            ?.message ||
            result.message ||
            "Failed to remove product from manual checkout."
        )
        return
      }
      ...
      showSuccessToast(
        updated.message || `${product.name} removed from manual checkout.`
      )
    } catch (error) {
      const apiError = error as { data?: { message?: string } }
      showErrorToast(
    ...
    <div className="space-y-5">
      ...
      >
        <div>
          <h1 className="text-xl font-bold text-slate-800">
            {isSupplierServicesPortal ? "Services" : "Products"}
          </h1>
          <p className="mt-0.5 text-sm text-slate-500 dark:text-slate-300">
            {manualHeaderToggle
              ? "Viewing products assigned to manual checkout."
              : isSupplierServicesPortal
        ...
        <div className="flex shrink-0 items-center gap-2">
            ...
            >
              ...
              >
                <path
                  ...
                  strokeLinejoin="round"
                  strokeWidth={2}
                  d="M15 5v2m0 4v2m0 4v2M5 5h14a2 2 0 0 1 2 2v3a2 2 0 1 0 0 4v3a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-3a2 2 0 1 0 0-4V7a2 2 0 0 1 2-2Z"
                />
              </svg>
          ...
          {!isSupplierServicesPortal && (
            ...
            >
              <div
                className={`relative h-5 w-9 rounded-full transition-colors ${manualHeaderToggle ? "bg-teal-500" : "bg-slate-200"}`}
              >
                <div
                  className={`absolute top-0.5 h-4 w-4 rounded-full bg-white transition-transform ${manualHeaderToggle ? "left-4" : "left-0.5"}`}
                />
              </div>
            ...
            >
              ...
              >
                <path
                  ...
                  strokeLinejoin="round"
                  strokeWidth={2}
                  d="M4 4h16v4H4zm0 8h16v8H4zm4 4h.01M12 16h4"
                />
              </svg>
              <span className="hidden sm:inline">{zqImportButtonLabel}</span>
            ...
            >
              ...
              >
                <path
                  ...
                  strokeLinejoin="round"
                  strokeWidth={2}
                  d="M9 5H7a2 2 0 0 0-2 2v10a2 2 0 0 0 2 2h10a2 2 0 0 0 2-2v-2M9 5a2 2 0 0 1 2-2h2a2 2 0 0 1 2 2M9 5a2 2 0 0 0 2 2h2a2 2 0 0 0 2-2m-6 9 2 2 4-4"
                />
              </svg>
          ...
          >
            ...
            <span className="hidden sm:inline">
              {isSupplierServicesPortal ? "Add Service" : "Add Product"}
            </span>
          </button>
        </div>
      </motion.div>
      ...
      {!isSupplierServicesPortal && (
        ...
        >
          <StatCard
            label={
              isSupplierServicesPortal ? "Total Services" : "Total Products"
            }
            value={
              zqInlineActive
                ? (
          ...
          <StatCard
            ...
            value={
              zqInlineActive
                ? (zqSummaryData?.active ?? 0).toLocaleString()
                : activeCountData
                  ? (activeCountData.meta?.total ?? 0).toLocaleString()
                  : "—"
            }
          ...
          <StatCard
            ...
            value={
              zqInlineActive
                ? (zqSummaryData?.inactive ?? 0).toLocaleString()
                : inactiveCountData
                  ? (inactiveCountData.meta?.total ?? 0).toLocaleString()
                  : "—"
            }
          ...
          {isSupplierServicesPortal ? (
            <StatCard
              ...
              value={
                newCountData
                  ? (newCountData.meta?.total ?? 0).toLocaleString()
                  : "—"
              }
            ...
            <StatCard
              ...
              value={
                pendingCountData
                  ? (
                      (pendingCountData.meta?.total ?? 0) + localPendingCount
                    ).toLocaleString()
                  : localPendingCount.toLocaleString()
              }
          ...
          <StatCard
            ...
            value={
              zqInlineActive
                ? (zqSummaryData?.low_stock ?? lowStockCount)
                : isLoading
                  ? "—"
                  : lowStockCount
            }
      ...
      >
        <ProductsToolbar
          ...
          isDuplicateFilterActive={
            isSupplierServicesPortal ? false : showDuplicateOnly
          }
          duplicateCount={
            isSupplierServicesPortal ? 0 : duplicateProductIds.size
          }
          onToggleDuplicateFilter={
            isSupplierServicesPortal ? undefined : handleToggleDuplicateFilter
          }
          manualCheckoutCount={
            isSupplierServicesPortal ? 0 : manualCheckoutProducts.length
          }
          onViewManualCheckout={
            isSupplierServicesPortal
              ? undefined
              : () => openManualSelectionModal(manualCheckoutProducts, "view")
          }
        />
      </motion.div>
      ...
      {!zqInlineActive && isError ? (
        ...
        <div className="space-y-2">
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-400 dark:text-slate-500">
                    {visibleMeta?.from || 0} to {visibleMeta?.to || 0} of{" "}
                    {visibleMeta?.total || visibleProducts.length}
                  </p>
                </div>
              </div>
              <div className="flex items-center gap-4">
          ...
          <DataTableShell
            ...
            subtitle={
              zqInlineActive
                ? "Fetched global supplier products are now displayed in the same products table area."
                : undefined
            }
          >
            {zqInlineActive &&
                ...
                <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                  <div>
                    <p className="text-sm font-bold text-slate-800 dark:text-slate-100">
                      {isSupplierPortal
                        ? "No synced Global Supplier products yet"
                        : "AF HOME GLOBAL SUPPLIER import is empty"}
                    </p>
                    <p className="mt-1 text-sm text-slate-500 dark:text-slate-300">
                      {isSupplierPortal
            ...
            <ProductsTable
              ...
              onViewProduct={(product) => {
                if (zqInlineActive) {
                  window.open(
                    `/global-product/${Math.abs(product.id)}?preview=1`,
                    "_blank",
                    "noopener,noreferrer"
                  )
                  return
                ...
                const productPath = buildStorefrontProductPath(
                  product.name,
                  product.id
                )
              ...
              isLoading={
                zqInlineActive && (isLoadingZqCached || isFetchingZqCached)
              }
      ...
      <AddProductModal
        ...
        onClose={() => {
          ...
          if (
            (searchParams.get("modal") ?? "").toLowerCase() === "add-product"
          ) {
      ...
      <EditProductModal
        ...
        isServicesView={
          ...
          (editProduct !== null &&
            ...
            Number(editProduct.catid ?? 0) === servicesCategoryId)
        }
      />
      {isSupplierPortal ? (
      ...
      {Array.from({ length: 8 }).map((_, ri) => (
        <div key={ri} className="grid grid-cols-9 gap-3">
          {Array.from({ length: 9 }).map((_, ci) => (
            <div
              key={ci}
              className="h-8 rounded bg-slate-100 dark:bg-slate-900"
            />
          ))}
        </div>
      ))}
