...
import {
  extractVariantOptionLabels,
  mergeVariantOptionLabelsMeta,
} from "@/libs/productVariantOptions"
...
const getEditProductDraftKey = (productId: number) =>
  `afhome:edit-product-draft:${productId}`
              ...
              >
                <div className="min-w-0">
                  ...
                  <p className="mt-0.5 font-mono text-[11px] text-slate-400 md:text-xs">
                    {pvStr} PV <span className="text-slate-300">×</span> {rate}{" "}
                    <span className="text-slate-300">=</span>{" "}
                    <span className="font-semibold text-slate-600">
...
const mapVariantToForm = (variant: ProductVariant): VariantFormState => ({
  ...
  pv_price_member:
    toOptionalPositiveNumber(variant.priceMember)?.toString() ?? "",
  pv_reversed_pv_multiplier: deriveMultiplierFromPv({
    transfer: variant.priceDp,
    pv: variant.prodpv,
  }),
  ...
  pv_images: Array.isArray(variant.images)
...
const normalizeComparableNumber = (
  value: string | number | null | undefined
) => {
  const trimmed = String(value ?? "").trim()
  if (!trimmed) return ""
...
const isGeneratedGlobalVariant = (
  ...
  const hasCustomSku =
    Boolean(variant.pv_sku.trim()) && !isAutoGeneratedVariantSku(variant.pv_sku)
  const hasCustomPricing = Boolean(
    (variant.pv_price_srp.trim() &&
      normalizeComparableNumber(variant.pv_price_srp) !==
        normalizeComparableNumber(baseDefaults?.srp)) ||
    (variant.pv_price_dp.trim() &&
      normalizeComparableNumber(variant.pv_price_dp) !==
        normalizeComparableNumber(baseDefaults?.dealer)) ||
    (variant.pv_price_member.trim() &&
      normalizeComparableNumber(variant.pv_price_member) !==
        normalizeComparableNumber(baseDefaults?.member)) ||
    (variant.pv_reversed_pv_multiplier.trim() &&
      normalizeComparableNumber(variant.pv_reversed_pv_multiplier) !==
        normalizeComparableNumber(baseDefaults?.multiplier))
  )
  const hasCustomVariantData = Boolean(
    variant.pv_style.trim() ||
  ...
  return (
    !hasCustomVariantData &&
    Boolean(
      variant.pv_name.trim() ||
      variant.pv_style.trim() ||
      variant.pv_size.trim() ||
...
const collapseGeneratedGlobalVariants = (
  ...
  const allGenerated = variants.every((variant) =>
    isGeneratedGlobalVariant(variant, baseDefaults)
  )
  if (!allGenerated) {
    return {
      visibleVariants: variants,
...
const getVariantFormKey = (variant: VariantFormState) => {
  ...
  return [
    ...
    normalizeVariantLabel(variant.pv_name).toLowerCase(),
    normalizeVariantLabel(variant.pv_style).toLowerCase(),
    dedupeVariantValues(variant.pv_extra_styles).join("|").toLowerCase(),
    normalizeVariantLabel(variant.pv_size).toLowerCase(),
    variant.pv_width.trim(),
    variant.pv_dimension.trim(),
    variant.pv_height.trim(),
...
const mapProductVariantsToFormStates = (productVariants: ProductVariant[]) => {
  ...
  const groupedVariants = productVariants.reduce((map, variant) => {
    ...
    const mergedStyles = dedupeVariantValues([
      current.pv_style,
      ...current.pv_extra_styles,
      variant.style ?? "",
    ])
    const [primaryStyle = "", ...extraStyles] = mergedStyles

    nextColor.forEach((color) => {
      const signature = getNormalizedColorSignature(color.name, color.hex)
      const alreadyExists = mergedColors.some(
        (existingColor) =>
          getNormalizedColorSignature(existingColor.name, existingColor.hex) ===
          signature
      )
      if (!alreadyExists) {
    ...
    map.set(groupKey, {
      ...current,
      pv_style: primaryStyle,
      pv_extra_styles: extraStyles,
...
const hasEditDraftContent = (
  ...
  const formChanged =
    initialForm !== null &&
    JSON.stringify(normalizeFormForComparison(currentForm)) !==
      JSON.stringify(normalizeFormForComparison(initialForm))
  const variantsChanged =
    JSON.stringify(normalizeVariantsForComparison(currentVariants)) !==
    JSON.stringify(normalizeVariantsForComparison(initialVariants))
...
const normalizeFormForComparison = (form: FormState) => ({
  ...
  pd_reversed_pv_multiplier: normalizeNumberField(
    form.pd_reversed_pv_multiplier
  ),
  pd_prodpv: deriveComputedPv({
...
const normalizeVariantsForComparison = (variants: VariantFormState[]) =>
  variants.map((variant) => ({
    ...
    pv_reversed_pv_multiplier: normalizeNumberField(
      variant.pv_reversed_pv_multiplier
    ),
    pv_prodpv: deriveComputedPv({
      transfer: variant.pv_price_dp,
      multiplier: variant.pv_reversed_pv_multiplier,
    }),
...
export default function EditProductModal({
  ...
  const [activeNewImageAdjustId, setActiveNewImageAdjustId] = useState<
    string | null
  >(null)
  ...
  const currentProductStatus = Number(
    openedProduct?.status ?? product?.status ?? form.pd_status ?? 0
  )
  const canEditProductStatus =
    !isSupplierScopedActor || isServicesView || currentProductStatus !== 3
  ...
  useEffect(() => {
    ...
    const optionLabels = extractVariantOptionLabels(
      openedProduct.specifications
    )
    const nextForm = {
      ...
      pd_room_type: openedProduct.roomType
        ...
        : "",
      pd_brand_type: openedProduct.brandType
        ? String(openedProduct.brandType)
        : "",
      ...
      pd_reversed_pv_multiplier: deriveMultiplierFromPv({
        transfer: openedProduct.priceDp,
        pv: openedProduct.prodpv,
      }),
      ...
      pd_assembly_required: Boolean(
        row.assemblyRequired ?? row.pd_assembly_required
      ),
      pd_parent_sku: generateSkuFromName(
        openedProduct.name ?? "",
        openedProduct.id
      ),
    ...
    const existing = dedupeImageUrls(
      Array.isArray(openedProduct.images) && openedProduct.images.length > 0
    ...
    const rawNextVariants = Array.isArray(openedProduct.variants)
      ? dedupeVariantFormStates(
          mapProductVariantsToFormStates(openedProduct.variants)
        )
      : []
    const collapsedInitialState = collapseGeneratedGlobalVariants(
      rawNextVariants,
      {
    ...
    if (typeof window !== "undefined") {
      try {
        const savedDraft = window.localStorage.getItem(
          getEditProductDraftKey(openedProduct.id)
        )
        if (savedDraft) {
          const parsedDraft = JSON.parse(
            savedDraft
          ) as Partial<EditProductDraft>
          if (
            parsedDraft.version === 1 &&
            parsedDraft.productId === openedProduct.id
          ) {
            const restoredRawVariants = Array.isArray(parsedDraft.variants)
              ? dedupeVariantFormStates(
                  parsedDraft.variants.map((variant) => ({
                    ...
                    ...variant,
                    pv_style:
                      typeof variant?.pv_style === "string"
                        ? variant.pv_style
                        : "",
                    pv_extra_styles: Array.isArray(
                      ...
                      ? dedupeVariantValues(
                          (variant as Partial<VariantFormState>)
                            .pv_extra_styles ?? []
                        )
                      : [],
                  }))
            ...
            const restoredCollapsedState = collapseGeneratedGlobalVariants(
              restoredRawVariants,
              {
                srp: nextForm.pd_price_srp,
                dealer: nextForm.pd_price_dp,
            ...
            setImageItems(
              (Array.isArray(parsedDraft.imageUrls)
                ? dedupeImageUrls(parsedDraft.imageUrls)
                : existing
              ).map(makeExistingImageItem)
            )
  ...
  useEffect(() => {
    ...
    setVariants((prev) => {
      ...
      return JSON.stringify(normalizeVariantsForComparison(prev)) ===
        JSON.stringify(normalizeVariantsForComparison(next))
        ? prev
        : next
    })
  ...
  const handleImageDragEnter =
    (targetIndex: number) => (event: React.DragEvent<HTMLElement>) => {
      ...
      setVariants((prev) =>
        prev.map((item, i) =>
          i === index
            ? {
                ...item,
                pv_images: dedupeImageUrls([...item.pv_images, ...uploaded]),
              }
            : item
        )
      )
  ...
  const validate = (): Errors => {
    ...
    if (!form.pd_price_srp.trim() || isNaN(Number(form.pd_price_srp)))
    ...
    if (form.pd_pswidth && isNaN(Number(form.pd_pswidth)))
      e.pd_pswidth = "Must be a valid number"
  ...
  const expandedVariants = variantRowsForExpansion
    ...
    .flatMap((v, index) => {
      const autoSku = buildVariantSku(
        form.pd_parent_sku || generateSkuFromName(form.pd_name, product?.id),
        index
      )
      ...
      const base = {
        ...
        pv_images:
          dedupeImageUrls(v.pv_images).length > 0
            ? dedupeImageUrls(v.pv_images)
            : undefined,
      }
  ...
  const handleSubmit = async (e: React.SyntheticEvent) => {
    ...
    const baseChanged =
      ...
      JSON.stringify(normalizeFormForComparison(initialForm ?? form))
    const variantsChanged =
      JSON.stringify(normalizeVariantsForComparison(variants)) !==
      JSON.stringify(normalizeVariantsForComparison(initialVariants))
    const globalPrimaryValuesChanged =
      JSON.stringify(dedupeVariantValues(globalPrimaryValues)) !==
      JSON.stringify(collectVariantNames(initialVariants))
    const globalSizeValuesChanged =
      JSON.stringify(dedupeVariantValues(globalSizeValues)) !==
      JSON.stringify(collectVariantSizes(initialVariants))
    const productTypeChanged =
      Number(form.pd_type) !== Number(initialForm?.pd_type ?? form.pd_type)
    const variantPayloadChanged =
      variantsChanged ||
      globalPrimaryValuesChanged ||
      globalSizeValuesChanged ||
      productTypeChanged
    const hasNewImages = imageItems.some((item) => item.kind === "new")
    const currentExistingImageUrls = imageItems
      .filter(
        (item): item is Extract<ProductImageItem, { kind: "existing" }> =>
          item.kind === "existing"
      )
      .map((item) => item.url)
    const existingImagesChanged =
      ...
      currentExistingImageUrls.some(
        (url, index) => url !== initialImageUrls[index]
      )
    ...
    try {
      ...
      const updatedProduct: Product = response.product
        ...
        : normalizeProduct({
            ...
            brandType: form.pd_brand_type
              ? Number(form.pd_brand_type)
              : undefined,
            brand:
              brands.find((brand) => brand.id === Number(form.pd_brand_type))
                ?.name ??
              product.brand ??
              null,
            ...
            priceMember: form.pd_price_member
              ? Number(form.pd_price_member)
              : undefined,
  ...
  const pendingImageCount = imageItems.filter(
    (item) => item.kind === "new"
  ).length
  ...
  const activeNewImageAdjustItem = activeNewImageAdjustId
    ? (imageItems.find(
        (item): item is Extract<ProductImageItem, { kind: "new" }> =>
          item.kind === "new" && item.id === activeNewImageAdjustId
      ) ?? null)
    : null
      ...
      {isOpen && (
        <>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-5">
            ...
            >
              {/* ── Header ── */}
              <div className="shrink-0 border-b border-slate-200/80 bg-gradient-to-r from-blue-50 via-white to-cyan-50 px-4 py-4 sm:px-6 sm:py-5 dark:border-slate-800 dark:from-slate-950 dark:via-slate-950 dark:to-blue-950/30">
                <div className="flex items-center justify-between gap-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-[11px] font-semibold tracking-[0.2em] text-blue-600 uppercase dark:text-blue-300">
                        Catalog Workspace
                      </p>
                      <h2 className="mt-1 text-lg leading-none font-bold text-slate-900 dark:text-slate-100">
                        {isServicesView ? "Edit Service" : "Edit Product"}
                      </h2>
                ...
                >
                    ...
                    {draftRestored && (
                      <div className="flex items-start gap-2.5 rounded-xl border border-amber-100 bg-amber-50 p-3.5">
                        ...
                        <p className="text-xs text-amber-700">
                          Local draft restored. Unsaved edits and uploaded image
                          links are back in this product form.
                        </p>
                      </div>
                    )}
                        ...
                        {showImageGrid ? (
                            ...
                            <div className="grid grid-cols-3 gap-2 sm:grid-cols-4 lg:grid-cols-5">
                                ...
                                return (
                                  ...
                                  >
                                    ...
                                    <div className="absolute bottom-1.5 left-1.5 flex flex-wrap gap-1">
                                      {index === 0 && (
                                        <span className="rounded-md bg-blue-500 px-1.5 py-0.5 text-[9px] font-bold text-white">
                                          Main
                                        </span>
                                      )}
                                      {item.kind === "new" && (
                                    ...
                                    <div className="absolute inset-0 flex items-center justify-center gap-1.5 opacity-0 transition-opacity group-hover:opacity-100">
                                      {item.kind === "new" && (
                                        <button
                                          type="button"
                                          onClick={() =>
                                            setActiveNewImageAdjustId(item.id)
                                          }
                              ...
                              {imageItems.length < 15 && (
                                ...
                                >
                                  ...
                                  <span className="text-[10px] font-medium text-slate-400 transition-colors group-hover:text-blue-500">
                                    Add
                                  </span>
                                </label>
                              )}
                          ...
                          >
                            <div className="flex h-14 w-14 items-center justify-center rounded-2xl bg-white shadow-md ring-1 ring-slate-100 transition-all group-hover:bg-blue-50 group-hover:ring-blue-200 dark:bg-slate-900 dark:ring-slate-800 dark:group-hover:bg-blue-950/40 dark:group-hover:ring-blue-800">
                              <svg
                                className="h-6 w-6 text-slate-400 transition-colors group-hover:text-blue-500"
                                fill="none"
                                stroke="currentColor"
                                viewBox="0 0 24 24"
                              >
                            ...
                            <div className="text-center">
                              <p className="text-sm font-semibold text-slate-700 transition-colors group-hover:text-blue-700 dark:text-slate-200 dark:group-hover:text-blue-300">
                  ...
                  <Card variant="default" className={sectionCardCls}>
                    <Card.Content className={`${sectionCardBodyCls} space-y-5`}>
                      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                        ...
                        <div className="rounded-[22px] border border-slate-200 bg-gradient-to-br from-slate-50 to-white px-4 py-4 shadow-sm">
                          ...
                          {product.uploaderRole ? (
                            <p className="mt-1 text-[11px] tracking-[0.12em] text-slate-400 uppercase">
                      ...
                      <Field
                        label={isServicesView ? "Company Name" : "Product Name"}
                        required
                            ...
                            onChange={(value) => {
                              ...
                              if (!roomTouched) {
                                ...
                                const inferredRoomType =
                                  inferRoomTypeFromCategory(selectedCategory)
                                set(
                                  "pd_room_type",
                                  inferredRoomType
                  ...
                  {isServicesView && (
                    <Field label="Status">
                      ...
                      {!canEditProductStatus && (
                        <p className="mt-1 text-[11px] text-slate-400">
                          Status is locked while the service is pending review.
                        </p>
                      )}
                    </Field>
                  )}
                    ...
                    <>
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        <Field label="Material">
                          <input
                            ...
                            onChange={(e) => set("pd_material", e.target.value)}
                        ...
                        {!isSupplierScopedActor && (
                          ...
                          >
                            <div className="space-y-1">
                              ...
                              />
                              <p className="text-[11px] text-slate-500">
                                Shown to member accounts. If blank, SRP will be
                                used.
                              </p>
                            </div>
                          </Field>
                      ...
                      <div className="grid grid-cols-2 gap-2 sm:grid-cols-3 sm:gap-3">
                        <Field label="Width / W (cm)" error={errors.pd_pswidth}>
                          <input
                            ...
                            onChange={(e) => set("pd_pswidth", e.target.value)}
                            placeholder="0"
                            className={inputCls(!!errors.pd_pswidth)}
                          />
                        </Field>
                        <Field
                          label="Length / L (cm)"
                        ...
                        >
                          <input
                            ...
                            onChange={(e) => set("pd_pslenght", e.target.value)}
                            placeholder="0"
                        ...
                        >
                          <input
                            ...
                            onChange={(e) => set("pd_psheight", e.target.value)}
                            placeholder="0"
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        ...
                        >
                          <input
                            ...
                            onChange={(e) => set("pd_psweight", e.target.value)}
                            placeholder="0.00"
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        ...
                        <Field label="Status">
                          ...
                          {!canEditProductStatus && (
                            <p className="mt-1 text-[11px] text-slate-400">
                              Status is locked while the product is pending
                              review.
                            </p>
                          )}
                        </Field>
                            ...
                            <div className="rounded-2xl border border-blue-100 bg-gradient-to-br from-blue-50 to-cyan-50 p-4">
                              <div className="flex items-start justify-between gap-3">
                                <div>
                                  ...
                                  <p className="mt-1 max-w-2xl text-xs leading-relaxed text-slate-500">
                                    ...
                                    choices for this product upload. These
                                    options are not limited to size and color;
                                    they can also include finishes, dimensions,
                                    materials, styles, thickness, or any custom
                                    values that apply to this product variant
                                    set.
                                  </p>
                                </div>
                              ...
                              <div className="mt-4 flex items-center gap-2 rounded-xl border border-blue-100 bg-white/80 p-2.5 dark:border-blue-900/50 dark:bg-slate-950/70">
                                <label className="group relative shrink-0 cursor-pointer">
                                  <div
                                    className="h-10 w-10 rounded-xl border-2 border-white shadow-sm ring-1 ring-slate-200 transition-all group-hover:ring-blue-400"
                                    style={{
                                      backgroundColor:
                                ...
                                <div className="flex-1 space-y-1">
                                  <input
                                    ...
                                    className="w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm text-slate-700 placeholder-slate-400 focus:border-blue-400 focus:ring-2 focus:ring-blue-400/40 focus:outline-none dark:border-slate-700 dark:bg-slate-950 dark:text-slate-100 dark:placeholder:text-slate-500"
                                  />
                                  <p className="text-[11px] text-slate-400">
                                    Use this when the same colors apply across
                                    the whole product.
                                  </p>
                                </div>
                                <button
                                  ...
                                  className="shrink-0 rounded-xl bg-blue-600 px-4 py-2.5 text-sm font-semibold text-white transition-colors hover:bg-blue-700"
                                >
                                  Add Color
                                </button>
                              </div>
                              ...
                              {globalColors.length > 0 && (
                                <div className="mt-3 flex flex-wrap gap-2">
                                  {globalColors.map((color, colorIndex) => (
                                    <span
                                      key={`${getVariantColorKey(color)}-${colorIndex}`}
                                      className="inline-flex items-center gap-1.5 rounded-full border border-blue-100 bg-white py-1 pr-2.5 pl-1 shadow-sm dark:border-blue-900/50 dark:bg-slate-900"
                                    >
                                      <span
                              ...
                              <div className="mt-4 space-y-4 rounded-xl border border-blue-100 bg-white/80 p-3 dark:border-blue-900/50 dark:bg-slate-950/70">
                                  ...
                                  <div className="space-y-1">
                                    ...
                                    <div className="flex gap-2">
                                      <input
                                        ...
                                        className="flex-1 rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm text-slate-700 placeholder-slate-400 focus:border-blue-400 focus:ring-2 focus:ring-blue-400/40 focus:outline-none dark:border-slate-700 dark:bg-slate-950 dark:text-slate-100 dark:placeholder:text-slate-500"
                                      />
                                      <button
                                        type="button"
                                    ...
                                    {globalPrimaryValues.map(
                                      (value, valueIndex) => (
                                        <span
                                          ...
                                          className="inline-flex items-center gap-1.5 rounded-full border border-blue-100 bg-blue-50/70 py-1.5 pr-2 pl-3 shadow-sm"
                                        >
                                          <span className="text-xs font-medium text-slate-700 dark:text-slate-200">
                                            {value}
                                          </span>
                                ...
                                <div className="space-y-1">
                                  ...
                                  <p className="text-[11px] text-slate-400">
                                    Add repeatable size values here if you do
                                    not need per-variant cards.
                                  </p>
                                </div>

                                {globalSizeValues.length > 0 && (
                                  <div className="flex flex-wrap gap-2">
                                    {globalSizeValues.map(
                                      (value, valueIndex) => (
                                        <span
                                          key={`${value}-${valueIndex}`}
                                          className="inline-flex items-center gap-1.5 rounded-full border border-blue-100 bg-white py-1.5 pr-2 pl-3 shadow-sm dark:border-blue-900/50 dark:bg-slate-900"
                                        >
                                          <span className="text-xs font-medium text-slate-700 dark:text-slate-200">
                                            {value}
                                          </span>
                                      ...
                                      <div className="space-y-2.5 px-4 py-3.5">
                                        ...
                                        <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
                                          ...
                                          <div className="space-y-1">
                                            ...
                                            <div className="flex items-center gap-2">
                                              ...
                                              <button
                                                ...
                                                className="shrink-0 rounded-lg border border-blue-200 bg-blue-50 px-3 py-2 text-xs font-semibold text-blue-700 transition-colors hover:bg-blue-100"
                                              >
                                                + Add
                                              </button>
                                            </div>
                                          ...
                                          <div className="space-y-1">
                                            ...
                                            />
                                            <p className="text-[10px] text-slate-400">
                                              Leave blank if this variant does
                                              not use a size value.
                                            </p>
                                          </div>
                                          <div className="space-y-1">
                                            <label className="block text-[11px] font-semibold text-slate-500">
