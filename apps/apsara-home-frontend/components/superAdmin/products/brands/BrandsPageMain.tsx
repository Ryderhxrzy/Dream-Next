          ...
          <form onSubmit={handleSubmit} className="mt-6 space-y-4">
            ...
            <div className="space-y-1.5">
              ...
              <div className="grid grid-cols-2 gap-2">
                {[
                  { value: "0", label: "Active", color: "teal" },
                  { value: "1", label: "Disabled", color: "slate" },
                ].map((opt) => (
                  <button
                    key={opt.value}
                    type="button"
          ...
          <div className="flex flex-col items-center text-center">
            ...
            <div className="mt-2 w-full rounded-xl border border-slate-100 bg-slate-50 px-4 py-3 text-left">
              ...
              <p className="mt-0.5 text-xs text-slate-500">
                Status:{" "}
                <span
                  className={`font-semibold ${brand.status === 0 ? "text-emerald-600" : "text-slate-500"}`}
                >
  ...
  return (
    <div className="rounded-2xl border border-slate-100 bg-white px-5 py-4 shadow-sm">
      <div className="flex items-center justify-between">
        ...
        <div
          className={`flex h-10 w-10 items-center justify-center rounded-2xl ${color === "text-slate-900" ? "bg-slate-100" : color === "text-emerald-600" ? "bg-emerald-50" : "bg-amber-50"}`}
        >
          {icon}
        </div>
      </div>
...
export default function BrandsPageMain() {
  ...
  const activeCount = useMemo(
    () => brands.filter((b) => b.status === 0).length,
    [brands]
  )
  const disabledCount = useMemo(
    () => brands.filter((b) => b.status === 1).length,
    [brands]
  )
    ...
    <div className="space-y-5">
      ...
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">
            Manage product brands used in product dropdown selections.
          </p>
        </div>
        <button
                ...
                paginatedBrands.map((brand, index) => (
                  ...
                  >
                    ...
                    <td className="px-5 py-4">
                      ...
                      >
                        <span
                          className={`h-1.5 w-1.5 rounded-full ${brand.status === 0 ? "bg-emerald-500" : "bg-slate-400"}`}
                        />
                        {brand.status === 0 ? "Active" : "Disabled"}
                      </span>
                    </td>
        ...
        {!isLoading && filteredBrands.length > PAGE_SIZE && (
          <div className="flex flex-wrap items-center justify-between gap-3 border-t border-slate-100 px-5 py-4">
            <p className="text-xs text-slate-400">
              ...
              –
              <span className="font-semibold text-slate-600">
                {Math.min(currentPage * PAGE_SIZE, filteredBrands.length)}
              </span>{" "}
              of{" "}
              <span className="font-semibold text-slate-600">
                {filteredBrands.length}
      ...
      {deletingBrand && (
        <DeleteConfirmModal
          ...
          onClose={() => {
            if (isDeletingId === null) setDeletingBrand(null)
          }}
        />
      )}
    </div>
