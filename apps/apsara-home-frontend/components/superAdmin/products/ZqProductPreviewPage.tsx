...
export const extractZqImportDetail = (
  ...
  return {
    ...
    importProductStatus:
      typeof data.importProductStatus === "string"
        ? data.importProductStatus
        : null,
    ...
    targetCurrency:
      typeof data.targetCurrency === "string" ? data.targetCurrency : null,
      ...
      return {
        ...
        salesPrice:
          typeof row.salesPrice === "number"
            ? row.salesPrice
            : typeof row.salesPrice === "string"
              ? Number(row.salesPrice)
              : null,
        amountOnSale:
          typeof row.amountOnSale === "number"
            ? row.amountOnSale
            : typeof row.amountOnSale === "string"
              ? Number(row.amountOnSale)
              : null,
        weight:
        ...
        attributes: attributesRaw.map((attribute) => {
          ...
          return {
            attributeId:
              attr.attributeId == null ? null : String(attr.attributeId),
            value: typeof attr.value === "string" ? attr.value : null,
            skuImageUrl:
              typeof attr.skuImageUrl === "string" ? attr.skuImageUrl : null,
          }
        }),
      }
    }),
...
export default function ZqProductPreviewPage({
  ...
  const heroImage =
    detail.images[0] ?? detail.specs.find((item) => item.image)?.image ?? null
  const primarySpec = detail.specs[0] ?? null
  const totalStock = detail.specs.reduce(
    (sum, spec) => sum + Number(spec.amountOnSale ?? 0),
    0
  )
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <h1 className="mt-1 text-2xl font-bold text-slate-900">
            Global Supplier Product Preview
          </h1>
          <p className="mt-1 text-sm text-slate-500">
            Viewing only ito. Hindi pa ito kasama sa local catalog o sa public
            All Products.
          </p>
        </div>
        <Link
      ...
      <div className="grid gap-6 lg:grid-cols-[420px_minmax(0,1fr)]">
              ...
              <div className="space-y-2">
                ...
                <div className="flex flex-wrap items-end gap-2">
                  <span className="text-3xl font-extrabold text-sky-700">
                    {centsToDisplay(primarySpec?.salesPrice ?? null)}
                  </span>
                  {primarySpec?.cost != null ? (
                    <span className="text-base text-slate-400 line-through">
                      {centsToDisplay(primarySpec.cost)}
                    </span>
                  ) : null}
                </div>
                <div className="flex flex-wrap gap-2 pt-1">
                  ...
                  <span className="rounded-full border border-slate-200 bg-slate-50 px-3 py-1 text-xs font-semibold text-slate-600">
                    {detail.specs.length} variant
                    {detail.specs.length !== 1 ? "s" : ""}
                  </span>
                </div>
              </div>
              <button
        ...
        <div className="space-y-5">
          <div className="space-y-3 rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
            ...
            {detail.subjectCn ? (
              <p className="text-sm text-slate-500">{detail.subjectCn}</p>
            ) : null}
            <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
              ...
              <div className="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3">
                ...
                <p className="mt-1 text-sm font-semibold text-slate-700">
                  {detail.updatedAt ?? detail.published ?? "—"}
                </p>
              </div>
            </div>
            {detail.productUrl ? (
          ...
          <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
            ...
            <p className="mt-2 text-xs text-slate-400">
              Source HTML is hidden here para sariling preview lang ang makita
              natin.
            </p>
          </div>
            ...
            <div className="mb-4 flex items-center justify-between gap-3">
              <div>
                ...
                <p className="text-sm text-slate-500">
                  Ito yung pwede nating i-map later sa variants, prices, at
                  stock.
                </p>
              </div>
              <span className="rounded-full border border-slate-200 bg-slate-50 px-3 py-1 text-xs font-semibold text-slate-600">
                  ...
                  {detail.specs.map((spec) => (
                    <tr key={spec.id}>
                      ...
                      <td className="px-4 py-3 font-semibold text-slate-700">
                        {centsToDisplay(spec.cost)}
                      </td>
                      <td className="px-4 py-3 font-semibold text-sky-700">
                        {centsToDisplay(spec.salesPrice)}
                      </td>
