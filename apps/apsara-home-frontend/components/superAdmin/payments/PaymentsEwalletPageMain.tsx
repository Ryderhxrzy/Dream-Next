...
const statusTone = (status: string) => {
  ...
  if (["approved_by_admin", "on_hold"].includes(normalized))
...
export default function PaymentsEwalletPageMain() {
  ...
  const encashment = useGetAdminEncashmentRequestsQuery({
    filter: "all",
    page: 1,
    perPage: 8,
  })
  ...
  const isLoading =
    encashment.isLoading || (overview.isLoading && !encashment.data)
  const hasFatalError = !encashment.data && encashment.isError
  const pendingRequests =
    overview.data?.encashment_summary.pending_requests ??
    encashment.data?.counts.pending ??
    0
  const releasedRequests =
    overview.data?.encashment_summary.released_requests ??
    encashment.data?.counts.released ??
    0
  const totalRequests =
    overview.data?.encashment_summary.total_requests ??
    encashment.data?.counts.all ??
    0
  const releasedAmount =
    overview.data?.encashment_summary.released_amount ??
      ...
      <div className="animate-pulse space-y-6">
        <div className="grid gap-4 md:grid-cols-3">
          {Array.from({ length: 3 }).map((_, index) => (
            <div
              key={index}
              className="h-32 rounded-2xl border border-slate-200 bg-white shadow-sm dark:border-slate-700 dark:bg-slate-800"
            />
          ))}
        </div>
  ...
  return (
    <div className="space-y-6">
      ...
      >
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500 dark:text-slate-300">
            Cash-out and wallet-related admin view powered by the current
            encashment workflow.
          </p>
        </div>
      ...
      {overview.isFetching || encashment.isFetching ? (
        ...
        <>
          {overview.isError ? (
            <div className="rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800 dark:border-amber-800 dark:bg-amber-950/30 dark:text-amber-300">
              Payments summary is temporarily unavailable, so this page is
              showing encashment-based fallback data.
            </div>
          ) : null}
          ...
          >
            <div className="rounded-2xl border border-amber-200 bg-white p-4 shadow-sm dark:border-amber-700 dark:bg-slate-800">
              ...
              <p className="mt-1 text-sm text-slate-500 dark:text-slate-300">
                Requests waiting for approval or release
              </p>
            </div>
            ...
            <div className="rounded-2xl border border-emerald-200 bg-white p-4 shadow-sm dark:border-emerald-700 dark:bg-slate-800">
              ...
              <p className="mt-1 text-sm text-slate-500 dark:text-slate-300">
                {releasedRequests.toLocaleString()} released request(s)
              </p>
            </div>
            ...
            <div className="rounded-2xl border border-sky-200 bg-white p-4 shadow-sm dark:border-sky-700 dark:bg-slate-800">
              ...
              <p className="mt-1 text-sm text-slate-500 dark:text-slate-300">
                Total encashment records connected to wallet activity
              </p>
            </div>
          </motion.div>
              ...
              <div className="flex items-center justify-between gap-3 border-b border-slate-100 px-5 py-4 dark:border-slate-700">
                <div>
                  ...
                  <p className="mt-1 text-xs text-slate-400 dark:text-slate-300">
                    These records drive the admin side of wallet release
                    monitoring
                  </p>
                </div>
              ...
              <div className="overflow-x-auto">
                <table className="min-w-full text-left">
                  ...
                  <tbody className="divide-y divide-slate-100 dark:divide-slate-700">
                    {encashment.data.requests.length ? (
                      encashment.data.requests.map((request) => (
                        <tr
                          key={request.id}
                        ...
                        >
                          ...
                          <td className="px-4 py-3.5">
                            <p className="text-sm font-semibold text-slate-800 dark:text-slate-100">
                              {request.affiliate_name || "Affiliate"}
                            </p>
                            <p className="mt-0.5 text-xs text-slate-400 dark:text-slate-400">
                              {request.affiliate_email || "No email"}
                            </p>
                          </td>
                          ...
                          <td className="px-4 py-3.5">
                            <span
                              className={`inline-flex rounded-full border px-2.5 py-1 text-[11px] font-semibold capitalize ${statusTone(request.status)}`}
                            >
            ...
            >
              ...
              <div className="mt-4 space-y-3">
                <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4 dark:border-slate-600 dark:bg-slate-700/50">
                  ...
                  <p className="mt-1 text-sm text-slate-500 dark:text-slate-300">
                    Wallet debits happen when encashment releases are finalized
                    on the backend.
                  </p>
                </div>
                <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4 dark:border-slate-600 dark:bg-slate-700/50">
                  ...
                  <p className="mt-1 text-sm text-slate-500 dark:text-slate-300">
                    Accounting and finance already manage this through the
                    existing encashment module.
                  </p>
                </div>
                <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4 dark:border-slate-600 dark:bg-slate-700/50">
                  <p className="text-sm font-semibold text-slate-800 dark:text-slate-100">
                  ...
                  <p className="mt-1 text-sm text-slate-500 dark:text-slate-300">
                    The wallet master view still lives under Members, so this
                    page acts as the finance-focused summary.
                  </p>
                </div>
              </div>
            </motion.section>
