...
const SalesChart = () => {
    ...
    return chartConfig.chartData.reduce((sum, row) => {
      return (
        ...
        chartConfig.series.reduce(
          (inner, item) => inner + Number(row[item.key] ?? 0),
          0
        )
      )
    }, 0)
  ...
  if (!mounted)
    return (
      <div className="h-85 animate-pulse rounded-2xl border border-gray-200 bg-white p-6 dark:border-gray-700 dark:bg-gray-800" />
    )
