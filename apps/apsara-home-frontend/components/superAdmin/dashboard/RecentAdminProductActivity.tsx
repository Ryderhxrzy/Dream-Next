...
const actionTone = (action: string) => {
  switch (action) {
    ...
    default:
      return "text-gray-700 bg-gray-50 border-gray-200 dark:border-gray-700 dark:bg-gray-800/80 dark:text-gray-200"
  }
}
      ...
      <div className="flex items-center justify-between gap-3 border-b border-gray-200 px-6 py-5 dark:border-gray-700">
        <div>
          ...
          <h2 className="mt-1 text-lg font-bold text-gray-900 dark:text-gray-100">
            Recent Admin Product Activity
          </h2>
          <p className="mt-1 text-sm text-gray-500 dark:text-gray-300">
            Latest product actions across admin accounts.
          </p>
        </div>
        <Link
          href="/admin/products"
          className="rounded-xl border border-gray-200 px-3 py-2 text-sm font-semibold text-gray-600 transition hover:border-cyan-200 hover:text-cyan-700 dark:border-gray-700 dark:bg-gray-800/80 dark:text-gray-200 dark:hover:border-cyan-500/30 dark:hover:text-cyan-300"
        >
          Open Products
        </Link>
      </div>
        ...
        {isLoading ? (
          <div className="space-y-3">
            {Array.from({ length: 4 }).map((_, index) => (
              <div
                key={index}
                className="h-16 animate-pulse rounded-2xl bg-gray-200 dark:bg-gray-800/80"
              />
            ))}
          </div>
        ) : logs.length === 0 ? (
          <div className="rounded-2xl border border-dashed border-gray-300 bg-gray-50 px-5 py-10 text-center dark:border-gray-700 dark:bg-gray-800/60">
            <p className="text-sm font-semibold text-gray-700 dark:text-gray-100">
              No admin product activity yet
            </p>
            <p className="mt-1 text-sm text-gray-500 dark:text-gray-300">
              New product uploads, edits, deletes, and failures will appear
              here.
            </p>
          </div>
        ) : (
          <div className="space-y-4">
            <div className="max-h-[30rem] space-y-3 overflow-y-auto pr-1">
              {logs.map((log) => (
                <div
                  key={log.id}
                  className="rounded-2xl border border-gray-200 px-4 py-3 dark:border-gray-700 dark:bg-gray-800/60"
                >
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0">
                      <div className="flex flex-wrap items-center gap-2">
                        <span
                          className={`inline-flex items-center rounded-full border px-2.5 py-1 text-[11px] font-semibold ${actionTone(log.action)}`}
                        >
                        ...
                        <span
                          className={`inline-flex items-center rounded-full px-2 py-1 text-[11px] font-semibold ${statusTone(log.status)}`}
                        >
                          {log.status}
                        </span>
                      </div>
                      <div className="mt-1 flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-gray-500 dark:text-gray-300">
                        ...
                        <span>
                          {log.actorRole?.replace(/_/g, " ") ?? "admin"}
                        </span>
                        <span>
            ...
            <div className="flex flex-col gap-3 border-t border-gray-200 pt-4 sm:flex-row sm:items-center sm:justify-between dark:border-gray-700">
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  className="rounded-xl border border-gray-200 px-3 py-2 text-sm font-semibold text-gray-600 transition hover:border-cyan-200 hover:text-cyan-700 disabled:cursor-not-allowed disabled:opacity-50 dark:border-gray-700 dark:bg-gray-800/80 dark:text-gray-200 dark:hover:border-cyan-500/30 dark:hover:text-cyan-300"
                >
                  Previous
                </button>
                <button
