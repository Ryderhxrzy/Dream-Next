...
export default function SupplierDashboardHome() {
  ...
  const effectiveSupplierId =
    supplierId > 0 ? supplierId : Number(fallbackSupplier?.id ?? 0)
  ...
  const isServicesView = useMemo(
    () =>
      (supplierCategoriesData?.categories ?? []).some(
  ...
  const { data: inquiryData, isLoading: inquiryLoading } =
    useGetSupplierServiceInquiriesQuery(
      { per_page: 100 },
      {
        skip:
          status !== "authenticated" || (categoriesKnown && !isServicesView),
  ...
  const inquiryNewCount = useMemo(
    () =>
      allInquiries.filter((i) => i.status === "new" && inqIsToday(i.created_at))
        .length,
    [allInquiries]
  )
  const inquiryPendingCount = useMemo(
    () =>
      allInquiries.filter(
        (i) => i.status === "new" && !inqIsToday(i.created_at)
      ).length,
    [allInquiries]
  )
  const inquiryCompleteCount = useMemo(
    () =>
      allInquiries.filter(
        (i) => i.status === "responded" || i.status === "closed"
      ).length,
    [allInquiries]
  )
  const inquiryTotalCount = inquiryData?.counts?.total ?? allInquiries.length
  const recentInquiries = useMemo(
    () => allInquiries.slice(0, 6),
    [allInquiries]
  )
  ...
  const inquiryTrendData = useMemo(() => {
    ...
    return days.map((day) => ({
      date: new Date(`${day}T00:00:00`).toLocaleDateString("en-PH", {
        month: "short",
        day: "numeric",
      }),
      Inquiries: cnts[day],
  ...
  const inquiryStatusTrendData = useMemo(() => {
    ...
    days.forEach((d) => {
      map[d] = { New: 0, Pending: 0, Complete: 0 }
    })
    for (const i of allInquiries) {
      const day = (i.created_at ?? "").slice(0, 10)
      if (!(day in map)) continue
      if (i.status === "new" && inqIsToday(i.created_at)) map[day].New++
      else if (i.status === "new" && !inqIsToday(i.created_at))
        map[day].Pending++
      else if (i.status === "responded" || i.status === "closed")
        map[day].Complete++
    }
    return days.map((day) => ({
      date: new Date(`${day}T00:00:00`).toLocaleDateString("en-PH", {
  ...
  const inquiryDonutData = useMemo(
    ...
    [inquiryNewCount, inquiryPendingCount, inquiryCompleteCount]
  )
  ...
  const { data: trendOrdersData, isLoading: ordersLoading } =
  ...
  const useBrandFallback = useMemo(
    () => brandType > 0 && (productsData?.meta?.total ?? 0) <= 0,
    [brandType, productsData?.meta?.total]
  )

  const effectiveProductsData = useBrandFallback
    ? brandProductsData
    : productsData
  const effectiveActiveProductsData = useBrandFallback
    ? brandActiveProductsData
    : activeProductsData
  const effectiveInactiveProductsData = useBrandFallback
    ? brandInactiveProductsData
  ...
  const recentProducts = useMemo(
    () => (effectiveProductsData?.products ?? []).slice(0, 6),
    [effectiveProductsData?.products]
  )

  const lowStockCount = useMemo(
    () =>
      (effectiveInventoryData?.products ?? []).filter(
        (p) => Number(p.qty ?? 0) > 0 && Number(p.qty ?? 0) <= 5
      ).length,
  ...
  const subSupplierCount = useMemo(
    () =>
      (supplierUsersData?.users ?? []).filter((u) => !u.is_main_supplier)
        .length,
    [supplierUsersData?.users]
  )
  ...
  const statusTrendData = useMemo(() => {
    ...
    const map: Record<string, Row> = {}
    days.forEach((d) => {
      map[d] = { Processing: 0, Shipped: 0, Delivered: 0, Cancelled: 0 }
    })
    for (const o of trendOrdersData?.orders ?? []) {
      const day = (o.created_at ?? "").slice(0, 10)
      if (!(day in map)) continue
  ...
  const productLineData = useMemo(
    () => [
      ...
      {
        name: "Inactive",
        value: effectiveInactiveProductsData?.meta?.total ?? 0,
      },
      { name: "Low Stock", value: lowStockCount },
    ],
    [
      effectiveProductsData,
      effectiveActiveProductsData,
      effectiveInactiveProductsData,
      lowStockCount,
    ]
  )
  ...
  const donutData = useMemo(
    () =>
      [
        ...
        {
          name: "To Receive",
          value: orderCounts?.to_receive ?? 0,
          fill: "#8b5cf6",
        },
        {
          name: "Completed",
  ...
  return (
    <div className="space-y-7">
        ...
        <div className="relative flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
          <div>
            ...
            <div className="mt-2.5 flex flex-wrap items-center gap-2">
              ...
              <span
                className={`inline-flex rounded-full px-3 py-1 text-[11px] font-semibold ${supplier?.status === 1 ? "bg-emerald-400/30 text-white" : "bg-white/15 text-white/80"}`}
              >
            ...
            <div className="mt-4 flex flex-wrap gap-3">
              {[
                {
                  label: isServicesView ? "Total Bookings" : "Total Orders",
                  value: (isServicesView ? inquiryLoading : ordersLoading)
                    ? "…"
                    : String(isServicesView ? inquiryTotalCount : totalOrders),
                },
                {
                  label: isServicesView ? "Services" : "Products",
                  value: String(effectiveProductsData?.meta?.total ?? 0),
                },
                {
                  label: "Categories",
                  value: String(
                    supplierCategoriesData?.categories?.length ?? 0
                  ),
                },
              ].map((s) => (
                <div
      ...
      <div>
        <SectionLabel
          icon={isServicesView ? CalendarDays : ShoppingCart}
          color="text-blue-500"
        >
          {isServicesView ? "Booking Analytics" : "Order Analytics"}
        </SectionLabel>
        <div
          className={`grid grid-cols-2 gap-3 sm:grid-cols-3 ${isServicesView ? "xl:grid-cols-4" : "xl:grid-cols-6"}`}
        >
          <StatCard
            label={isServicesView ? "Total Inquiries" : "Total Orders"}
            value={
              (isServicesView ? inquiryLoading : ordersLoading)
                ? "—"
                : String(isServicesView ? inquiryTotalCount : totalOrders)
            }
            icon={isServicesView ? CalendarDays : ShoppingCart}
          ...
          />
          <StatCard
            label={isServicesView ? "New" : "Pending Payment"}
            value={
              (isServicesView ? inquiryLoading : ordersLoading)
                ? "—"
                : String(
                    isServicesView
                      ? inquiryNewCount
                      : (orderCounts?.to_pay ?? 0)
                  )
            }
            icon={isServicesView ? MessageSquare : Clock3}
            gradient="from-amber-400 to-orange-500"
            href="/supplier/orders?filter=to_pay"
            alert={
              isServicesView
                ? inquiryNewCount > 0
                : (orderCounts?.to_pay ?? 0) > 0
            }
          />
          <StatCard
            label={isServicesView ? "Pending" : "To Ship"}
            value={
              (isServicesView ? inquiryLoading : ordersLoading)
                ? "—"
                : String(
            ...
            icon={isServicesView ? Settings : Package}
            gradient="from-sky-400 to-cyan-500"
            href="/supplier/orders?filter=to_ship"
          />
          {!isServicesView && (
            <StatCard
              label="To Receive"
              value={ordersLoading ? "—" : String(orderCounts?.to_receive ?? 0)}
              icon={Truck}
              gradient="from-violet-500 to-purple-600"
              href="/supplier/orders?filter=to_receive"
            />
          )}
          <StatCard
            label={isServicesView ? "Complete" : "Completed"}
            value={
              (isServicesView ? inquiryLoading : ordersLoading)
                ? "—"
                : String(
                    isServicesView
                      ? inquiryCompleteCount
                      : (orderCounts?.completed ?? 0)
                  )
            }
            icon={CheckCircle2}
            gradient="from-emerald-400 to-teal-500"
            href="/supplier/orders?filter=completed"
          />
          {!isServicesView && (
            <StatCard
              label="Cancelled"
              value={ordersLoading ? "—" : String(orderCounts?.cancelled ?? 0)}
              icon={XCircle}
              gradient="from-rose-400 to-red-500"
              href="/supplier/orders?filter=cancelled"
            />
          )}
        </div>
      ...
      <div className="grid gap-5 xl:grid-cols-[1.5fr_1fr]">
        ...
        {isServicesView ? (
          <ChartCard
            ...
            loading={!mounted || inquiryLoading}
            empty={inquiryStatusTrendData.every(
              (d) => d.New === 0 && d.Pending === 0 && d.Complete === 0
            )}
      ...
      <div>
        <SectionLabel
          icon={isServicesView ? MessageSquare : Package}
          color="text-sky-500"
        >
        ...
        {!isServicesView && (
          <div className="grid grid-cols-2 gap-3 xl:grid-cols-4">
            ...
            <StatCard
              ...
              alert={(effectiveInactiveProductsData?.meta?.total ?? 0) > 0}
            />
            <StatCard
              label="Low Stock"
      ...
      <div className="grid gap-5 xl:grid-cols-[1.5fr_1fr]">
        ...
        <ChartCard
          ...
          subtitle={
            isServicesView
              ? "Current snapshot across your services"
              : "Current snapshot across your catalog"
          }
          loading={!mounted}
          empty={productLineData.every((d) => d.value === 0)}
          emptyMessage={
            isServicesView ? "No services added yet" : "No products added yet"
          }
        ...
        >
          ...
          <div className="mt-4 grid grid-cols-4 divide-x divide-slate-100 border-t border-slate-100 pt-4 dark:divide-slate-800 dark:border-slate-800">
            {[
              {
                label: "Total",
                value: effectiveProductsData?.meta?.total ?? 0,
                color: "#0ea5e9",
              },
              {
                label: "Active",
                value: effectiveActiveProductsData?.meta?.total ?? 0,
                color: "#10b981",
              },
              {
                label: "Inactive",
              ...
              {
                label: isServicesView ? "Needs Update" : "Low Stock",
                value: lowStockCount,
                color: "#f59e0b",
              },
            ].map((m) => (
            ...
            <div className="flex flex-col items-center gap-4">
              ...
              <div className="w-full space-y-1.5">
                {inquiryDonutData.map((item) => (
                  <div
                    key={item.name}
                    className="flex items-center gap-2 rounded-lg px-2 py-1.5 transition hover:bg-slate-50 dark:hover:bg-slate-800/50"
                  >
                    <span
                      className="h-2.5 w-2.5 shrink-0 rounded-full"
                      style={{ backgroundColor: item.fill }}
                    />
                    <span className="flex-1 text-[11.5px] text-slate-600 dark:text-slate-300">
                      {item.name}
                    </span>
                    <span className="text-[11.5px] font-bold text-slate-800 dark:text-slate-100">
                      {item.value}
                    </span>
                    <span className="text-[10px] text-slate-400 dark:text-slate-500">
                      {inquiryTotalCount > 0
                        ? `${Math.round((item.value / inquiryTotalCount) * 100)}%`
                        : "—"}
                    </span>
                  </div>
                ))}
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-sm font-bold text-slate-900 dark:text-white">
                {isServicesView ? "Recent Bookings" : "Recent Orders"}
              </h2>
              <p className="text-[11px] text-slate-400 dark:text-slate-500">
                {isServicesView
                  ? "Latest booking requests from your clients"
                  : "Latest incoming from your storefront"}
              </p>
            </div>
          </div>
          <Link
            href="/supplier/orders"
            className="inline-flex items-center gap-1.5 rounded-xl bg-linear-to-r from-sky-500 to-blue-600 px-4 py-2 text-xs font-bold text-white shadow-sm transition hover:from-sky-600 hover:to-blue-700"
          >
            {isServicesView ? "View all bookings" : "View all orders"}{" "}
          ...
          {isServicesView ? (
            inquiryLoading ? (
              ...
              <div className="p-5">
                <EmptyState
                  ...
                  sub="Inquiry requests from clients will appear here"
                />
              </div>
            ) : (
                  ...
                  <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                    {recentInquiries.map((inq) => {
                      ...
                      return (
                        <tr
                          key={inq.id}
                          className="group transition hover:bg-sky-50/50 dark:hover:bg-sky-500/5"
                        >
                          <td className="px-4 py-3.5 text-[13px] font-bold text-indigo-500 dark:text-indigo-400">
                            #{inq.id}
                          </td>
                          ...
                          <td className="px-4 py-3.5 text-[13px] text-slate-600 dark:text-slate-300">
                            {inq.contact || "—"}
                          </td>
                          <td className="px-4 py-3.5">
                            <span
                              className={`inline-flex items-center gap-1.5 rounded-full border px-2.5 py-1 text-[11px] font-semibold ${statusStyle}`}
                            >
                              <span
            ...
            <div className="p-5">
              <EmptyState
                ...
                sub="Orders from your storefront will appear here"
              />
            </div>
          ) : (
            <div className="overflow-x-auto">
                      ...
                      <td className="px-4 py-3.5">
                        <div className="flex items-center gap-3">
                          ...
                          <div className="min-w-0">
                            <p className="max-w-[200px] truncate text-[13px] font-semibold text-slate-800 dark:text-slate-100">
                              {order.product_name}
                            </p>
          ...
          {(orderCounts?.return ?? 0) > 0 && (
            <div className="m-5 flex items-center gap-3 rounded-xl border border-orange-200 bg-orange-50 px-4 py-3 dark:border-orange-500/20 dark:bg-orange-500/10">
              ...
              <p className="flex-1 text-xs font-semibold text-orange-700 dark:text-orange-300">
                {orderCounts?.return} return
                {(orderCounts?.return ?? 0) !== 1 ? "s" : ""} need attention
              </p>
              <Link
                href="/supplier/orders?filter=return"
      ...
      <div className="grid gap-5 xl:grid-cols-2">
        ...
        <div className="rounded-2xl border border-slate-200/80 bg-white/95 shadow-sm dark:border-slate-700/50 dark:bg-slate-900">
          <div className="flex items-center justify-between border-b border-slate-100 px-5 py-4 dark:border-slate-800">
            <div className="flex items-center gap-3">
              <span
                className={`flex h-9 w-9 items-center justify-center rounded-xl ${isServicesView ? "bg-teal-50 text-teal-500 dark:bg-teal-500/10 dark:text-teal-400" : "bg-amber-50 text-amber-500 dark:bg-amber-500/10 dark:text-amber-400"}`}
              >
                {isServicesView ? (
              ...
              <div>
                <h2 className="text-sm font-bold text-slate-900 dark:text-white">
                  {isServicesView ? "Services Status" : "Inventory Attention"}
                </h2>
                <p className="text-[11px] text-slate-400 dark:text-slate-500">
                  {isServicesView
                    ? "Overview of your service listings"
            ...
            <Link
              href={
                isServicesView ? "/supplier/products" : "/supplier/inventory"
              }
              className="inline-flex items-center gap-1 rounded-xl border border-slate-200 bg-slate-50 px-3 py-1.5 text-[11px] font-semibold text-slate-500 transition hover:border-amber-200 hover:bg-amber-50 hover:text-amber-700 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-400 dark:hover:text-amber-400"
            >
              Review
            </Link>
          </div>
          <div className="space-y-3 p-5">
            <InventoryBar
              label={isServicesView ? "Active Services" : "Active Products"}
              value={effectiveActiveProductsData?.meta?.total ?? 0}
              total={effectiveProductsData?.meta?.total ?? 0}
              tone="sky"
              note={isServicesView ? "Currently bookable" : "Currently live"}
            />
            <InventoryBar
              label={isServicesView ? "Inactive Services" : "Inactive Products"}
              value={effectiveInactiveProductsData?.meta?.total ?? 0}
              total={effectiveProductsData?.meta?.total ?? 0}
              tone={
            ...
            {isServicesView ? (
              ...
              <InventoryBar
                ...
                tone="amber"
                note={
                  lowStockCount > 0 ? "Needs restock" : "No low stock alerts"
                }
              />
            )}
          </div>
        ...
        <div className="rounded-2xl border border-slate-200/80 bg-white/95 shadow-sm dark:border-slate-700/50 dark:bg-slate-900">
          <div className="flex items-center justify-between border-b border-slate-100 px-5 py-4 dark:border-slate-800">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-[11px] text-slate-400 dark:text-slate-500">
                  Who owns and uses this workspace
                </p>
              </div>
            </div>
            <Link
              href="/supplier/users"
              className="inline-flex items-center gap-1 rounded-xl border border-slate-200 bg-slate-50 px-3 py-1.5 text-[11px] font-semibold text-slate-500 transition hover:border-sky-200 hover:bg-sky-50 hover:text-sky-700 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-400 dark:hover:text-sky-400"
            >
              Manage
            </Link>
          </div>
          <div className="space-y-3 p-5">
            <TeamRow
              ...
              label="Main Supplier"
              value={
                mainSupplierUser?.fullname ||
                mainSupplierUser?.username ||
                supplierName
              }
              badge="Owner"
              tone="sky"
            ...
            <TeamRow
              icon={Users}
              label="Sub Suppliers"
              value={`${subSupplierCount} sub-supplier${subSupplierCount !== 1 ? "s" : ""}`}
              badge={subSupplierCount > 0 ? "Active" : "None"}
              tone={subSupplierCount > 0 ? "emerald" : "slate"}
            />
            <TeamRow
              icon={ShieldCheck}
              label="Your Role"
              value={
                isMainSupplier
                  ? "Can manage supplier users"
                  : "Product & company access only"
              }
              badge={isMainSupplier ? "Main" : "Sub"}
              tone={isMainSupplier ? "sky" : "slate"}
            />
            <div className="flex items-start gap-3 rounded-2xl bg-slate-50 px-4 py-3 dark:bg-slate-800/50">
              ...
              <p className="text-[11.5px] leading-relaxed text-slate-500 dark:text-slate-400">
                {isMainSupplier
                  ? "Invite sub-supplier users from the Users page to give your team separate logins."
                  : "Contact the main supplier to request user management or category changes."}
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="grid gap-5 xl:grid-cols-[1.2fr_0.8fr]">
        ...
        <div className="rounded-2xl border border-slate-200/80 bg-white/95 shadow-sm dark:border-slate-700/50 dark:bg-slate-900">
          <div className="flex items-center justify-between border-b border-slate-100 px-5 py-4 dark:border-slate-800">
            <div className="flex items-center gap-3">
              <span
                className={`flex h-9 w-9 items-center justify-center rounded-xl ${isServicesView ? "bg-teal-50 text-teal-500 dark:bg-teal-500/10 dark:text-teal-400" : "bg-sky-50 text-sky-500 dark:bg-sky-500/10 dark:text-sky-400"}`}
              >
                {isServicesView ? (
                  <MessageSquare className="h-4.5 w-4.5" />
                ) : (
              ...
              <div>
                <h2 className="text-sm font-bold text-slate-900 dark:text-white">
                  {isServicesView ? "My Services" : "Recent Products"}
                </h2>
                <p className="text-[11px] text-slate-400 dark:text-slate-500">
                  {isServicesView
                    ? "Your listed services available for booking"
                    : "Latest from your supplier catalog"}
                </p>
              </div>
            </div>
            <Link
              href="/supplier/products"
              className={`inline-flex items-center gap-1.5 rounded-xl px-4 py-2 text-xs font-bold text-white shadow-sm transition ${isServicesView ? "bg-linear-to-r from-teal-500 to-cyan-600 hover:from-teal-600 hover:to-cyan-700" : "bg-linear-to-r from-violet-500 to-indigo-600 hover:from-violet-600 hover:to-indigo-700"}`}
            >
              <Settings className="h-3.5 w-3.5" />{" "}
              {isServicesView ? "Manage Services" : "Manage Products"}
            </Link>
          </div>
          <div>
              ...
              <div className="p-5">
                <div className="flex items-start gap-3 rounded-2xl border border-amber-200 bg-amber-50 p-4 dark:border-amber-400/20 dark:bg-amber-500/10">
                  ...
                  <p className="text-sm text-amber-700 dark:text-amber-300">
                    {isServicesView
                      ? "No services yet. Start by adding your first service listing."
              ...
              <div className="overflow-x-auto">
                <table className="w-full">
                  <thead>
                    <tr className="border-b border-slate-100 bg-slate-50/80 dark:border-slate-800 dark:bg-slate-800/50">
                      {[
                        isServicesView ? "Service" : "Product",
                        "SKU",
                        isServicesView ? "Slots" : "Quantity",
                        "Status",
                        "Updated",
                        "Actions",
                      ].map((h) => (
        ...
        <div className="rounded-2xl border border-slate-200/80 bg-white/95 shadow-sm dark:border-slate-700/50 dark:bg-slate-900">
          <div className="flex items-center justify-between border-b border-slate-100 px-5 py-4 dark:border-slate-800">
            ...
            <Link
              ...
              className="inline-flex items-center gap-1 rounded-xl border border-slate-200 bg-slate-50 px-3 py-1.5 text-[11px] font-semibold text-slate-500 transition hover:border-violet-200 hover:bg-violet-50 hover:text-violet-700 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-400 dark:hover:text-violet-400"
            >
              View all
            </Link>
          </div>
          <div className="p-5">
            {(supplierCategoriesData?.categories?.length ?? 0) === 0 ? (
              <div className="flex items-start gap-3 rounded-2xl border border-amber-200 bg-amber-50 p-4 dark:border-amber-400/20 dark:bg-amber-500/10">
                ...
                <p className="text-sm text-amber-700 dark:text-amber-300">
                  {isServicesView
                    ? "No categories assigned yet. Ask the admin team."
                    : "No categories assigned yet. Ask the admin team."}
                </p>
              </div>
            ) : (
                  ...
                  {(supplierCategoriesData?.categories ?? [])
                    ...
                    .map((c, i) => {
                      ...
                      return (
                        <span
                          key={c.id}
                          className={`inline-flex items-center gap-1.5 rounded-xl border px-3 py-1.5 text-[11px] font-semibold transition hover:shadow-sm ${colors[i % colors.length]}`}
                        >
      ...
      <div className="rounded-2xl border border-slate-200/80 bg-white/95 shadow-sm dark:border-slate-700/50 dark:bg-slate-900">
        <div className="flex items-center justify-between border-b border-slate-100 px-5 py-4 dark:border-slate-800">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[11px] text-slate-400 dark:text-slate-500">
                Core company details and workspace ownership
              </p>
            </div>
          </div>
          <Link
            href="/supplier/company"
            className="inline-flex items-center gap-1.5 rounded-xl border border-slate-200 bg-slate-50 px-3 py-1.5 text-[11px] font-semibold text-slate-500 transition hover:border-indigo-200 hover:bg-indigo-50 hover:text-indigo-700 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-400 dark:hover:text-indigo-400"
          >
            Edit profile
          </Link>
        </div>
        <div className="grid gap-3 p-5 sm:grid-cols-2 xl:grid-cols-3">
          <SnapCard
            label="Company"
            value={supplier?.company || supplier?.name || "Not linked"}
            icon={Building2}
            accent="indigo"
          />
          <SnapCard
            label="Contact"
            value={supplier?.contact || "Not provided"}
            icon={UserRound}
            accent="sky"
          />
          <SnapCard
          ...
          <SnapCard
            ...
            icon={ShieldCheck}
            accent={supplier?.status === 1 ? "emerald" : "slate"}
          />
          <SnapCard
            label="Main Supplier"
            value={
...
function StatCard({
  ...
  return (
    ...
    >
      ...
      {/* Gradient icon blob */}
      <div
        className={`mb-4 inline-flex h-10 w-10 items-center justify-center rounded-xl bg-linear-to-br ${gradient} text-white shadow-sm`}
      >
      ...
      {/* bottom accent line */}
      <div
        className={`absolute bottom-0 left-0 h-0.5 w-full bg-linear-to-r ${gradient} opacity-0 transition-opacity duration-200 group-hover:opacity-100`}
      />
    </Link>
  )
}
    ...
    <div className="rounded-2xl border border-slate-200/80 bg-white/95 shadow-sm dark:border-slate-700/50 dark:bg-slate-900">
      <div className="flex items-start justify-between border-b border-slate-100 px-5 py-4 dark:border-slate-800">
        ...
        {action && (
          <Link
            href={action.href}
            className="inline-flex items-center gap-1 rounded-lg border border-slate-200 bg-slate-50 px-3 py-1 text-[11px] font-semibold text-slate-500 transition hover:border-sky-200 hover:bg-sky-50 hover:text-sky-700 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-400 dark:hover:text-sky-300"
          >
...
function FulfillmentBadge({ status }: { status: string }) {
  const map: Record<string, { label: string; cls: string }> = {
    processing: {
      label: "Processing",
      cls: "bg-blue-50 text-blue-600 border-blue-200 dark:bg-blue-500/10 dark:text-blue-300 dark:border-blue-500/20",
    },
    packed: {
      label: "Packed",
      cls: "bg-sky-50 text-sky-600 border-sky-200 dark:bg-sky-500/10 dark:text-sky-300 dark:border-sky-500/20",
    },
    shipped: {
      label: "Shipped",
      cls: "bg-violet-50 text-violet-600 border-violet-200 dark:bg-violet-500/10 dark:text-violet-300 dark:border-violet-500/20",
    },
    out_for_delivery: {
      label: "Out for Delivery",
      cls: "bg-amber-50 text-amber-600 border-amber-200 dark:bg-amber-500/10 dark:text-amber-300 dark:border-amber-500/20",
    },
    delivered: {
      label: "Delivered",
      cls: "bg-emerald-50 text-emerald-600 border-emerald-200 dark:bg-emerald-500/10 dark:text-emerald-300 dark:border-emerald-500/20",
    },
    cancelled: {
      label: "Cancelled",
      cls: "bg-rose-50 text-rose-600 border-rose-200 dark:bg-rose-500/10 dark:text-rose-300 dark:border-rose-500/20",
    },
    returned: {
      label: "Returned",
      cls: "bg-orange-50 text-orange-600 border-orange-200 dark:bg-orange-500/10 dark:text-orange-300 dark:border-orange-500/20",
    },
  }
  const { label, cls } = map[status] ?? {
    label: status,
    cls: "bg-slate-50 text-slate-500 border-slate-200 dark:bg-slate-800 dark:text-slate-400 dark:border-slate-700",
  }
  return (
    <span
...
function InventoryBar({
  ...
  const s = {
    sky: {
      ...
      numBg: "bg-sky-100 dark:bg-sky-500/15",
      num: "text-sky-600 dark:text-sky-300",
      bar: "bg-linear-to-r from-sky-400 to-cyan-400",
      track: "bg-sky-100 dark:bg-sky-900/30",
    },
    amber: {
      border: "border-l-amber-400",
      numBg: "bg-amber-100 dark:bg-amber-500/15",
    ...
    rose: {
      ...
      numBg: "bg-rose-100 dark:bg-rose-500/15",
      num: "text-rose-600 dark:text-rose-300",
      bar: "bg-linear-to-r from-rose-400 to-pink-400",
      track: "bg-rose-100 dark:bg-rose-900/30",
    },
    emerald: {
      border: "border-l-emerald-400",
      numBg: "bg-emerald-100 dark:bg-emerald-500/15",
  ...
  return (
    ...
    >
      ...
      >
        <span className={`text-xl font-bold tabular-nums ${s.num}`}>
          {value}
        </span>
      </div>
      <div className="min-w-0 flex-1">
        <div className="mb-2 flex items-center justify-between">
          ...
          <span className={`text-xs font-bold tabular-nums ${s.num}`}>
            {pct}%
          </span>
        </div>
      ...
      <td className="px-4 py-3.5">
        <div className="flex items-center gap-3">
          {product.image ? (
            ...
            <div
              className={`flex h-12 w-12 shrink-0 items-center justify-center rounded-xl ${isServicesView ? "bg-linear-to-br from-teal-100 to-cyan-100 dark:from-teal-500/15 dark:to-cyan-500/10" : "bg-linear-to-br from-sky-100 to-cyan-100 dark:from-sky-500/15 dark:to-cyan-500/10"}`}
            >
              {isServicesView ? (
          ...
          <div className="min-w-0">
            <p className="max-w-55 truncate text-[13px] font-bold text-slate-800 dark:text-slate-100">
              {product.name}
            </p>
            {product.description ? (
