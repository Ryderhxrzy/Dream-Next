...
const quickActions = [
  {
    label: "Add Product",
    color:
      "bg-teal-50 hover:bg-teal-100 text-teal-700 border border-teal-200 dark:bg-teal-500/10 dark:hover:bg-teal-500/15 dark:text-teal-300 dark:border-teal-500/25",
    icon: (
      <svg
        className="h-4 w-4"
        fill="none"
  ...
  },
  {
    label: "New Order",
    color:
      "bg-blue-50 hover:bg-blue-100 text-blue-700 border border-blue-200 dark:bg-blue-500/10 dark:hover:bg-blue-500/15 dark:text-blue-300 dark:border-blue-500/25",
    icon: (
      <svg
        className="h-4 w-4"
        fill="none"
  ...
  },
  {
    label: "Add Member",
    color:
      "bg-purple-50 hover:bg-purple-100 text-purple-700 border border-purple-200 dark:bg-purple-500/10 dark:hover:bg-purple-500/15 dark:text-purple-300 dark:border-purple-500/25",
    icon: (
      <svg
        className="h-4 w-4"
        fill="none"
  ...
  },
  {
    label: "View Reports",
    color:
      "bg-orange-50 hover:bg-orange-100 text-orange-700 border border-orange-200 dark:bg-orange-500/10 dark:hover:bg-orange-500/15 dark:text-orange-300 dark:border-orange-500/25",
    icon: (
      <svg
        className="h-4 w-4"
        fill="none"
...
const TopProducts = () => {
  ...
  const products = useMemo(() => {
    ...
    const peakSoldCount = Math.max(
      ...rows.map((product) => Number(product.soldCount ?? 0)),
      0
    )
    ...
    return rows.map((product) => {
      ...
      const pct =
        peakSoldCount > 0
          ? Math.max(8, Math.round((sold / peakSoldCount) * 100))
          : 8
      const roomLabel = getRoomOptionById(product.roomType)?.label
      const metaLabel =
        roomLabel ||
        product.brand ||
        (product.sku ? `SKU ${product.sku}` : "Uncategorized")
        ...
        <div className="mb-4 flex items-center justify-between">
          <div>
            ...
            <p className="mt-0.5 text-[11px] text-gray-400 dark:text-gray-500">
              {isLoading
                ? "Loading live product rankings..."
                : "Based on actual sold count from the database"}
            </p>
          </div>
          <Link
            href="/admin/products"
          ...
          {!isLoading &&
            ...
            products.map((product, index) => (
              ...
              >
                <div className="mb-1.5 flex items-start justify-between gap-2">
                  <div className="flex items-center gap-2">
                    <span className="w-4 shrink-0 text-xs font-bold text-gray-300 dark:text-gray-500">
                      {index + 1}
                    </span>
