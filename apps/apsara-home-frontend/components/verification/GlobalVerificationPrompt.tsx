...
export default function GlobalVerificationPrompt() {
  ...
  const shouldPrompt =
    ...
    hasLoadedCustomer &&
    !isVerified &&
    rawStatus === "not_verified"
  const storageKey = me?.id
    ? `verification-prompt-dismissed:${me.id}:${rawStatus}`
    : null
  const successStorageKey = me?.id
    ? `verification-success-seen:${me.id}:verified`
    : null
  ...
  const successDismissed = successStorageKey
    ? dismissedSuccessKey === successStorageKey ||
      (typeof window !== "undefined" &&
        window.localStorage.getItem(successStorageKey) === "1")
    : false
  const shouldShowSuccess =
    isCustomerSession && hasLoadedCustomer && isVerified && !successDismissed
          ...
          >
            <div className="bg-[radial-gradient(circle_at_top,_rgba(16,185,129,0.22),_transparent_52%),linear-gradient(180deg,#ecfdf5_0%,#ffffff_100%)] px-6 pb-6 pt-7">
              ...
              <p className="mt-3 text-sm leading-7 text-slate-600">
                Your account is now approved. You can now enjoy more verified
                features across your AF Home account.
              </p>
        ...
        >
          ...
          >
            <div className="bg-[radial-gradient(circle_at_top,_rgba(251,146,60,0.22),_transparent_52%),linear-gradient(180deg,#fff7ed_0%,#ffffff_100%)] px-6 pb-6 pt-7">
              ...
              <p className="mt-3 text-sm leading-7 text-slate-600">
                Complete your KYC details to unlock verification-based features
                and keep your affiliate account ready for payouts and referrals.
              </p>
