...
export default function VerificationOverviewPage() {
  ...
  const isCustomerSession =
    sessionStatus === "authenticated" && (role === "customer" || role === "")
  const { data: me } = useMeQuery(undefined, { skip: !isCustomerSession })

  const rawStatus = me?.verification_status ?? "not_verified"
  const status: VerificationStatus =
    rawStatus === "verified" || me?.account_status === 1
      ? "verified"
      : rawStatus === "pending_review" || me?.account_status === 2
        ? "pending_review"
        : rawStatus === "on_hold"
          ? "on_hold"
          : "not_verified"
  ...
  const {
    ...
    ctaHref,
    ctaPrimary,
  } = getStatusInfo(status, profileBasePath)
        ...
        >
          ...
          <div className="mx-auto max-w-3xl text-center lg:text-left">
            ...
            <p className="mt-3 text-sm leading-7 text-slate-600 md:text-base">
              Before AF Home releases your payout, we need to verify your
              identity and collect your payout details. The next step will open
              your profile encashment form where you can submit KYC documents
              and the payout request together.
            </p>
          </div>
        </motion.div>
        ...
        <div className="grid gap-6 lg:grid-cols-[1.2fr_0.8fr]">
          ...
          >
            ...
            <h2 className="mt-3 text-xl font-semibold sm:mt-4 sm:text-2xl">
              This page explains the requirement before opening the form.
            </h2>
            <p className="mt-3 text-sm leading-7 text-slate-300 sm:mt-4">
              Once you continue, you will be sent to your profile Encashment
              tab. Complete the verification fields, enter your payout method,
              then submit one request for admin review.
            </p>
            ...
            <div className="mt-6 space-y-4 rounded-2xl border border-white/10 bg-white/5 p-4 sm:mt-8 sm:p-5">
              <div>
                ...
                <div className="mt-1 text-sm font-medium text-white">
                  {me?.name || me?.username || "AF Home Member"}
                </div>
              </div>
              <div>
                <div className="text-xs uppercase tracking-[0.2em] text-slate-400">
