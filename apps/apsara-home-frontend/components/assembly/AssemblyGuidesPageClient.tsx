...
export default function AssemblyGuidesPageClient({
  localGuides = [],
  initialCategories,
}: Props) {
  ...
  const { data, isLoading, isError } =
    useGetPublicWebPageItemsQuery("assembly-guides")
  const items = useMemo(() => {
    ...
    const baseItems = [...localItems, ...normalizedAdminItems].filter(
      (item) => {
    ...
    return [...filtered].sort((a, b) => {
      const left = String(a.title ?? "").localeCompare(
        String(b.title ?? ""),
        undefined,
        { sensitivity: "base" }
      )
      ...
      <main className="min-h-screen bg-gradient-to-b from-blue-50 via-white to-white dark:from-gray-900 dark:via-gray-900 dark:to-gray-800">
        ...
        >
          ...
          >
            <div
              aria-hidden
              className="absolute inset-0 bg-[radial-gradient(ellipse_at_top_left,rgba(37,99,235,0.18),transparent_55%)]"
            />
            <div
              aria-hidden
              className="absolute inset-0 animate-pulse bg-[radial-gradient(ellipse_at_bottom_right,rgba(56,189,248,0.14),transparent_50%)]"
            />
            ...
            <div className="relative flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
              <div>
                ...
                <p className="mt-2 text-sm text-gray-600 dark:text-gray-300">
                  {items.length.toLocaleString()} guide
                ...
                {paginatedItems.map((item, idx) => {
                  const card = (
                    ...
                    >
                      <div
                        aria-hidden
                        className="absolute inset-0 bg-[radial-gradient(circle_at_top_left,rgba(37,99,235,0.14),transparent_55%)] opacity-0 transition-opacity group-hover:opacity-100"
                      />
