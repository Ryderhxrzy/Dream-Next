        ...
        <AnimatePresence initial={false}>
          {cards.map((card, index) => (
            ...
            >
              <Link
                href={
                  card.url.startsWith("/") ? card.url : `/category/${card.url}`
                }
                className="group relative block h-64 w-full overflow-hidden rounded-2xl shadow-sm transition-shadow duration-500 hover:shadow-xl md:h-80"
              >
                <Image
                  src={card.image}
                  alt={card.name}
