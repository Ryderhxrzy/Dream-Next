...
const formatPeso = (value: number) =>
  `P${Number(value || 0).toLocaleString("en-PH", { minimumFractionDigits: 0, maximumFractionDigits: 2 })}`
...
const isGlobalSupplierBrandName = (value: string) => {
  ...
  return (
    normalized === "af-home-global-brand" ||
    normalized === "af-home-global-supplier" ||
    normalized.includes("global")
  )
}

const zqPrice = (product: ZqCachedProduct) =>
  Number(product.priceMinCents ?? product.priceMaxCents ?? 0) / 100
const zqComparePrice = (product: ZqCachedProduct) =>
    ...
    >
      ...
      <div className="flex flex-1 flex-col border-t border-gray-100 p-3">
        ...
        <div className="mt-auto flex flex-wrap items-end gap-2 pt-3">
          ...
          <span className="ml-auto rounded-full border border-sky-200 bg-sky-50 px-2 py-1 text-[10px] font-semibold text-sky-600">
            PV 0
          </span>
        </div>
        <p className="mt-1 text-xs text-gray-400">
          Stock {product.totalStock.toLocaleString()} · Global Supplier
        </p>
      </div>
    </Link>
  )
...
function ZqBrandItemCard({
  ...
  const discount = showMemberPrice
    ? Math.max(1, Math.round(((srp - memberPrice) / srp) * 100))
    : 6
...
export default function ByBrandPageMain() {
  ...
  const selectedBrand = decodeURIComponent(searchParams.get("brand") || "")
    .trim()
    .toLowerCase()
  const [letterFilter, setLetterFilter] = useState<string>("ALL")
  const [searchQuery, setSearchQuery] = useState("")
  const [sortBy, setSortBy] = useState<
    "name-asc" | "name-desc" | "price-asc" | "price-desc" | "default"
  >("default")
  ...
  const [hoveringShareProductId, setHoveringShareProductId] = useState<
    number | null
  >(null)
  ...
  const availableLetters = useMemo(() => {
    const letters = new Set(
      allBrands.map((b) => b.name.charAt(0).toUpperCase())
    )
  ...
  const brands = useMemo(() => {
    let rows = allBrands
    if (selectedBrand)
      rows = rows.filter((brand) => toSlug(brand.name) === selectedBrand)
    else {
      if (searchQuery.trim())
        rows = rows.filter((b) =>
        ...
        rows = rows.filter(
          (b) => b.name.charAt(0).toUpperCase() === letterFilter
        )
  ...
  const isGlobalSupplierBrand = selectedBrandItem
    ? isGlobalSupplierBrandName(selectedBrandItem.name)
    : false
  ...
  const { data: brandInfo } = useGetProductBrandQuery(
    selectedBrandItem?.id ?? 0,
    {
  ...
  const {
    data: zqProductsData,
    isFetching: isFetchingZqProducts,
    isLoading: isLoadingZqProducts,
  } = useGetPublicZqProductsQuery(
  ...
  const productTotalCount =
    (productsMeta?.total ?? brandProducts.length) + (zqProductsMeta?.total ?? 0)
  const brandTotalProducts = Number(
    brandInfo?.totalProducts ?? productTotalCount
  )
  const isFetchingBrandProducts =
    isFetchingProducts || (isGlobalSupplierBrand && isFetchingZqProducts)
  const isLoadingBrandProducts =
    isLoadingProducts || (isGlobalSupplierBrand && isLoadingZqProducts)
            ...
            <div className="container mx-auto px-4 py-6">
              <div className="mt-4">
                ...
                <p className="mt-2 text-gray-600 dark:text-gray-400">
                  Browse featured product brands from the catalog. Pick a brand
        ...
        <div className="container mx-auto px-4 py-8 space-y-8">
                ...
                <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:gap-6">
                  ...
                  >
                    {selectedBrandItem.image ? (
                      <Image
                        src={selectedBrandItem.image}
                        alt={selectedBrandItem.name}
                        fill
                  ...
                  <div className="flex-1">
                    ...
                    <div className="mt-3 flex flex-wrap items-center justify-center sm:justify-start gap-4 text-sm">
                      <div className="flex items-center gap-1 text-gray-600 dark:text-gray-400">
                        ...
                        <span>
                          Chat Performance: {brandInfo?.chatPerformance ?? 95}%
                        </span>
                      </div>
                      <div className="flex items-center gap-1 text-gray-600 dark:text-gray-400">
                        <svg
                        ...
                        <span>
                          Overall Rating:{" "}
                          {brandInfo?.overallRating
                            ? brandInfo.overallRating.toFixed(1)
                            : "N/A"}
                        </span>
                      </div>
                      <div className="flex items-center gap-1 text-gray-600 dark:text-gray-400">
                      ...
                      <div className="flex items-center gap-1 text-gray-600 dark:text-gray-400">
                        ...
                        <span>
                          Joined:{" "}
                          {brandInfo?.joinedDate
                            ? new Date(brandInfo.joinedDate).toLocaleDateString(
                                "en-US",
                                {
                                  year: "numeric",
                                  month: "short",
                ...
                <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                  ...
                  {!searchQuery && (
                    <div className="flex flex-wrap gap-2">
                      <span className="px-3 py-2 text-sm font-semibold text-gray-500 dark:text-gray-400">
                        Filter:
                      </span>
            ...
            brandProducts.length > 0 && (
              ...
              >
                ...
                <button
                  onClick={() =>
                    setCurrentSlide((prev: number) =>
                      prev === 0
                        ? Math.min(4, brandProducts.length - 1)
                        : prev - 1
                    )
                  }
                ...
                <button
                  onClick={() =>
                    setCurrentSlide((prev: number) =>
                      prev === Math.min(4, brandProducts.length - 1)
                        ? 0
                        : prev + 1
                    )
                  }
              ...
              >
                ...
                <div className="mt-6">
                  {isFetchingBrandProducts ? (
                    ...
                    >
                      {Array.from({ length: viewType === "grid" ? 12 : 8 }).map(
                        (_, i) =>
                          viewType === "grid" ? (
                            <ProductCardSkeleton key={i} />
                          ) : (
                              ...
                              >
                                ...
                                <div className="absolute top-2 right-2 flex flex-col gap-2 z-10">
                                  ...
                                  <button
                                    ...
                                    onMouseEnter={() =>
                                      setHoveringShareProductId(product.id)
                                    }
                                    onMouseLeave={() =>
                                ...
                                <div className="flex flex-col justify-center flex-1 p-4 relative">
                                  ...
                                  <div className="flex items-baseline gap-2 mb-2">
                                    <span className="text-lg font-bold text-sky-500 dark:text-sky-400">
                                      {"\u20b1"}
                                      {Number(
                                        product.priceSrp ?? product.priceDp ?? 0
                                      ).toLocaleString()}
                                    </span>
                                    {product.priceMember &&
                                      Number(product.priceMember) > 0 &&
                              ...
                              >
                                ...
                                <div className="min-w-0 flex-1">
                                  ...
                                  <p className="mt-2 text-lg font-black text-sky-500">
                                    {formatPeso(zqPrice(product))}
                                  </p>
                                  <p className="text-xs text-gray-400">
                                    Stock {product.totalStock.toLocaleString()}{" "}
                                    · Global Supplier
                                  </p>
                                </div>
                              </Link>
          ...
          {selectedBrandItem && (
            <div className="rounded-lg bg-white dark:bg-gray-800 p-6 border border-gray-200 dark:border-gray-700">
              ...
              ) : allBrands.filter((b) => b.id !== selectedBrandItem.id)
                  .length === 0 ? (
