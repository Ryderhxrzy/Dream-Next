...
function readCheckoutDraft(): CustomerCheckoutData | null {
  ...
  try {
    ...
    const normalizedProductId =
      toPositiveDraftNumber(parsed?.product?.id) ??
...
const CustomerCheckoutMain = ({
  ...
  const hasPartnerContext =
    isPartnerStorefront || Boolean(storefrontDisplayName?.trim())
  ...
  const {
    ...
    isFetching: shippingRatesFetching,
  } = useGetPublicShippingRatesQuery()
  const [fetchProduct, { data: fullProductData }] =
    useLazyGetPublicProductQuery()
  const [checkoutRefreshTrigger, setCheckoutRefreshTrigger] = useState(0)

  const partnerName =
    storefrontDisplayName?.trim() ||
    (normalizedPartner ? normalizedPartner.replace(/-/g, " ") : "AF Home")
  const partnerLogo = storefrontLogoUrl || storefrontTabLogoUrl
  const shouldShowReferralField = true
  const backToShopHref = isPartnerStorefront
    ? `/shop/${normalizedPartner}/product`
    : "/shop"
  const checkoutHeaderLabel = isPartnerStorefront
    ? `${partnerName} Secure Checkout`
  ...
  const isZqCheckout =
    checkoutData?.sourceType === "zq" ||
    checkoutData?.product?.sourceType === "zq"
  const storedReferral = useMemo(() => readStoredReferral(), [])
  const storefrontReferral = useMemo(
    () => extractReferralUsername(String(storefrontReferralCode ?? "")),
    [storefrontReferralCode]
  )
  const memberReferral = (meData?.referrer_username ?? "").trim()
  const effectiveReferral =
    storefrontReferral || memberReferral || storedReferral
  ...
  const requiredFieldOrder = useMemo(
    () =>
      isLoggedIn
        ? REQUIRED_FIELD_ORDER.filter((key) => key !== "referred_by")
        : REQUIRED_FIELD_ORDER,
    [isLoggedIn]
  )
  const [selectedMethod, setSelectedMethod] = useState<PaymentMethod>("gcash")
  const [selectedOnlineBankingProvider, setSelectedOnlineBankingProvider] =
    useState<CheckoutOnlineBankingProvider>("dob")
  const paymentModeEnabledByAdmin = Boolean(
    publicSettingsData?.settings?.enable_test_payments
  )
  const manualCheckoutModeEnabledByAdmin = Boolean(
    publicSettingsData?.settings?.enable_manual_checkout_mode
  )
  const useManualCheckoutShipping =
    manualCheckoutModeEnabledByAdmin && !isZqCheckout
  ...
  const { data: walletOverview } = useGetWalletOverviewQuery(
    { page: 1, perPage: 1, walletType: "all" },
    {
      skip: !isLoggedIn,
    }
  )
  const canSwitchPaymentMode = isLocalPaymentHost || paymentModeEnabledByAdmin
  const effectivePaymentMode: PaymentMode = canSwitchPaymentMode
    ? paymentMode
    : "live"
  ...
  const shippingAddressLabel = [form.city.trim(), form.province.trim()]
    .filter(Boolean)
    .join(", ")
  const isShippingRatePending =
    useManualCheckoutShipping &&
    hasShippingAddress &&
    (shippingRatesLoading || shippingRatesFetching)
  const isShippingRateUnavailable =
    useManualCheckoutShipping &&
    hasShippingAddress &&
    !isShippingRatePending &&
    shippingFee === null
  const canProceedWithoutShippingRate =
    useManualCheckoutShipping && isShippingRateUnavailable
  const resolvedShippingFee = shippingFee ?? 0

  const voucherDiscount = useMemo(
    () => Math.max(0, Number(voucherInfo?.discount ?? 0)),
    [voucherInfo?.discount]
  )
  const availableCashbackBalance = useMemo(
    () =>
      Math.max(
        0,
        Number(
          walletOverview?.summary?.personal_cashback_balance ??
            walletOverview?.summary?.cashback_balance ??
            0
        )
      ),
    [
  ...
  const availableEgcBalance = useMemo(
    () =>
      Math.max(0, Number(walletOverview?.summary?.available_egc_balance ?? 0)),
    [walletOverview?.summary?.available_egc_balance]
  )
  ...
  const guestPricing = useMemo(() => {
    ...
    const normalizedSku = String(checkoutData.selectedSku ?? "")
      .trim()
      .toLowerCase()
    const normalizedVariantId = toPositiveNumber(checkoutData.variantId)
    const normalizedColor = String(checkoutData.selectedColor ?? "")
      .trim()
      .toLowerCase()
    const normalizedSize = String(checkoutData.selectedSize ?? "")
      .trim()
      .toLowerCase()
    const normalizedType = String(checkoutData.selectedType ?? "")
      .trim()
      .toLowerCase()
    ...
    const matchedVariant =
      variants.find(
        (variant) =>
          ...
          Number(variant.id ?? 0) === normalizedVariantId
      ) ??
      variants.find(
        (variant) =>
          normalizedSku !== "" &&
      ...
      variants.find((variant) => {
        const vColor = String(variant.color ?? "")
          .trim()
          .toLowerCase()
        const vSize = String(variant.size ?? "")
          .trim()
          .toLowerCase()
        const vType = String(variant.name ?? "")
          .trim()
          .toLowerCase()
  ...
  const effectiveSubtotal = useMemo(() => {
    ...
    return !isLoggedIn
      ? guestPricing.subtotal
      : toPositiveNumber(checkoutData.subtotal)
  ...
  const effectiveProductPv = useMemo(() => {
    if (!checkoutData) return 0
    return !isLoggedIn
      ? guestPricing.productPv
      : toPositiveNumber(checkoutData.product.prodpv ?? 0)
  }, [checkoutData, guestPricing.productPv, isLoggedIn])

  const computedTotal = useMemo(() => {
    if (!checkoutData) return 0
    const effectiveCashbackAmount = Math.min(
      appliedCashbackAmount,
      Math.max(0, effectiveSubtotal - voucherDiscount)
    )
    const effectiveEgcAmount = Math.min(
      appliedEgcAmount,
      Math.max(0, effectiveSubtotal - voucherDiscount - effectiveCashbackAmount)
    )
    return (
      Math.max(
        0,
  ...
  const maxApplicableCashback = useMemo(
    () =>
      Math.max(
        0,
        Math.min(availableCashbackBalance, effectiveSubtotal - voucherDiscount)
      ),
  ...
  const maxApplicableEgc = useMemo(
    () =>
      Math.max(
        ...
        Math.min(
          availableEgcBalance,
          effectiveSubtotal - voucherDiscount - appliedCashbackAmount
        )
      ),
    [
      appliedCashbackAmount,
  ...
  const draftSourceSlug = String(checkoutData?.sourceSlug ?? "")
    .trim()
    .toLowerCase()
  ...
  useEffect(() => {
    ...
    const handle = setTimeout(async () => {
      try {
        ...
        setVoucherInfo(null)
        setVoucherError(
          apiError?.data?.message || "Voucher code is invalid or expired."
        )
      }
    }, 450)
      ...
      try {
        ...
        setCashbackError(
          apiError?.data?.message ||
            "Personal cashback is not available for this order."
        )
      }
    }, 300)
  ...
  useEffect(() => {
    const handle = setTimeout(async () => {
      ...
      try {
        ...
        setAppliedEgcAmount(0)
        setEgcError(
          apiError?.data?.message || "AF-GC is not available for this order."
        )
      }
    }, 300)
  ...
  const setField = useCallback(
    (key: keyof GuestForm, value: string) => {
  ...
  const validate = (): FormErrors => {
    ...
    if (shouldRequireReferral && !form.referred_by.trim())
      e.referred_by = "Please enter a referral code."
    if (!form.address.trim())
      e.address = "Please enter your street or house number."
  ...
  const showValidationError = useCallback(
    (validationErrors: FormErrors) => {
      const firstErrorKey = requiredFieldOrder.find((key) =>
      ...
      const message =
        ...
        `${FIELD_LABELS[firstErrorKey] ?? "This field"} is required.`
      const missingCount =
  ...
  const handleSubmit = async () => {
    ...
    if (isShippingRateUnavailable && !canProceedWithoutShippingRate) {
      notify.error("No shipping rate for this location.", {
        description:
          "Please choose another province or city before placing your order.",
      })
      return
    }
    ...
    if (appliedCashbackAmount > maxApplicableCashback) {
      const message =
        "Personal cashback amount exceeds the available discount for this order."
    ...
    if (appliedEgcAmount > maxApplicableEgc) {
      const message =
        "AF-GC amount exceeds the available discount for this order."
      setEgcError(message)
      notify.error(message)
      return
    }
    ...
    try {
      ...
      const data = await createCheckoutSession({
        ...
        online_banking_provider:
          selectedMethod === "online_banking" && showOnlineBankingProviderPicker
            ? selectedOnlineBankingProvider
            : undefined,
        voucher_code: voucherInfo?.code,
        cashback_amount:
          appliedCashbackAmount > 0 ? appliedCashbackAmount : undefined,
        ...
        order: {
          ...
          product_id: Number.isFinite(normalizedProductId)
            ...
            : undefined,
          product_sku:
            checkoutData.selectedSku ?? checkoutData.product.sku ?? null,
          ...
          zq_product_id:
            checkoutData.zqProductId ??
            checkoutData.product.zqProductId ??
            null,
          zq_external_id:
            checkoutData.zqExternalId ??
            checkoutData.product.zqExternalId ??
            null,
          zq_offer_id:
            checkoutData.zqOfferId ?? checkoutData.product.zqOfferId ?? null,
        },
      }).unwrap()
      ...
      if (data.checkout_id) {
        ...
        localStorage.setItem(
          "last_checkout_payment_mode",
          canSwitchPaymentMode
            ? data.payment_mode || effectivePaymentMode
            : "live"
        )
        ...
        try {
          ...
          if (
            typeof window !== "undefined" &&
            /(^|\\.)afhome\\.ph$/i.test(window.location.hostname)
          ) {
      ...
      notify.error(
        gatewayError || apiError?.data?.message || "Something went wrong",
        {
  ...
  if (!checkoutData) {
    return (
      <LoadingScreen
        logoSrc={
          hasPartnerContext ? partnerLogo || null : "/Images/af_home_logo.png"
        }
        logoAlt={`${partnerName} Logo`}
        brandText={partnerName.toUpperCase()}
        tagline={
          hasPartnerContext ? "Partner Storefront" : "Your Trusted Home Partner"
        }
      />
    )
  }
      ...
      <div className="border-b border-slate-200 bg-white dark:border-slate-800 dark:bg-slate-950">
        <div className="container mx-auto flex items-center justify-between px-4 py-4">
          <div className="flex items-center gap-3">
            <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-2xl border border-slate-200 bg-slate-100 dark:border-slate-700 dark:bg-slate-800">
          ...
          <Link
            href={backToShopHref}
            className="flex items-center gap-1.5 rounded-xl border border-slate-200 bg-slate-100 px-3 py-2 text-xs font-semibold text-slate-600 transition-colors hover:bg-slate-200 hover:text-slate-900 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-300 dark:hover:bg-slate-700 dark:hover:text-white"
          >
        ...
        <div className="grid grid-cols-1 items-start gap-6 lg:grid-cols-3">
          ...
          <div className="lg:sticky lg:top-4">
            <CustomerCheckoutOrderSummary
              ...
              voucher={
                voucherInfo
                  ? {
                      ...
                      sourceType: voucherInfo.sourceType,
                    }
                  : null
              }
              ...
              unitPriceOverride={
                !isLoggedIn && !hasMultiItemsCheckout
                  ? guestPricing.unitPrice
                  : undefined
              }
