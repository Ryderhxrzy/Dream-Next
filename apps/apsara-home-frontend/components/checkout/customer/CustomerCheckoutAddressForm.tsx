...
const regionSupportsProvince = (
  regionName: string,
  supportedProvinceKeys: Set<string>
) => {
  const region = normalizeLocationKey(regionName)

  if (
    supportedProvinceKeys.has("manila") &&
    (region.includes("national capital") || region === "ncr")
  )
    return true
  if (
    (supportedProvinceKeys.has("bulacan") ||
      supportedProvinceKeys.has("pampanga")) &&
...
export default function CustomerCheckoutAddressForm({
  ...
  } = useCustomerAddressesQuery(undefined, { skip: !isLoggedIn })
  ...
  const effectiveLoggedIn =
    isLoggedIn && !forceGuestMode && !hasUnauthorizedAddressAccess

  const addresses = useMemo(() => data?.addresses ?? [], [data?.addresses])
  const activeShippingRates = useMemo(
    () => shippingRates.filter((rate) => rate.status),
    [shippingRates]
  )
  const shouldRestrictToShippingRates =
    restrictToShippingRates && activeShippingRates.length > 0
  const supportedProvinceKeys = useMemo(
    () =>
      new Set(
        activeShippingRates.map((rate) =>
          normalizeProvinceKey(rate.provinceKey || rate.province)
        )
      ),
    [activeShippingRates]
  )
  const supportedRegions = useMemo(
    () =>
      shouldRestrictToShippingRates
        ? ph.regions.filter((region) =>
            regionSupportsProvince(region.name, supportedProvinceKeys)
          )
        : ph.regions,
  ...
  const supportedProvinces = useMemo(
    () =>
      shouldRestrictToShippingRates
        ? ph.provinces.filter((province) =>
            supportedProvinceKeys.has(normalizeProvinceKey(province.name))
          )
        : ph.provinces,
  ...
  const supportedCities = useMemo(() => {
    ...
    return ph.cities.filter((city) =>
      activeShippingRates.some((rate) => {
        const rateProvinceKey = normalizeProvinceKey(
          rate.provinceKey || rate.province
        )
        const rateCityKey = normalizeCityKey(rate.cityKey || rate.city)

        return (
          rateProvinceKey === selectedProvinceKey &&
          rateCityKey === normalizeCityKey(city.name)
        )
      })
    )
  }, [
  ...
  useEffect(() => {
    ...
    setField(
      "province",
      ph.noProvince ? ph.address.region : ph.address.province
    )
  ...
  const updateDraft = (
    key: keyof CheckoutAddressDraft,
  ...
  const handleMakeDefault = async (address: CustomerAddress) => {
    try {
      ...
      const status = (error as { status?: number; data?: { message?: string } })
        ?.status
      if (status === 401) {
      ...
      setActionError(
        message || "Unable to set that address as default right now."
      )
    }
  }
    ...
    try {
      ...
      setActionError(
        firstValidationMessage ||
          ...
          "Unable to save the new shipping address right now."
      )
    }
  }
            ...
            >
              ...
              <select
                ...
                disabled={
                  ph.noProvince
                    ? !ph.regionCode
                    : !ph.provinceCode || ph.loadingCities
                }
                onChange={(e) => {
                  const option = e.target.options[e.target.selectedIndex]
                    ...
                    >
                      <div className="rounded-2xl border border-slate-100 bg-slate-50 px-4 py-3 dark:border-slate-700 dark:bg-slate-800">
                        ...
                        <p className="mt-1.5 text-xs text-slate-400 dark:text-slate-500">
                          Uses the name and phone entered in your contact info.
                        </p>
                      </div>
                      ...
                      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                        <Field
                          ...
                          onChange={(v) => updateDraft("zip_code", v)}
                          placeholder="e.g. 1100"
                        />
                        <Field
                          label="Address Type"
                          value={draft.address_type}
                          onChange={(v) => updateDraft("address_type", v)}
                      ...
                      <label className="flex cursor-pointer items-center gap-3 rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 transition-colors hover:bg-slate-100 dark:border-slate-700 dark:bg-slate-800 dark:hover:bg-slate-700">
                        <input
                          ...
                          onChange={(e) =>
                            updateDraft("set_default", e.target.checked)
                          }
