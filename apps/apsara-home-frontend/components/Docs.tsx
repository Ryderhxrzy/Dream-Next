...
function DiagramLightbox({
  ...
  const screenToSvg = useCallback((sx: number, sy: number, currentVb: VB) => {
    ...
    if (!el)
      return {
        x: currentVb.x + currentVb.w / 2,
        y: currentVb.y + currentVb.h / 2,
      }
  ...
  const zoomAt = useCallback(
    (factor: number, pivotX?: number, pivotY?: number) => {
      setVb((prev) => {
        ...
        const newW = Math.min(
          Math.max(prev.w * factor, origVB.w * 0.08),
          origVB.w * 6
        )
  ...
  const clampPan = useCallback(
    (v: VB): VB => {
      ...
      return {
        ...
        x: Math.max(
          origVB.x - v.w * slack,
          Math.min(origVB.x + origVB.w - v.w * (1 - slack), v.x)
        ),
        y: Math.max(
          origVB.y - v.h * slack,
          Math.min(origVB.y + origVB.h - v.h * (1 - slack), v.y)
        ),
      }
    },
    [origVB]
  ...
  const onTouchMove = (e: React.TouchEvent) => {
    ...
    if (e.touches.length === 1) {
      const dx =
        ((e.touches[0].clientX - lastTouches.current.x) / r.width) *
        vbRef.current.w
      const dy =
        ((e.touches[0].clientY - lastTouches.current.y) / r.height) *
        vbRef.current.h
      setVb((prev) => clampPan({ ...prev, x: prev.x - dx, y: prev.y - dy }))
      lastTouches.current = { x: e.touches[0].clientX, y: e.touches[0].clientY }
...
function Badge({
  ...
  label: string
  color?: "green" | "amber" | "blue" | "red" | "purple" | "gray"
}) {
  const map = {
      ...
      (entries) => {
        ...
        if (visible.length) {
          ...
          const parent = NAV.find(
            (n) => n.sub?.some((s) => s.id === id) || n.id === id
          )
        ...
        <div className="docs-content max-w-3xl mx-auto px-6 py-10 pb-24 space-y-0">
          ...
          <section id="overview" data-section>
            ...
            <p className="text-gray-600 text-[15px] leading-relaxed mb-6">
              ...
              and partner storefronts through a single unified system built with
              Next.js and Laravel.
            </p>
            <Note type="info">
              The platform targets the Philippine market, uses PayMongo for
              payments, J&T and XDE for shipping, and includes a
              referral/affiliate commission system, member tier rewards, and an
              AI-powered customer support layer.
            </Note>
          </section>
          ...
          <section id="overview-apps" data-section className="pt-2">
            ...
            <InfoTable
              ...
              rows={[
                [
                  "Apsara Home (Frontend)",
                  "app.afhome.com",
                  "Main e-commerce platform",
                  "NextAuth + Sanctum (4 actors)",
                ],
                [
                  "Dreambuild Landing Page",
                  "dreambuild.afhome.com",
                  "Brand / portfolio entry point",
                  "Redirects to Apsara Home login",
                ],
                [
                  "AF Nexus (Planned)",
                  "nexus.afhome.com",
                  "Community + chat + learning",
                  "Validates via GET /api/customer/me",
                ],
              ]}
            />
            <Note type="info">
              All three apps share the <strong>.afhome.com</strong> cookie
              domain — the NextAuth session cookie from Apsara Home is
              accessible on every subdomain, no second login required.
            </Note>
          </section>
          ...
          <section id="overview-frontend" data-section className="pt-2">
            ...
            <InfoTable
              ...
              rows={[
                ...
                [
                  "Auth",
                  "NextAuth v4 — 4 separate session configs (customer, admin, supplier, partner)",
                ],
                [
                  "State / API",
                  "Redux Toolkit + RTK Query for all Laravel API communication",
                ],
            ...
            />
            <InfoTable
              headers={["Route Area", "Routes"]}
              rows={[
                [
                  "Public Storefront",
                  "/, /shop, /product/[slug], /category, /by-room, /by-brand, /search",
                ],
                [
                  "Customer / Member",
                  "/login, /verification, /mfa-approval, /profile, /orders, /wishlist, /checkout",
                ],
                [
                  "Admin Console",
                  "/admin/*, /super_admin/* — dashboard, members, products, orders, finance, chat",
                ],
                [
                  "Supplier Console",
                  "/supplier/* — login, dashboard, products, orders, categories, users, reports",
                ],
                [
                  "Partner Storefront",
                  "/partner/*, /shop/[partner], /[partner]",
                ],
                [
                  "Content / Info",
                  "/about, /faq, /blog, /shipping, /returns, /privacy, /terms, /branches",
                ],
              ]}
            />
          </section>
          ...
          <section id="overview-backend" data-section className="pt-2">
            ...
            <InfoTable
              ...
              rows={[
                [
                  "Framework",
                  "Laravel 12 (PHP) — API-first, no frontend rendering",
                ],
                ...
                [
                  "Payments",
                  "PayMongo — checkout sessions, webhooks, mobile payment",
                ],
                [
                  "Shipping",
                  "J&T Express + XDE Logistics — waybills, rates, tracking",
                ],
                ["Media", "Cloudinary — image upload and CDN"],
                [
                  "AI",
                  "Gemini + OpenAI — support, vision, embeddings, recommendations",
                ],
                ["Realtime", "Pusher — customer-admin chat events"],
                [
                  "External Sync",
                  "AF HOME GLOBAL SUPPLIER — product and order data synchronization",
                ],
              ]}
            />
          </section>
...
composer dev        # Full dev: server + queue + logs + Vite`}
          ...
          <section id="arch-session" data-section className="pt-2">
            ...
            <InfoTable
              headers={["Route Prefix", "NextAuth Config", "Session Route"]}
              rows={[
                [
                  "/admin/*, /super_admin/*",
                  "libs/adminAuth.ts",
          ...
          <section id="arch-db" data-section className="pt-2">
            ...
            <InfoTable
              ...
              rows={[
                ...
                ["tbl_admin", "id"],
                [
                  "tbl_checkout_history",
                  "ch_id (internal) · ch_checkout_id (PayMongo ref)",
                ],
              ]}
            />
          </section>
          ...
          <section id="arch-security" data-section className="pt-2">
            ...
            <InfoTable
              ...
              rows={[
                [
                  "Token isolation",
                  "Each actor has its own Sanctum token — a customer token cannot auth an admin request",
                ],
                [
                  "Cookie isolation",
                  "Each actor has its own NextAuth cookie — logout one actor, others unaffected",
                ],
                [
                  "Role narrowing",
                  "Admin routes require admin.token.validation AND admin.role:* for that route group",
                ],
                [
                  "No NEXT_PUBLIC_ secrets",
                  "Payment keys, AI keys, OAuth secrets must stay server-only",
                ],
                [
                  "Upload gates",
                  "All Cloudinary signing endpoints behind protected Next.js API routes with role checks",
                ],
                [
                  "Webhook validation",
                  "PayMongo webhooks use signature verification — not open POST",
                ],
              ]}
            />
          </section>
          ...
          <section id="arch-risks" data-section className="pt-2">
            ...
            <Note type="danger">
              These are active issues requiring remediation before production
              hardening.
            </Note>
          ...
          <section id="integrations" data-section>
            ...
            <InfoTable
              headers={["Service", "Purpose", "Actor"]}
              rows={[
                [
                  "PayMongo",
                  "Checkout sessions, mobile payments, webhooks",
                  "Customer checkout",
                ],
                [
                  "J&T Express",
                  "Shipping rates, waybills, tracking",
                  "Admin / shipping",
                ],
                [
                  "XDE Logistics",
                ...
                [
                  "Google Drive",
                  "File storage for exports and imports",
                  "Admin",
                ],
                ...
                [
                  "Gemini AI",
                  "AI support, content generation, vision",
                  "Admin / customer support",
                ],
                [
                  "OpenAI",
                  "AI-assisted features, embeddings",
                  "Backend services",
                ],
                [
                  "Pusher",
                  "Realtime websocket events (chat)",
                  "Customer / Admin",
                ],
                [
                  "AF HOME GLOBAL SUPPLIER",
                  "External product / order data synchronization",
                  "Admin",
                ],
          ...
          <section id="api-auth" data-section className="pt-2">
            ...
            <InfoTable
              ...
              rows={[
                [
                  "POST",
                  "/auth/register · /auth/mobile/register",
                  "Register (web Turnstile, throttle 10/min) / mobile",
                ],
                [
                  "POST",
                  "/auth/login · /auth/mobile/login",
                  "Login — throttle member-login 3/min",
                ],
                [
                  "GET",
                  "/auth/register/check-email · check-username · check-referral",
                  "Live availability / referral validation",
                ],
                [
                  "POST",
                  "/auth/register/verify-otp · resend-otp",
                  "Registration OTP (resend throttle otp 5/min)",
                ],
                [
                  "POST",
                  "/auth/send-sms-otp · verify-sms-otp",
                  "SMS OTP send / verify",
                ],
                [
                  "POST",
                  "/auth/login/mfa/status · /mfa/respond · /2fa/resend",
                  "Login 2FA gate",
                ],
                [
                  "POST",
                  "/auth/forgot-password · verify-reset-otp · reset-password",
                  "Password reset flow",
                ],
                [
                  "POST/GET",
                  "/auth/passkeys/{login|register}/{options|verify}",
                  "Passkey (WebAuthn)",
                ],
                ["POST", "/auth/totp/setup · enable · disable", "TOTP 2FA"],
                [
                  "POST/GET",
                  "/auth/qr/generate · /qr/{id}/status · /qr/complete",
                  "QR login (desktop ↔ mobile)",
                ],
                [
                  "POST",
                  "/auth/callback/google · /callback/facebook",
                  "OAuth callbacks",
                ],
                [
                  "POST/GET",
                  "/auth/link/{provider} · /unlink/{provider} · /linked-accounts",
                ...
                [
                  "POST",
                  "/me/avatar",
                  "Avatar upload — throttle uploads 20/min",
                ],
                [
                  "GET/DELETE",
                  "/sessions · /login-history · /sessions/{id}",
                  "Session list / revoke",
                ],
                [
                  "POST/GET",
                  "/username-change/send-otp · /submit · /latest",
            ...
            <InfoTable
              ...
              rows={[
                [
                  "GET",
                  "/products",
                  "List + filters (category, price, keyword)",
                ],
                [
                  "GET",
                  "/products/{id} · /summary · /reviews · /brand",
                  "Product detail data",
                ],
                [
                  "GET",
                  "/products/slug/{slug} · /products/cards",
                  "By slug / card list",
                ],
                ["GET", "/categories · /rooms · /product-brands", "Taxonomy"],
                [
                  "GET",
                  "/product-brands/{id}/profile · /with-products",
                  "Brand storefront",
                ],
                [
                  "GET",
                  "/search · /search/live · /search/recommendations",
                ...
                [
                  "GET",
                  "/products/zq/cached · /zq/cached/{id}",
                  "Cached supplier (ZQ) products",
                ],
                [
                  "POST",
                  "/products/{id}/viewers/heartbeat",
            ...
            <InfoTable
              ...
              rows={[
                ...
                [
                  ...
                  "PayMongo session — throttle checkout 20/min",
                ],
                [
                  "GET",
                ...
                [
                  "POST",
                  "/payments/validate-cashback",
                  "Validate auto-applied personal cashback discount",
                ],
                [
                  "POST",
                  "/payments/validate-egc",
                  "Validate auto-applied E-GC store credit",
                ],
                [
                  "POST/GET",
                  "/mobile/payments/create · /{id}/status",
                  "Mobile payments — throttle 10/min",
                ],
              ]}
            />
          </section>
          ...
          <section id="api-orders" data-section className="pt-2">
            ...
            <InfoTable
              ...
              rows={[
                [
                  "GET",
                  "/orders/history · /orders/counts",
                  "Customer order history + counts",
                ],
                [
                  "POST",
                  "/orders/{id}/confirm · /orders/{id}/refund",
                  "Confirm receipt / request refund",
                ],
                ["GET", "/orders/track", "Guest order tracking (public)"],
                [
                  "GET",
                  "/admin/orders · /admin/orders/counts",
                  "Admin order list",
                ],
                [
                  "PATCH",
                  "/admin/orders/{id}/approve · reject · status",
                  "Order status transitions",
                ],
                [
                  "PATCH",
                  "/admin/orders/{id}/fulfillment-mode · shipment-status",
                  "Fulfillment",
                ],
                [
                  "POST/GET",
                  "/admin/orders/{id}/shipping/jnt/book · track",
                  "J&T booking / tracking",
                ],
                [
                  "POST/GET",
                  "/admin/orders/{id}/shipping/xde/book · track · waybill · epod",
                  "XDE booking / waybill / POD",
                ],
                [
                  "GET/POST/PUT/DELETE",
                  "/admin/shipping/rates",
                  "Shipping rates CRUD (public read /shipping-rates)",
                ],
                [
                  "POST/GET",
                  "/admin/orders/{id}/zq/push · detail · tracking",
                  "Supplier (ZQ) order sync",
                ],
              ]}
            />
          </section>
          ...
          <section id="api-mlm" data-section className="pt-2">
            <SubHeading id="api-mlm">
              Member, Commission &amp; Encashment
            </SubHeading>
            <InfoTable
              headers={["Method", "Endpoint", "Purpose"]}
              rows={[
                [
                  "GET",
                  "/account/snapshot · /referral-tree",
                  "Tier, PV, wallet summary + downline",
                ],
                [
                  "GET",
                  "/public/community-stats · /public/top-members",
                  "Public stats / leaderboard",
                ],
                [
                  "GET",
                  "/encashment/wallet",
                  "Balance, ledger, vouchers, unilevel",
                ],
                [
                  "POST/GET",
                  "/encashment/requests",
                  "Submit / list encashment requests",
                ],
                [
                  "POST/DELETE",
                  "/encashment/payout-methods · /{id}",
                  "Manage payout methods",
                ],
                ["POST", "/encashment/vouchers", "Create affiliate voucher"],
                [
                  "POST",
                  "/encashment/verification-request[-with-payout]",
                  "KYC verify + payout",
                ],
                ["GET", "/admin/encashment", "Admin request list"],
                [
                  "PATCH",
                  "/admin/encashment/{id}/approve · reject · release",
                  "Encashment workflow",
                ],
                [
                  "CRUD",
                  "/admin/member-tiers[/{id}]",
                  "Member tier management",
                ],
                [
                  "GET",
                  "/admin/members · stats · referrals · top-earners",
                  "Member admin",
                ],
                [
                  "PATCH/POST",
                  "/admin/members/{id}/assign-sponsor · temporary-password",
                  "Member ops",
                ],
              ]}
            />
          ...
          <section id="api-supplier" data-section className="pt-2">
            ...
            <InfoTable
              ...
              rows={[
                [
                  "POST/GET",
                  "/supplier/auth/login · forgot-password · reset-password · me · logout",
                  "Supplier auth (throttle auth)",
                ],
                ["CRUD", "/supplier/warehouse[/{id}]", "Supplier warehouse"],
                [
                  "GET/PATCH/POST",
                  "/supplier/orders · /{id}/fulfillment · tracking · approve · push-to-zq",
                  "Supplier order fulfillment",
                ],
                [
                  "POST/GET/PATCH",
                  "/supplier/products/zq/* (fetch · sync · pricing · mappings)",
                  "ZQ catalog & pricing (variant / bulk)",
                ],
                [
                  "GET/POST",
                  "/supplier/chat/conversations[...]",
                  "Supplier ↔ admin chat",
                ],
                [
                  "POST/GET",
                  "/supplier/push-notifications/send · history",
                  "Supplier push notifications",
                ],
                [
                  "CRUD",
                  "/admin/suppliers[/{id}] · /admin/supplier-users",
                  "Supplier management (admin)",
                ],
                [
                  "CRUD",
                  "/admin/partner-users · /admin/partner-members",
                  "Partner users (admin, web_content)",
                ],
                [
                  "POST/GET",
                  "/webstore-requests · receipt · payment-session · latest · sync-account",
                  "Webstore / storefront requests",
                ],
                [
                  "GET/PATCH/DELETE",
                  "/admin/partner/webstore-requests[...]",
                  "Admin webstore review + renewal",
                ],
              ]}
            />
            <Note type="info">
              ...
              admin voucher rules remain visible in the super admin payments
              voucher page.
            </Note>
          </section>
          ...
          <section id="api-admin" data-section className="pt-2">
            ...
            <InfoTable
              ...
              rows={[
                [
                  "POST",
                  "/admin/auth/login · /login/2fa/resend",
                  "Admin login — throttle admin-login 3/min",
                ],
                ["GET/PUT", "/admin/auth/me", "Admin profile"],
                [
                  "CRUD",
                  "/admin/users[/{id}] · /{id}/ban · /unban",
                  "Admin user mgmt (super_admin, admin)",
                ],
                [
                  "GET/POST",
                ...
                [
                  "CRUD",
                  "/admin/products[/{id}] · /export · /import",
                  "Product CRUD + CSV",
                ],
                [
                  "POST",
                  "/admin/products/bulk-price · bulk-update (preview/apply)",
                  "Bulk product ops",
                ],
                [
                  "CRUD",
                  "/admin/categories · /admin/product-brands",
                  "Catalog admin",
                ],
                [
                  "GET/PATCH",
                  "/admin/members/kyc · /{id}/approve · reject",
                  "KYC review",
                ],
                [
                  "POST",
                  "/admin/email-blast/send · /admin/sms-blast/send",
                  "Email / SMS blast",
                ],
                [
                  "CRUD",
                  "/admin/web-pages/{type}[/{id}]",
                  "CMS pages — throttle admin-write",
                ],
                [
                  "GET/PATCH",
                  "/admin/interior-requests · /{id}",
                  "Interior service requests",
                ],
                ["GET/POST", "/admin/conversations[...]", "CSR conversations"],
                [
                  "GET/PUT/DELETE",
                  "/admin/qa/test-statuses",
                  "QA Testing board persistence (admin.actor)",
                ],
              ]}
            />
          </section>
          ...
          <section id="api-webhooks" data-section className="pt-2">
            <SubHeading id="api-webhooks">
              Webhooks, Notifications &amp; Misc
            </SubHeading>
            <InfoTable
              headers={["Method", "Endpoint", "Purpose"]}
              rows={[
                [
                  "POST",
                  "/payments/webhooks/paymongo",
                  "PayMongo webhook — throttle 30/min",
                ],
                [
                  "POST",
                  "/jnt/webhook/logistics-trackback · order-status (+ sandbox)",
                  "J&T shipping webhooks",
                ],
                [
                  "POST/GET",
                  "/notifications/{fcm|expo|onesignal} (register · send)",
                  "Push notification tokens & send",
                ],
                [
                  "POST",
                  "/realtime/pusher/auth · /broadcasting/auth",
                  "Pusher channel auth",
                ],
                [
                  "POST/GET",
                  "/ai-support · /gemini/chat · /gemini/models",
                  "AI support / Gemini (throttle auth)",
                ],
                [
                  "POST",
                  "/meilisearch/sync-products · sync-product/{id} · clear-index",
                  "Search index (admin only)",
                ],
                [
                  "POST",
                  "/leads · /leads/batch",
                  "Lead capture — throttle auth",
                ],
                [
                  "GET",
                  "/address/regions · provinces · cities · barangays",
                  "PH address data (public)",
                ],
              ]}
            />
          </section>
          ...
          <section id="api-guards" data-section className="pt-2">
            ...
            <InfoTable
              ...
              rows={[
                ...
                [
                  "Guard",
                  "customer.actor / admin.actor / supplier.actor",
                  "Restrict a route to one actor type (else 403)",
                ],
                [
                  "Guard",
                  "admin.role:…",
                  "super_admin, admin, csr, merchant_admin, web_content, accounting, finance_officer",
                ],
                [
                  "Guard",
                  "admin.token.validation",
                  "Rejects revoked admin tokens",
                ],
                [
                  "Rate",
                  "member-login / admin-login",
                  "3 / min per IP + identifier",
                ],
                ...
                [
                  "Global",
                  "CORS · RequestAbuseGuard · SecurityHeaders · MediaCacheHeaders",
          ...
          <section id="api-fields" data-section className="pt-2">
            ...
            <InfoTable
              ...
              rows={[
                ...
                [
                  "password",
                  "string, confirmed; strict: min:8 + upper/lower/digit/special (else min:6)",
                  "Yes",
                ],
                ["email", "email, unique", "—"],
                ["middle_name · phone · partner_slug", "string", "—"],
                [
                  "birth_date · gender · occupation · work_location · country",
                  "date · in:male,female,other · string · in:local,overseas · string",
                  "—",
                ],
                [
                  "address · barangay · city · province · region (+ *_code, zip_code)",
                  "string — PH address parts",
                  "—",
                ],
              ]}
            />
            ...
            <InfoTable
              ...
              rows={[
                [
                  "identifier OR email",
                  "string, max:255 (one required) — forgot-password",
                  "Yes*",
                ],
                ...
                [
                  "password",
                  "string, confirmed (same policy as register) — reset-password",
                  "Yes",
                ],
              ]}
            />
            ...
            <InfoTable
              ...
              rows={[
                [
                  "current_password",
                  "string (nullable only when a forced change is required)",
                  "Yes",
                ],
                [
                  "new_password",
                  "string, confirmed (same policy as register)",
                  "Yes",
                ],
              ]}
            />
            ...
            <InfoTable
              ...
              rows={[
                ...
                [
                  "birth_date · gender · occupation · address parts",
                  "same as register",
                  "—",
                ],
              ]}
            ...
            <InfoTable
              ...
              rows={[
                ...
                [
                  "payment_method",
                  "in: online_banking, card, gcash, maya",
                  "Yes",
                ],
                ["payment_mode", "in: test, live", "—"],
                ["online_banking_provider", "in: dob, ubp", "—"],
                [
                  "voucher_code · cashback_amount · egc_amount",
                  "string max:80 · numeric min:0 · numeric min:0",
                  "—",
                ],
                [
                  "customer{}",
                  "object: name, email, phone, address, referred_by, is_member",
                  "—",
                ],
                [
                  "order{}",
                  "object: product_name, product_id, product_sku, product_pv, quantity (1–1000), selected_*, subtotal, handling_fee, source_type (local|zq), zq_*",
                  "—",
                ],
              ]}
            />
            ...
            <InfoTable
              ...
              rows={[
                ...
                [
                  "refund_videos[]",
                  "mp4/mov/webm, max:100MB each, up to 5",
                  "—",
                ],
                [
                  "status",
                  "in: pending, processing, packed, shipped, out_for_delivery, delivered, cancelled, refunded",
                  "Yes",
                ],
              ]}
            />
            ...
            <InfoTable
              ...
              rows={[
                ...
                [
                  "account_name · account_number · mobile_number · email_address",
                  "string / email",
                  "—",
                ],
                [
                  "bank_name · bank_code · account_type",
                  "string · string · in: savings, checking",
                  "—",
                ],
                [
                  "card_holder_name · card_brand · card_last4",
                  "string · in: visa,mastercard,jcb,amex,other · max:4",
                  "—",
                ],
            ...
            <InfoTable
              ...
              rows={[
                ...
                [
                  "contact_number · address_line · city · province · postal_code · country",
                  "string",
                  "Yes",
                ],
                [
            ...
            <InfoTable
              ...
              rows={[
                ...
                [
                  "pd_price_dp · pd_price_member · pd_prodpv · pd_qty",
                  "numeric, min:0",
                  "—",
                ],
                [
                  "pd_room_type · pd_brand_type · pd_catsubid · pd_type",
                  "integer",
                  "—",
                ],
                [
                  "pd_weight · pd_psweight · pd_pslenght · pd_psheight · pd_pswidth",
                  "numeric (dimensions)",
                  "—",
                ],
                [
                ...
                [
                  "pd_assembly_required · pd_musthave · pd_bestseller · pd_salespromo · pd_manual_checkout_enabled",
                  "boolean",
                  "—",
                ],
                ...
                [
                  "pd_variants[]",
                  "array: pv_sku, pv_name, pv_color (+hex), pv_size, pv_style, pv_price_srp/dp/member, pv_prodpv, pv_qty, pv_status, pv_images[]",
                  "—",
                ],
              ]}
            />
            ...
            <InfoTable
              ...
              rows={[
                ...
                [
                  "min_direct_referrals · min_group_volume",
                  "integer, min:0",
                  "Yes",
                ],
                [
                ...
                [
                  "province · city · fee",
                  "string · string · numeric 0–999999 (shipping rate)",
                  "Yes",
                ],
            ...
            <InfoTable
              ...
              rows={[
                [
                  "full_name · username · display_name",
                  "string, max:255",
                  "Yes",
                ],
            ...
            <InfoTable
              ...
              rows={[
                ...
                ["email", "email, max:255", "Yes"],
                [
                  "property_type · project_scope · budget · style_preference · flexibility · target_timeline",
                  "string",
                  "—",
                ],
          ...
          <section id="api-responses" data-section className="pt-2">
            ...
            <p className="text-xs text-gray-500 mb-1.5">
              ...
              (random | bestseller | newest | price_asc | price_desc) — all
              optional
            </p>
            <CodeBlock
              lang="json"
...
{ brands: [ { id, name, image, status } ], total }`}
            ...
            <p className="text-xs text-gray-500 mb-1.5">
              ...
              · sort. Live: <span className="font-semibold">q (2–255)</span> ·
              limit (1–20, def 10)
            </p>
            <CodeBlock
              lang="json"
...
// GET /referral-tree
{
  ...
  children: [ { ...node, children_count, children[] } ]
            ...
            <p className="text-xs text-gray-500 mb-1.5">
              ...
              page · per_page (1–100, def 20) · wallet_type (all | cash | pv |
              rewards)
            </p>
            <CodeBlock
              lang="json"
              code={`// GET /encashment/wallet
{
  summary: {
...
201 { message, request: { ... }, eligibility, policy }`}
            ...
            <p className="text-xs text-gray-500 mb-1.5">
              ...
              filter · q · released_from · released_to · page · per_page.{" "}
              <span className="font-semibold text-gray-700">
                Query (members):
              </span>{" "}
              per_page (1–100, def 25) · page · q · status
              (blocked|pending|kyc_review|active) · tier · registration
              (new|referred|direct) · profile_photo · sort
            </p>
            <CodeBlock
              lang="json"
              code={`// GET /admin/encashment
          ...
          <section id="cs-frontend" data-section className="pt-2">
            ...
            <InfoTable
              ...
              rows={[
                [
                  "Next.js App Router",
                  "All routing, layouts, server components, route handlers",
                ],
                [
                  "TypeScript (strict: true)",
                  "All files — no any unless unavoidable and documented",
                ],
                ["RTK Query (store/api/*)", "All reusable Laravel API calls"],
                [
                  "NextAuth",
                  "All session management — no manual token storage",
                ],
            ...
            />
            <Note type="info">
              <strong>Data fetching:</strong> Use RTK Query for all reusable
              Laravel calls. Use direct <code>fetch</code> only for Next.js
              route handlers (server-side: auth callbacks, upload proxies,
              Sheets, Cloudinary signing).
            </Note>
            <CodeBlock
              lang="typescript"
              code={`// Always include in Laravel API calls
headers: {
  ...
  'Authorization': \`Bearer \${token}\`,
          ...
          <section id="cs-backend" data-section className="pt-2">
            ...
            <InfoTable
              headers={["Middleware Alias", "Purpose"]}
              rows={[
                [
                  "auth:sanctum",
                  "Validates bearer token; rejects unauthenticated requests",
                ],
                [
                  "customer.actor",
                  "Confirms token belongs to a customer/member model",
                ],
                [
                  "admin.token.validation",
                  "Validates admin-specific token type and ban status",
                ],
                ["admin.role:*", "Checks admin has a specific named role"],
                [
                  "supplier.actor",
                  "Confirms token belongs to a supplier user model",
                ],
                [
                  "request.abuse.guard",
                  "Guards against request abuse patterns",
                ],
              ]}
            />
            <Note type="warning">
              Controllers must do exactly 4 things: validate (via Form Request),
              call a service or model, format the response, return. Business
              logic, external calls, and DB queries belong in service classes.
            </Note>
          </section>
          ...
          <section id="cs-database" data-section className="pt-2">
            ...
            <InfoTable
              ...
              rows={[
                [
                  "Legacy tables",
                  "Use tbl_* prefix — do not rename without full migration + model + contract update",
                ],
                [
                  "Column prefixes",
                  "pd_ · ch_ · c_ · su_ — preserve these across all queries and responses",
                ],
                ["New tables", "Standard Laravel snake_case (no tbl_ prefix)"],
                [
                  "Migrations",
                  "Always additive — never destructive column renames on legacy tables in production",
                ],
                [
                  "Indexes",
                  "Add for all foreign keys and columns used in WHERE or ORDER BY",
                ],
              ]}
            />
          </section>
          ...
          <section id="cs-secrets" data-section className="pt-2">
            ...
            <Note type="danger">
              Never put real secrets in <Pill color="red">.env.example</Pill>.
              Never move payment keys, AI keys, or OAuth secrets to{" "}
              <Pill color="red">NEXT_PUBLIC_*</Pill> — these are exposed to the
              browser.
            </Note>
            <InfoTable
              headers={["Variable", "Rule"]}
              rows={[
          ...
          <section id="ui-routes" data-section className="pt-2">
            ...
            <InfoTable
              headers={["Actor", "Entry", "Key Pages"]}
              rows={[
                [
                  "Public",
                  "/",
                  "Home, shop, product, category, by-room, by-brand, search, blog, about, faq",
                ],
                [
                  "Customer / Member",
                  "/login",
                  "Profile, orders, wishlist, checkout, rewards, referrals, interior",
                ],
                [
                  "Admin",
                  "/admin",
                  "Dashboard, members, products, orders, finance, encashment, CMS, chat, settings, reports",
                ],
                ["Super Admin", "/super_admin", "Extended admin controls"],
                [
                  "Supplier",
                  "/supplier/login",
                  "Dashboard, products, orders, categories, users, company, reports",
                ],
                [
                  "Partner",
                  "/partner/login",
                  "Dashboard, webpages, branded storefront at /shop/[partner]",
                ],
              ]}
            />
          </section>
          ...
          <section id="ui-components" data-section className="pt-2">
            ...
            <InfoTable
              ...
              rows={[
                ...
                [
                  "components/ui/",
                  "Shared buttons, inputs, modals, cards, badges",
                ],
                [
                  "components/product/",
                  "Product listing cards, detail, gallery, reviews",
                ],
                [
                  "components/checkout/",
                  "Cart, checkout steps, payment form, confirmation",
                ],
                [
                  "components/admin/",
                  "Admin dashboard, tables, forms for all admin features",
                ],
                ["components/supplier/", "Supplier-specific UI components"],
                [
                  "components/profile/",
                  "Profile settings, KYC, passkey management",
                ],
          ...
          <section id="progress-docs" data-section className="pt-2">
            ...
            <InfoTable
              headers={["Document", "Status", "Last Reviewed"]}
              rows={[
                [
                  "project-overview.md",
                ...
                [
                  "progress-tracker.md",
                  <Badge label="Current" color="blue" />,
                  "2026-05-11",
                ],
                [
                  "api-reference.md",
          ...
          <section id="progress-remediation" data-section className="pt-2">
            ...
            <Note type="danger">
              <strong>Critical:</strong> Rotate the OpenAI API key in{" "}
              <Pill color="red">Apsara-Home-Backend/.env.example</Pill>. Treat
              it as compromised. Rotate in the OpenAI dashboard, then replace
              with placeholder <code>sk-your-openai-api-key</code>.
            </Note>
            <InfoTable
              headers={["Priority", "Task", "Area", "Status"]}
              rows={[
                [
                  <Badge label="High" color="amber" />,
                  "Remove hardcoded local backend URLs from frontend route handlers",
                  "Frontend",
                  "Not started",
                ],
                [
                  <Badge label="High" color="amber" />,
                  "Decide on typescript.ignoreBuildErrors: true in next.config.ts",
                  "Frontend",
                  "Not started",
                ],
                [
                  <Badge label="High" color="amber" />,
                  "Document production PostgreSQL / Neon environment variables",
                  "DevOps",
                  "Not started",
                ],
                [
                  <Badge label="Medium" color="blue" />,
                  "Normalize remaining fetch calls to RTK Query endpoints",
                  "Frontend",
                  "Not started",
                ],
                [
                  <Badge label="Medium" color="blue" />,
                  "Expand backend tests — payment, auth, role gates, order state",
                  "Backend",
                  "Not started",
                ],
                [
                  <Badge label="Medium" color="blue" />,
                  "Audit all Cloudinary upload handlers for session + role checks",
                  "Backend",
                  "Not started",
                ],
                [
                  <Badge label="Medium" color="blue" />,
                  "Confirm PayMongo webhook signature validation in all handlers",
                  "Backend",
                  "Not started",
                ],
                [
                  <Badge label="Low" color="gray" />,
                  "Add database indexes to high-traffic query columns",
                  "Database",
                  "Not started",
                ],
                [
                  <Badge label="Low" color="gray" />,
                  "Split routes/api.php by bounded domain",
                  "Backend",
                  "Not started",
                ],
              ]}
          ...
          <section id="progress-risks" data-section className="pt-2">
            ...
            <InfoTable
              headers={["Risk", "Severity", "Status", "Mitigation"]}
              rows={[
                [
                  "Real OpenAI key in .env.example",
                  <Badge label="Critical" color="red" />,
                  "Open",
                  "Rotate key immediately",
                ],
                [
                  "Hardcoded localhost URLs in chat proxy",
                  <Badge label="High" color="amber" />,
                  "Open",
                  "Replace with LARAVEL_API_URL env var",
                ],
                [
                  "TypeScript build bypass enabled",
                  <Badge label="High" color="amber" />,
                  "Open",
                  "Fix type errors, remove ignoreBuildErrors",
                ],
                [
                  "Upload endpoints without role gate",
                  <Badge label="Medium" color="blue" />,
                  "Open",
                  "Audit all Cloudinary signing handlers",
                ],
                [
                  "PayMongo webhooks without signature check",
                  <Badge label="Medium" color="blue" />,
                  "Open",
                  "Add signature verification to all handlers",
                ],
                [
                  "Missing DB indexes on high-traffic columns",
                  <Badge label="Low" color="gray" />,
                  "Open",
                  "Add indexes per query analysis",
                ],
              ]}
            />
          </section>
...
└── Unit/                  # Unit tests`}
          ...
          <section id="ai-security" data-section className="pt-2">
            ...
            <Note type="danger">
              Never echo secrets in docs, comments, or output. Never move
              secrets to NEXT_PUBLIC_* variables. Never weaken security controls
          ...
          <section id="ai-highrisk" data-section className="pt-2">
            ...
            <InfoTable
              headers={["Area", "Why High Risk", "Rule"]}
              rows={[
                [
                  "PayMongo checkout & webhooks",
                  "Real money, complex state machine",
                  "Read existing flow end-to-end before changing anything",
                ],
                [
                  "Order status transitions",
                  "Tied to shipping, wallet, commission, notifications",
                  "Map all side effects before touching order state",
                ],
                [
                  "Encashment requests",
                  "Affects wallet balances and real payouts",
                  "Never approve or modify without balance validation",
                ],
                [
                  "Referral commissions",
                  "Multi-level, complex calculation rules",
                  "Test against known calculation fixtures",
                ],
                [
                  "Role and actor middleware",
                  "If weakened, all access controls fail",
                  "Never remove or comment out middleware",
                ],
                [
                  "MFA and passkeys",
                  "Auth bypass risk if broken",
                  "Test full flow — do not skip verification steps",
                ],
                [
                  "Cloudinary upload endpoints",
                  "Unauthorized upload or signed URL abuse",
                  "Always verify session + role before signing",
                ],
                [
                  "Migrations on legacy tables",
                  "Breaking change risk to production data",
                  "Always additive — never destructive renames",
                ],
              ]}
            />
            <Note type="warning">
              <strong>Suggested AI work loop:</strong> (1) Identify the bounded
              area. (2) Read frontend surface: routes, components, RTK Query
              endpoints. (3) Read backend surface: routes, middleware,
              controller, service, model. (4) Make the smallest change that
              achieves the goal. (5) Verify correctness. (6) Update docs.
            </Note>
          </section>
        </div>
      </div>
