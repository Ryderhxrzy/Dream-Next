...
const resolveAdsPageFromPathname = (
  pathname: string
): "shop" | "home" | "landing" | "product" | "category" | "brand" | null => {
  ...
  if (pathname === "/category" || pathname.startsWith("/category/"))
    return "category"
  if (pathname === "/by-brand" || pathname.startsWith("/by-brand/"))
    return "brand"
...
export default function AdsPopup() {
  ...
  const adsPage = useMemo(
    () => resolveAdsPageFromPathname(pathname),
    [pathname]
  )
  ...
  const activeItems = useMemo(() => {
    const normalizedAdsPage = String(adsPage ?? "")
      .trim()
      .toLowerCase()
    const items = (data?.items ?? []).filter((item) => {
      const itemPage = String(item.page ?? "")
        .trim()
        .toLowerCase()
      ...
      return (
        isPageMatch &&
        (item.status ?? 1) === 0 &&
        (Boolean(item.image_url) || Boolean(item.video_url))
      )
    })
    const sorted = [...items].sort((a, b) => {
      const dateA = a.date_created
        ? new Date(`${a.date_created}T00:00:00`).getTime()
        : Number.NaN
      const dateB = b.date_created
        ? new Date(`${b.date_created}T00:00:00`).getTime()
        : Number.NaN
  ...
  if (!adsPage || isLoading || isError || activeItems.length === 0 || !isOpen)
    return null
