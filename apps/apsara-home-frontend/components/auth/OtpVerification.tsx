...
const OtpVerification = ({
  ...
  const [verifyRegisterOtp, { isLoading: isVerifying }] =
    useVerifyRegisterOtpMutation()
  const [resendRegistrationOtp, { isLoading: isResending }] =
    useResendRegisterOtpMutation()
  ...
  const handleVerify = async (e: React.FormEvent) => {
    ...
    const result = await verifyRegisterOtp({
      ...
      otp,
      debug_trace_id: requestTraceId,
    })

    if ("error" in result) {
      const errPayload = result.error as {
  ...
  const handleResend = async () => {
    ...
    if ("error" in result) {
      ...
      const msg = err?.errors
        ? Object.values(err.errors)[0][0]
        : err?.message || "Unable to resend the code."
      showError(msg)
      showErrorToast(msg)
      return
    }
  ...
  return (
    ...
    >
      ...
      <p className="mb-8 text-sm font-semibold text-sky-600 dark:text-sky-400">
        {maskedEmail}
      </p>
      {senderName ? (
      ...
      {error && (
        <div
          ref={errorRef}
          className="mb-4 w-full rounded-xl border border-red-200 bg-red-50 px-4 py-2.5 text-left text-sm text-red-600 dark:border-red-400/20 dark:bg-red-500/20 dark:text-red-300"
        >
          {error}
          {debugInfo ? (
