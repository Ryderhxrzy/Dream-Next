...
function OtpInput({ value, onChange, length = 4, disabled, hasError, autoFocus }: {
  value: string
...
export default function ForgotPasswordForm({ turnstileSiteKey = '' }: Props) {
  ...
  const passwordRequirements = useMemo(() => getPasswordChecks(password, strictPassword), [password, strictPassword])
  ...
  const requestOtp = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(firstError || data?.message || 'Unable to send reset OTP.')
      }
      ...
      if (data?.reset_token) {
        ...
        setSuccess(data?.message || 'If that account exists, a password reset OTP has been sent.')
      }
    } catch (err) {
  ...
  const verifyOtp = async (code: string) => {
    ...
    try {
      ...
      if (!res.ok) {
        const firstError = getFirstApiError((data as { errors?: unknown } | null)?.errors)
        throw new Error(firstError || data?.message || 'Invalid verification code.')
      }
          ...
          <div className="overflow-hidden bg-white dark:bg-slate-800 border border-gray-200 dark:border-white/10 rounded-3xl shadow-2xl p-8">
            <div className="mb-8">
              ...
              <p className="mt-2 text-sm text-gray-500 dark:text-white/70">
                {step === 'identifier' && 'Enter your AF Home email, username, or mobile number to receive an SMS OTP.'}
                {step === 'otp' && 'Enter the 4-digit code we sent to your mobile number.'}
            ...
            {step === 'identifier' ? (
              ...
              <form onSubmit={requestOtp} className="space-y-4">
                ...
                <div>
                  <label className="mb-1.5 block text-xs font-semibold text-gray-600 dark:text-white/80">Email, Username, or Mobile</label>
                  <input
                    type="text"
                    value={identifier}
                    onChange={(e) => {
              ...
              <div className="space-y-5">
                {maskedPhone && (
                  <div className="rounded-xl border border-gray-200 dark:border-white/10 bg-gray-50 dark:bg-white/5 p-4 text-sm text-gray-700 dark:text-white/80">
                    OTP sent to <span className="font-semibold text-gray-900 dark:text-white">{maskedPhone}</span>
                  </div>
                )}

                <div>
                  <label className="mb-3 block text-center text-xs font-semibold text-gray-600 dark:text-white/80">Enter the 4-digit code</label>
                  <OtpInput
                    value={otp}
                    onChange={(next) => {
                      setOtp(next)
                ...
                <button
                  type="button"
                  onClick={() => requestOtp()}
                  disabled={isSubmitting || isVerifying || resendCooldown > 0}
                ...
                >
                  {isSubmitting ? 'Resending...' : resendCooldown > 0 ? `Resend code in ${resendCooldown}s` : 'Resend code'}
                </button>
              </div>
              </motion.div>
            ) : (
                ...
                <div className="rounded-xl border border-gray-200 dark:border-white/10 bg-gray-50 dark:bg-white/5 p-4">
                  ...
                  <div className="mt-2 grid grid-cols-1 gap-1">
                    {passwordRequirements.map((item) => (
                      <p key={item.label} className={`flex items-center gap-2 text-[11px] ${item.passed ? 'text-emerald-600 dark:text-emerald-300' : 'text-gray-400 dark:text-white/55'}`}>
