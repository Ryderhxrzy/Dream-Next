...
export default function CartDrawer() {
  ...
  const checkoutItems: CustomerCheckoutLineItem[] = selectedItems.map(
    (item) => ({
      ...
      const totalPv =
        checkoutItems.length === 1
          ...
          : checkoutItems.reduce(
              (sum, item) =>
                sum +
                (item.prodpv ?? 0) * Math.max(1, Number(item.quantity ?? 1)),
              0
            )
      ...
      const nextDraft = {
        ...
        product: {
          ...
          name:
            checkoutItems.length === 1
              ? firstItem.name
              : `${checkoutItems.length} selected items from AF Home`,
          image: firstItem.image,
          price: subtotal,
          prodpv: totalPv,
          sku:
            checkoutItems.length === 1
              ? (firstItem.selectedSku ?? undefined)
              : undefined,
          manualCheckoutEnabled: checkoutItems.every(
            (item) => item.manualCheckoutEnabled === true
          ),
        },
        quantity: totalQuantity,
        variantId:
          checkoutItems.length === 1 ? (firstItem.variantId ?? null) : null,
        selectedColor:
          checkoutItems.length === 1 ? (firstItem.selectedColor ?? null) : null,
        selectedStyle:
          checkoutItems.length === 1 ? (firstItem.selectedStyle ?? null) : null,
        selectedSize:
          checkoutItems.length === 1 ? (firstItem.selectedSize ?? null) : null,
        selectedType:
          checkoutItems.length === 1 ? (firstItem.selectedType ?? null) : null,
        selectedSku:
          checkoutItems.length === 1 ? (firstItem.selectedSku ?? null) : null,
        items: checkoutItems,
        subtotal,
  ...
  const handleCustomerCheckout = () => {
    ...
    const totalSelectedQuantity = checkoutItems.reduce(
      (sum, item) => sum + Math.max(1, Number(item.quantity ?? 1)),
      0
    )
        ...
        product: {
          ...
          prodpv:
            checkoutItems.length === 1
              ? (firstItem.prodpv ?? 0)
              : checkoutItems.reduce(
                  (sum, item) => sum + (item.prodpv ?? 0) * item.quantity,
                  0
                ),
          sku:
            checkoutItems.length === 1
                            ...
                            >
                              ...
                              <div className="min-w-0 flex-1">
                                ...
                                <div className="mt-2 flex items-center gap-2">
                                  ...
                                  <span className="w-5 text-center text-sm font-semibold text-slate-700 dark:text-gray-300">
                                    {item.quantity}
                                  </span>
                                  <button
                                    onClick={(event) => {
