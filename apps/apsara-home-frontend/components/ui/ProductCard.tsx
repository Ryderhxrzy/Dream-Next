...
export default function ProductCard({
  ...
  >(variants ?? [])
  const [fetchPublicProduct, { isFetching: isFetchingProductDetails }] =
  ...
  const slug = safeName
    ...
    .replace(/^-+|-+$/g, "")
  const productPath =
    typeof id === "number" ? `/product/${slug}-i${id}` : `/product/${slug}`
  ...
  const visibleVariantChoices = effectiveSelectedColor
    ? variantChoices.filter((choice) =>
        choice.variants.some(
          (variant) =>
  ...
  const activeChoice =
    visibleVariantChoices.find((choice) =>
      choice.variants.some(
  ...
  const activeVariant = activeChoice
    ? (activeChoice.variants.find(
        (variant) => variant.color === effectiveSelectedColor
      ) ?? activeChoice.variants[0])
    : undefined
  ...
  const addResolvedVariantToCart = (variant?: VariantOption) => {
    const variantSrp =
      Number(variant?.priceSrp ?? displayPrice ?? 0) || displayPrice
    const variantMember = Number(variant?.priceMember ?? 0)
    const hasVariantMember =
      isLoggedIn && variantMember > 0 && variantMember < variantSrp
    ...
    addToCart({
      ...
      price: variantPrice,
      originalPrice:
        typeof variantOriginal === "number" ? variantOriginal : null,
                ...
                {ratingValue !== null || normalizedReviewCount !== null ? (
                  <div className="mt-1 flex items-center gap-1.5 text-xs text-slate-500">
                    ...
                    {normalizedReviewCount !== null ? (
                      <span>{`(${normalizedReviewCount})`}</span>
                    ) : null}
                  </div>
                ) : (
              ...
              <div className="flex flex-row items-center justify-between gap-3 sm:w-44 sm:flex-col sm:items-end sm:justify-between">
                ...
                >
                  <svg
                    ...
                    height="16"
                    viewBox="0 0 24 24"
                    fill={isWishlisted ? "#38bdf8" : "none"}
                    stroke={isWishlisted ? "#38bdf8" : "currentColor"}
                    strokeWidth="2"
                  >
          ...
          >
            <div
              className={`relative overflow-hidden bg-gray-50 ${
                isList
                  ? "aspect-[4/3] sm:aspect-auto sm:h-40 sm:w-56 sm:min-w-[14rem]"
                  : "aspect-square"
              }`}
            >
              <Image
            ...
            <div className={`p-4 ${isList ? "sm:flex-1" : ""}`}>
              <h3 className="truncate text-sm font-medium text-gray-800 transition-colors duration-200 group-hover:text-sky-500">
                {safeName}
              </h3>
              {(ratingValue !== null || normalizedReviewCount !== null) && (
                <div className="mt-1 flex items-center gap-1.5 text-xs text-slate-500">
                  ...
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      ...
                      <p className="mt-1 text-xs text-slate-500">
                        Choose a variant before adding this product to cart.
                      </p>
                    </div>
                    <button
                      type="button"
                        ...
                        {colorOptions.map((color) => (
                          <button
                            ...
                            onClick={() => {
                              ...
                              const nextVariant =
                                nextChoice?.variants.find(
                                  (variant) => variant.color === color.name
                                ) ?? nextChoice?.variants[0]
                              if (nextVariant?.sku) {
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
                      {visibleVariantChoices.map((choice) => {
                        ...
                        const isActive = choice.variants.some(
                          (variant) =>
                            (variant.sku ?? "") === (activeVariant?.sku ?? "")
                        )
                        const choicePrice =
                          Number(
                            choiceVariant?.priceMember ??
                              choiceVariant?.priceSrp ??
                              displayPrice ??
                              0
                          ) || displayPrice
