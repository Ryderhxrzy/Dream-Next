...
import {
  ShoppingBag, BarChart2, AlignLeft, Tag,
  RotateCcw, Filter, ChevronDown,
} from 'lucide-react'
...
const extractErrorMessage = (error: unknown): string => {
  ...
  const e = error as { status?: number | string; data?: { message?: string; error?: string }; error?: string }
...
function EmptyOrdersIllustration() {
  return (
    <div className="relative mx-auto flex h-28 w-28 items-center justify-center">
      {/* sparkle dots */}
      <span className="absolute left-0 top-4 text-teal-300 text-lg select-none">+</span>
      <span className="absolute right-1 top-2 text-teal-200 text-sm select-none">+</span>
      <span className="absolute left-3 bottom-2 text-teal-200 text-xs select-none">✦</span>
      <span className="absolute right-2 bottom-4 text-teal-300 text-xs select-none">✦</span>
      {/* circle bg */}
      <div className="flex h-20 w-20 items-center justify-center rounded-full bg-teal-50 shadow-inner">
        {/* document + search SVG */}
        <svg width="52" height="52" viewBox="0 0 52 52" fill="none">
          {/* document */}
          <rect x="6" y="4" width="28" height="36" rx="4" fill="#ccfbf1" stroke="#5eead4" strokeWidth="1.5"/>
          <line x1="13" y1="14" x2="27" y2="14" stroke="#14b8a6" strokeWidth="2" strokeLinecap="round"/>
          <line x1="13" y1="20" x2="27" y2="20" stroke="#14b8a6" strokeWidth="2" strokeLinecap="round"/>
          <line x1="13" y1="26" x2="21" y2="26" stroke="#14b8a6" strokeWidth="2" strokeLinecap="round"/>
          {/* magnifying glass */}
          <circle cx="36" cy="36" r="10" fill="white" stroke="#5eead4" strokeWidth="1.5"/>
          <circle cx="35" cy="35" r="5.5" stroke="#0d9488" strokeWidth="2"/>
          <line x1="39" y1="39" x2="44" y2="44" stroke="#0d9488" strokeWidth="2.5" strokeLinecap="round"/>
        </svg>
      </div>
    </div>
  )
...
export default function PartnerOrdersPage() {
  ...
  const partnerOrders = useMemo(() => {
    return (ordersData?.orders ?? []).filter((order) => {
      if (statusFilter !== 'all') {
        ...
        if (fulfillmentStatus !== statusFilter.trim().toLowerCase()) return false
      }
  ...
  const loading = isMeLoading || isStorefrontLoading || isOrdersLoading || isCategoriesLoading
  ...
  const categoryOptions = useMemo(() => {
    ...
    for (const c of allowedCategories) options.push({ value: String(c.id), label: c.name })
    return options
  }, [allowedCategories])

  const resetFilters = () => { setStatusFilter('all'); setCategoryFilter('all') }
      ...
      <div className="relative overflow-hidden rounded-3xl border border-slate-100 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900">
        ...
        <div className="relative flex flex-col gap-3 p-6 sm:flex-row sm:items-center sm:justify-between sm:p-8">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-2xl font-extrabold tracking-tight text-slate-900 dark:text-white">Partner Orders</h1>
              <p className="mt-0.5 text-sm text-slate-500 dark:text-slate-400">Orders placed from your assigned storefronts.</p>
            </div>
          </div>
      ...
      <div className="rounded-2xl border border-slate-100 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900">
        <div className="grid grid-cols-1 items-end gap-4 md:grid-cols-3">
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <p className="text-center text-[11px] text-slate-400 dark:text-slate-500">Use dropdowns to refine results</p>
          </div>
      ...
      <div className="rounded-3xl border border-slate-100 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900">
        ...
        {!loading && !error && storefrontIds.length === 0 && (
          <div className="py-16 text-center">
            ...
            <p className="mt-5 text-base font-bold text-slate-800 dark:text-slate-100">No storefront assigned.</p>
            <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">Contact your admin to get a storefront assigned to your account.</p>
          </div>
        )}
        ...
        {!loading && !error && storefrontIds.length > 0 && partnerOrders.length === 0 && (
          <div className="py-16 text-center">
            <EmptyOrdersIllustration />
            <p className="mt-5 text-base font-bold text-slate-800 dark:text-slate-100">No orders match your current filters.</p>
            <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">Try adjusting your filters or reset to see more results.</p>
            <button
              type="button"
              onClick={resetFilters}
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full border-separate border-spacing-0">
              <thead className="sticky top-0 z-10 bg-slate-50 dark:bg-slate-800/60">
                <tr>
                  {['Order', 'Storefront', 'Customer', 'Product', 'Amount', 'Payment', 'Status', 'Date'].map((h) => (
                    <th key={h} className="px-4 py-3 text-left text-[11px] font-bold uppercase tracking-widest text-slate-500 dark:text-slate-400">
                      {h}
                    </th>
                  ))}
                </tr>
                  ...
                  return (
                    ...
                    >
                      <td className="px-4 py-3">
                        ...
                        <p className="text-xs text-slate-400 dark:text-slate-500">{order.checkout_id}</p>
                      </td>
                      ...
                      <td className="px-4 py-3 text-sm text-slate-700 dark:text-slate-300">
                        <div className="flex max-w-xs gap-3">
                          <div className="relative h-12 w-12 shrink-0 overflow-hidden rounded-xl border border-slate-100 bg-slate-50 dark:border-slate-700 dark:bg-slate-900">
                            {order.product_image ? (
                              <Image src={order.product_image} alt={order.product_name} fill sizes="48px" className="object-cover" />
                            ) : (
                              <div className="flex h-full w-full items-center justify-center text-[10px] text-slate-400">N/A</div>
                            )}
                          </div>
                          <div className="min-w-0">
                            <p className="truncate font-semibold text-slate-800 dark:text-slate-100">{order.product_name}</p>
                      ...
                      <td className="px-4 py-3">
                        <span className={`inline-flex rounded-full px-2.5 py-1 text-xs font-semibold capitalize ${statusClass(order.fulfillment_status)}`}>
                          {String(order.fulfillment_status ?? '-').replace(/_/g, ' ')}
                        </span>
                      </td>
