  ...
  return (
    <div className="bg-[#0a0a0f] font-sans text-white">
      ...
      <section className="relative overflow-hidden" style={{ minHeight: 580 }}>
        <img src={heroImage} alt="Hero" className="absolute inset-0 h-full w-full object-cover opacity-30" />
        ...
        <div className="relative z-10 flex flex-col items-center justify-center px-6 py-32 text-center">
          <span className="mb-5 inline-block rounded-full border px-4 py-1 text-xs font-semibold uppercase tracking-widest"
            style={{ borderColor: `${primaryColor}60`, color: primaryColor }}>
            Partner Storefront
          </span>
          <h1 className="max-w-3xl text-5xl font-black leading-tight lg:text-6xl">{tagline}</h1>
          <p className="mt-5 max-w-xl text-base leading-relaxed text-white/60">{description}</p>
          <div className="mt-10 flex flex-wrap items-center justify-center gap-4">
            <button type="button" className="rounded-full px-8 py-3.5 text-sm font-bold text-white shadow-2xl transition hover:opacity-90" style={{ backgroundColor: primaryColor }}>
              {heroBtnPrimary}
            </button>
            <button type="button" className="rounded-full border border-white/20 px-8 py-3.5 text-sm font-medium text-white/80 transition hover:bg-white/10">
              {heroBtnSecondary}
            </button>
          </div>
        </div>
      ...
      <section className="px-10 py-20">
        <div className="mx-auto max-w-6xl">
          ...
          <div className="grid gap-6 md:grid-cols-3">
            {[
              { icon: '🛋️', title: 'Curated Catalog', desc: 'Premium furniture sourced and quality-checked by Apsara Home.' },
              { icon: '🎨', title: 'Your Brand', desc: 'Your logo, your colors, your store. Fully white-labeled.' },
              { icon: '📦', title: 'Zero Logistics', desc: 'We handle storage, packing, and delivery. You just sell.' },
              { icon: '💰', title: 'Earn Commissions', desc: 'Get paid on every completed order automatically.' },
              { icon: '⚡', title: 'Launch in Days', desc: 'Go live fast. No technical setup required.' },
              { icon: '📊', title: 'Live Dashboard', desc: 'Track orders, earnings, and performance in real time.' },
            ].map((f) => (
              <div key={f.title} className="rounded-2xl border border-white/10 bg-white/5 p-6 transition hover:border-white/20">
      ...
      <section className="px-10 py-24 text-center">
        <div className="mx-auto max-w-2xl">
          ...
          <button type="button" className="mt-10 rounded-2xl px-12 py-4 text-sm font-bold text-white shadow-2xl transition hover:opacity-90" style={{ backgroundColor: primaryColor }}>
            {ctaBtnText}
          </button>
        </div>
      </section>
