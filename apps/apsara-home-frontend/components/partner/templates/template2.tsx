...
export default function Template2({
  ...
  return (
    <div className="bg-[#faf7f3] font-sans text-[#1e293b]">
      ...
      {(aboutTitle || aboutBody) && (
        <section className="px-10 py-20">
          <div className={`mx-auto grid max-w-6xl items-center gap-12 ${aboutImage ? 'md:grid-cols-2' : ''}`}>
            {aboutImage && (
              <motion.div {...REVEAL} className="overflow-hidden rounded-3xl shadow-xl">
                <img src={aboutImage} alt={aboutTitle ?? storeName} className="w-full object-cover" style={{ maxHeight: 420 }} />
              </motion.div>
            )}
            <motion.div {...REVEAL}>
              {aboutTitle && <h2 className="text-3xl font-bold text-slate-900">{aboutTitle}</h2>}
              {aboutBody && <p className="mt-4 leading-relaxed text-slate-500">{aboutBody}</p>}
              {Array.isArray(aboutHighlights) && aboutHighlights.length > 0 && (
                <ul className="mt-6 space-y-2">
                  {aboutHighlights.map((h, i) => (
                    <li key={h?.text ?? i} className="flex items-center gap-3 text-sm text-slate-600">
      ...
      {/* Footer */}
      <footer className="border-t border-slate-200 bg-white px-10 py-10">
        <div className="mx-auto grid max-w-6xl grid-cols-1 gap-8 md:grid-cols-3">
          <div>
            ...
            {tagline && <p className="mt-1 text-xs text-slate-400">{tagline}</p>}
          </div>
          <div>
            <p className="mb-3 text-[10px] font-bold uppercase tracking-widest text-slate-400">Quick Links</p>
            <div className="flex flex-col gap-2 text-sm text-slate-500">
              {navLinks.map((l) => (
                <a key={l.label} href={l.href} className="transition hover:text-slate-900">{l.label}</a>
              ))}
            </div>
          </div>
        ...
        <p className="mt-8 text-center text-xs text-slate-400">© {new Date().getFullYear()} {storeName} · Powered by Apsara Home</p>
      </footer>
    </div>
  )
}
