...
export default function Template2({
  ...
  stat1Value = '500+', stat1Label = 'Active Partners',
  ...
  return (
    <div className="bg-[#faf7f3] font-sans text-[#1e293b]">
      ...
      <section className="mx-auto grid max-w-7xl items-center gap-12 px-10 py-20 md:grid-cols-2">
        <div>
          <span className="mb-4 inline-block rounded-full px-3 py-1 text-xs font-semibold uppercase tracking-wider"
            style={{ backgroundColor: `${primaryColor}18`, color: primaryColor }}>
            Partner Storefront
          </span>
          <h1 className="text-5xl font-black leading-tight lg:text-6xl">{tagline}</h1>
          <p className="mt-5 max-w-md text-base leading-relaxed text-slate-500">{description}</p>
          <div className="mt-8 flex flex-wrap items-center gap-4">
            <button type="button" className="rounded-2xl px-8 py-3.5 text-sm font-bold text-white shadow-lg transition hover:opacity-90" style={{ backgroundColor: primaryColor }}>
              {heroBtnPrimary}
            </button>
            <button type="button" className="rounded-2xl border border-slate-300 bg-white px-8 py-3.5 text-sm font-medium text-slate-600 transition hover:bg-slate-50">
              {heroBtnSecondary}
            </button>
          </div>
          <div className="mt-10 flex flex-wrap items-center gap-5">
            {['Free Delivery', 'Easy Returns', 'Quality Guarantee'].map((badge) => (
              <span key={badge} className="flex items-center gap-1.5 text-xs font-semibold text-slate-500">
                <span className="h-1.5 w-1.5 rounded-full" style={{ backgroundColor: primaryColor }} />{badge}
              </span>
            ))}
          </div>
        </div>
        ...
        <div className="mx-auto max-w-6xl">
          ...
          <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
            {[
              ...
              <div key={f.title} className="rounded-2xl border border-slate-100 p-6 transition hover:shadow-md">
      ...
      <section className="px-10 py-20">
        <div className="mx-auto max-w-3xl rounded-3xl p-10 text-center text-white shadow-xl" style={{ backgroundColor: primaryColor }}>
          <p className="text-2xl font-bold leading-relaxed">"{testimonialText}"</p>
      ...
      {/* CTA */}
      <section className="bg-[#faf7f3] px-10 py-24 text-center">
        <h2 className="text-4xl font-black text-slate-900">{ctaTitle}</h2>
        <p className="mt-4 text-base text-slate-500">{ctaSubtitle}</p>
        <button type="button" className="mt-10 rounded-2xl px-12 py-4 text-sm font-bold text-white shadow-xl transition hover:opacity-90" style={{ backgroundColor: primaryColor }}>
          {ctaBtnText}
        </button>
      </section>

      {/* Footer */}
      <footer className="border-t border-slate-200 bg-white px-10 py-8 text-center text-xs text-slate-400">
