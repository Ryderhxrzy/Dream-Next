...
import { AlertTriangle, Calendar, CheckCircle2, ChevronDown, Clock3, FileText, Loader2, Lock, RotateCcw, ShieldCheck, Star, Store, Upload, Users, X } from 'lucide-react'
...
type RenewalDraftPayload = { selectedPlan?: PlanKey; selectedBillingOption?: BillingOption; selectedPaymentMethod?: PaymentMethod; renewalEnabled?: boolean; storefrontKey?: string }
...
const getRequestTimestamp = (request?: { reviewed_at?: string | null; created_at?: string | null; approved_at?: string | null } | null) => {
  if (!request) return 0
...
export default function PartnerStorefrontRenewalPage() {
  ...
  const [successModalData, setSuccessModalData] = useState<{
    checkoutId: string; paymentReference: string; paymentIntent: string
    paymentMethod: string; customerName: string; email: string
    plan: PlanKey; billingOption: BillingOption; fee: number
  } | null>(null)
  const [pendingFinalizePayload, setPendingFinalizePayload] = useState<SubmitWebstoreRequestPayload | null>(null)
  const [reuploadSource, setReuploadSource] = useState<'success' | 'rejection' | null>(null)
  const [reuploadFiles, setReuploadFiles] = useState<Array<{ name: string; preview: string; file: File; isRejected?: boolean }>>([])
  ...
  const totalReceiptCount = useMemo(() =>
    selectedStorefrontRequests.reduce((sum, req) => sum + (req.receipt_items?.length ?? 0), 0),
    [selectedStorefrontRequests],
  )
  ...
  const isLatestReceiptRejected = latestReceiptItem?.approval_status === 'rejected'
    ...
    const finalize = async () => {
      try {
        ...
        setSuccessModalData({
          ...
          paymentReference: verified.payment_reference || verified.payment_intent_id || checkoutId,
          paymentIntent: verified.payment_intent_id || '-',
          paymentMethod: effectivePaymentMethod,
          customerName: String(adminMe?.name ?? '').trim() || selectedStorefrontName,
          email: String(adminMe?.email ?? '').trim() || '',
          plan: effectivePlan,
          billingOption: effectiveBillingOption,
          fee: effectiveBillingOption === 'full' ? (planOpt?.full ?? 0) : (planOpt?.monthly ?? 0),
        })
      } catch (error) {
  ...
  const openReuploadModalWithPrefill = async () => {
    ...
    const urls = (latestReceiptItem?.receipt_urls ?? []).filter((u): u is string => Boolean(u))
    if (urls.length > 0) {
      try {
        const loaded = await Promise.all(
          urls.map(async (url) => {
            ...
            const fileName = decodeURIComponent(url.split('/').pop()?.split('?')[0] ?? 'receipt.jpg')
            const file = new File([blob], fileName, { type: blob.type || 'image/jpeg' })
            return { name: fileName, preview: URL.createObjectURL(file), file, isRejected: true }
          })
        )
  ...
  const handleReuploadFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setReuploadFiles(files.map((f) => ({ name: f.name, preview: URL.createObjectURL(f), file: f })))
    if (reuploadInputRef.current) reuploadInputRef.current.value = ''
  }

  const submitPendingFinalizePayload = async (overrideReceiptUrls?: string[]) => {
  ...
  const handleSubmitReupload = async () => {
    ...
    try {
      ...
      if (uploadedUrls.length === 0) throw new Error('No receipt images were uploaded successfully.')
      ...
      // Rejection flow
      const targetRequest = latestApprovedRequest
        ?? selectedStorefrontRequests.find((r) => r.status === 'pending_review')
        ?? null
      if (!targetRequest) throw new Error('No active subscription found.')
      const paymentRef = targetRequest.payment_reference || targetRequest.checkout_id || `WEB-${Date.now()}`
      await submitWebstoreRequest({
        ...
        slug_name: selectedStorefrontSlug,
        display_name: selectedStorefrontName,
        plan: (targetRequest.plan ?? 'test') as SubmitWebstoreRequestPayload['plan'],
        billing_option: (targetRequest.billing_option ?? 'full') as 'full' | 'monthly',
        payment_method: (targetRequest.payment_method ?? 'gcash') as PaymentMethod,
      ...
      const apiErr = error as { data?: { message?: string }; message?: string }
      const text = apiErr?.data?.message || apiErr?.message || 'Failed to submit receipt.'
      setMessage({ type: 'error', text })
      showErrorToast(text)
    } finally {
  ...
  const handleDownloadSuccessImage = async () => {
    ...
    const billingLabel = successModalData.billingOption === 'full' ? 'Full Payment' : 'Monthly'
    const methodLabel = successModalData.paymentMethod === 'gcash' ? 'GCash' : successModalData.paymentMethod === 'grab_pay' ? 'GrabPay' : successModalData.paymentMethod === 'maya' ? 'Maya' : 'Card'
...
<svg xmlns="http://www.w3.org/2000/svg" width="1200" height="1500" viewBox="0 0 1200 1500">
  ...
  <g transform="translate(76 982)">
    ...
    try {
      ...
      const fileRef = (successModalData.paymentReference || successModalData.checkoutId || 'receipt').replace(/[^a-zA-Z0-9_-]/g, '-')
    ...
    <div className="space-y-5">
          ...
          <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900">
            ...
            <div className="mt-5 grid grid-cols-3 gap-3">
              ...
              <div className="flex items-center gap-3 rounded-2xl border border-slate-100 bg-slate-50 p-4 dark:border-slate-700/50 dark:bg-slate-800/60">
                ...
                <div className="min-w-0">
                  <p className="text-[10px] font-bold uppercase tracking-wider text-slate-400">Payments</p>
                  <p className="mt-0.5 text-sm font-bold text-slate-900 dark:text-slate-100">{totalReceiptCount}</p>
                </div>
              </div>
            </div>
          </div>
          ...
          {isLatestReceiptRejected ? (
            <div className="flex gap-3 rounded-3xl border border-rose-200 bg-linear-to-r from-rose-50 to-pink-50 p-5 shadow-sm dark:border-rose-500/30 dark:from-rose-500/10 dark:to-pink-500/10">
              ...
              <div className="flex flex-1 min-w-0 items-start justify-between gap-3">
                <div className="min-w-0">
                  <p className="font-bold text-rose-900 dark:text-rose-100">Payment receipt rejected</p>
                  <p className="mt-1 text-sm text-rose-800/80 dark:text-rose-100/80">
                    Your latest submitted receipt was rejected. Please upload a new receipt.
                    {latestReceiptItem?.submitted_at ? (
                      <span className="block mt-0.5 text-xs text-rose-600/70 dark:text-rose-300/70">
                        Submitted: {new Date(latestReceiptItem.submitted_at).toLocaleDateString()}
                      </span>
                    ) : null}
                  </p>
                </div>
          ...
          <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900">
            ...
            <div className="mt-4 space-y-2">
              ...
              <div className="flex items-center gap-3 rounded-xl bg-slate-50 px-4 py-2.5 dark:bg-slate-800/60">
                ...
                <span className="flex-1 text-sm text-slate-500 dark:text-slate-400">Payment receipts</span>
                <span className="text-sm font-semibold text-slate-900 dark:text-slate-100">{totalReceiptCount}</span>
              </div>
            </div>
          </div>
        ...
        <div className="fixed inset-0 z-230 flex items-center justify-center bg-slate-950/55 px-3 py-4 backdrop-blur-sm">
          <div className="relative w-full max-w-lg overflow-hidden rounded-[28px] border border-emerald-200 bg-white shadow-[0_28px_90px_rgba(15,23,42,0.35)] dark:border-slate-700 dark:bg-slate-900">
            ...
            <div className="bg-linear-to-r from-teal-500 to-emerald-500 px-6 py-5 text-center text-white">
              ...
              <h3 className="text-xl font-black tracking-tight">Webstore Payment Success</h3>
              <p className="mt-1 text-xs text-white/90">Your payment is confirmed and renewal request has been submitted.</p>
            </div>
            ...
            <div className="px-6 py-5 space-y-2.5">
              ...
              <div className="grid grid-cols-2 gap-2.5">
                <div className="rounded-2xl border border-slate-100 bg-slate-50 px-4 py-3 dark:border-slate-700 dark:bg-slate-800/50">
                  <p className="text-[10px] font-bold uppercase tracking-widest text-slate-400">Checkout ID</p>
                  <p className="mt-0.5 truncate text-xs font-bold text-slate-800 dark:text-slate-100">{successModalData.checkoutId}</p>
                </div>
                <div className="rounded-2xl border border-slate-100 bg-slate-50 px-4 py-3 dark:border-slate-700 dark:bg-slate-800/50">
                  <p className="text-[10px] font-bold uppercase tracking-widest text-slate-400">Status</p>
                  <span className="mt-1 inline-block rounded-full bg-emerald-100 px-3 py-0.5 text-xs font-bold text-emerald-700 dark:bg-emerald-500/20 dark:text-emerald-300">Paid</span>
                </div>
              </div>
              {/* Row 2: Payment Reference + Payment Intent */}
              <div className="grid grid-cols-2 gap-2.5">
                <div className="rounded-2xl border border-slate-100 bg-slate-50 px-4 py-3 dark:border-slate-700 dark:bg-slate-800/50">
                  <p className="text-[10px] font-bold uppercase tracking-widest text-slate-400">Payment Reference</p>
                  <p className="mt-0.5 truncate text-xs font-bold text-slate-800 dark:text-slate-100">{successModalData.paymentReference}</p>
                </div>
                <div className="rounded-2xl border border-slate-100 bg-slate-50 px-4 py-3 dark:border-slate-700 dark:bg-slate-800/50">
                  <p className="text-[10px] font-bold uppercase tracking-widest text-slate-400">Payment Intent</p>
                  <p className="mt-0.5 truncate text-xs font-bold text-slate-800 dark:text-slate-100">{successModalData.paymentIntent}</p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-2.5">
                <div className="rounded-2xl border border-slate-100 bg-slate-50 px-4 py-3 dark:border-slate-700 dark:bg-slate-800/50">
                  ...
                  <p className="mt-0.5 truncate text-xs font-bold text-slate-800 dark:text-slate-100">{successModalData.customerName}</p>
                </div>
                <div className="rounded-2xl border border-slate-100 bg-slate-50 px-4 py-3 dark:border-slate-700 dark:bg-slate-800/50">
                  <p className="text-[10px] font-bold uppercase tracking-widest text-slate-400">Email</p>
                  <p className="mt-0.5 truncate text-xs font-bold text-slate-800 dark:text-slate-100">{successModalData.email}</p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-4 gap-2.5">
                <div className="rounded-2xl border border-slate-100 bg-slate-50 px-3 py-3 dark:border-slate-700 dark:bg-slate-800/50">
                  ...
                  <p className="mt-0.5 text-xs font-bold text-slate-800 dark:text-slate-100">
                    {successModalData.paymentMethod === 'gcash' ? 'GCash' : successModalData.paymentMethod === 'grab_pay' ? 'GrabPay' : successModalData.paymentMethod === 'maya' ? 'Maya' : 'Card'}
                  </p>
                </div>
                <div className="rounded-2xl border border-slate-100 bg-slate-50 px-3 py-3 dark:border-slate-700 dark:bg-slate-800/50">
                  <p className="text-[10px] font-bold uppercase tracking-widest text-slate-400">Plan</p>
                  <p className="mt-0.5 text-xs font-bold text-slate-800 dark:text-slate-100">{planOptions.find((p) => p.key === successModalData.plan)?.title ?? successModalData.plan}</p>
                </div>
                <div className="rounded-2xl border border-slate-100 bg-slate-50 px-3 py-3 dark:border-slate-700 dark:bg-slate-800/50">
                  <p className="text-[10px] font-bold uppercase tracking-widest text-slate-400">Billing</p>
                  <p className="mt-0.5 text-xs font-bold text-slate-800 dark:text-slate-100">{successModalData.billingOption === 'full' ? 'Full' : 'Monthly'}</p>
                </div>
                <div className="rounded-2xl border border-emerald-100 bg-emerald-50 px-3 py-3 dark:border-emerald-500/30 dark:bg-emerald-500/10">
                  <p className="text-[10px] font-bold uppercase tracking-widest text-emerald-600 dark:text-emerald-400">Amount</p>
                  <p className="mt-0.5 text-xs font-bold text-emerald-700 dark:text-emerald-300">PHP {successModalData.fee.toLocaleString()}</p>
                </div>
              </div>
            </div>
            ...
            <div className="flex items-center justify-end gap-3 border-t border-slate-100 px-6 py-4 dark:border-slate-800">
              ...
              <button
                ...
                onClick={async () => {
                  ...
                  try {
                    imageFile = (await handleDownloadSuccessImage()) ?? undefined
                  } catch {
                    // download failed — still open re-upload modal so user can submit manually
                  }
                  setReuploadSource('success')
                  if (imageFile) {
                    setReuploadFiles([{ name: imageFile.name, preview: URL.createObjectURL(imageFile), file: imageFile }])
                  } else {
                    setReuploadFiles([])
                  }
                  setSuccessModalData(null)
      ...
      {isReuploadModalOpen ? (
        <div
          ...
          onClick={() => { if (!isReuploadingReceipt) { void closeReuploadModal() } }}
        >
          <div
          ...
          >
            <button
              ...
              onClick={() => { void closeReuploadModal() }}
            ...
            <div className="bg-linear-to-r from-sky-500 via-blue-500 to-indigo-500 px-6 py-7 text-center text-white">
              ...
              <h3 className="text-2xl font-black tracking-tight">Upload Payment Receipt</h3>
              <p className="mt-1.5 text-sm text-white/90">Upload your new receipt image to re-submit for review.</p>
            </div>
            ...
            <div className="flex-1 overflow-y-auto px-6 py-6 space-y-4">
              ...
              {/* Drop zone */}
              <div
                onDragEnter={(e) => { e.preventDefault(); e.stopPropagation(); setIsDraggingReupload(true) }}
                onDragOver={(e) => { e.preventDefault(); e.stopPropagation(); setIsDraggingReupload(true) }}
                onDragLeave={(e) => { e.preventDefault(); e.stopPropagation(); setIsDraggingReupload(false) }}
                onDrop={(e) => {
                  e.preventDefault()
                  e.stopPropagation()
                  setIsDraggingReupload(false)
                  const files = Array.from(e.dataTransfer.files ?? []).filter((f) => f.type.startsWith('image/') && f.size <= 10 * 1024 * 1024)
                  setReuploadFiles((prev) => [...prev, ...files.map((f) => ({ name: f.name, preview: URL.createObjectURL(f), file: f }))])
                }}
              ...
              >
                {reuploadFiles.length === 0 ? (
                  ...
                  >
                    ...
                    <p className="text-sm font-semibold text-slate-500 dark:text-slate-400">Click or drag images here</p>
                    <p className="mt-0.5 text-xs text-slate-400">PNG, JPG, WEBP up to 10MB</p>
                  </button>
                ) : (
                  <div className="p-4">
            ...
            <div className="flex items-center justify-end gap-3 border-t border-slate-100 px-6 py-4 dark:border-slate-800">
              ...
              >
                {isReuploadingReceipt ? <Loader2 className="h-4 w-4 animate-spin" /> : <Upload className="h-4 w-4" />}
