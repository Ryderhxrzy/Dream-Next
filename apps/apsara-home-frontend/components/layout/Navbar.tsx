...
const getCustomerNotificationTimestamp = (value: string | null | undefined) => {
  ...
  const timestamp = new Date(
    normalizeCustomerNotificationTimestamp(value)
  ).getTime()
...
const navLinks: NavLink[] = [
  ...
  },
  {
    label: "Shop By Room",
    href: "/by-room",
    mega: Object.fromEntries(
      ROOM_OPTIONS.map((room) => [room.label.toUpperCase(), []])
    ),
  },
...
const roomIcons: Record<string, React.ReactNode> = {
  BEDROOM: (
    ...
    >
      ...
      <path d="M21 10H3V7a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2v3z" />
      <path d="M6 19v2" />
      <path d="M18 19v2" />
    </svg>
  ),
  ...
  OUTDOOR: (
    ...
    >
      <path d="M17 14h.01" />
      <path d="M7 7h12a2 2 0 0 1 2 2v6a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h14" />
...
const highlightText = (
  text: string,
  searchTerm: string
): Array<{ type: "highlight" | "normal"; text: string }> => {
...
function NavbarInner({
  ...
  const avatarUrl = isCustomerSession
    ? meData?.avatar_url || user?.image || null
    : null
  ...
  const [readCustomerNotificationKeys, setReadCustomerNotificationKeys] =
    useState<string[]>([])
  const [
    surfacedCustomerNotificationKeys,
    setSurfacedCustomerNotificationKeys,
  ] = useState<string[]>([])
  const [realtimeNotification, setRealtimeNotification] =
    useState<CustomerNotificationItem | null>(null)
  ...
  useEffect(() => {
    ...
    const timeoutId = window.setTimeout(
      ...
      REALTIME_NOTIFICATION_DURATION_SECONDS * 1000
    )
  ...
  useEffect(() => {
    ...
    const onNotificationCreated = (
      event: CustomerRealtimeNotificationEvent
    ) => {
      const item: CustomerNotificationItem = {
        ...
        latest_at:
          event.latest_at ?? event.created_at ?? new Date().toISOString(),
      }

      dispatch(
        customerNotificationsApi.util.updateQueryData(
          "getCustomerNotifications",
          customerNotificationCacheKey,
          (draft) => {
      ...
      dispatch(
        baseApi.util.invalidateTags(["Encashment", "CustomerNotifications"])
      )
  ...
  useEffect(() => {
    ...
    const storedEmail =
      window.localStorage.getItem("afhome_new_registration_email")?.trim() ?? ""
  ...
  const { data: searchedProductsData, isFetching: isSearchingProducts } =
    ...
    return rows
      .map((product) => {
        ...
        if (isPartnerSearchScoped) {
          if (
            !partnerSelectedProductIdSet ||
            partnerSelectedProductIdSet.size === 0
          )
            return null
          if (!id || !partnerSelectedProductIdSet.has(id)) return null
        }
        const priceMember =
          typeof product.priceMember === "number" ? product.priceMember : null
        const priceDp =
          typeof product.priceDp === "number" ? product.priceDp : null
        const priceSrp =
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (stored) {
        ...
        nextReadKeys = Array.isArray(parsed)
  ...
  useEffect(() => {
    ...
    try {
      window.localStorage.setItem(
        customerNotificationStorageKey,
        JSON.stringify(readCustomerNotificationKeys)
      )
    } catch {
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (stored) {
        const parsed = JSON.parse(stored)
        nextSurfacedKeys = Array.isArray(parsed)
          ? parsed.filter((entry): entry is string => typeof entry === "string")
          : []
      }
    } catch {
  ...
  useEffect(() => {
    ...
    try {
      window.localStorage.setItem(
        customerNotificationSurfacedStorageKey,
        JSON.stringify(surfacedCustomerNotificationKeys)
      )
    } catch {
      // Ignore localStorage write failures.
    }
  ...
  const visibleCustomerNotifications = useMemo(() => {
    const items = (notificationsData?.items ?? []).filter(
      (item) => item.count > 0
    )

    return [...items].sort((a, b) => {
      const aUnread = readCustomerNotificationKeys.includes(
        getCustomerNotificationReadKey(a)
      )
        ? 1
        : 0
      const bUnread = readCustomerNotificationKeys.includes(
        getCustomerNotificationReadKey(b)
      )
        ? 1
        : 0
      ...
      const timeDiff =
        getCustomerNotificationTimestamp(b.latest_at) -
        getCustomerNotificationTimestamp(a.latest_at)
  ...
  const unreadNotificationCount = useMemo(() => {
    return visibleCustomerNotifications.reduce((total, item) => {
      const isRead = readCustomerNotificationKeys.includes(
        getCustomerNotificationReadKey(item)
      )
      return isRead ? total : total + item.count
    }, 0)
  }, [readCustomerNotificationKeys, visibleCustomerNotifications])

  const markCustomerNotificationAsRead = (item: {
    id: string
    title: string
    description: string
    count: number
  ...
  const markAllCustomerNotificationsAsRead = () => {
    setReadCustomerNotificationKeys((current) => {
      ...
      visibleCustomerNotifications.forEach((item) =>
        next.add(getCustomerNotificationReadKey(item))
      )
  ...
  useEffect(() => {
    ...
    const nextUnreadNotification = visibleCustomerNotifications.find((item) => {
      const readKey = getCustomerNotificationReadKey(item)
      return (
        !readCustomerNotificationKeys.includes(readKey) &&
        !surfacedCustomerNotificationKeys.includes(readKey)
      )
    })
  ...
  const shopCategoryItems = useMemo(() => {
    ...
    if (partnerSlug) {
      ...
      const nonInteriorItems = scopedItems.filter(
        (item) =>
          item.label.trim().toLowerCase() !== interiorLabel.toLowerCase()
      )
  ...
  const getNotificationIcon = (title: string) => {
    ...
    if (
      ...
      return {
        ...
        icon: (
          <svg
            className={`${iconClass} text-pink-600 dark:text-pink-400`}
            fill="none"
            stroke="currentColor"
            viewBox="0 0 24 24"
          >
    ...
    return {
      ...
      icon: (
        <svg
          className={`${iconClass} text-slate-500 dark:text-slate-400`}
          fill="none"
          stroke="currentColor"
          viewBox="0 0 24 24"
        >
            ...
            >
              <div className="container mx-auto flex min-h-11 items-center justify-between gap-4 px-4 py-2">
                <p className="flex-1 leading-snug font-medium">
                  ...
                  <Link
                    href={profileHref}
                    className="font-semibold text-sky-600 underline underline-offset-4 transition hover:text-sky-700 dark:text-sky-300 dark:hover:text-sky-200"
                  >
                          ...
                          <div className="max-h-[60vh] overflow-y-auto overscroll-contain sm:max-h-[420px]">
                            {isNotificationsLoading ? (
                              ...
                              <div className="divide-y divide-slate-100 dark:divide-slate-800/80">
                                {visibleCustomerNotifications.map(
                                  (item, idx) => {
                                    const isRead =
                                      readCustomerNotificationKeys.includes(
                                        getCustomerNotificationReadKey(item)
                                      )
                                    const isFreshRealtime =
                                      freshRealtimeNotificationIds.includes(
                                        item.id
                                      )
                  ...
                  <div className="flex flex-col gap-3">
                    <div className="grid grid-cols-4 gap-1 lg:grid-cols-8">
                      {navbarBrandItems.length > 0 ? (
                        ...
                        <p className="col-span-full py-4 text-center text-sm text-gray-400 dark:text-gray-500">
                          No brands available
                        </p>
                      )}
                    </div>
                ...
                {isLoggedIn ? (
                  <div className="mb-3 overflow-hidden rounded-2xl border border-sky-100 shadow-sm dark:border-sky-900/50 dark:shadow-none">
                    ...
                    {(() => {
                      ...
                      const gradient =
                        tier === "Lifestyle Elite"
                          ? "from-sky-400 via-sky-400 to-rose-400"
                          : tier === "Lifestyle Consultant"
                            ...
                            {link.mega ? (
                              ...
                              <div className="ml-4 space-y-1 border-l-2 border-sky-200 py-2 pl-4 dark:border-sky-800">
                                ...
                                {(() => {
                                  ...
                                  const rooms = Object.keys(link.mega!).filter(
                                    ...
                                    return (
                                      <div key={room} className="mb-2">
                                        ...
                                        >
                                          ...
                                          <span className="text-xs font-bold tracking-wider text-sky-600">
                                            {room}
                                          </span>
                                        </Link>
                                      </div>
                              ...
                              <div className="ml-4 space-y-2 border-l-2 border-sky-200 py-2 pl-4 dark:border-sky-800">
                                ...
                                {(mobileBrandSearch
                                  ? shopBrandItems.filter((b) =>
                                      b.label
                                        .toLowerCase()
                                        .includes(
                                          mobileBrandSearch.toLowerCase()
                                        )
                                    )
                                  : navbarBrandItems
                                ).map((item) => (
                          ...
                          >
                            ...
                            <div className="mx-4 mb-3.5 h-[3px] overflow-hidden rounded-full bg-[#2c2c2e]">
                              <motion.div
                                ...
                                transition={{
                                  duration:
                                    REALTIME_NOTIFICATION_DURATION_SECONDS -
                                    0.45,
                                  ease: "linear",
                                }}
                              />
                ...
                >
                  ...
                  >
                    <div className="flex items-start gap-3 p-4">
                      <div
                        className={`mt-0.5 flex h-10 w-10 shrink-0 items-center justify-center rounded-2xl ${getNotificationIcon(realtimeNotification.title).bg}`}
                      >
                    ...
                    <div className="h-1 bg-slate-100 dark:bg-slate-800">
                      <motion.div
                        ...
                        animate={{ scaleX: 0 }}
                        transition={{
                          duration: REALTIME_NOTIFICATION_DURATION_SECONDS,
                          ease: "linear",
                        }}
                      />
                    </div>
                        ...
                        <div className="max-h-[calc(100vh-180px)] overflow-y-auto px-3 py-4 sm:px-4 sm:py-5">
                          ...
                          {debouncedSearchQuery.length < 2 ? (
                              ...
                              {!isLoggedIn ? (
                                ...
                                >
                                  <div className="mx-auto flex h-12 w-12 items-center justify-center rounded-lg bg-slate-200 dark:bg-gray-700">
                                    <svg
                                      className="h-6 w-6 text-slate-600 dark:text-gray-400"
                                      fill="none"
                                      stroke="currentColor"
                                      viewBox="0 0 24 24"
                                    >
                                  ...
                                  <p className="mt-3 text-sm font-medium text-slate-900 dark:text-gray-200">
                                    Sign in to save searches
                                  </p>
                                  <p className="mt-1 text-xs text-slate-600 dark:text-gray-400">
                                    Your search history will appear here once
                                    you sign in
                                  </p>
                                </motion.div>
                                  ...
                                  <div className="flex flex-wrap gap-2">
                                    {searchHistoryData?.data?.map(
                                      (item, index) => (
                                        ...
                                        >
                                          <svg
                                            className="h-3.5 w-3.5 shrink-0 text-slate-400 dark:text-gray-500"
                                            fill="none"
                                            stroke="currentColor"
                                            viewBox="0 0 24 24"
                                          >
                                          ...
                                          >
                                            <svg
                                              className="h-3.5 w-3.5 text-slate-400 hover:text-red-500 dark:text-gray-500 dark:hover:text-red-400"
                                              fill="none"
                                              stroke="currentColor"
                                              viewBox="0 0 24 24"
                                            >
                            ...
                            >
                              ...
                              <p className="mt-2 text-sm font-medium text-slate-900 dark:text-gray-200">
                                No products found
                              </p>
                              <p className="mt-1 text-xs text-slate-600 dark:text-gray-400">
                                No matches for &quot;
                              ...
                              {searchedProducts.map((product, index) => (
                                ...
                                >
                                  ...
                                  >
                                    ...
                                    <svg
                                      className="h-5 w-5 shrink-0 text-slate-300 transition group-hover:translate-x-1 dark:text-gray-600"
                                      fill="none"
                                      stroke="currentColor"
                                      viewBox="0 0 24 24"
                                    >
