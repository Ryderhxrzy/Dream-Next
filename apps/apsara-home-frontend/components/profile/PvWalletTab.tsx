        ...
        <div className="relative flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
          <div className="flex flex-1 items-start gap-4">
            <div className="flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl border border-white/80 bg-white px-2 shadow-sm dark:border-white/10 dark:bg-white/10">
              <img src="/af_home_logo.png" alt="AF Home" className="h-7 w-auto object-contain" />
            </div>
            <div>
              <p className={`text-[11px] font-bold uppercase tracking-widest ${isGroupBonusActive ? 'text-emerald-600 dark:text-emerald-400' : 'text-amber-600 dark:text-amber-400'}`}>
              ...
              <p className="mt-1.5 text-sm leading-relaxed text-slate-600 dark:text-gray-300">
                Personal PV this {monthlyActivation?.month_label ?? 'month'} determines if you can receive Group Purchase Bonus.
              </p>
            </div>
          </div>
          ...
          <span className={`inline-flex shrink-0 items-center gap-2 self-start rounded-xl px-4 py-2 text-sm font-bold shadow-sm ${
            ...
            <span className={`h-1.5 w-1.5 rounded-full ${isGroupBonusActive ? 'bg-emerald-200' : 'bg-amber-200'}`} />
            {isGroupBonusActive ? 'Active for Group Bonus' : 'Inactive for Group Bonus'}
          </span>
        </div>
