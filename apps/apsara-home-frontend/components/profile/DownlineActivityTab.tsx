      ...
      <section className="relative overflow-hidden rounded-2xl border border-sky-200/70 bg-gradient-to-br from-sky-50 via-white to-emerald-50 p-5 dark:border-sky-800/50 dark:from-slate-900 dark:via-slate-900 dark:to-emerald-950/30">
        ...
        <div className="relative flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          <div>
            ...
            <p className="mt-1 max-w-xl text-sm text-slate-500 dark:text-slate-400">
              Track downline orders, PV movement, status, and any bonus
              generated from eligible purchases.
            </p>
          </div>
          <button
            type="button"
          ...
          >
            <span
              className={
                isFetching
                  ? "h-3 w-3 animate-spin rounded-full border-2 border-sky-500 border-t-transparent"
        ...
        <div className="relative mt-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
          <SummaryCard
            ...
            value={numberFmt(summary?.total_orders ?? 0)}
            sub="matching orders"
            color="sky"
          />
          <SummaryCard
            icon={Zap}
            label="Total PV"
            value={`${numberFmt(summary?.total_pv ?? 0)} PV`}
            sub="network purchase PV"
            color="blue"
          />
          <SummaryCard
            icon={WalletCards}
            label="Bonus Generated"
            value={peso(summary?.total_bonus ?? 0)}
            sub="credited when eligible"
            color="emerald"
          />
          <SummaryCard
            icon={Users}
            label="Active Buyers"
            value={numberFmt(summary?.active_downlines ?? 0)}
        ...
        {isLoading ? (
          ...
          <div className="px-6 py-14 text-center">
            ...
            <p className="mt-3 font-bold text-slate-700 dark:text-slate-200">
              No downline purchases found
            </p>
            <p className="mt-1 text-sm text-slate-400 dark:text-slate-500">
              Orders from your referral network will appear here once they are
              created.
            </p>
          </div>
        ) : (
              ...
              <thead>
                <tr className="bg-slate-50/80 text-left text-[10px] font-black tracking-widest text-slate-400 uppercase dark:bg-slate-800/40 dark:text-slate-500">
                  {[
                    ...
                    <th
                      key={heading}
                      className={`px-4 py-3 first:pl-5 last:pr-5 ${["PV", "Amount", "Bonus"].includes(heading) ? "text-right" : ""}`}
                    >
                      {heading}
                    </th>
                  ))}
                ...
                {rows.map((row, index) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-4 py-3.5">
                        <span
                          className={`inline-flex rounded-full px-2.5 py-1 text-[11px] font-bold ring-1 ${statusClass(statusValue)}`}
                        >
                      ...
                      <td className="px-4 py-3.5 text-right">
                        <p
                          className={`font-black tabular-nums ${row.bonus_amount > 0 ? "text-emerald-600 dark:text-emerald-400" : "text-slate-400"}`}
                        >
                          {row.bonus_amount > 0
                            ? `+${peso(row.bonus_amount)}`
                            : peso(0)}
                        </p>
                        {row.bonus_rate ? (
...
const SUMMARY_STYLES: Record<SummaryColor, string> = {
  ...
  violet:
    "border-violet-200 bg-violet-50 text-violet-700 dark:border-violet-800/60 dark:bg-violet-900/20 dark:text-violet-300",
}
