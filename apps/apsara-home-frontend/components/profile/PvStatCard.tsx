...
const accentStyles = {
  blue: {
    ...
    badge: 'bg-sky-50 text-sky-600 ring-sky-100 dark:bg-sky-950/60 dark:text-sky-300 dark:ring-sky-800/70',
  ...
  sky: {
    ...
    badge: 'bg-cyan-50 text-cyan-600 ring-cyan-100 dark:bg-cyan-950/60 dark:text-cyan-300 dark:ring-cyan-800/70',
    label: 'text-sky-600 dark:text-sky-400',
    value: 'text-sky-900 dark:text-sky-100',
  },
  emerald: {
    ...
    badge: 'bg-emerald-50 text-emerald-600 ring-emerald-100 dark:bg-emerald-950/60 dark:text-emerald-300 dark:ring-emerald-800/70',
  ...
  violet: {
    ...
    badge: 'bg-violet-50 text-violet-600 ring-violet-100 dark:bg-violet-950/60 dark:text-violet-300 dark:ring-violet-800/70',
...
const PvStatCard = ({
  ...
  return (
    <div className={`group relative flex min-h-[170px] flex-col overflow-hidden rounded-2xl border ${s.card} shadow-sm shadow-slate-200/70 transition duration-200 hover:-translate-y-0.5 hover:shadow-lg hover:shadow-slate-200/80 dark:shadow-black/20`}>
      <div className={`absolute inset-x-0 top-0 h-1 bg-gradient-to-r ${s.bar}`} />
      <div className={`pointer-events-none absolute -right-10 -top-12 h-28 w-28 rounded-full bg-gradient-to-br ${s.halo}`} />
      <div className={`relative flex min-h-[58px] items-center justify-between gap-4 border-b border-slate-100/80 px-5 py-3 ring-1 ring-inset ${s.badge} dark:border-slate-700/70`}>
        <div className="flex min-w-0 items-center gap-2.5">
          {icon && <span className="flex h-5 w-5 shrink-0 items-center justify-center">{icon}</span>}
          <p className={`whitespace-normal break-words text-[11px] font-black uppercase leading-snug tracking-[0.12em] ${s.label}`}>{label}</p>
        </div>
        <img
      ...
      <div className="relative px-5 pt-5">
        <p className={`text-3xl font-black tabular-nums tracking-normal ${s.value}`}>
          {display}
        </p>
      </div>
