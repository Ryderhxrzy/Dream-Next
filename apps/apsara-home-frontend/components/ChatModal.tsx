...
export default function ChatModal({ onClose }: { onClose: () => void }) {
    ...
    const fetchConversations = async () => {
      try {
        ...
        const response = await fetch(
          `${process.env.NEXT_PUBLIC_LARAVEL_API_URL}/api/conversations`,
          {
            method: "GET",
            headers: {
  ...
  const handleSelectConversation = async (conversation: Conversation) => {
    ...
    console.log(
      "Fetching messages from API for conversation ID:",
      conversation.id
    )
    try {
      const sessionResponse = await fetch("/api/auth/session", {
    ...
    if (currentConversationId) {
      try {
        ...
        if (sessionResponse.ok) {
          ...
          if (accessToken) {
            const response = await fetch(
              `${process.env.NEXT_PUBLIC_LARAVEL_API_URL}/api/conversations/${currentConversationId}/messages`,
              {
                method: "POST",
                headers: {
                  Authorization: `Bearer ${accessToken}`,
