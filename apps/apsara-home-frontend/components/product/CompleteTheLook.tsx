...
const resolveBundleCopy = (categoryLabel?: string, categorySlug?: string) => {
  ...
  const isAccessory =
    /(accessor|mobile|gadget|wearable|smart|tech|electronics|device|band|strap|watch)/.test(
      label
    )
  const isRoom =
    /(sofa|chair|table|rack|cabinet|bed|dining|living|room|home|furniture)/.test(
      label
    )
  ...
  if (isAccessory) {
    return {
      ...
      subtitle:
        "Pair this with compatible add-ons and essentials for daily use.",
    }
  }
...
const CompleteTheLook = ({
  ...
  const forceRealPrice =
    !canUseMemberPrice || (Boolean(partnerSlug) && !enableActivateDiscount)
  ...
  const bundleItems: BundleItem[] = useMemo(() => {
    if (Array.isArray(presetItems) && presetItems.length > 0) {
      return presetItems
        .filter((item) =>
          currentProductId ? item.id !== currentProductId : true
        )
        .slice(0, 6)
        .map((item) => ({
          id: Number(item.id ?? 0),
    ...
    return products
      .filter((product) =>
        currentProductId ? product.id !== currentProductId : true
      )
      .slice(0, 6)
      .map((product) => {
        ...
        const effectivePrice =
          !forceRealPrice && memberPrice > 0 && memberPrice < srpPrice
            ? memberPrice
            : srpPrice
  ...
  const selectedSet = useMemo(() => {
    ...
    const next = new Set(
      Array.from(selected).filter((id) => defaultSelection.has(id))
    )
  ...
  const selectedItems = useMemo(
    () => bundleItems.filter((item) => selectedSet.has(item.id)),
    [bundleItems, selectedSet]
  )
  const total = useMemo(
        ...
        <div className="bg-gray-50 dark:bg-gray-800 px-4 py-4 flex items-center justify-between gap-4 flex-wrap">
          <div>
            ...
            <p className="text-lg font-bold text-slate-900 dark:text-gray-100">
              Total:{" "}
              <span className="text-sky-500 dark:text-gray-400">
                {formatMoney(total)}
              </span>
            </p>
          </div>
          ...
          >
            ...
            */}
            <CartIcon />
            Add {selectedSet.size > 0
              ? `${selectedSet.size} Items`
              : "Items"}{" "}
            to Cart
          </PrimaryButton>
        </div>
