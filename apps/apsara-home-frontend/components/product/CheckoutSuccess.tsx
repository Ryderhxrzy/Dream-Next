...
function CheckoutSuccessPage() {
  ...
  const effectiveCheckoutSourceSlug = (partnerSlugFromPath || "")
    .trim()
    .toLowerCase()
  const trackOrderBaseHref = effectiveCheckoutSourceSlug
    ? `/${effectiveCheckoutSourceSlug}/track-order`
    : "/track-order"
  const homeHref = effectiveCheckoutSourceSlug
    ? `/shop/${effectiveCheckoutSourceSlug}`
    : "/"
  ...
  const matchedStorefront = (partnerStorefrontData?.items ?? []).find(
    (item) =>
      getPartnerStorefrontConfig(item)?.slug === effectiveCheckoutSourceSlug
  )
  ...
  const partnerLogoAlt =
    partnerStorefrontConfig?.displayName || "Partner Storefront"
  ...
  useEffect(() => {
    ...
    const verify = async (isInitial = false) => {
      ...
      const canUseLocalPaymentMode =
        typeof window !== "undefined" &&
        LOCAL_PAYMENT_MODE_HOSTS.has(window.location.hostname)
      ...
      try {
        ...
        try {
          data = await verifyCheckoutSession({
            checkoutId,
        ...
        if (fallbackCheckoutId) {
          ...
          setError(
            "We’re still confirming your payment. If this takes too long, you can track your order instead."
          )
          setLoading(false)
          stopPolling()
          return
  ...
  // -- LOADING --------------------------------------------------
  if (loading) {
    const loadingBrandText =
      partnerStorefrontConfig?.displayName ||
              ...
              >
                ...
                >
                  <path
                    ...
                    strokeLinejoin="round"
                    strokeWidth={2}
                    d="M10 17h4V5H2v12h3m9 0h4m0 0a2 2 0 100 4 2 2 0 000-4zm-10 0a2 2 0 100 4 2 2 0 000-4m10 0V12l-4-4h-4"
                  />
                </svg>
                Track This Order
              </Link>
                ...
                {(result?.order_summary?.product_name ||
                  ...
                  <div className="divide-y divide-gray-100 overflow-hidden rounded-2xl border border-gray-200 bg-white dark:divide-slate-700 dark:border-slate-700 dark:bg-gray-950">
                    ...
                    <div className="flex items-center justify-between px-4 py-3">
                      ...
                      <span className="text-xs font-semibold text-gray-700 dark:text-gray-200">
                        {typeof result?.order_summary?.amount === "number"
                          ? `PHP ${result.order_summary.amount.toLocaleString()}`
                          : "N/A"}
                      </span>
                    </div>
                  </div>
