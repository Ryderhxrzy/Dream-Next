...
const BuyNowOptionsModal = ({
  ...
  const [modalSelectedVariantKey, setModalSelectedVariantKey] = useState(
    getStableVariantKey(selectedVariant)
  )
  const loading = false
  ...
  const activeVariant = useMemo(() => {
    ...
    if (effectiveVariantKey) {
      return (
        variantOptions.find(
          (variant) => getStableVariantKey(variant) === effectiveVariantKey
        ) ??
        selectedVariant ??
        variantOptions[0]
      )
  ...
  const memberPrice =
    toPositiveNumber(activeVariant?.priceMember) ??
  ...
  const unitPvRaw =
    toPositiveNumber(activeVariant?.prodpv) ?? Number(product.prodpv ?? 0)
  ...
  const visibleVariantChoices = useMemo(() => {
    ...
    return variantOptions.filter(
      (variant) => !variant.color || variant.color === activeSelectedColor
    )
  }, [activeSelectedColor, variantOptions])
  const variantChoiceLabel = visibleVariantChoices.some((variant) =>
    variant.size?.trim()
  )
    ? "Size"
    : "Variant"
                ...
                <div className="border-b border-gray-200 bg-white p-5 sm:p-6 lg:col-span-2 lg:border-r lg:border-b-0 dark:border-gray-700 dark:bg-gray-800">
                  ...
                  <div className="mt-3 overflow-hidden rounded-2xl border border-gray-200 bg-white dark:border-gray-700 dark:bg-gray-800">
                    ...
                    <div className="flex items-center justify-between border-t border-gray-200 bg-sky-50 px-4 py-3 dark:border-gray-700 dark:bg-sky-900/20">
                      ...
                      <span className="text-xl font-extrabold text-sky-500 dark:text-sky-400">
                  ...
                  <div className="mt-2 flex items-center gap-2 rounded-xl border border-green-200 bg-green-50 px-3 py-2 dark:border-green-800 dark:bg-green-900/20">
                    ...
                    <p className="text-xs font-medium text-green-700 dark:text-green-400">
                      Shipping fee is temporarily set to P0.00 for checkout
                      testing.
                    </p>
                  </div>
                ...
                <div className="p-5 sm:p-6 lg:col-span-3">
                  ...
                  <div className="rounded-2xl border border-gray-200 bg-gray-50 p-4 dark:border-gray-700 dark:bg-gray-800">
                    ...
                    <p className="mt-2 text-[11px] leading-relaxed text-gray-500 dark:text-gray-400">
                      We removed payment selection from this modal so the final
                      checkout page can handle address, vouchers, and payment
                      method in one place.
                    </p>
                  </div>
                          ...
                          <div className="flex items-start justify-between gap-3">
                            <div>
                              ...
                              <p className="mt-1 text-[11px] text-sky-600 dark:text-sky-400">
                                Checkout will use the exact variant you select
                                here.
                              </p>
                            </div>
                            <OutlineButton
                                ...
                                <div className="space-y-4 rounded-2xl border border-sky-100 bg-white p-3 dark:border-sky-900/50 dark:bg-gray-800">
                                    ...
                                    <div>
                                      ...
                                      <div className="flex flex-wrap gap-2">
                                        {colorOptions.map((color) => {
                                          ...
                                          const isActive =
                                            activeSelectedColor === color.name
                                  ...
                                  <div>
                                    ...
                                    <div className="grid max-h-64 grid-cols-1 gap-2 overflow-y-auto pr-1 sm:grid-cols-2">
                                      {visibleVariantChoices.map(
                                        (variant, index) => {
                                          const isActive = isSameVariant(
                                            activeVariant,
                                            variant
                                          )
                                          const variantLabel =
                  ...
                  <div className="mt-3 rounded-2xl border border-gray-200 bg-gray-50 p-4 dark:border-gray-700 dark:bg-gray-800">
                    ...
                    <div className="space-y-2.5">
                      {[
                        {
                          n: "1",
                          text: "Continue to the checkout page",
                          active: true,
                        },
                        {
                          n: "2",
                          text: "Review address, voucher, and final payment option",
                          active: false,
                        },
                        {
                          n: "3",
                          text: "PayMongo processes your payment securely after checkout",
                          active: false,
                        },
                      ].map((step) => (
                        <div key={step.n} className="flex items-start gap-3">
                          ...
                          <p
                            className={`text-xs leading-relaxed ${step.active ? "font-semibold text-gray-800 dark:text-white" : "text-gray-400 dark:text-gray-500"}`}
                          >
                            {step.text}
                          </p>
                        </div>
                        ...
                        >
                          {loading ? (
                            ...
                            <>
                              ...
                              <span>
                                Continue to Checkout P{total.toLocaleString()}
                              </span>
                            </>
                          )}
                        </PrimaryButton>
