...
const buildVariantTitleParts = (
  variant?: NonNullable<CategoryProduct["variants"]>[number]
) => {
...
const ProductInfo = ({
  ...
  const checkoutTarget = partnerSlug
    ? `/${partnerSlug}/checkout/customer`
    : "/checkout/customer"
  ...
  const optionLabels = useMemo(
    () => extractVariantOptionLabels(product.specifications),
  ...
  useEffect(() => {
    ...
    const sendHeartbeat = async () => {
      try {
        const response = await heartbeatProductViewer({
          productId,
  ...
  const handleWishlistToggle = async () => {
    if (!useAccountWishlist) {
      if (allowGuestWishlist && typeof window !== "undefined" && product.id) {
        ...
        const currentlyInWishlist = currentItems.some(
          ...
          : [
              ...currentItems,
              {
                ...
                priceMember: product.priceMember
                  ? Number(product.priceMember)
                  : undefined,
                priceDp: product.priceDp ? Number(product.priceDp) : undefined,
                priceSrp: product.priceSrp
                  ? Number(product.priceSrp)
                  : undefined,
        ...
        const deduped = Array.from(
          nextItems
            .reduce((map, item) => {
              ...
              return map
            }, new Map<number, GuestWishlistItem>())
            .values()
        )
        setGuestWishlistItems(deduped)
        window.localStorage.setItem(
          GUEST_WISHLIST_ITEMS_STORAGE_KEY,
          JSON.stringify(deduped)
        )
        window.localStorage.setItem(
          GUEST_WISHLIST_STORAGE_KEY,
          JSON.stringify(deduped.map((item) => item.productId))
        )
  ...
  const groupedVariantChoices = useMemo(() => {
    ...
    const grouped = variantOptions.reduce((map, variant, index) => {
      ...
      map.set(groupKey, {
        key: groupKey,
        label:
          sizeLabel || (variant.name ?? "").trim() || `Variant ${index + 1}`,
        meta: metaParts.join(" ? "),
  ...
  const logicalSizeChoices = useMemo(() => {
    return groupedVariantChoices.filter((choice) =>
      Boolean(
        choice.groupVariants?.some((variant) => (variant.size ?? "").trim())
      )
    )
  ...
  const effectiveSelectedPrimaryName =
    selectedVariantName ||
    (hasPrimaryOptionSelector ? variantNameOptions[0]?.name || "" : "")
  const displayedStyleOptions = useMemo(() => {
    let filteredVariants = variantOptions

    if (hasColorSelector && effectiveSelectedColor) {
      filteredVariants = filteredVariants.filter(
        (variant) => !variant.color || variant.color === effectiveSelectedColor
      )
    }
    ...
    effectiveSelectedPrimaryName,
    hasColorSelector,
    hasPrimaryOptionSelector,
    variantOptions,
  ])
  const hasStyleSelector = displayedStyleOptions.length > 0
  const effectiveSelectedStyle = displayedStyleOptions.some(
    (option) => option.name === selectedStyle
  )
    ? selectedStyle
  ...
  const secondaryOptionLabel =
    optionLabels.secondaryLabel?.trim() ||
    (hasSecondarySizeValues ? "Size" : "")
  const displayedSizeChoices = useMemo(() => {
    let filteredChoices = logicalSizeChoices
    if (hasColorSelector && effectiveSelectedColor) {
      filteredChoices = logicalSizeChoices.filter((choice) =>
        (choice.groupVariants ?? []).some(
          (variant) =>
            !variant.color || variant.color === effectiveSelectedColor
        )
      )
    }
    if (hasPrimaryOptionSelector && effectiveSelectedPrimaryName) {
    ...
    if (hasStyleSelector && effectiveSelectedStyle) {
      filteredChoices = filteredChoices.filter((choice) =>
        (choice.groupVariants ?? []).some(
          (variant) => (variant.style ?? "").trim() === effectiveSelectedStyle
        )
      )
    }
  ...
  const shouldShowSecondaryOption =
    secondaryOptionLabel.length > 0 && displayedSizeChoices.length > 0
  const effectiveSelectedSizeKey =
    selectedSizeKey || displayedSizeChoices[0]?.key || ""
  const effectiveSelectedSize =
    selectedSize || displayedSizeChoices[0]?.label || ""

  const selectedVariant = useMemo(() => {
    if (variantOptions.length === 0) return undefined
    const selectedSizeChoice = displayedSizeChoices.find(
      (choice) => choice.key === effectiveSelectedSizeKey
    )
    if (selectedSizeChoice) {
      const groupedVariants =
        selectedSizeChoice.groupVariants ??
        (selectedSizeChoice.variant ? [selectedSizeChoice.variant] : [])
      return (
    ...
    if (
      ...
      return (
        ...
        variantOptions.find(
          (variant) => variant.color === effectiveSelectedColor
        ) ??
        variantOptions.find(
          (variant) => (variant.style ?? "").trim() === effectiveSelectedStyle
        ) ??
    ...
    return (
      variantOptions.find(
        (variant) =>
          variant.color === effectiveSelectedColor &&
          ((variant.size ?? "").trim() === effectiveSelectedSize ||
            (variant.name ?? "").trim() === effectiveSelectedSize)
      ) ??
      variantOptions.find(
        (variant) => variant.color === effectiveSelectedColor
      ) ??
      variantOptions.find(
        (variant) =>
          ...
          (variant.name ?? "").trim() === selectedSize
      ) ??
      variantOptions[0]
    )
  }, [
    displayedSizeChoices,
    variantOptions,
    effectiveSelectedColor,
    effectiveSelectedPrimaryName,
    effectiveSelectedSize,
    effectiveSelectedSizeKey,
    effectiveSelectedStyle,
    hasColorSelector,
    hasPrimaryOptionSelector,
    hasStyleSelector,
  ...
  const handleModalVariantSelect = useCallback(
    (variant: VariantOption) => {
      ...
      const matchingChoice = groupedVariantChoices.find((choice) =>
        (choice.groupVariants ?? (choice.variant ? [choice.variant] : [])).some(
          (choiceVariant) => {
            if (variant.sku && choiceVariant.sku)
              return choiceVariant.sku === variant.sku
            if (
              typeof variant.id === "number" &&
              typeof choiceVariant.id === "number"
            )
              return choiceVariant.id === variant.id
  ...
  const baseSrp =
    toPositiveNumber(product.originalPrice) ??
    toPositiveNumber(product.price) ??
    0
  const variantSrp = toPositiveNumber(selectedVariant?.priceSrp) ?? baseSrp
  const variantMember =
    toPositiveNumber(selectedVariant?.priceMember) ??
    toPositiveNumber(product.priceMember) ??
    0
  ...
  const shouldDisplayMemberPrice =
    hasMemberPrice && !forceRealPrice && canUseMemberPrice
  ...
  const displayOriginalPrice = shouldDisplayMemberPrice
    ...
    : !forceRealPrice &&
        ...
        product.originalPrice > variantSrp
      ? product.originalPrice
      : undefined
  ...
  const displayStock =
    typeof selectedVariant?.qty === "number"
      ? selectedVariant.qty
      : typeof totalVariantStock === "number"
        ? totalVariantStock
        : product.stock
  ...
  const displaySku =
    selectedVariant?.sku && selectedVariant.sku.trim().length > 0
      ? selectedVariant.sku
      : product.sku && product.sku.trim().length > 0
        ? product.sku
        : ""
  const displayWidth =
    toPositiveNumber(selectedVariant?.width) ??
    toPositiveNumber(product.pswidth)
  const displayDimension =
    toPositiveNumber(selectedVariant?.dimension) ??
    toPositiveNumber(product.pslenght)
  const displayHeight =
    toPositiveNumber(selectedVariant?.height) ??
    toPositiveNumber(product.psheight)
  const hasDisplayDimensions = Boolean(
    displayWidth || displayDimension || displayHeight
  )
  const selectedVariantImage = selectedVariant?.images?.find(
  ...
  const handleAddToCart = () => {
    ...
    const variantLabel = [
      ...
      .filter(Boolean)
      .join(" ? ")
    const cartItemIdBase = product.id
      ? String(product.id)
      : product.name.toLocaleLowerCase().replace(/\s+/g, "-")
    const cartItemId = selectedVariant?.sku
      ? `${cartItemIdBase}::${selectedVariant.sku}`
      : cartItemIdBase
  ...
  const shareUrl = useMemo(() => {
    const productSlug = product.id
      ? `${toSlug(product.name)}-i${product.id}`
      : toSlug(product.name)
    ...
    >
      ...
      <ShareModal
        ...
        product={{
          ...
          priceSrp: variantSrp,
          priceDp:
            toPositiveNumber(selectedVariant?.priceDp) ||
            toPositiveNumber(product.priceDp),
