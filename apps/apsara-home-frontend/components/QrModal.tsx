...
const QrModal = ({
  ...
  const [status, setStatus] = useState<"pending" | "approved" | "expired">(
    "pending"
  )
  ...
  const generateQrSession = useCallback(async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          errorData?.message ||
            `Failed to generate QR session (${response.status})`
        )
      }
      ...
      setTimeout(() => {
        if (qrCanvasRef.current && data.qr_data) {
          ...
          console.warn("Canvas ref or QR data not available:", {
            ref: qrCanvasRef.current,
            data: data.qr_data,
          })
        }
      }, 0)
    ...
    const checkStatus = async () => {
      try {
        const response = await fetch(
          `${apiBaseUrl}/api/auth/qr/${sessionId}/status`,
          {
            method: "GET",
            headers: {
              "Content-Type": "application/json",
  ...
  const handleQrApproved = useCallback(
              ...
              <div className="space-y-3">
                ...
                <ol className="space-y-2 text-xs text-gray-600 dark:text-white/70">
                  <li className="flex gap-2">
                    ...
                    <span>
                      Open the{" "}
                      <span className="font-medium">{accountLabel}</span> mobile
                      app
                    </span>
                  </li>
                  <li className="flex gap-2">
                    ...
                    <span>
                      Go to <span className="font-medium">Profile</span> →{" "}
                  ...
                  <li className="flex gap-2">
                    ...
                    <span>
                      Select <span className="font-medium">Security</span>{" "}
                      section
                    </span>
                  </li>
                  <li className="flex gap-2">
                    ...
                    <span>
                      Tap <span className="font-medium">"Scan QR Code"</span>{" "}
                      button
                    </span>
                  </li>
                  <li className="flex gap-2">
