...
export default function ItemCard({
  ...
  const [submitInquiry, { isLoading: isSubmittingInquiry }] =
    useSubmitServiceInquiryMutation()
  const [addToCartApi, { isLoading: isAddingToCart }] = useAddToCartMutation()
  const [loadProductDetails, { isFetching: isFetchingProductDetails }] =
  ...
  const [addWishlist, { isLoading: isAddingToWishlist }] =
    useAddWishlistMutation()
  const [removeWishlist, { isLoading: isRemovingFromWishlist }] =
    useRemoveWishlistMutation()
  const [guestWishlistItems, setGuestWishlistItems] = useState<
    GuestWishlistItem[]
  >(() => readGuestWishlistItems())
  const [variantPickerOpen, setVariantPickerOpen] = useState(false)
  const [openCartAfterVariantPicker, setOpenCartAfterVariantPicker] =
    useState(false)
  ...
  const [loadedVariants, setLoadedVariants] = useState<ProductVariant[] | null>(
    null
  )
  const activeVariants = useMemo(
    () =>
      (loadedVariants ?? product.variants ?? []).filter(
    ...
    () =>
      selectedColor
        ? activeVariants.filter(
            (variant) => !variant.color || variant.color === selectedColor
          )
        : activeVariants,
    ...
    if (!useAccountCart) {
      if (allowGuestAddToCart) {
        ...
        addToLocalCart({
          ...
          originalPrice:
            variantPrice.strike > variantPrice.display
              ? variantPrice.strike
              : null,
  ...
  const handleAddToCart = async (e: React.MouseEvent) => {
    ...
    if (loadedVariants === null) {
      try {
        ...
        const fetchedVariants = (productDetails.variants ?? []).filter(
          (variant) => (variant.status ?? 1) === 1
        )
        ...
        toast.error(
          "Unable to load product options. Please open the product page to choose a variant."
        )
        return
      }
    }
  ...
  const handleWishlist = async (e: React.MouseEvent) => {
    ...
    try {
      ...
      const errorMessage =
        error?.data?.message || error?.message || "Failed to update wishlist"
      toast.error(errorMessage)
    }
  }
  ...
  const strikePrice =
    product.originalPrice && product.originalPrice > srpPrice
      ? product.originalPrice
      : 0
  const displayPv = Number(product.prodpv ?? 0)
  const getVariantDisplayPrice = (variant?: ProductVariant) => {
    const variantSrp =
      (variant?.priceSrp ? Number(variant.priceSrp) : undefined) ?? srpPrice
    const variantMember =
      (variant?.priceMember ? Number(variant.priceMember) : undefined) ??
      memberPrice
    const variantHasMemberPrice =
      variantMember > 0 && variantMember < variantSrp
    const variantStrikeBase = variantSrp
    const variantOriginal = variant?.priceSrp
      ? variant?.priceSrp
      : (variant?.priceDp ?? variant?.priceMember)
    const variantStrike =
      variantOriginal && Number(variantOriginal) > variantStrikeBase
        ? Number(variantOriginal)
        : 0
    const variantPv = Number(variant?.prodpv ?? displayPv)
    return {
      display: variantSrp,
      strike: variantStrike,
      pv: variantPv,
      variantHasMemberPrice,
    }
  }
              ...
              {isAddingToCart || isFetchingProductDetails ? (
                <>
                  ...
                  <span className="hidden sm:inline">
                    {isFetchingProductDetails ? "Loading..." : "Adding..."}
                  </span>
                </>
              ) : (
                <>
                  ...
                  <div className="flex items-center gap-4">
                    <div className="flex-1">
                      ...
                      <p className="mt-1.5 text-xs leading-relaxed text-gray-500 dark:text-gray-400">
                        {
                          "We're here to make things simple, safe and stress-free for you."
                        }
                      </p>
                    </div>
                    {product.image && (
                      ...
                      <div className="flex flex-wrap gap-2">
                        {serviceTypes.map((type, i) => {
                          ...
                          return (
                            <span
                              key={type}
                              className={`flex items-center gap-1.5 rounded-full border px-3 py-1 text-xs font-medium ${c.wrap}`}
                            >
                              <svg
                                xmlns="http://www.w3.org/2000/svg"
                                width="11"
              ...
              {inquireSubmitted ? (
                <div className="flex flex-col items-center gap-3 px-5 py-12 text-center">
                  ...
                  <p className="max-w-xs text-sm leading-relaxed text-gray-500 dark:text-gray-400">
                    ...
                    Please ensure all submitted information is accurate and up
                    to date.
                  </p>
                  <button
                    type="button"
                ...
                <>
                  ...
                  <div className="flex gap-3 border-t border-gray-100 px-5 py-4 dark:border-gray-800">
                    ...
                    <button
                      ...
                      disabled={
                        isSubmittingInquiry ||
                        !inquireForm.fullname.trim() ||
                        !inquireForm.email.trim() ||
                        !inquireForm.contact.trim() ||
                        !inquireForm.address.trim() ||
                        !inquireForm.intent.trim()
                      }
                      onClick={async () => {
                        try {
                          await submitInquiry({
          ...
          >
            ...
            >
              ...
              <div className="max-h-[70vh] overflow-y-auto p-4">
                  ...
                  <div className="mb-4">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {colorOptions.map((color) => {
                        const firstIndex = activeVariants.findIndex(
                        ...
                        return (
                          <button
                            ...
                            title={color.name}
                            onClick={() =>
                              setSelectedVariantIndex(Math.max(0, firstIndex))
                            }
                ...
                <div>
                  <p className="mb-2 text-sm font-semibold text-gray-700 dark:text-gray-200">
                    {visibleVariants.some((variant) => variant.size?.trim())
                      ? "Size"
                      : "Variant"}
                  </p>
                  <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
                    {visibleVariants.map((variant) => {
                      const realIndex = activeVariants.findIndex(
                        (entry) => entry === variant
                      )
                      const isActive =
                        activeVariants[selectedVariantIndex] === variant
                      const variantPrice = getVariantDisplayPrice(variant)
                      const label =
                        variant.size?.trim() ||
                        variant.name?.trim() ||
                      ...
                      return (
                        <button
                          ...
                          type="button"
                          onClick={() =>
                            setSelectedVariantIndex(Math.max(0, realIndex))
                          }
                          ...
                          <div className="flex gap-2">
                            ...
                            <div className="min-w-0 flex-1">
                              ...
                              <p className="mt-1 text-xs font-bold text-sky-500">
                                {"\u20b1"}
                                {variantPrice.display.toLocaleString()}
                              </p>
                            </div>
                          </div>
                        </button>
