...
const fetchPsgcList = async (
  ...
  const request = requestJson(url, timeoutMs)
    ...
    .then(
      (payload: Array<{ code?: string; name?: string; regionName?: string }>) =>
        (payload ?? [])
          .map((item) => ({
...
const loadWithFallback = async (
  ...
  pathBySource: {
    backend: { path: string; params?: Record<string, string> }
    psgc: { path: string }
  }
) => {
  ...
  if (preferredSource === "backend") {
    return fetchAddressList(
      pathBySource.backend.path,
      pathBySource.backend.params
    )
  }

  const backendResult = await loadBySource(
    "backend",
    pathBySource.backend.path,
    pathBySource.backend.params
  )
...
export function usePhAddress(
  ...
  useEffect(() => {
    ...
    const load = async () => {
      if (source === "psgc") {
        const provinceResult = await fetchPsgcList(
          `/regions/${regionCode}/provinces/`
        )
        if (!provinceResult.items.length) {
          const cityResult = await fetchPsgcList(
      ...
      if (
        source === "backend" &&
        options?.legacyNoProvinceRegions &&
        LEGACY_NO_PROVINCE_REGION_CODES.has(regionCode)
      ) {
        const cityResult = await fetchAddressList("cities", {
