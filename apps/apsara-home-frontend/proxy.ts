...
const PARTNER_ALLOWED_PREFIXES = ["/partner/webpages"]
...
const PARTNER_LOOKUP_CACHE_TTL_MS = Math.max(
  5_000,
  Number(process.env.FRONTEND_PROXY_LOOKUP_CACHE_MS ?? 15_000)
)
let partnerStorefrontCache: {
  expiresAt: number
  slugToId: Map<string, number>
} | null = null
const partnerStorefrontIdsCache = new Map<
  string,
  { expiresAt: number; ids: number[] }
>()
...
const resolveStorefrontSlugToIdMap = async (): Promise<Map<string, number>> => {
  ...
  try {
    ...
    for (const item of data?.items ?? []) {
      ...
      const fieldSlug = String(fields?.slug ?? "")
        .trim()
        .toLowerCase()
      const slug = fieldSlug || keySlug
      if (slug) map.set(slug, id)
    }
    partnerStorefrontCache = {
      expiresAt: now + PARTNER_LOOKUP_CACHE_TTL_MS,
      slugToId: map,
    }
    return map
  } catch {
...
const fetchLivePartnerStorefrontIds = async (
  accessToken: string
): Promise<number[] | null> => {
  const apiUrl = getLaravelApiUrl()
  ...
  try {
    ...
    const ids = Array.isArray(data?.storefront_ids)
      ...
      : []
    partnerStorefrontIdsCache.set(accessToken, {
      expiresAt: now + PARTNER_LOOKUP_CACHE_TTL_MS,
      ids,
    })
    return ids
...
const getRequiredWebContentPermission = (pathname: string): string | null => {
  for (const [permission, prefixes] of Object.entries(
    WEB_CONTENT_SECTION_PREFIXES
  )) {
...
const canAccessWebContentPath = (
  permissions: string[],
  pathname: string
): boolean => {
  const sectionPermissions = permissions.filter((permission) =>
  ...
  return (
    ...
    sectionPermissions.includes(requiredPermission)
  )
}
...
export async function proxy(req: NextRequest) {
  ...
  const passwordChangeRequired = Boolean(
    (token as { passwordChangeRequired?: boolean } | null)
  ...
  const isPartnerLoginPage =
    pathname === "/partner/login" || pathname.startsWith("/partner/login/")
  ...
  const isAuthRequiredRoute = AUTH_REQUIRED_PREFIXES.some((prefix) =>
  ...
  if (isAdminLoginPage) {
    const role = String(
      (adminToken as { role?: string } | null)?.role ?? ""
    ).toLowerCase()
    const userLevelId = Number(
      (adminToken as { userLevelId?: number } | null)?.userLevelId ?? 0
    )
  ...
  if (isPartnerLoginPage) {
    ...
    const role = String(
      (partnerToken as { role?: string } | null)?.role ?? ""
    ).toLowerCase()
    const userLevelId = Number(
      (partnerToken as { userLevelId?: number } | null)?.userLevelId ?? 0
    )
    const isWebContent = role === "web_content" || userLevelId === 4
    const storefrontIds = Array.isArray(
    ...
    if (partnerToken && isWebContent) {
      return NextResponse.redirect(
        new URL("/partner/webpages/partner-storefronts", req.url)
      )
    }
  ...
  if (isSupplierPublicPage) {
    const role = String(
      (supplierToken as { role?: string } | null)?.role ?? ""
    ).toLowerCase()
  ...
  if (isAdminRoute) {
    ...
    const isWebContent = role === "web_content" || userLevelId === 4
    const rawAdminPermissions = (
      (adminToken as { adminPermissions?: string[] } | null)
        ?.adminPermissions ?? []
    ).filter(
    ...
    const hasCustomAdminPermissions =
      (role === "admin" || userLevelId === 2) && adminPermissions.length > 0
    const adminAllowedPrefixes = hasCustomAdminPermissions
      ? [
          "/admin/dashboard",
          ...adminPermissions.flatMap(
            (permission) => ADMIN_PERMISSION_PREFIXES[permission] ?? []
          ),
        ]
    ...
    if (isAccounting) {
      const allowed = ACCOUNTING_ALLOWED_PREFIXES.some((prefix) =>
        pathname.startsWith(prefix)
      )
      if (!allowed) {
    ...
    if (isFinanceOfficer) {
      const allowed = FINANCE_ALLOWED_PREFIXES.some((prefix) =>
        pathname.startsWith(prefix)
      )
      if (!allowed) {
    ...
    if (isSupplierAdmin) {
      const allowed =
        pathname === "/admin" ||
        ADMIN_SUPPLIER_ALLOWED_PREFIXES.some((prefix) =>
          pathname.startsWith(prefix)
        )
      if (!allowed) {
  ...
  if (isPartnerRoute) {
    ...
    const isWebContent = role === "web_content" || userLevelId === 4
    const tokenStorefrontIds = Array.isArray(
      (partnerToken as { storefrontIds?: number[] } | null)?.storefrontIds
    )
      ? ((partnerToken as { storefrontIds?: number[] } | null)?.storefrontIds ??
        [])
    ...
    const partnerAccessToken = String(
      (partnerToken as { accessToken?: string } | null)?.accessToken ?? ""
    )
    const liveStorefrontIds = partnerAccessToken
      ? await fetchLivePartnerStorefrontIds(partnerAccessToken)
      : null
    const storefrontIds =
      liveStorefrontIds ?? (partnerAccessToken ? [] : tokenStorefrontIds)
    const hasStorefrontAccess = storefrontIds.length > 0
    const rawPartnerPermissions = (
      (partnerToken as { adminPermissions?: string[] } | null)
        ?.adminPermissions ?? []
    ).filter(
      (permission): permission is string => typeof permission === "string"
    )
    ...
    if (!hasStorefrontAccess) {
      return NextResponse.redirect(
        new URL("/partner/login?reason=subscription_expired", req.url)
      )
    }
  ...
  if (isShopRoute && partnerToken) {
    ...
    const isWebContent = role === "web_content" || userLevelId === 4
    const disabledStorefrontIds = Array.isArray(
      (partnerToken as { disabledStorefrontIds?: number[] } | null)
        ?.disabledStorefrontIds
    )
      ? ((partnerToken as { disabledStorefrontIds?: number[] } | null)
          ?.disabledStorefrontIds ?? [])
      : []
    ...
    if (isWebContent) {
      ...
      const partnerSlug = String(segments[1] ?? "")
        .trim()
        .toLowerCase()
      if (partnerSlug) {
        ...
        if (
          targetStorefrontId > 0 &&
          disabledStorefrontIds.includes(targetStorefrontId)
        ) {
