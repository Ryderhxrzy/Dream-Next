...
export default function FacebookAuthCallback() {
  ...
  useEffect(() => {
      ...
      if (window.opener) {
        try {
          window.opener.postMessage(
            { ...payload, type: "FACEBOOK_AUTH_CALLBACK" },
    ...
    fetch(
      `https://graph.facebook.com/v18.0/me?fields=id,name,email,first_name,last_name&access_token=${accessToken}`
    )
      .then((res) => res.json())
      .then((data) => {
        if (data.error) {
        ...
        if (!email) {
          throw new Error(
            "Facebook did not return an email. Make sure your Facebook account has a verified email."
          )
        }
