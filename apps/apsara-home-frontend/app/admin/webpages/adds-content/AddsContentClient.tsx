...
export default function AddsContentClient() {
  ...
  const [deleteConfirm, setDeleteConfirm] = useState<{ id: number } | null>(
    null
  )
  const [createAddsContent, { isLoading }] = useCreateAddsContentMutation()
  const [updateAddsContent, { isLoading: isUpdating }] =
    useUpdateAddsContentMutation()
  const [updateStatus, { isLoading: isStatusLoading }] =
    useUpdateAddsContentStatusMutation()
  const [deleteAddsContent, { isLoading: isDeleteLoading }] =
    useDeleteAddsContentMutation()
  const { data: addsContentData, isLoading: isAddsLoading } =
    useGetAddsContentQuery()
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-2 max-w-3xl text-sm leading-relaxed text-slate-500 dark:text-slate-400">
          This page will manage advertising placements, promos, and sponsored
          content blocks once connected to the web content service.
        </p>
      </div>
        ...
        <div className="relative flex flex-wrap items-center justify-between gap-4">
          <div>
            ...
            <p className="mt-2 max-w-2xl text-sm leading-relaxed text-slate-500 dark:text-slate-400">
              Create advertising placements, promo tiles, and sponsored content
              blocks with a single action. You can later attach images, links,
            ...
            <div className="flex items-start justify-between gap-4">
              <div>
                ...
                <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">
                  Fill in image, video, and schedule details.
                </p>
              </div>
              <button
                type="button"
            ...
            <div className="mt-6 grid gap-5 sm:grid-cols-2">
                    ...
                    <input
                      ...
                      onChange={async (event) => {
                        ...
                        if (sizeMb > MAX_VIDEO_MB) {
                          showErrorToast(
                            `The maximum video size is ${MAX_VIDEO_MB}MB.`
                          )
                          event.target.value = ""
                          setVideoFile(null)
                          return
              ...
              <div className="relative sm:col-span-2">
                ...
                <p className="mt-1 text-[11px] text-slate-500">
                  Pick the date this content should be recorded.
                </p>
              ...
              <button
                ...
                onClick={async () => {
                  ...
                  try {
                    ...
                    showErrorToast(
                      getApiErrorMessage(error, "Failed to save content.")
                    )
                  }
                }}
            ...
            <div className="flex items-start justify-between gap-4">
              <div>
                ...
                <h3 className="mt-2 text-lg font-bold text-slate-900">
                  {statusConfirm.label} Content
                </h3>
                <p className="mt-2 text-sm text-slate-500">
                  Are you sure you want to set this content to{" "}
                  <span className="font-semibold">{statusConfirm.label}</span>?
                </p>
              </div>
              <button
                type="button"
            ...
            <div className="mt-6 flex items-center justify-end gap-3">
              ...
              <button
                ...
                onClick={async () => {
                  ...
                  try {
                    const response = await deleteAddsContent({
                      id: deleteConfirm.id,
                    }).unwrap()
                    showSuccessToast(response.message || "Content deleted.")
                  } catch (error: any) {
                    showErrorToast(
                      error?.data?.message || "Failed to delete content."
                    )
                  } finally {
      ...
      <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900">
        <div className="flex flex-wrap items-center justify-between gap-3">
          ...
          <span className="text-xs text-slate-500 dark:text-slate-400">
            {isAddsLoading
              ? "Loading..."
              : `${addsContentData?.items?.length ?? 0} items`}
          </span>
        </div>
