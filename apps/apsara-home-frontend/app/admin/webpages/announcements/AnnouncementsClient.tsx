...
export default function AnnouncementsClient() {
  ...
  const [emojiMenuPos, setEmojiMenuPos] = useState<{ top: number; left: number } | null>(null)
  ...
  const isAllVisibleSelected = useMemo(() => {
    ...
    return filteredMemberEmails.every((item) => selectedEmails.includes(item.value))
  ...
  useEffect(() => {
    ...
    emailDraftSnapshotRef.current = emailDraftHtml || emailDraftSnapshotRef.current || initialContentHtml
    setEmailDraftHtml('')
  ...
  useEffect(() => {
    const loadRecipients = async () => {
      ...
      try {
        ...
        const recipientPath = deliveryChannel === 'sms' ? '/api/admin/sms-blast/recipients' : '/api/admin/email-blast/recipients'
        const response = await fetch(`${baseUrl}${recipientPath}?${params.toString()}`, {
          headers: {
        ...
        const recipients: RecipientOption[] = Array.isArray(data?.recipients)
          ? (data.recipients as unknown[])
              .map((item: unknown) => {
                ...
                return value ? { value: deliveryChannel === 'email' ? value.toLowerCase() : value, label: value } : null
              })
              .filter((item: RecipientOption | null): item is RecipientOption => Boolean(item))
          : []
        const normalizedRecipients = Array.from(new Map(recipients.map((item: RecipientOption) => [item.value, item])).values())
        setMemberEmails(normalizedRecipients)
      } catch {
        toast.error(deliveryChannel === 'sms' ? 'Unable to load mobile numbers.' : 'Unable to load member emails.')
      } finally {
  ...
  useEffect(() => {
    if (sendTimingMode === 'scheduled' && !scheduledFor) {
      setScheduledFor(formatLocalDatetimeInput(new Date(Date.now() + 60 * 60 * 1000)))
    }
  ...
  const toggleSelectVisible = () => {
    ...
    setSelectedEmails((prev) => {
      if (isAllVisibleSelected) {
        return prev.filter((email) => !filteredMemberEmails.some((item) => item.value === email))
      }
  ...
  const sendAnnouncement = async () => {
    ...
    try {
      ...
      const sendPath = isEmailMode ? '/api/admin/email-blast/send' : '/api/admin/sms-blast/send'
      ...
      if (data?.scheduled) {
        const scheduledLabel = data?.scheduled_at ? new Date(String(data.scheduled_at)).toLocaleString() : scheduledDate?.toLocaleString() ?? 'later'
  ...
  const renderEmailComposer = () => (
    <div data-announcement-email-editor className="overflow-hidden rounded-xl border border-[#d6dff7]">
          ...
          <button
            ...
            onClick={() => {
              if (!showEmojiMenu && emojiButtonRef.current) {
                ...
                const left = spaceOnRight >= pickerWidth ? rect.left : Math.max(8, rect.right - pickerWidth)
                setEmojiMenuPos({
                  top: rect.bottom + 8,
                  left,
                })
  ...
  const renderSmsComposer = () => (
    <div className="space-y-3 rounded-xl border border-[#d6dff7] bg-white p-4">
      <div className="rounded-lg border border-[#d6dff7] bg-[#f7f9ff] px-3 py-2 text-sm text-[#4e67ab]">
        SMS supports plain text only. Images and rich formatting are disabled for this channel.
      </div>
      <textarea
        value={smsMessage}
        onChange={(event) => {
      ...
      <div className="flex items-center justify-between text-xs text-[#4f75dc]">
        ...
        <span>Characters: {smsCharacterCount} | Words: {smsWordCount}</span>
      </div>
    </div>
  )

  return (
    <div data-announcement-sms-mode={deliveryChannel === 'sms' ? 'true' : 'false'} className="mt-5 grid grid-cols-1 gap-4 xl:grid-cols-12">
      <style>{`
      ...
      <div className="space-y-5 rounded-2xl border border-[#dce5ff] bg-white p-4 shadow-sm xl:col-span-8 md:p-5">
        ...
        >
          ...
          <p className="text-sm text-[#4e67ab]">Choose between email with rich content or SMS with text only.</p>
          <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
            <label
              ...
              className={`relative flex cursor-pointer items-center gap-3 rounded-xl border p-4 transition ${
                deliveryChannel === 'email' ? 'border-[#3f69db] bg-[#f3f6ff]' : 'border-[#d6dff7] bg-white'
              }`}
            >
              <input
              ...
              <span className="flex-1">
                ...
                <span className="block text-sm text-[#4e67ab]">Rich text, images, links and formatting</span>
              </span>
              <span className={`h-5 w-5 rounded-full border-2 ${deliveryChannel === 'email' ? 'border-[#2457e7]' : 'border-[#a7b8e6]'}`}>
                <span className={`m-[3px] block h-2.5 w-2.5 rounded-full ${deliveryChannel === 'email' ? 'bg-[#2457e7]' : 'bg-transparent'}`} />
              </span>
            </label>
            ...
            <label
              ...
              className={`relative flex cursor-pointer items-center gap-3 rounded-xl border p-4 transition ${
                deliveryChannel === 'sms' ? 'border-[#3f69db] bg-[#f3f6ff]' : 'border-[#d6dff7] bg-white'
              }`}
            >
              <input
              ...
              <span className="flex-1">
                ...
                <span className="block text-sm text-[#4e67ab]">Plain text only, no images or rich formatting</span>
              </span>
              <span className={`h-5 w-5 rounded-full border-2 ${deliveryChannel === 'sms' ? 'border-[#2457e7]' : 'border-[#a7b8e6]'}`}>
                <span className={`m-[3px] block h-2.5 w-2.5 rounded-full ${deliveryChannel === 'sms' ? 'bg-[#2457e7]' : 'bg-transparent'}`} />
              </span>
            </label>
          </div>
        </section>
        ...
        <section className="space-y-3">
          <h2 className="text-[20px] font-bold text-[#17398d]">3. {deliveryChannel === 'email' ? 'Email Content' : 'SMS Message'}</h2>
          ...
          {deliveryChannel === 'email' ? renderEmailComposer() : renderSmsComposer()}
          ...
          <div className="relative flex flex-wrap items-center justify-between gap-3 rounded-xl border border-[#d6dff7] bg-[#eef3ff] px-3 py-2">
            <div className="flex items-center gap-2 text-sm text-[#2f58c8]">
              ...
              <span>{deliveryChannel === 'email' ? 'Use variables to personalize your announcement.' : 'SMS is plain text only; variables still work.'}</span>
            </div>
          ...
          {recipientMode === 'specific' ? (
            <div className="space-y-2">
              ...
              <div className="max-h-64 overflow-auto rounded-lg border border-[#d6dff7] p-1.5">
                {isLoadingRecipients ? (
                  <p className="px-2 py-1 text-[11px] text-[#7f95d4]">Loading {recipientUnitLabelPlural}...</p>
                ) : filteredMemberEmails.length === 0 ? (
                  <p className="px-2 py-1 text-[11px] text-[#7f95d4]">No {recipientUnitLabelPlural} found.</p>
                ) : (
                  filteredMemberEmails.map((item) => (
                    <label key={item.value} className="flex cursor-pointer items-center gap-2 rounded px-2 py-1 text-[13px] text-[#2f4177] hover:bg-[#eef3ff]">
                      <input
                        type="checkbox"
              ...
              {selectedEmails.length > 0 ? (
                <div className="max-h-24 overflow-auto rounded-lg border border-[#d6dff7] bg-[#f7f9ff] p-2">
                  <p className="mb-1 text-[11px] font-semibold text-[#4f75dc]">Selected {recipientUnitLabelPlural}</p>
          ...
          <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
            <label
              className={`flex cursor-pointer items-center gap-2 rounded-lg border px-3 py-2 text-sm transition ${
                sendTimingMode === 'now' ? 'border-[#3f69db] bg-[#f3f6ff] text-[#17398d]' : 'border-[#d6dff7] bg-white text-[#2f4177]'
              }`}
            >
              <input
                type="radio"
            ...
            <label
              className={`flex cursor-pointer items-center gap-2 rounded-lg border px-3 py-2 text-sm transition ${
                sendTimingMode === 'scheduled' ? 'border-[#3f69db] bg-[#f3f6ff] text-[#17398d]' : 'border-[#d6dff7] bg-white text-[#2f4177]'
              }`}
            >
              <input
                type="radio"
          ...
          {sendTimingMode === 'scheduled' ? (
            <div className="space-y-2 rounded-lg border border-[#d6dff7] bg-[#f7f9ff] p-3">
              ...
              <p className="text-[11px] leading-relaxed text-[#7f95d4]">
                Scheduled announcements are queued and sent automatically when the selected time is reached.
              </p>
            </div>
          ) : (
            <p className="text-[11px] leading-relaxed text-[#7f95d4]">
              Send immediately, or switch to scheduling and queue this announcement for later.
            </p>
          )}
        </div>
            ...
            >
              <div className="flex items-start justify-between gap-4 border-b border-white/70 bg-white/70 px-6 py-5 backdrop-blur">
                <div className="space-y-1">
                  ...
                  <h3 className="text-2xl font-extrabold tracking-tight text-slate-900">Review your message before sending</h3>
              ...
              <div className="grid max-h-[calc(92vh-88px)] gap-0 overflow-auto lg:grid-cols-[1.4fr_0.8fr]">
                    ...
                    <div className="mx-auto max-w-3xl overflow-hidden rounded-[26px] border border-slate-200 bg-white shadow-[0_18px_50px_rgba(15,23,42,0.08)]">
                      ...
                      <div className="space-y-4 px-5 py-5">
                        <div className="rounded-2xl border border-[#dfe7ff] bg-[#f8fbff] p-4">
                          ...
                          <p className="mt-1 text-sm font-semibold text-slate-900">AF Home &lt;support@afhome.com&gt;</p>
                        </div>
                        ...
                        <div className="rounded-3xl border border-slate-200 bg-gradient-to-b from-white to-[#fbfcff] p-5">
                          <div className="mb-4 flex items-center gap-3 border-b border-slate-100 pb-4">
                            ...
                            <div>
                              <p className="text-xs font-semibold uppercase tracking-[0.18em] text-slate-400">Announcement Preview</p>
                              <p className="text-lg font-bold text-slate-900">{preview.title || 'Untitled'}</p>
                            </div>
                          </div>
                      ...
                      <div className="w-full rounded-[34px] border border-slate-200 bg-slate-900 p-3 shadow-[0_25px_70px_rgba(15,23,42,0.28)]">
                        <div className="rounded-[28px] bg-[#f7f9ff] p-4">
                          <div className="mb-4 flex items-center justify-between">
                            <div>
                              <p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-slate-400">SMS Preview</p>
                              <p className="text-lg font-bold text-slate-900">AF Home</p>
                            </div>
                          ...
                          <div className="rounded-[24px] bg-slate-900 p-4 text-white shadow-inner">
                            <div className="mb-4 flex justify-end">
                              <div className="max-w-[90%] rounded-[22px] rounded-tr-md bg-gradient-to-br from-[#2457e7] to-[#6d86ff] px-4 py-3 shadow-lg">
                                <p className="whitespace-pre-wrap break-words text-sm leading-6 text-white">{preview.plainText}</p>
                              </div>
                            </div>
                ...
                <div className="space-y-4 p-5">
                  <div className="rounded-2xl border border-white/70 bg-white/80 p-4 shadow-sm backdrop-blur">
                    <p className="text-[11px] font-bold uppercase tracking-[0.18em] text-slate-400">Channel</p>
                    <p className="mt-1 text-base font-bold text-slate-900">{isEmailPreview ? 'Email' : 'SMS'}</p>
                  </div>
                  <div className="rounded-2xl border border-white/70 bg-white/80 p-4 shadow-sm backdrop-blur">
                    <p className="text-[11px] font-bold uppercase tracking-[0.18em] text-slate-400">Title</p>
                    <p className="mt-1 text-base font-bold text-slate-900">{preview.title || 'Untitled'}</p>
                  </div>
                  <div className="rounded-2xl border border-white/70 bg-white/80 p-4 shadow-sm backdrop-blur">
                  ...
                  <div className="rounded-2xl border border-white/70 bg-white/80 p-4 shadow-sm backdrop-blur">
                    <p className="text-[11px] font-bold uppercase tracking-[0.18em] text-slate-400">Length</p>
                    <p className="mt-1 text-sm font-semibold text-slate-900">
                      {isEmailPreview
