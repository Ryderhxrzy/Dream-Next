...
export async function GET() {
  ...
  try {
    const res = await fetch(LARAVEL_BASE, { headers: authHeaders(token), cache: 'no-store' })
    const data = await res.json().catch(() => ({}))
    return NextResponse.json(data, { status: res.status })
  } catch (error) {
    console.error('QA status load failed:', error)
    return NextResponse.json({ error: 'Failed to load QA statuses' }, { status: 502 })
  }
}
...
export async function PUT(request: Request) {
  ...
  try {
    ...
    return NextResponse.json({ error: 'Failed to save QA status' }, { status: 502 })
  }
}
...
export async function DELETE() {
  ...
  try {
    const res = await fetch(LARAVEL_BASE, { method: 'DELETE', headers: authHeaders(token) })
    const data = await res.json().catch(() => ({}))
    return NextResponse.json(data, { status: res.status })
  } catch (error) {
    console.error('QA status reset failed:', error)
    return NextResponse.json({ error: 'Failed to reset QA statuses' }, { status: 502 })
  }
}
