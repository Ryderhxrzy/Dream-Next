...
const supportReasonFor = (question: string) => {
  ...
  if (/\b(encashment|payout|withdraw|cash out|cashout|wallet balance|wrong wallet|missing wallet)\b/i.test(question)) return "encashment_verification"
  if (/\b(commission status|missing commission|commission missing|uncredited commission|commission.*(?:missing|not credited|status|wrong)|referral earning|not credited|missing credit|hindi pumasok|hindi na-credit|bonus.*(?:missing|not credited|status)|voucher.*(?:missing|not credited|status)|pv.*(?:missing|not credited|status)|points.*(?:missing|not credited|status))\b/i.test(question)) return "affiliate_record_verification"
