...
export async function POST(req: NextRequest) {
  try {
    ...
    const partnerSession = adminSession
      ? null
      : await getServerSession(partnerAuthOptions)
    const supplierSession =
      !adminSession && !partnerSession
        ? await getServerSession(supplierAuthOptions)
        : null
    const customerSession =
      !adminSession && !partnerSession && !supplierSession
        ? await getServerSession(authOptions)
        : null
    const session =
      adminSession ?? partnerSession ?? supplierSession ?? customerSession
    const role = String(
    ...
    const isCustomerProfileUpload =
      role === "customer" && folderType === "profile"
    const isPartnerUpload =
      !!partnerSession?.user && folderType === "partner-storefronts"
    const isSupplierCatalogueUpload =
      !!supplierSession?.user && folderType === "merchant-catalogues"
    if (
      !session?.user ||
      (!isAdminUpload &&
        !isCustomerProfileUpload &&
        !isPartnerUpload &&
        !isSupplierCatalogueUpload)
    ) {
