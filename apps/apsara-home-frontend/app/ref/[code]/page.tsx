...
async function fetchReferrerProfile(username: string) {
  try {
    ...
    const response = await fetch(
      `${apiUrl}/api/public/profile/${encodeURIComponent(username)}`,
      {
        method: "GET",
        headers: {
...
function normalizeAvatarUrl(value?: string | null): string {
  ...
  const unescaped = unquoted.replace(/\\\//g, "/").replace(/&amp;/g, "&").trim()
...
export async function generateMetadata({ params }: ReferralPageProps) {
  ...
  return buildPageMetadata({
    ...
    description:
      "Open an AF Home referral link and continue with sign up or shopping.",
    path: `/ref/${code}`,
    image: profileImage,
  })
}
