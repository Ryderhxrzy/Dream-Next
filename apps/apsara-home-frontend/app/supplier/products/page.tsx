...
const pickBestBrandId = (
  ...
  const exactMatch = brands.find((brand) => {
    ...
    return (
      brandKey !== "" && candidates.some((candidate) => candidate === brandKey)
    )
  })
  if (exactMatch?.id) {
...
async function resolveSupplierBrandType(
  apiUrl: string,
  accessToken: string,
  supplierId: number,
  ...
  try {
    ...
    const suppliersJson = suppliersRes.ok
      ? ((await suppliersRes.json()) as {
          suppliers?: Array<{ id: number; name?: string; company?: string }>
        })
      : { suppliers: [] }
    const supplier = (suppliersJson.suppliers ?? []).find(
...
async function getInitialProducts(): Promise<{
  ...
  const supplierId = Number(
    (session?.user as { supplierId?: number | null } | undefined)?.supplierId ??
      0
  )
  const supplierName =
    (session?.user as { supplierName?: string | null } | undefined)
      ?.supplierName ?? null
  ...
  const brandType =
    supplierId > 0
      ? await resolveSupplierBrandType(
          apiUrl,
          accessToken,
          supplierId,
          supplierName
  ...
  const res = await fetch(
    `${apiUrl}/api/admin/products?page=1&per_page=25${supplierQuery}`,
    {
      method: "GET",
      headers: {
        Accept: "application/json",
  ...
  return {
    data: normalizeProductsResponse((await res.json()) as ProductsResponse),
    brandType,
  }
}
...
export default async function SupplierProductsPage() {
  ...
  return (
    <ProductsPageMain initialData={initialData} initialBrandType={brandType} />
  )
}
