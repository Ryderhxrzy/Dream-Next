...
export async function setRsvp(
  ...
  const [going, interested] = await Promise.all([
    ...
    prisma.communityEventRsvp.count({ where: { postId, status: "INTERESTED" } }),
  ]);
...
export async function createRepost(
  ...
  if (originalOwnerId && originalOwnerId !== authorId) {
    const author = {
      ...
      name: [repost.author.firstName, repost.author.lastName].filter(Boolean).join(" ") || "Community Member",
...
export async function toggleReaction(postId: bigint, userId: bigint) {
  ...
  if (existing) {
    ...
    await prisma.communityReaction.create({ data: { postId, authorId: userId } });
    ...
    if (post && post.authorId !== userId) {
      const reactor = await prisma.customer.findUnique({ where: { id: userId } });
      const author = {
        id: userId.toString(),
        name: [reactor?.firstName, reactor?.lastName].filter(Boolean).join(" ") || "Community Member",
