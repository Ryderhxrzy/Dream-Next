...
communityPostRoutes.get("/", async (c) => {
  ...
  return c.json(
    posts.map((post) => serializeCommunityPostListItem(post, viewer?.id))
  )
})
...
communityPostRoutes.get("/saved", requireAuth, async (c) => {
  ...
  return c.json(
    posts.map((post) =>
      serializeCommunityPostListItem(post, c.get("customer").id)
    )
  )
})
...
communityPostRoutes.post("/:id/repost", requireAuth, async (c) => {
  ...
  return c.json(
    serializeCommunityPostListItem(data!, c.get("customer").id),
    201
  )
})
...
communityPostRoutes.patch("/:id/hide", requireAuth, async (c) => {
  ...
  if (error === "not_found") return c.json({ message: "Post not found" }, 404)
  if (error === "forbidden")
    return c.json({ message: "You can only hide your own posts" }, 403)
...
communityPostRoutes.patch("/:id/unhide", requireAuth, async (c) => {
  ...
  if (error === "not_found") return c.json({ message: "Post not found" }, 404)
  if (error === "forbidden")
    return c.json({ message: "You can only unhide your own posts" }, 403)
