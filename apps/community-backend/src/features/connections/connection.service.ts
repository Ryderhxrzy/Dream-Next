...
async function buildAuthor(userId: bigint) {
  ...
  return {
    id: userId.toString(),
    name: [u?.firstName, u?.lastName].filter(Boolean).join(" ") || "Community Member",
...
export async function sendRequest(requesterId: bigint, addresseeId: bigint) {
  if (requesterId === addresseeId) return { error: "cannot_connect_self" as const };
