...
messageRoutes.post("/images", requireAuth, async (c) => {
  ...
  const upload = await uploadImageToCloudinary(image, config.cloudinary.communityChatFolder);
...
messageRoutes.post("/conversations", requireAuth, async (c) => {
  ...
  if (error === "self") return c.json({ message: "Cannot message yourself" }, 422);
...
messageRoutes.get("/conversations/:id/messages", requireAuth, async (c) => {
  ...
  if (error === "forbidden") return c.json({ message: "Not in this conversation" }, 403);
...
messageRoutes.post("/conversations/:id/messages", requireAuth, async (c) => {
  ...
  const { data, error } = await sendMessage(id, c.get("customer").id, content, imageUrl);
