...
export function parseUpdateProfileInput(body: unknown) {
  ...
  if (location && location.length > MAX_LOCATION) {
    return { data: null, error: `location must be at most ${MAX_LOCATION} characters` };
  }
  ...
  if (occupation && occupation.length > MAX_SHORT) {
    return { data: null, error: `occupation must be at most ${MAX_SHORT} characters` };
  }
  ...
  const data: UpdateProfileInput = { bio, location, coverUrl, occupation, role, interests };
