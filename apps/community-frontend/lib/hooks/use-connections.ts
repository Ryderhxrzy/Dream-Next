...
export function useMutualConnections(userId: string | undefined, enabled: boolean) {
  const token = useAuthStore((s) => s.token);
  return useQuery({
    queryKey: ["mutual", userId, token],
    queryFn: () => api<ConnectionUser[]>(`/connections/${userId}/mutual`, { token }),
...
export function useConnectionActions(userId: string) {
  ...
  const accept = useMutation({
    mutationFn: () => api(`/connections/${userId}/accept`, { method: "POST", token }),
    onSuccess: invalidate,
  });
  const remove = useMutation({
    mutationFn: () => api(`/connections/${userId}`, { method: "DELETE", token }),
    onSuccess: invalidate,
  });
