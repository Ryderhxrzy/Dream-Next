...
import {
  Loader2, FileText, MapPin, Pencil, ImagePlus, Check, CheckCircle2,
  UserPlus, UserCheck, Clock3, MessageCircle, X,
} from "lucide-react"
...
import { useMyProfile, useUpdateProfile, useUploadProfileImage, type CommunityProfile } from "@/lib/hooks/use-profile"
import { useUserProfile, useConnectionActions } from "@/lib/hooks/use-connections"
...
const EMPTY_PROFILE: CommunityProfile = {
  bio: null, location: null, coverUrl: null, occupation: null, role: null, interests: [],
}
...
export function ProfileView({ userId }: { userId?: string }) {
  ...
  const { data: otherBundle, isLoading: otherLoading } = useUserProfile(isOwn ? undefined : userId)
  ...
  const profile: CommunityProfile = (isOwn ? myProfile : otherBundle) ?? EMPTY_PROFILE
  const name = isOwn ? getFullName(currentUser) : otherBundle?.name ?? "Member"
  const avatarUrl = isOwn ? currentUser?.avatarUrl : otherBundle?.avatarUrl
  const initials = isOwn ? getInitials(currentUser) : initialsFromName(otherBundle?.name ?? "?")
  const handle = isOwn && currentUser?.email ? currentUser.email.split("@")[0] : null
  const connectionCount = isOwn ? myProfile?.connectionCount ?? 0 : otherBundle?.connectionCount ?? 0
  ...
  const visible = useMemo(
    () => (tab === "events" ? myPosts.filter((p) => p.category === "EVENT") : myPosts),
    [myPosts, tab],
  )
  ...
  function handleMessage() {
    ...
    startConversation.mutate(userId, {
      ...
      onError: (er) => toast.error(er.message ?? "Failed to start conversation"),
    })
  }
      ...
      <div className="grid grid-cols-1 gap-5 xl:grid-cols-[minmax(0,1fr)_340px]">
          ...
          <div className="overflow-hidden rounded-2xl border border-border bg-card">
              ...
              {isOwn && (
                <>
                  <Button
                    ...
                    disabled={coverBusy}
                    size="sm"
                    className="absolute right-3 top-3 h-8 rounded-full bg-black/30 px-3 text-xs font-medium text-white backdrop-blur-sm hover:bg-black/40"
                  >
                    {coverBusy ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <ImagePlus className="mr-1.5 h-3.5 w-3.5" />}
                    Edit Cover
                  </Button>
            ...
            <div className="px-5 pb-5 sm:px-6">
              ...
              <div className="relative -mt-12 mb-3 w-fit sm:-mt-14">
                ...
                <span className="absolute bottom-2 right-2 h-4 w-4 rounded-full bg-emerald-500 ring-2 ring-card" />
              </div>
              ...
              <div className="flex items-start justify-between gap-3">
                ...
                {isOwn ? (
                  <Button
                    ...
                    className="shrink-0 rounded-full bg-primary px-4 text-sm font-semibold text-primary-foreground hover:bg-primary/90"
                  >
                  ...
                  <div className="flex shrink-0 items-center gap-2">
                    {status === "NONE" && (
                      <Button onClick={() => connect.mutate()} disabled={connect.isPending} className="rounded-full px-4 text-sm font-semibold">
                        <UserPlus className="mr-1.5 h-4 w-4" />
                        Connect
                      </Button>
                    )}
                    {status === "PENDING_OUTGOING" && (
                      <Button onClick={() => remove.mutate()} disabled={remove.isPending} variant="outline" className="rounded-full px-4 text-sm font-semibold">
                    ...
                    {status === "PENDING_INCOMING" && (
                      <>
                        <Button onClick={() => accept.mutate()} disabled={accept.isPending} className="rounded-full px-4 text-sm font-semibold">
                          <Check className="mr-1.5 h-4 w-4" />
                          Accept
                        </Button>
                        <Button onClick={() => remove.mutate()} disabled={remove.isPending} variant="outline" size="icon" className="rounded-full">
                          <X className="h-4 w-4" />
                        </Button>
                      </>
                    )}
                    ...
                    <Button onClick={handleMessage} disabled={startConversation.isPending} variant="outline" size="icon" className="rounded-full">
              ...
              {/* Bio */}
              {profile.bio && (
                <p className="mt-3 whitespace-pre-line text-sm leading-relaxed text-foreground/90">{profile.bio}</p>
              )}
              ...
              {profile.interests.length > 0 && (
                <div className="mt-3 flex flex-wrap gap-1.5">
                  {profile.interests.map((i) => (
                    <span key={i} className="rounded-full border border-border bg-accent/50 px-3 py-1 text-xs font-medium text-foreground">
                      {i}
                    </span>
                  ))}
                </div>
              ...
              {/* Stats */}
              <div className="mt-5 grid grid-cols-2 border-t border-border pt-4 sm:grid-cols-4 sm:divide-x sm:divide-border">
                {statsArr.map((s) => (
        ...
        <aside className="hidden xl:block">
          <div className="sticky top-18">
            <ProfileRightPanel profile={profile} stats={statsObj} isOwn={isOwn} userId={targetId ?? ""} />
          </div>
        </aside>
      </div>

      {isOwn && (
        <EditProfileModal open={editOpen} onClose={() => setEditOpen(false)} profile={myProfile ?? EMPTY_PROFILE} />
      )}
    </div>
  )
}
