...
import { eventDateOf, formatEventTimeRange, initials, to12h } from "./event-utils"
    ...
    <div className="flex items-center gap-2.5">
      {going.length > 0 && (
        <div className="flex -space-x-2.5">
          {going.map((r) => (
            <Avatar key={r.userId} className="w-8 h-8 ring-2 ring-violet-600">
              <AvatarImage src={r.avatarUrl ?? undefined} />
              <AvatarFallback className="bg-white text-violet-700 text-[11px] font-semibold">
                {initials(r.name)}
              </AvatarFallback>
            </Avatar>
          ))}
  ...
  async function handleShare() {
    ...
    try {
      if (typeof navigator !== "undefined" && navigator.share) {
        await navigator.share({ title: event.title, text: event.content ?? "", url })
      ...
      <div className="relative flex flex-col gap-5 sm:flex-row sm:items-center sm:justify-between sm:gap-6">
        ...
        <div className="order-2 min-w-0 flex-1 space-y-4 sm:order-1">
          ...
          <div className="space-y-2">
            <h2 className="text-xl font-bold leading-tight sm:text-2xl lg:text-3xl">{event.title} 🎉</h2>
            {event.content && (
              <p className="max-w-lg text-sm leading-relaxed text-white/80 line-clamp-2">
                {event.content}
              </p>
            )}
          </div>
        ...
        <div className="order-1 w-28 shrink-0 self-start overflow-hidden rounded-2xl bg-white text-center shadow-lg sm:order-2 sm:w-32 sm:self-auto">
          <div className="bg-red-500 py-1.5 text-xs font-bold uppercase tracking-wide text-white">
            {format(date, "MMMM")}
          </div>
          <div className="px-2 pt-3">
            <div className="text-5xl font-bold leading-none text-zinc-900">{format(date, "d")}</div>
            <div className="pt-1.5 text-xs font-medium text-zinc-500">{format(date, "EEEE")}</div>
          </div>
